...
/// v1.18: Added `shell_version`: the webuild-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like webuild-desktop.
...
/// OS-level sandbox state for a trace turn (local `xai-webuild-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// Version of the webuild-shell agent binary that handled this turn
    /// (`xai_webuild_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like webuild-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
