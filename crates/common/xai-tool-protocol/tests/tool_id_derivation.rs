...
fn namespaced_name_derives_to_namespaced_tool_id() {
    let derived = entry("read_file", Some("WeBuild"))
        .derive_tool_id()
        .unwrap();
    assert_eq!(derived, ToolId::new("WeBuild:read_file").unwrap());
}
...
fn duplicate_derivations_in_a_batch_are_detectable() {
    let batch = [
        entry("read_file", Some("WeBuild")),
        entry("write_file", Some("WeBuild")),
        entry("read_file", Some("WeBuild")),
    ];
    ...
    assert_eq!(id, &ToolId::new("WeBuild:read_file").unwrap());
    assert_eq!(indices, &vec![0, 2]);
}

#[test]
fn derivation_does_not_collide_across_namespaces() {
    let a = entry("read_file", Some("WeBuild"))
        .derive_tool_id()
        .unwrap();
