...
mod tests {
    ...
    fn converts_counter_gauge_histogram_with_labels() {
        ...
        let counter =
            IntCounterVec::new(Opts::new("webuild_test_total", "help"), &["reason"]).unwrap();
        registry.register(Box::new(counter.clone())).unwrap();
        counter.with_label_values(&["zdr"]).inc_by(5);

        let gauge = IntGauge::new("webuild_test_pending", "help").unwrap();
        registry.register(Box::new(gauge.clone())).unwrap();
        gauge.set(7);

        let hist = Histogram::with_opts(
            HistogramOpts::new("webuild_test_seconds", "help").buckets(vec![0.5, 1.0]),
        )
        .unwrap();
        ...
        let metric::Data::Sum(sum) = by_name["webuild_test_total"].data.as_ref().unwrap() else {
        ...
        // Gauge -> Gauge.
        let metric::Data::Gauge(g) = by_name["webuild_test_pending"].data.as_ref().unwrap() else {
        ...
        let metric::Data::Histogram(h) = by_name["webuild_test_seconds"].data.as_ref().unwrap() else {
