...
//!
//! - WeBuild chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `WeBuildTurn::get_num_tokens`.
...
//! (WeBuild chat: `ItemTokenCounter<Arc<WeBuildTurn>>`), so exactly one mechanism
//! is in play.
