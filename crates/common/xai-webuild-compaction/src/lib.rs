...
//! stay in each product host (for example `xai-webuild-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `xai-webuild-sampling-types`. It is decoupled from both WeBuild chat and
...
//! - [`intra_compaction`] — WeBuild chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — WeBuild chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by WeBuild chat and
