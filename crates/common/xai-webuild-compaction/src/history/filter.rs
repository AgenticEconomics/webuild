...
//!
//! Behavior is byte-for-byte identical for WeBuild chat (`T = Arc<WeBuildTurn>`).
...
///   summaries so their `<webuild_user_queries>` sections can be split out.
...
/// Extracts **all** `<webuild_user_queries>...</webuild_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<webuild_user_queries>";
    let end_tag = "</webuild_user_queries>";
...
/// Extract a `<webuild_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<webuild_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<webuild_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<webuild_user_queries>\n");
    ...
    result.push_str("</webuild_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<webuild_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<webuild_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<webuild_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<webuild_user_queries>` block.
...
/// `<webuild_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<webuild_user_queries>` from earlier rounds.
...
///   would see the prior `<webuild_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<webuild_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<webuild_user_queries>"));
        assert!(out.ends_with("</webuild_user_queries>"));
        assert!(out.contains("<webuild_query>hello world</webuild_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<webuild_user_queries>\n<webuild_query>first</webuild_query>\n</webuild_user_queries>";
        let inner2 = "<webuild_user_queries>\n<webuild_query>second</webuild_query>\n</webuild_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<webuild_user_queries>\n<webuild_query>Q1</webuild_query>\n</webuild_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<webuild_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<webuild_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<webuild_user_queries>\n<webuild_query>Q</webuild_query>\n</webuild_user_queries>";
