...
//! harness implements [`CompactionItem`] for its `WeBuildTurn`;
//! grok-build implements it for `xai_webuild_sampling_types::ConversationItem`.
//!
...
//! `<webuild_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `WeBuildRole` (WeBuild chat) and the
/// `ConversationItem` variants (grok-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (WeBuild chat) — maps to System on harnesses without a
...
/// extraction (`<webuild_file id=".." name=".." />` lines in the
/// `<webuild_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (WeBuild chat's
    /// `WeBuildTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (WeBuild chat: a
    ...
    /// strips their `<webuild_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<webuild_file>` lines in the
    /// `<webuild_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (WeBuild chat: a `Developer` turn with category
    ...
    /// requests/results (WeBuild chat: keep only `Channel` contents of an
...
/// purpose. `CompactionItemBuilder` is already implemented by WeBuild chat's
/// `WeBuildTurn`; adding these constructors to it as required methods would break
/// that impl. They are also grok-build-specific (WeBuild chat's tail-keep path
...
/// operate over `&[Arc<T>]` (WeBuild chat stores turns as `Arc<WeBuildTurn>`).
