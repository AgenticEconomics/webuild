...
/// `<webuild_user_queries>` preamble: the shared `code_compaction` summarizer
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    //     Shared with WeBuild chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<webuild_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<webuild_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<webuild_user_queries>` preamble below — stays intra's.
    ...
    // 3c. For `History` target, prepend a `<webuild_user_queries>` preamble so
...
/// metric label. Keep these in sync with the doc string on WeBuild chat's
...
/// the literal error messages produced by the WeBuild chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the WeBuild chat sampler when the
    ...
    /// — guards the forwarding that WeBuild chat (`Arc<WeBuildTurn>`) relies on.
    #[test]
