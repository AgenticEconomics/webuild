...
/// `filename`, `path`, `metadata`) that the WeBuild SLOP converter uses to
...
pub enum ContentBlock {
    ...
    ///
    /// Optional metadata fields are used by the WeBuild SLOP converter to
    ...
    /// - `path`: file path on the WeBuild Computer filesystem
