...
Sample hooks for WeBuild. Copy to `~/.webuild/hooks/` to enable globally, or to `<project>/.webuild/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.webuild/hooks/bin
cp examples/hooks/safe-shell.json ~/.webuild/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.webuild/hooks/bin/
chmod +x ~/.webuild/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.webuild/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.webuild/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.webuild/hooks/bin/
chmod +x ~/.webuild/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.webuild/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.webuild/hooks/bin
cp examples/hooks/session-log.json ~/.webuild/hooks/
cp examples/hooks/bin/session-log.sh ~/.webuild/hooks/bin/
chmod +x ~/.webuild/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.webuild/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.webuild/hooks/bin
cp examples/hooks/tool-logger.json ~/.webuild/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.webuild/hooks/bin/
chmod +x ~/.webuild/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match WeBuild names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.webuild/hooks/`. The hook stops running on the next session.