//! Integration tests for xai-webuild-hooks.
//!
...
use xai_webuild_hooks::discovery::load_hooks;
use xai_webuild_hooks::dispatcher;
use xai_webuild_hooks::event::*;
use xai_webuild_hooks::result::HookDecision;
use xai_webuild_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        xai_webuild_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$WEBUILD_HOOK_EVENT" > {f}; echo "NAME=$WEBUILD_HOOK_NAME" >> {f}; echo "SESSION=$WEBUILD_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`WEBUILD_HOOK_EVENT`, `WEBUILD_HOOK_NAME`,
/// `WEBUILD_SESSION_ID`, `WEBUILD_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$WEBUILD_HOOK_EVENT" > {f}; echo "NAME=$WEBUILD_HOOK_NAME" >> {f}; echo "SESSION=$WEBUILD_SESSION_ID" >> {f}; echo "ROOT=$WEBUILD_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "WEBUILD_HOOK_EVENT": "spoofed_event",
                                "WEBUILD_HOOK_NAME": "spoofed_name",
                                "WEBUILD_SESSION_ID": "spoofed_session",
                                "WEBUILD_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "WEBUILD_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed WEBUILD_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "WEBUILD_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed WEBUILD_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "WEBUILD_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed WEBUILD_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use xai_webuild_hooks::result::HookRunResult;
    let info = match run {
