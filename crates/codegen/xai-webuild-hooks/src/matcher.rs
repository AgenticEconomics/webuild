use regex::Regex;
use xai_webuild_tools::types::{claude_names_for, webuild_names_for};
...
///   **exact** match against each name (after external→WeBuild alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the WeBuild tool `run_terminal_command`).
///
...
/// plus any WeBuild tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to WeBuild registry in
/// `xai-webuild-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for webuild_name in webuild_names_for(term) {
            push(webuild_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_webuild_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // WeBuild name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_webuild_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // WeBuild equivalent
        assert!(m.is_match("hashline_edit")); // second WeBuild alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_webuild_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_webuild_tool() {
        // A regex written against an external alias still matches the WeBuild tool
