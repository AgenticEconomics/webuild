...
// (`~/.webuild/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_webuild_home>/trusted-hook-projects`), or `None` when no user webuild home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_webuild_config::user_webuild_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user webuild home (set $WEBUILD_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$WEBUILD_HOME/disabled-hooks`, or `None` when no user webuild
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_webuild_config::user_webuild_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let webuild_dir = dir.join(".webuild");
        std::fs::create_dir_all(&webuild_dir).unwrap();
        webuild_dir.join("trusted-hook-projects")
    }
