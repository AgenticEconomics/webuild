...
//!
//! Extracted from `xai-webuild-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// `xai-webuild-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use xai_webuild_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item(text))
    }
