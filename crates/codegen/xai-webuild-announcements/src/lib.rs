//! Shared announcement types, persistence, and formatting for WeBuild CLI apps.
//!
//! This crate provides the common logic used by `xai-webuild-shell` and
//! `xai-webuild-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.webuild/announcements.json`.
...
/// Write hidden announcement ids to `~/.webuild/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    xai_webuild_tools::util::webuild_home::webuild_home().join("announcements.json")
}
...
///
/// Precedence: `WEBUILD_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("WEBUILD_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid WEBUILD_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("WEBUILD_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("WEBUILD_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get SuperWeBuild","url":"https://x.ai/webuild","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get SuperWeBuild"));
        assert_eq!(cta.url.as_deref(), Some("https://x.ai/webuild"));
