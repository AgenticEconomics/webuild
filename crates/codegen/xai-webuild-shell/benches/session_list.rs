...
//!
//! Run: `cargo bench -p xai-webuild-shell --bench session_list`
...
use xai_webuild_shell::session::persistence::Summary;
use xai_webuild_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use xai_webuild_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = xai_webuild_shell::util::webuild_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = xai_webuild_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = xai_webuild_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(xai_webuild_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("WEBUILD_HOME", home.path());
    }
    assert_eq!(xai_webuild_shell::util::webuild_home::webuild_home(), home.path());
