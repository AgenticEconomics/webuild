...
//! `WEBUILD_HOME` (the `webuild_home` `OnceLock` allows a single value per process)
...
use xai_webuild_shell::config::ServingIdentity;
use xai_webuild_test_support::spawn_counting_server;
...
/// Shared temp dir used as WEBUILD_HOME for the whole test binary (the webuild_home
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("WEBUILD_HOME", &path);
            ...
            for var in [
                "WEBUILD_DEPLOYMENT_KEY",
                "WEBUILD_MANAGED_CONFIG",
                "WEBUILD_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS",
                "WEBUILD_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
                "HTTP_PROXY",
                "HTTPS_PROXY",
                "ALL_PROXY",
                "http_proxy",
            ...
            std::env::set_var("WEBUILD_DEPLOYMENT_CONFIG_BACKOFF_MS", "10");
        }
        path
    })
}
...
fn write_team_auth_expiry(home: &std::path::Path, team_id: &str, expires_at: &str) {
    let scope = xai_webuild_shell::auth::WeBuildComConfig::default().auth_scope();
...
async fn team_sync_writes_files() {
    ...
    let wrote = xai_webuild_shell::managed_config::sync()
        .await
...
async fn marker_dir_squat_is_cleared_and_marker_written() {
    ...
    let marker_path = home.join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE);
...
async fn padded_team_id_is_one_identity() {
    ...
    xai_webuild_shell::managed_config::sync()
        .await
        .expect("sync should succeed");
    let marker =
        std::fs::read_to_string(home.join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE)).unwrap();
    let v: serde_json::Value = serde_json::from_str(&marker).unwrap();
    assert_eq!(
        v["principal"].as_str(),
        Some("team-007"),
    ...
    assert_eq!(
        xai_webuild_config::confirmed_team_switch("team-007"),
        None,
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    assert!(!xai_webuild_shell::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !xai_webuild_shell::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = xai_webuild_shell::managed_config::sync()
        .await
...
async fn served_then_deleted_refetches_best_effort() {
    ...
    let auth_manager = std::sync::Arc::new(xai_webuild_shell::auth::AuthManager::new(
        &home,
        xai_webuild_shell::auth::WeBuildComConfig::default(),
    ));
    xai_webuild_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    assert!(
        home.join("requirements.toml").exists(),
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !xai_webuild_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        xai_webuild_shell::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        xai_webuild_shell::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    xai_webuild_shell::config::mark_managed_config_synced(xai_webuild_shell::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = xai_webuild_shell::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(xai_webuild_shell::auth::AuthManager::new(
        &home,
        xai_webuild_shell::auth::WeBuildComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match xai_webuild_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
/// Live wiring guard: an offline `WEBUILD_DEPLOYMENT_KEY` switch on a fail_closed install must FAIL CLOSED, else a
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    unsafe { std::env::set_var("WEBUILD_DEPLOYMENT_KEY", "KEY-AAA") };
    xai_webuild_shell::managed_config::sync()
        .await
    ...
    // WEBUILD_MANAGED_CONFIG=0 disables any incidental background fetch (the gate is sync anyway).
    // SAFETY: #[serial] test; restored before any assertion below.
    unsafe { std::env::set_var("WEBUILD_MANAGED_CONFIG", "0") };

    // Same key A → matching fingerprint → ALLOW (exact-equality vs recorded blake3).
    let gate_same_key = xai_webuild_shell::managed_config::managed_policy_gate();

    // Switch to a different key B → REFUSE: exercises the full offline wiring.
    // SAFETY: #[serial] test; restored immediately below.
    unsafe { std::env::set_var("WEBUILD_DEPLOYMENT_KEY", "KEY-BBB") };
    let gate_switched_key = xai_webuild_shell::managed_config::managed_policy_gate();

    // SAFETY: #[serial] test; restore env BEFORE asserting so a failed assert can't leak it to later tests.
    unsafe {
        std::env::remove_var("WEBUILD_DEPLOYMENT_KEY");
        std::env::remove_var("WEBUILD_MANAGED_CONFIG");
    }
...
async fn identity_change_permits_offline_team_switch_and_purges_prior_team() {
    ...
    for f in xai_webuild_shell::managed_config::MANAGED_ARTIFACT_FILES
        .into_iter()
        .chain([xai_webuild_config::MANAGED_CONFIG_CACHE_FILE])
    {
        assert!(
            !home.join(f).exists(),
...
async fn gate_purge_retries_past_a_transient_lock_holder() {
    ...
    assert!(
        !home
            .join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE)
            .exists(),
...
/// The session-start gate reads no env: `WEBUILD_MANAGED_CONFIG_FAIL_CLOSED=0` must NOT disarm a fail_closed
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    unsafe {
        std::env::set_var("WEBUILD_MANAGED_CONFIG", "0"); // offline (the gate is sync anyway)
        std::env::set_var("WEBUILD_MANAGED_CONFIG_FAIL_CLOSED", "0"); // attempt a local disarm
    }
    let gate = xai_webuild_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("WEBUILD_MANAGED_CONFIG");
        std::env::remove_var("WEBUILD_MANAGED_CONFIG_FAIL_CLOSED");
    }

    assert!(
        gate.is_err(),
        "WEBUILD_MANAGED_CONFIG_FAIL_CLOSED=0 must not disarm the session-start gate (no local bypass)"
    );
}
...
/// `webuild setup` with config served but the lock held by another writer reports the
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = xai_webuild_shell::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            xai_webuild_shell::managed_config::SetupOutcome::Skipped
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = xai_webuild_shell::http::shared_client();
...
/// `WEBUILD_MANAGED_CONFIG=0` is an explicit opt-out: the post-login sync must
/// make zero requests.
#[tokio::test]
#[serial]
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("WEBUILD_MANAGED_CONFIG", "0") };
    let outcome = xai_webuild_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("WEBUILD_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        xai_webuild_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: xai_webuild_shell::auth::WeBuildAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = xai_webuild_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        xai_webuild_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        xai_webuild_shell::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
...
async fn purge_crash_prefixes_stay_armed_and_converge() {
    let home = test_home().clone();
    let artifacts = xai_webuild_shell::managed_config::MANAGED_ARTIFACT_FILES;
    ...
    for prefix_len in 0..=artifacts.len() {
        ...
        assert!(
            home.join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE)
                .exists(),
        ...
        write_team_auth(&home, "team-b");
        assert_eq!(
            xai_webuild_config::confirmed_team_switch("team-b").as_deref(),
            Some("team-a"),
...
async fn contended_sync_writes_no_marker() {
    ...
    let lock = std::fs::OpenOptions::new()
        ...
        .unwrap();
    lock.lock().unwrap();
    let synced = xai_webuild_shell::managed_config::sync()
        .await
...
async fn dk_synced_marker_survives_config_blip_with_team_signed_in() {
    ...
    write_team_auth(&home, "team-b");
    assert_eq!(
        xai_webuild_config::confirmed_team_switch("team-b"),
        None,
