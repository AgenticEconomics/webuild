...
//! - Session resume
//!
//! Each test spawns a real `webuild agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p xai-webuild-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".webuild").join("models_cache.json");
    if cache.exists() {
...
#[tokio::test]
#[ignore]
async fn test_default_model_uses_webuild_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("WeBuild") || sys_prompt.contains("webuild"),
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_webuild =
            original_sys_prompt.contains("WeBuild") || original_sys_prompt.contains("webuild");
        let resumed_has_webuild =
            resumed_sys_prompt.contains("WeBuild") || resumed_sys_prompt.contains("webuild");
        assert_eq!(
            original_has_webuild,
            resumed_has_webuild,
            "resumed session should use the same harness as the original\n\
             original identity markers: webuild={original_has_webuild}\n\
             resumed identity markers: webuild={resumed_has_webuild}\n\
...
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_webuild_build() {
    with_local_set(|| async {
...
/// The `WEBUILD_AGENT` escape hatch should override the model's agent_type.
/// Setting `WEBUILD_AGENT=grok-build` with an alternate-agent model should use
/// grok-build harness.
#[tokio::test]
#[ignore]
async fn test_webuild_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            xai_webuild_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("WEBUILD_AGENT", "grok-build");
            let mut child = cmd.spawn().expect("spawn webuild");
            ...
            assert!(
                sys_prompt.contains("WeBuild") || sys_prompt.contains("webuild"),
                "WEBUILD_AGENT=grok-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
