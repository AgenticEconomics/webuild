...
//! - `WEBUILD_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/grok-build-public-artifacts/cli/webuild-<ver>-linux-x86_64`).
//! - `WEBUILD_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! WEBUILD_BINARY_LEADER=/path/to/webuild-old WEBUILD_BINARY_CLIENT=/path/to/webuild-new \
//!   cargo test -p xai-webuild-shell --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
use xai_webuild_test_support::leader::{
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: WEBUILD_BINARY_LEADER/WEBUILD_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".webuild").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set WEBUILD_BINARY_LEADER/WEBUILD_BINARY_CLIENT and run with --ignored"]
...
/// `webuild update`'s relaunch signal against a REAL old leader: connect,
/// require `relaunch_v1`, send `RelaunchForUpdate`, and the leader exits so
/// the surviving client re-elects. Mirrors the private
/// `signal_leaders_to_relaunch` in `xai-webuild-pager-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".webuild").join("leader.sock"),
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".webuild").join("auth.json");
