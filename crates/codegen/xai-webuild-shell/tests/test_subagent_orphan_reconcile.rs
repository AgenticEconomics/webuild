...
//!
//! This test spawns a real `webuild agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p xai-webuild-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // WeBuildStdioClient sets HOME=<temp>; the binary uses <HOME>/.webuild as WEBUILD_HOME.
        let webuild_home = shared_home.path().join(".webuild");
        let session_dir = locate_session_dir(&webuild_home, session_id.0.as_ref());
