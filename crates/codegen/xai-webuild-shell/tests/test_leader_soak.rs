...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p xai-webuild-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = webuild_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            // the same wiring: xai-webuild-pager/src/app/leader_cluster/mod.rs
