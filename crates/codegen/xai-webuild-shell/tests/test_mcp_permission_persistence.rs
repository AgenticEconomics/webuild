...
use xai_webuild_workspace::permission::types::{
...
/// Shared `WEBUILD_HOME` for the entire test binary. The `OnceLock` in
/// `xai-webuild-config` only allows one value per process, so all tests share
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("WEBUILD_HOME", &path) };
        path
    })
}
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::WeBuildPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / WeBuildWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use xai_webuild_workspace::permission::types::{PermissionConfig, PromptPolicy};
