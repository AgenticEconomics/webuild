...
//! `.webuild`, `.cursor`, and `.claude` vendor dirs, spawns a real `webuild agent
//! stdio` process against the mock inference server (toggling the
//! `WEBUILD_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the WeBuild-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p xai-webuild-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_WEBUILD_SKILL: &str = "ZZ_WEBUILD_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".webuild", "webuild-skill", MARKER_WEBUILD_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = WeBuildStdioClient::spawn_with_home_and_env(&server, workdir.path(), home, env).await;
...
/// Defaults (all vendors on): webuild + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_WEBUILD_SKILL),
            "webuild-skill must always be present"
        );
        assert!(
...
/// `WEBUILD_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; webuild stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_WEBUILD_SKILL),
            "webuild-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `WEBUILD_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; webuild stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_WEBUILD_SKILL),
            "webuild-skill always present"
        );
...
/// `WEBUILD_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `WEBUILD_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `WEBUILD_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `WEBUILD_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("WEBUILD_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("WEBUILD_CURSOR_SKILLS_ENABLED", "false"),
            ("WEBUILD_CURSOR_RULES_ENABLED", "false"),
            ("WEBUILD_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_WEBUILD_SKILL), "webuild always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("WEBUILD_CLAUDE_SKILLS_ENABLED", "false"),
            ("WEBUILD_CLAUDE_RULES_ENABLED", "false"),
            ("WEBUILD_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only webuild-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
