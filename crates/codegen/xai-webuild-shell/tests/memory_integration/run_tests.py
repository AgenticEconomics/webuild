...
Launches webuild agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/xai-webuild-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/xai-webuild-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/xai-webuild-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    WEBUILD_BINARY=/path/to/xai-webuild-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a webuild agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated webuild environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.webuild/auth.json   (copied from real home)
      ~/.webuild/memory/     (pre-populated by tests)
      ~/.webuild/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="webuild-memtest-")
        ...
        # Create .webuild dirs in fake home
        self.webuild_home = os.path.join(self.fake_home, ".webuild")
        self.memory_dir = os.path.join(self.webuild_home, "memory")
        self.logs_dir = os.path.join(self.webuild_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.webuild/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.webuild_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.webuild/config.toml (where the agent reads config)."""
        with open(os.path.join(self.webuild_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("WEBUILD_BINARY", "webuild")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}webuild binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
        env["HOME"] = self.fake_home
        env["WEBUILD_MEMORY"] = "1"
        env["RUST_LOG"] = "warn"
        # Disable leader mode
        env["WEBUILD_NO_LEADER"] = "1"
        if extra_env:
...
def _default_config() -> str:
    embed_model = os.environ.get("WEBUILD_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"WEBUILD_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """WEBUILD_MEMORY=1 env var enables memory even without config."""
    section("13. WEBUILD_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"WEBUILD_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with WEBUILD_MEMORY=1")
        else:
            fail("no index despite WEBUILD_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.webuild/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("WEBUILD_BINARY", "webuild")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["WEBUILD_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["WEBUILD_NO_LEADER"] = "1"
...
def build_binary():
    """Build xai-webuild-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "xai-webuild-pager")

    # Find rg for WEBUILD_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["WEBUILD_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building xai-webuild-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "xai-webuild-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "WEBUILD_BINARY" in os.environ:
        binary = os.environ["WEBUILD_BINARY"]
        print(f"Binary: {binary} (from $WEBUILD_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["WEBUILD_BINARY"] = binary
