...
/// All tests share this as WEBUILD_HOME (the `OnceLock` in xai-webuild-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = xai_webuild_shell::config::load_effective_config().unwrap();
    let effective_cfg = xai_webuild_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    xai_webuild_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = xai_webuild_shell::util::config::load_config_from_toml(&user_toml);
