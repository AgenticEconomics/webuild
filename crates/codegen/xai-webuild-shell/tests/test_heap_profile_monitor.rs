...
use xai_webuild_shell::auth::{AuthManager, AuthMode, WeBuildAuth, WeBuildComConfig};
use xai_webuild_shell::heap_profile::{
...
use xai_webuild_test_support::{EnvGuard, MockInferenceServer};
use xai_webuild_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = WeBuildComConfig::default().auth_scope();
...
fn assert_storage_auth(uploads: &[xai_webuild_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), WeBuildComConfig::default()));
