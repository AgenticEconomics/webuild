...
use xai_webuild_sampler::RetryPolicy;
use xai_webuild_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
use xai_webuild_shell::sampling::{
...
use xai_webuild_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// `x-webuild-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p xai-webuild-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = xai_webuild_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let webuild_home = home.path().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create .webuild home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(xai_webuild_test_support::webuild_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_webuild_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("WEBUILD_HOME", webuild_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("WEBUILD_LEADER_SOCKET");

    let result = xai_webuild_test_support::run_headless_with_cmd(cmd).await;
    xai_webuild_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-webuild-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-webuild-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-webuild-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-webuild-doom-loop-check"),
            None::<&str>,
