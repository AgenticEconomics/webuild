...
//!
//! Each test spawns a real `webuild agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p xai-webuild-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.webuild/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".webuild").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let webuild_dir = home.path().join(".webuild");
        std::fs::create_dir_all(&webuild_dir).expect("create .webuild dir");
        std::fs::write(
...
"#,
        ...
        let client = WeBuildStdioClient::spawn_with_home(&server, workdir.path(), home).await;
