...
use xai_webuild_shell::session::storage::JsonlStorageAdapter;
use xai_webuild_test_support::sse::responses_api_reasoning_and_text_events;
use xai_webuild_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "webuild-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy grok-build reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_webuild_legacy"},"model_id":"grok-build"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use xai_webuild_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use xai_webuild_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use xai_webuild_shell::sampling::rs::OutputMessageContent;
...
async fn test_request_includes_headers() {
    ...
    assert_eq!(request.header("x-webuild-conv-id"), Some("conv-12345"));
    assert_eq!(request.header("x-webuild-req-id"), Some("req-67890"));
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-webuild-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use xai_webuild_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-webuild-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-webuild-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `xai-webuild-sampler` selects the
