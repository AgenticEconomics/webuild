...
//!
//! Knobs: WEBUILD_PERF_GIT_FILES (default 300), WEBUILD_PERF_GIT_PICKS (default 6).
...
fn install_global_scan_counter() -> ScanCounter {
    // WEBUILD_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("WEBUILD_E2E_LOG").ok();
    ScanCounter(
...
async fn run_storm(
    ...
    let files = env_usize("WEBUILD_PERF_GIT_FILES", 300);
    let picks = env_usize("WEBUILD_PERF_GIT_PICKS", 6);
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let webuild_home = TempDir::new().expect("webuild home");
