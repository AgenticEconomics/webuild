...
//! 4. Smoke-validate session JSON under `$WEBUILD_HOME/sessions/` after exit.
//!
//! Requires a built `webuild` binary (`WEBUILD_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-webuild-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-webuild-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_webuild_agent::plugins::SharedPluginRegistryHandle;
use xai_webuild_agent::plugins::discovery::DiscoveryConfig;
use xai_webuild_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _webuild_guard = EnvVarGuard::set("WEBUILD_HOME", &webuild_home);
    ...
    let mut registry = InstallRegistry::empty(webuild_home.join("installed-plugins"));
    ...
    // webuild_home() caches the first WEBUILD_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_webuild_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built webuild binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/WEBUILD_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via webuild_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _webuild_guard = EnvVarGuard::set("WEBUILD_HOME", &webuild_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("WEBUILD_HOME", &webuild_home);
    ...
    // Smoke: session storage under WEBUILD_HOME/sessions has JSON artifacts.
