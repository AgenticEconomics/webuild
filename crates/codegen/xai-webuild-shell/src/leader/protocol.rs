...
pub enum ClientMode {
    /// Headless mode (webuild agent, webuild agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (webuild agent stdio, webuild -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `webuild update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `webuild update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `webuild update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["webuild-a".into(), "webuild-b".into()],
            pid: 4242,
        };
