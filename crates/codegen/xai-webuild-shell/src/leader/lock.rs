...
use fs2::FileExt;
use xai_webuild_workspace::util::is_lock_contended;
...
pub fn compute_ws_url_suffix(ws_url: &str) -> String {
    ...
    // `webuild_ws_url` is always the *relay* endpoint (see
...
/// `~/.webuild/leader.sock` — e.g. run a local branch build's leader without
...
/// bypassed entirely.
pub const LEADER_SOCKET_ENV: &str = "WEBUILD_LEADER_SOCKET";
...
pub fn lock_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_lock_path(leader_socket_override(), &webuild_home(), ws_url)
}
...
pub fn socket_path_for_ws_url(ws_url: &str) -> PathBuf {
    resolve_socket_path(leader_socket_override(), &webuild_home(), ws_url)
}
...
impl LeaderLock {
    /// Create a new LeaderLock using the default paths in webuild home.
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        let root = Path::new("/home/u/.webuild");
        let override_sock = PathBuf::from("/home/u/.webuild/leader-branch.sock");
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.webuild/leader-branch.lock")
        );
    }
