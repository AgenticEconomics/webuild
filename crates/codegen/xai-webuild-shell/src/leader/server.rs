...
use xai_webuild_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "webuild-tui", "grok-code-extension").
...
impl AuthProvider for LeaderAuthProvider {
    ...
    /// read. Mirrors the in-process path (`mvp_agent`): prefer `WeBuildAuth.team_id`
...
async fn handle_workspace_start(
    ...
    let status_config = xai_webuild_workspace::StatusConfig::from_env();
    let alpha_test_key = None;
    let auth = wait_for_leader_auth(ws, &cancel).await?;
    let server_id = workspace_server_id();
    let metadata = serde_json::json!(
        { "source" : "webuild-workspace", "hostname" : gethostname::gethostname()
        .to_string_lossy(), "cwd" : cwd_path.display().to_string(), }
    );
    let upload_queue_enabled =
        std::env::var("WEBUILD_WORKSPACE_UPLOAD_QUEUE_ENABLED").as_deref() != Ok("false");
    ...
    let handle = xai_webuild_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
    ...
    Some(
        serde_json::json!(
            ...
            format!("Client version {client_version} differs from leader version \
                     {leader_version}. Restart the webuild binary to use the same version.")
            } }
        )
        .to_string(),
    )
...
pub async fn run_leader_server(
    ...
    loop {
        tokio::select! {
            ...
            xai_webuild_telemetry::unified_log::info("leader.client.registered", None,
            ...
            xai_webuild_telemetry::unified_log::info("leader.client.disconnected", None,
            ...
            xai_webuild_telemetry::unified_log::warn("leader.response.orphaned", None,
            ...
            xai_webuild_telemetry::unified_log::warn("leader.response.send_failed", None,
...
mod tests {
    ...
    async fn workspace_start_errors_when_cancelled_before_auth() {
        let state = default_test_control_state(Path::new("/tmp/webuild-ws-auth-test.sock"));
        let cancel = CancellationToken::new();
        cancel.cancel();
    ...
    async fn relay_demand_signals_only_on_headless_registration() {
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _stdio =
            connect_and_register_with_mode(&sock_path, "webuild-tui", ClientMode::Stdio).await;
        ...
        let _headless =
            connect_and_register_with_mode(&sock_path, "webuild-headless", ClientMode::Headless).await;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "webuild-tui",
    ...
    async fn initialize_preserves_existing_client_identifier() {
        ...
        .unwrap();
        let _: ServerMessage = read_message(&mut reader).await.unwrap();
        let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"webuild-web"}}}"#;
        write_message(
            &mut writer,
            &ClientMessage::Acp {
                payload: payload.into(),
        ...
        let json: serde_json::Value = serde_json::from_str(&received).unwrap();
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "webuild-web",
    ...
    fn patch_initialize_response_preserves_other_fields() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"webuildShell":true,"modelState":{"currentModelId":"grok-3","availableModels":[{"modelId":"grok-3"},{"modelId":"grok-3-fast"}]}}}}"#,
        );
        let default_model = Some("grok-3-fast".to_string());
        assert!(patch_initialize_response_model(&mut json, &default_model));
        assert_eq!(json["result"]["meta"]["webuildShell"], true);
        assert_eq!(
            json["result"]["meta"]["modelState"]["currentModelId"],
            "grok-3-fast"
        );
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "webuild-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "webuild-tui", ClientId(42));
        assert_eq!(
    ...
    fn inject_yolo_notification_adds_client_identifier() {
        ...
        assert!(inject_client_identity_into_yolo_notification(
            &mut json, "webuild-tui"
        ));
        assert_eq!(json["params"]["clientIdentifier"], "webuild-tui");
    ...
    fn inject_client_identity_adds_identifier_to_initialize() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "webuild-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated, "should have injected the identifier");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "webuild-tui");
    }
    #[test]
    fn inject_client_identity_does_not_override_existing() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"webuild-web"}}}"#,
        );
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "webuild-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(!mutated, "existing identifier means nothing was injected");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "webuild-web");
    }
    #[test]
    ...
    async fn set_model_updates_default_model_for_next_session_new() {
        ...
        write_message(
            ...
            &ClientMessage::Register {
                ...
                capabilities: ClientCapabilities {
                    yolo_mode: false,
                    default_model: Some("webuild-original".to_string()),
                    ..Default::default()
                },
            },
        )
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "webuild-tui").await;
    ...
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/models/update","params":{"currentModelId":"webuild-new","availableModels":[{"modelId":"webuild-new","name":"WeBuild New"}]}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a.as_deref().is_some_and(|p| p.contains("webuild-new")),
            "client A must receive the models broadcast, got {got_a:?}"
        );
        assert!(
            got_b.as_deref().is_some_and(|p| p.contains("webuild-new")),
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/mcp/servers_updated","params":{"method":"x.ai/mcp/servers_updated","params":{"mcpServers":[{"name":"webuild_com_slack","source":"managed"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a
                .as_deref()
                .is_some_and(|p| p.contains("webuild_com_slack")),
    ...
    fn inject_capabilities_sets_code_nav_enabled_true() {
        ...
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "webuild-web", ClientId(1));
        assert_eq!(
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"webuild-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "webuild-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut web_json, &web_caps, "webuild-web", ClientId(1));
        let mut tui_json = pv(session_new);
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "webuild-tui", ClientId(2));
        assert_eq!(
