//! Leader-follower IPC architecture for webuild-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.webuild/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use xai_webuild_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::WeBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = xai_webuild_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`webuild -p`).
...
pub struct LeaderEnvUrls {
    pub webuild_ws_url: String,
    pub webuild_ws_origin: String,
}
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<WeBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(WeBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<WeBuildEnvironment> {
    let environments: &[WeBuildEnvironment] = &[WeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<WeBuildEnvironment> {
    let environments: &[WeBuildEnvironment] = &[WeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::webuild_home::webuild_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : xai_webuild_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::WeBuildComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::WeBuildComConfig) -> Self {
        Self {
            webuild_ws_url: c.webuild_ws_url.clone(),
            webuild_ws_origin: c.webuild_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`webuild -p` where hanging forever is unacceptable.
...
///     "webuild-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.webuild_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "webuild-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.webuild_ws_url);
...
///
/// For a **managed install** — the running binary lives under `webuild_home`
/// (e.g. `~/.webuild/...`) — prefer the managed `~/.webuild/bin/webuild` symlink. After an
/// auto-update or `webuild update` atomically swaps that symlink, `current_exe()`
...
/// `xai_webuild_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `webuild_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.webuild/bin/webuild` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::webuild_home::webuild_home())
}
fn resolve_binary_with_home(webuild_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(webuild_home, std::env::current_exe().ok())
}
/// Binary file name for the managed webuild install (`webuild` / `webuild.exe`).
fn managed_webuild_bin_name() -> &'static str {
    if cfg!(windows) { "webuild.exe" } else { "webuild" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = webuild_home.join("bin").join(managed_webuild_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, webuild_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--webuild-ws-url").arg(&env_urls.webuild_ws_url);
    cmd.arg("--webuild-ws-origin").arg(&env_urls.webuild_ws_origin);
    ...
    for key in [
        "WEBUILD_DEBUG_LOG",
        "WEBUILD_HOOKS_LOG",
        "WEBUILD_LOG_SAMPLING",
        "WEBUILD_INSTRUMENTATION",
    ] {
    ...
    let log_path = crate::util::webuild_home::webuild_home().join("leader.log");
    ...
    let leader_log = std::env::var("WEBUILD_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "xai_webuild_shell=info,xai_acp_lib=warn,xai_webuild_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            webuild_ws_url: "wss://test.invalid".into(),
            webuild_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("webuild"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("webuild-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("webuild")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("webuild-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("webuild");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("webuild-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_webuild_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_webuild_bin_name());
