...
//!   into `\\.\pipe\webuild-leader-<hash>` so callers keep their path-based API.
//!
#[cfg(unix)]
...
mod windows_impl {
    ...
    /// Deterministic leaf name (`webuild-leader-<hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("webuild-leader-{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_is_deterministic() {
            let a = path_to_pipe_name(Path::new("/tmp/webuild.sock"));
            let b = path_to_pipe_name(Path::new("/tmp/webuild.sock"));
            assert_eq!(a, b);
        }
        ...
        fn pipe_name_has_correct_prefix() {
            ...
            assert!(s.starts_with(r"\\.\pipe\webuild-leader-"), "got: {s}");
        }
        ...
        fn pipe_name_is_bounded() {
            ...
            // \\.\pipe\webuild-leader- (20 chars) + 16 hex chars = 36 total
        ...
        async fn listener_is_ready_tracks_pipe_lifecycle() {
            ...
            let path =
                std::env::temp_dir().join(format!("webuild-ready-probe-{}.sock", std::process::id()));
