// Re-export all types from the standalone xai-webuild-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_webuild_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-webuild-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-webuild-sampler` crate can reference them without a circular
// dep on `xai-webuild-shell`.

// Tests for the types now live in xai-webuild-sampling-types crate.

use xai_webuild_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-webuild-sampling-types` or
/// `xai-webuild-tools`) so neither of those crates needs to depend on
