...
pub use self::types::*;
pub use xai_webuild_sampler::ApiBackend;
pub use xai_webuild_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// xai-webuild-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `xai-webuild-sampler` crate. We re-export the public surface here so
...
// `xai_webuild_sampler::*`. The shell-side `sampling::client::Config`
