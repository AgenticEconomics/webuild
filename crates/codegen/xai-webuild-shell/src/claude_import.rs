// claude_import.rs
// Scans Claude settings and generates TOML patches for .webuild/config.toml.
//
...
use xai_webuild_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use xai_webuild_workspace::permission::rules::parse_permission_rule;
use xai_webuild_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.webuild/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.webuild/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.webuild/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.webuild/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.webuild/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.webuild/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.webuild/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::webuild_home::webuild_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.webuild/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.webuild/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::webuild_home::webuild_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.webuild/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::webuild_home::webuild_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".webuild").join("config.toml");
        ...
        let hooks_dir = project_root.join(".webuild").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .webuild/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `xai-webuild-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.webuild/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .webuild/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_WEBUILD_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = xai_webuild_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = xai_webuild_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.webuild) don't interfere.
        let has_probe = |reg: &xai_webuild_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `WEBUILD_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_WEBUILD_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        // managed settings, and the developer's real `~/.webuild/config.toml`.
        // We can't isolate `webuild_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.webuild
        // config rules (if any) are out of scope for this test.
        let resolved =
            xai_webuild_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use xai_webuild_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.webuild/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `webuild_home()`
