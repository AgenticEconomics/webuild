...
use xai_webuild_telemetry::id::agent_id;
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `webuild sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.webuild/config.toml`,
...
/// Hot-reload the model catalog from `~/.webuild/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by webuild-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .webuild/plugins under it, plus
