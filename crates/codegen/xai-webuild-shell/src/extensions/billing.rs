...
//!
//! Fetches the authenticated user's WeBuild billing configuration
...
/// Current billing configuration for WeBuild coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `WeBuildBillingConfig` fields
...
/// against both the new `GetWeBuildCreditsConfig` and the legacy
/// `GetWeBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetWeBuildCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `WeBuildCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetWeBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/webuild/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperWeBuild Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.webuild/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `webuild login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetWeBuildCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            crate::auth::WeBuildComConfig::default().token_header,
        )
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `webuild login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperWeBuild");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetWeBuildCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_WEBUILD_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
