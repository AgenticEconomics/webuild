...
use xai_webuild_hooks::event::{HookEventEnvelope, HookEventName};
...
pub fn hook_spec_to_info(spec: &xai_webuild_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: xai_webuild_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
mod tests {
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use xai_webuild_hooks::event::{HookEventEnvelope, HookPayload};
