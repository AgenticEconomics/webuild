...
) -> Result<crate::auth::WeBuildAuth, acp::Error> {
    super::auth_gate::require_xai_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `webuild login` to authenticate.",
    )
}
...
mod tests {
    ...
    #[test]
    fn share_rejects_non_xai_auth_with_actionable_webuild_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            WeBuildComConfig::default(),
