...
//! Delegates to `xai-webuild-plugin-marketplace` crate for scanning and install logic.
...
fn load_filtered_marketplace_sources() -> Vec<xai_webuild_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                xai_webuild_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                xai_webuild_plugin_marketplace::SourceKind::Local { path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        xai_webuild_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let xai_webuild_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = xai_webuild_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = xai_webuild_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
...
async fn handle_update(
    ...
    let source_identity = |s: &xai_webuild_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            xai_webuild_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            xai_webuild_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match xai_webuild_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        xai_webuild_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = xai_webuild_plugin_marketplace::git::default_cache_root();
            match xai_webuild_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                xai_webuild_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = xai_webuild_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = xai_webuild_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = xai_webuild_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        ...
        Err(xai_webuild_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            xai_webuild_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match xai_webuild_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    xai_webuild_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
async fn auto_install_defaults(
    agent: &MvpAgent,
    sources: &[xai_webuild_plugin_marketplace::MarketplaceSource],
    ...
    for (source, scan) in sources.iter().zip(results.iter()) {
        ...
        let mut reg = xai_webuild_agent::plugins::install_registry::InstallRegistry::load();
        ...
        if let Some((existing_key, _)) = existing {
            ...
            let _ = reg.save();
            reg = xai_webuild_agent::plugins::install_registry::InstallRegistry::load();
        }
...
fn scan_source(
    source: &xai_webuild_plugin_marketplace::MarketplaceSource,
    ...
    let scan = xai_webuild_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = xai_webuild_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match xai_webuild_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: xai_webuild_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.webuild/config.toml`.
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &xai_webuild_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if xai_webuild_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
...
fn remove_source_locked(source_url_or_path: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let is_official = xai_webuild_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<webuild_home>/.config-init.lock`,
...
fn acquire_init_lock(webuild_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(webuild_home);
    let lock_path = webuild_home.join(".config-init.lock");
...
pub fn ensure_official_marketplace_source(webuild_home: &std::path::Path) {
    let config_path = webuild_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(webuild_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %webuild_home.join(".config-init.lock").display(),
    ...
    // (settings.json / known_marketplaces.json) under webuild_home. Scoped to
    // webuild_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = xai_webuild_plugin_marketplace::load_sources(&parsed);
    let json_sources = xai_webuild_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&webuild_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, xai_webuild_plugin_marketplace::SourceKind::Git { url, .. }
            if xai_webuild_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<xai_webuild_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        xai_webuild_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xai_webuild_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                xai_webuild_plugin_marketplace::SourceKind::Git { url, .. }
                    if xai_webuild_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xai_webuild_plugin_marketplace::SourceKind::Git { url, .. }
                if url == xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = xai_webuild_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
