...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&webuild_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("XAI_API_KEY") };
        } else {
            crate::auth::store_api_key(&webuild_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `webuild-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a webuild-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("webuild-asset:///{key}")),
        None => None,
    };
