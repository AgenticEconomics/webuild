...
/// back atomically. `webuild_root` is injectable for tests.
async fn repair_on_disk(webuild_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &webuild_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(webuild_root.to_path_buf());
