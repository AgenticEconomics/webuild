...
fn to_code_nav_ext_response(resp: xai_webuild_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for webuild-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
