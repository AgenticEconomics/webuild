...
mod tests {
    ...
    fn test_auth() -> crate::auth::WeBuildAuth {
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".webuild/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".webuild/roles")).unwrap();
        std::fs::write(
            project_root.join(".webuild/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".webuild/roles/reviewer.toml"),
