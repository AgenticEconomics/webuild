...
fn marker_is_not_a_managed_artifact() {
    assert!(
        !MANAGED_ARTIFACT_FILES.contains(&xai_webuild_config::MANAGED_CONFIG_CACHE_FILE),
...
fn purge_keeps_marker_when_an_artifact_removal_fails() {
    ...
    std::fs::write(home.join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE), "{}").unwrap();
...
fn served_principal_prefers_deployment_id() {
    use xai_webuild_config::signed_policy::SignedPayload;
    let payload = |dep: Option<&str>, team: Option<&str>| SignedPayload {
        typ: xai_webuild_config::signed_policy::MANAGED_POLICY_TYP.into(),
        version: 1,
...
fn claim_persists_only_when_bound_to_served_principal() {
    let claim = |principal: &str| xai_webuild_config::signed_policy::ManagedIdentityClaim {
        typ: xai_webuild_config::signed_policy::MANAGED_IDENTITY_TYP.into(),
