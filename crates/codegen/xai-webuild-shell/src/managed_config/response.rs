...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that WEBUILD_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `webuild login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.webuild. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<xai_webuild_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<xai_webuild_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            xai_webuild_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[xai_webuild_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: xai_webuild_config::signed_policy::SignatureEnvelope,
    pub(super) payload: xai_webuild_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use xai_webuild_config::signed_policy;
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use xai_webuild_config::signed_policy::SignatureEnvelope;
