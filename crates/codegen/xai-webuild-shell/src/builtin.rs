//! Built-in files extracted to `~/.webuild/` on startup.
...
///
/// These directories under `~/.webuild/skills/` will be deleted on startup
...
/// `<webuild_home>/skills/<name>/SKILL.md`. Exact-path (not prefix) so a
/// user-authored skill that reuses a bundled name — even elsewhere under
/// `<webuild_home>/skills/` — is never labeled bundled. Lives beside the
...
pub(crate) fn is_extracted_bundled_skill(
    name: &str,
    path: &std::path::Path,
    webuild_home: &std::path::Path,
) -> bool {
    BUNDLED_SKILLS.iter().any(|&(n, _)| n == name)
        && path == webuild_home.join("skills").join(name).join("SKILL.md")
}

/// Resolve the content for a skill, applying any name-specific transforms.
fn resolve_skill_content(name: &str, raw: &str, webuild_home: &std::path::Path) -> String {
    match name {
        // Help skill needs path substitution so absolute paths work.
        "help" => {
            let webuild_home_str = format!("{}/", webuild_home.to_string_lossy());
            raw.replace("~/.webuild/", &webuild_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.webuild/` on startup.
///
...
pub fn extract_bundled_files(webuild_home: &std::path::Path) {
    ...
    remove_legacy_bundled_skills(webuild_home);

    let version = xai_webuild_version::VERSION;
    let marker = webuild_home.join(".metadata_version");
        ...
        && existing.trim() == version
    {
        ...
        extract_missing_skills(webuild_home);
        return;
    }
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(webuild_home.join(stale));
    }

    for &(filename, content) in BUNDLED_FILES {
        if let Err(e) = std::fs::write(webuild_home.join(filename), content) {
    ...
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_dir = webuild_home.join("skills").join(name);
        let _ = std::fs::create_dir_all(&skill_dir);
        let content = resolve_skill_content(name, raw, webuild_home);
...
fn extract_missing_skills(webuild_home: &std::path::Path) {
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_md = webuild_home.join("skills").join(name).join("SKILL.md");
        if skill_md.exists() {
            continue;
        }
...
fn remove_legacy_bundled_skills(webuild_home: &std::path::Path) {
    remove_legacy_skills(webuild_home, LEGACY_BUNDLED_SKILL_NAMES, BUNDLED_SKILLS);
}
...
fn remove_legacy_skills(
    ...
    for name in legacy_names {
        ...
        let dir = webuild_home.join("skills").join(name);
        if dir.exists() {
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".webuild").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".webuild/skills/help/SKILL.md"),
        )
        .unwrap();
