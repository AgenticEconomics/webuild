...
use xai_webuild_agent::plugins::discovery::PluginScope;
use xai_webuild_agent::plugins::git_install::{self, UpdateStatus};
use xai_webuild_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use xai_webuild_plugin_marketplace::git::{self, SourceCacheLease};
use xai_webuild_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.webuild/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &xai_webuild_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `webuild plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `webuild plugin install ./{name}`, or add a \
                     source with `webuild plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `webuild plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `webuild plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    xai_webuild_config::load_effective_config_disk_only()
        .map(|c| xai_webuild_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| xai_webuild_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &xai_webuild_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.webuild/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user webuild via user_webuild_home() (None when no home resolves) and
    // home separately, so removal still runs from $WEBUILD_HOME when no home dir
    // exists, and never touches a cwd-relative .webuild.
    let home = dirs::home_dir();
    let webuild = xai_webuild_config::user_webuild_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref webuild) = webuild {
        settings_candidates.push(webuild.join("settings.local.json"));
        settings_candidates.push(webuild.join("settings.json"));
    }
    ...
    if let Some(ref webuild) = webuild {
        known_candidates.push(webuild.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("webuild plugin install ./sentry"), "{msg}");
        assert!(msg.contains("webuild plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("webuild plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> xai_webuild_workspace::permission::resolution::MarketplaceAllowlist {
        xai_webuild_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = xai_webuild_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
