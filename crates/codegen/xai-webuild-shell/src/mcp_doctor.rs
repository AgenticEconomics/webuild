//! `webuild mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = webuild_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.webuild/config.toml".to_string(),
...
/// Discover managed `webuild_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&webuild_home, webuild_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `webuild login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use xai_webuild_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `webuild mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `webuild mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `webuild mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `webuild mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
