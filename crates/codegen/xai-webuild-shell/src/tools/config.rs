...
use xai_webuild_sampler::SamplerConfig;
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `WEBUILD_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("WEBUILD_WEB_FETCH_PROXY"))
...
/// from `xai_webuild_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `WeBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `WeBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "WeBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "WeBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"WeBuild:read_file"));
        assert!(ids.contains(&"WeBuild:search_replace"));
        assert!(ids.contains(&"WeBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"WeBuildHashline:hashline_read"));
        assert!(ids.contains(&"WeBuildHashline:hashline_edit"));
        assert!(ids.contains(&"WeBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            xai_webuild_agent::config::AgentDefinition::plan(),
            xai_webuild_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "WeBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"WeBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"WeBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"WeBuildHashline:hashline_edit"),
