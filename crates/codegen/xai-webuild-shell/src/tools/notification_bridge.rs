//! Notification bridge: translates `xai-webuild-tools` `ToolNotification` events
//! into `xai-webuild-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use xai_webuild_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use xai_webuild_tools::types::output::{BashOutput, ToolOutput};
...
pub struct NotificationBridgeConfig {
    ...
    pub auto_wake_delivered: xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds,
    ...
    /// next-turn exit reminder. WeBuild-build leaves this `false` — its
...
async fn emit_current_mode_update(
    ...
    mode: xai_webuild_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == xai_webuild_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    xai_webuild_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    xai_webuild_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = xai_webuild_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, xai_webuild_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, xai_webuild_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use xai_webuild_tools::computer::types::TaskKind;
    use xai_webuild_tools::types::TaskSnapshot;
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(xai_webuild_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            xai_webuild_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            xai_webuild_tools::notification::types::BashOutputChunk {
                base: xai_webuild_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            xai_webuild_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            xai_webuild_tools::notification::types::BashExecutionBackgrounded {
                base: xai_webuild_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            xai_webuild_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(xai_webuild_tools::notification::types::PlanModeExited {
    ...
    /// Default (webuild) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(xai_webuild_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-webuild".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            xai_webuild_tools::notification::types::PlanModeEntered {
