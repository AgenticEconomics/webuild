...
/// Resolved absolute path to bash. On Unix uses the `xai_webuild_config` shell
/// resolution cascade (`$WEBUILD_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_webuild_config::shell::unix_shell_path(xai_webuild_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-webuild-tools`.
pub use xai_webuild_tools::util::pager_env;
...
/// xai-webuild-pager which renders its own UI and doesn't need raw ANSI codes).
///
