...
/// Path format: `~/.webuild/sessions/{session_id}/tasks/{task_id}.log`
pub fn get_task_output_path(session_id: &str, task_id: &str) -> PathBuf {
    use crate::util::webuild_home::webuild_home;

    let tasks_dir = webuild_home().join("sessions").join(session_id).join("tasks");
...
pub struct BackgroundTaskManifestEntry {
    ...
    pub cwd: String,
    #[serde(default)]
    pub kind: xai_webuild_tools::computer::types::TaskKind,
}
...
pub fn format_resumed_tasks_reminder(entries: &[BackgroundTaskManifestEntry]) -> String {
    ...
    for entry in entries {
        ...
        let kind_label = match entry.kind {
            xai_webuild_tools::computer::types::TaskKind::Monitor => " [monitor]",
...
mod tests {
    ...
    fn format_reminder_labels_monitor_tasks() {
        ...
        entry.kind = xai_webuild_tools::computer::types::TaskKind::Monitor;
        let reminder = format_resumed_tasks_reminder(&[entry]);
