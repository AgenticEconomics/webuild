//! AcpTerminalAdapter: implements `xai-webuild-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates xai-webuild-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (webuild-desktop) spawns with `shell: true`
...
/// Wraps xai-webuild-shell's ACP gateway to satisfy xai-webuild-tools' TerminalBackend.
