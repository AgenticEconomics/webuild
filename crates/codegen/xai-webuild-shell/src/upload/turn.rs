...
pub(crate) use xai_webuild_telemetry::session_metrics::TraceUploadReason;
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<xai_webuild_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match xai_webuild_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = % def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return xai_webuild_agent::discovery::by_name(name);
    }
    tracing::warn!(
