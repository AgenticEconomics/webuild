...
use xai_webuild_workspace::permission::PermissionEvent;
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        xai_webuild_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        xai_webuild_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = xai_webuild_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: xai_webuild_sandbox::is_active(),
    })
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&xai_webuild_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        xai_webuild_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(xai_webuild_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[xai_webuild_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn xai_webuild_auth::AuthCredentialProvider>> {
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    xai_webuild_workspace::recovery::run_startup_recovery(&webuild_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    xai_webuild_workspace::recovery::purge_spilled_items(&webuild_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::webuild_home::webuild_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(webuild_home, resolver, UploadRetryPolicy::default());
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let webuild_com_config = WeBuildComConfig::default();
        let scope = webuild_com_config.auth_scope();
        let initial_auth = WeBuildAuth {
        ...
        let refreshed_auth = WeBuildAuth {
    ...
    async fn resolve_async_falls_back_when_no_refresher() {
        ...
        let auth_json = serde_json::to_string_pretty(&store).unwrap();
        std::fs::write(dir.path().join("auth.json"), &auth_json).unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), webuild_com_config));
        let resolver = DynamicResolver {
            auth_manager,
            base_config: TraceExportConfig {
                bucket_url: None,
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::WeBuildAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        let webuild_com_config = WeBuildComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(dir.path(), webuild_com_config));
        auth_manager.hot_swap(WeBuildAuth {
            key: "expired-oidc".into(),
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::WeBuildAuth {
