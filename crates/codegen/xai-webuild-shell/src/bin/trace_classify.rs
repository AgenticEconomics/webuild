...
//!       [--api-key <key> | $XAI_API_KEY | <webuild-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use xai_webuild_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// `$XAI_API_KEY`, then `<webuild-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$WEBUILD_HOME` or `~/.webuild`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    webuild_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.webuild_home.is_none());
    }
    ...
    #[test]
    fn cli_webuild_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.webuild_home, Some(PathBuf::from("/tmp/scratch-webuild")));
    }
