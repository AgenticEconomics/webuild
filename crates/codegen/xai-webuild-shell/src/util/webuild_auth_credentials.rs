...
use std::sync::Arc;
/// Credentials for authenticating with webuild backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: xai-webuild-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct WeBuildAuthCredentials {
...
impl std::fmt::Debug for WeBuildAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WeBuildAuthCredentials")
            .field(
                "user_token",
...
impl WeBuildAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your WEBUILD_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `webuild login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> WeBuildAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> WeBuildAuthCredentials {
...
impl xai_webuild_auth::HttpAuth for WeBuildAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        WeBuildAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = WeBuildAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = WeBuildAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = WeBuildAuthCredentials::new(Some("static-token".into()));
