pub mod config;
pub mod webuild_auth_credentials;
pub mod hooks;

// The foundation utilities live in `xai-webuild-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `xai_webuild_shell::util::…` users compile
// unchanged.
pub use xai_webuild_shell_base::util::*;
