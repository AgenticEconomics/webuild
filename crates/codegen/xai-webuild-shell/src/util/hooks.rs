...
use xai_webuild_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .webuild/hooks/ directories are still scanned (they hold
    ...
    // user_webuild_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .webuild.
    ...
    if let Some(ref webuild) = webuild {
        global.push(webuild.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = webuild
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".webuild").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    xai_webuild_hooks::discovery::HookRegistry,
    Vec<xai_webuild_hooks::error::HookError>,
) {
