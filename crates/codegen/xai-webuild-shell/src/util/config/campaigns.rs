...
use xai_webuild_config::campaigns::{
    CampaignEntry, filter_active_campaigns, ids_touching_paths, merge_campaign_entries,
};
use xai_webuild_config::config_override::{PatchPath, patch_touches_any};
use xai_webuild_config::{
    CampaignsState, ConfigLayers, campaigns_state_path, load_dismissed_ids_from_home,
    user_webuild_home,
};
use xai_webuild_config_types::{CampaignOverride, RemoteSettings};
...
pub fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_webuild_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several webuild processes share `$WEBUILD_HOME`; the in-process mutex alone would
...
/// `WEBUILD_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("WEBUILD_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid WEBUILD_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `WEBUILD_CAMPAIGNS_OVERRIDE` (replaces all
...
/// `WEBUILD_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<xai_webuild_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    use xai_webuild_config::ConfigLayers;
    ...
    /// `WEBUILD_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("WEBUILD_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `WEBUILD_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("WEBUILD_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `WEBUILD_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("WEBUILD_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("WEBUILD_CAMPAIGNS", "0");
    ...
    /// `WEBUILD_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("WEBUILD_CAMPAIGNS_OVERRIDE", "[]");
