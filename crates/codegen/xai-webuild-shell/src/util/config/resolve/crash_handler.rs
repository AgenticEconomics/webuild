...
pub(crate) const ENV_CRASH_HANDLER: &str = "WEBUILD_CRASH_HANDLER";
...
/// Precedence: requirements > env (`WEBUILD_CRASH_HANDLER`) >
...
/// Merge system-managed policy (`/etc/webuild`) under home `managed_config.toml`
...
fn load_managed_toml_layers() -> Option<TomlValue> {
    ...
    match (system, managed) {
        ...
        (None, Some(m)) => Some(m),
        (Some(mut s), Some(m)) => {
            xai_webuild_config::deep_merge_toml(&mut s, &m);
            Some(s)
        }
    }
}
...
mod crash_handler_gate_tests {
    ...
    // `WEBUILD_CRASH_HANDLER` is process-global; serialize and force it unset at
