...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "WEBUILD_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `WEBUILD_AUTO_COMPACT_THRESHOLD_PERCENT`
...
///      (populated from `webuild_build_models[i].auto_compact_threshold_percent`;
...
///      (populated from `webuild_build_settings.auto_compact_threshold_percent`)
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "WEBUILD_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `WEBUILD_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes WEBUILD_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
