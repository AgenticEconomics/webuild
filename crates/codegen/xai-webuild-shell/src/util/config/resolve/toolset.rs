...
/// [`xai_webuild_config::env_bool`] parser (`WEBUILD_TOOLS_FIND_BFS` /
/// `WEBUILD_TOOLS_GREP_UGREP`, plus the `WEBUILD_FIND_BFS` / `WEBUILD_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.webuild/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`xai_webuild_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = xai_webuild_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = xai_webuild_config::env_bool(primary).or_else(|| xai_webuild_config::env_bool(alias));
    ...
    (
        resolve("WEBUILD_TOOLS_FIND_BFS", "WEBUILD_FIND_BFS", "find_bfs"),
        resolve("WEBUILD_TOOLS_GREP_UGREP", "WEBUILD_GREP_UGREP", "grep_ugrep"),
    )
}
...
/// the shared [`xai_webuild_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "WEBUILD_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`WEBUILD_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            xai_webuild_tools::implementations::webuild_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`WEBUILD_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        xai_webuild_tools::implementations::webuild_build::ask_user_question::response_timeout_env_secs(),
...
) -> xai_webuild_tools::implementations::webuild_build::ask_user_question::AskUserQuestionParams {
...
mod ask_user_question_timeout_tests {
    ...
    use xai_webuild_tools::implementations::webuild_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = xai_webuild_tools::implementations::webuild_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes WEBUILD_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
