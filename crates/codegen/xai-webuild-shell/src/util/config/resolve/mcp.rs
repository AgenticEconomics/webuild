...
/// | env          | `WEBUILD_MCP_LIVENESS_WATCHERS` (handled by `BoolFlag::env`)       |
/// | config       | `[features] mcp_liveness_watchers` in `~/.webuild/config.toml`     |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `WEBUILD_MCP_AUTO_RESTART` (handled by `BoolFlag::env`)            |
/// | config       | `[features] mcp_auto_restart` in `~/.webuild/config.toml`          |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `WEBUILD_MCP_PUSH_SERVER_STATUS` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_push_server_status` in `~/.webuild/config.toml`    |
...
/// | cli          | (none — no CLI flag)                                                |
/// | env          | `WEBUILD_MCP_RECURSIVE_CONFIG_WATCH` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_recursive_config_watch` in `~/.webuild/config.toml`    |
...
/// Kept in sync with `xai_webuild_mcp::servers`'s standalone fallback.
...
/// Env override for the MCP startup timeout, in seconds (webuild-native).
const ENV_MCP_STARTUP_TIMEOUT_SECS: &str = "WEBUILD_MCP_STARTUP_TIMEOUT_SECS";
...
/// `WEBUILD_MCP_STARTUP_TIMEOUT_SECS` secs) > effective `config.toml [mcp]` >
...
/// `WEBUILD_MCP_STARTUP_TIMEOUT_SECS` (secs). Unparseable/zero values are ignored.
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = xai_webuild_tools::MCP_MAX_OUTPUT_BYTES;
...
pub fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    xai_webuild_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
///   2. env `WEBUILD_MAX_MCP_OUTPUT_BYTES` / `MAX_MCP_OUTPUT_BYTES`
///      (WeBuild-native wins when both set)
...
pub fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        xai_webuild_tools::mcp_max_output_bytes_from_env(),
...
/// `.webuild/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = xai_webuild_config::load_config_file(&config_path)
...
pub fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || xai_webuild_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".webuild")).unwrap();
        std::fs::create_dir_all(root.join(".webuild")).unwrap();
        std::fs::write(
            root.join(".webuild/config.toml"),
        ...
        std::fs::write(sub.join(".webuild/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .webuild files with the key anywhere → None.
        std::fs::remove_file(root.join(".webuild/config.toml")).unwrap();
