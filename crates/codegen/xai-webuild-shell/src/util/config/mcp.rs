...
use xai_webuild_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use xai_webuild_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `xai-webuild-config-types` (config
...
pub use xai_webuild_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use xai_webuild_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.webuild/config.toml` at each level.
/// Project-scoped `.webuild/config.toml` entries override global `~/.webuild/config.toml`
...
/// `.webuild/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.webuild/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.webuild/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.webuild/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .webuild/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    xai_webuild_config::webuild_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    xai_webuild_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.webuild/config.toml`.
...
/// Upsert an MCP server entry in `~/.webuild/config.toml`.
///
...
/// config file, e.g. a project-scoped `.webuild/config.toml`.
...
/// Delete an MCP server entry from `~/.webuild/config.toml`.
///
...
/// config file, e.g. a project-scoped `.webuild/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = xai_webuild_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.webuild/config.toml` files.
...
// `.mcp.json` discovery moved to `xai-webuild-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use xai_webuild_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.webuild/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::webuild_home::webuild_home().join("config.toml")
}

/// Path to the user-level config file (`~/.webuild/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.webuild/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".webuild").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.webuild/skills", "~/.webuild/skills/special/SKILL.md"]
ignore = ["~/.webuild/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.webuild/skills", "~/.webuild/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.webuild/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${WEBUILD_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&webuild_dir).unwrap();
        std::fs::write(
            webuild_dir.join("config.toml"),
            r#"
