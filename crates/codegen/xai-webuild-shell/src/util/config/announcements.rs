...
/// Re-exported from `xai-webuild-announcements` for backward compatibility.
pub use xai_webuild_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `WEBUILD_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("WEBUILD_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid WEBUILD_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `WEBUILD_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
