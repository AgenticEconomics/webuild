...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("WEBUILD_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.webuild/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.webuild/config.toml` `[permission]` section
///
