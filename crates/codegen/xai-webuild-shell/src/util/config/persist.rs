...
/// Process-wide write lock for `~/.webuild/config.toml`.
///
...
mod tests {
    ...
    fn merge_section_updates_modeled_fields_preserving_unmodeled() {
        ...
        ui.insert(
            ...
            TomlValue::String("webuildday".into()),
        );
        table.insert("ui".into(), TomlValue::Table(ui));
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("webuildday"));
...
"#;
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("webuild-new".to_string()),
            ..Default::default()
        };
        merge_section(&mut table, "models", &cfg);
        let m = table.get("models").unwrap().as_table().unwrap();
        assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("webuild-new"));
        assert_eq!(
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("webuild-persisted".to_string());
        ...
        assert_eq!(reloaded.models.default.as_deref(), Some("webuild-persisted"));
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("webuild-persisted"));
    }
    ...
    mod resolve_auto_compact {
        ...
        /// Serialize tests that mutate `WEBUILD_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("webuildday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("webuildday".to_string()));
