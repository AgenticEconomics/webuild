...
/// env master `WEBUILD_CONTEXTUAL_HINTS` (all-on/off) > user config
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("WEBUILD_CONTEXTUAL_HINTS")
            .config(user)
...
fn merge_hints_config_layers(
    ...
    if let Some(m) = managed {
        xai_webuild_config::deep_merge_toml(&mut merged, m);
    }
    if let Some(u) = user {
        xai_webuild_config::deep_merge_toml(&mut merged, u);
    }
    if let Some(r) = requirements {
        xai_webuild_config::deep_merge_toml(&mut merged, r);
    }
    merged
}
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "WEBUILD_CONTEXTUAL_HINTS";

    // `WEBUILD_CONTEXTUAL_HINTS` is process-global; serialize the tests reading it
