//! Shim — see `xai_webuild_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `xai_webuild_shell::instrumentation_timer!`). Keeping the macro here
...
pub use xai_webuild_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `xai_webuild_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `xai_webuild_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `xai_webuild_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `xai_webuild_telemetry::instrumentation`.
#[macro_export]
