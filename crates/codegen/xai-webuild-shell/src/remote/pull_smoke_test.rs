...
//!
//! Run with: `cargo test -p xai-webuild-shell -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<WeBuildAuth> {
        let path = crate::util::webuild_home::webuild_home().join("auth.json");
        let contents = std::fs::read_to_string(&path).ok()?;
        let store: BTreeMap<String, WeBuildAuth> = serde_json::from_str(&contents).ok()?;
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `webuild login`");
