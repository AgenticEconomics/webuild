...
/// Body for `PUT /rest/app-chat/conversations/{id}` (webuild-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("WEBUILD_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("WEBUILD_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| WEBUILD_WEB_URL.to_string());
        Self {
    ...
    async fn require_xai_auth(&self) -> Result<WeBuildAuth, ConvError> {
    ...
    fn apply_auth_headers(
        ...
        let mut builder = builder
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.webuild_com_config().token_header.clone(),
            )
