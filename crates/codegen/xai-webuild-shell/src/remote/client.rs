...
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("WEBUILD_CODE_WEB_URL").unwrap_or_else(|_| WEBUILD_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-XAI-Token-Auth", WeBuildComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-webuild-client-version", xai_webuild_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::webuild_auth_credentials::WeBuildAuthCredentials::new(
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn xai_webuild_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("WEBUILD_CODE_BACKEND_URL")
                .unwrap_or_else(|_| WEBUILD_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&WeBuildAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn xai_webuild_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<WeBuildAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_webuild_cli_token.
    /// See: crates/codegen/xai-webuild-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-XAI-Token-Auth",
            required(&WeBuildComConfig::default().token_header, "X-XAI-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-webuild-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-webuild-client-version",
            HeaderValue::from_static(xai_webuild_version::VERSION),
        );
        Ok(headers)
    }
...
/// Fetch `webuild_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-webuild-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(xai_webuild_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-webuild-agent-id", agent_id)
        .header("x-webuild-client-version", xai_webuild_version::VERSION)
        .header(
            "x-webuild-client-identifier",
            crate::http::process_client_identifier(),
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-XAI-Token-Auth", "xai-webuild-cli")
                .header("x-userid", &auth.user_id)
                .header("x-webuild-client-version", xai_webuild_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| xai_webuild_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(xai_webuild_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<xai_webuild_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<xai_webuild_sampling_types::CompactionsRemaining> {
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-webuild-agent-id"),
                            client_identifier: header_str(&headers, "x-webuild-client-identifier"),
                            client_version: header_str(&headers, "x-webuild-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-webuild-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-webuild-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-webuild-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::WeBuildComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("xai-webuild-cli"));
    ...
    /// REGRESSION: `webuild setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("WEBUILD_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("WEBUILD_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
