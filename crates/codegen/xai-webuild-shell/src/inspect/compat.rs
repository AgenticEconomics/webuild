//! Vendor-compat resolution for `webuild inspect`.
//!
//! Resolves the local env/config/default stack into a diagnostic report.

use serde::Serialize;
use xai_webuild_tools::types::compat::{COMPAT_CELLS, CompatCell, CompatConfig};
...
/// `None` for native `.webuild`/`.agents` paths.
...
pub(super) fn resolve_inspect_compat(
    ...
    resolve_inspect_compat_with_env(effective_config, |cell| {
        xai_webuild_config::env_bool(cell.env_var())
    })
}
...
mod tests {
    ...
    fn inspect_compat_uses_env_config_default_precedence() {
        ...
        let report = resolve_inspect_compat_with_env(Ok(&effective_config), |cell| {
            (cell.env_var() == "WEBUILD_CURSOR_SKILLS_ENABLED").then_some(true)
        });
