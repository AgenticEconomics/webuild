//! `webuild inspect` — configuration introspection.
//!
//! Shows everything WeBuild discovers in the current directory: project
...
use xai_webuild_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[webuild_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `WEBUILD_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .webuild/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `webuild inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = xai_webuild_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = xai_webuild_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = xai_webuild_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        webuild_version: xai_webuild_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".webuild")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = xai_webuild_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        xai_webuild_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let webuild_home = Some(crate::util::webuild_home::webuild_home());
    ...
    // `xai_webuild_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both WeBuild TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use xai_webuild_workspace::permission::resolution;
...
/// (`[webuild_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let webuild_com_config = config
        .map(|c| c.webuild_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: webuild_com_config.api_key_auth_disabled(),
        disable_api_key_auth: webuild_com_config.disable_api_key_auth,
        force_login_team_uuid: webuild_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[xai_webuild_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = xai_webuild_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        xai_webuild_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);

    let home_dir = dirs::home_dir();
    let webuild_home = xai_webuild_config::webuild_home();

    let mut entries: Vec<HookEntry> = registry
        .all_hooks()
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&webuild_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &xai_webuild_agent::plugins::PluginRegistry,
    skills_config: &xai_webuild_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = xai_webuild_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &webuild_home);
...
/// `<webuild_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
fn skill_entry_source(
    s: &xai_webuild_agent::prompt::skills::SkillInfo,
    webuild_home: &Path,
) -> ConfigSource {
    use xai_webuild_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, webuild_home) {
...
fn list_agents(
    ...
    let agents = xai_webuild_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[xai_webuild_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                xai_webuild_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                xai_webuild_agent::plugins::PluginScope::Project => Scope::Project,
                xai_webuild_agent::plugins::PluginScope::User => Scope::User,
                xai_webuild_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: xai_webuild_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    xai_webuild_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = xai_webuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `webuild mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.webuild/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_webuild_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.webuild_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `webuild mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use xai_webuild_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use xai_webuild_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.webuild/rules/team.md", r"C:\repo\.webuild\rules\team.md"] {
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.webuild/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.webuild");

        let s = skill_fixture("a", "/repo/.webuild/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.webuild/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.webuild/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.webuild/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.webuild/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<webuild_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.webuild/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.webuild/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        // Bundled name in a user dir outside <webuild_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <webuild_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.webuild/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <webuild_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.webuild/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.webuild dirs.
        ...
        let registry = xai_webuild_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
