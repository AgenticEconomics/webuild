...
//!   under `~/.webuild/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.webuild/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.webuild/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.webuild/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.webuild/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.webuild/`.
...
/// `CleanAll` mode. Placed under `~/.webuild/worktree_pool/<instance_id>/<uuid>/`.
