...
//!
//! These tests verify the parallel dispatch path (WEBUILD_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and webuild_build's
...
#[test]
fn lock_path_for_args_matches_webuild_build_file_path() {
    // webuild_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_webuild_build_target_file() {
    // webuild_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_webuild_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. webuild_build search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&webuild), lock_path_for_args(&compat));
}
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use xai_webuild_sampling_types::{ConversationItem, SyntheticReason};
