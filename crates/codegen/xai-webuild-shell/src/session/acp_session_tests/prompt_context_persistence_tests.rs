...
fn test_json_round_trip() {
    let ctx = xai_webuild_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: xai_webuild_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `webuild_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = xai_webuild_agent::PromptContext::default();
    ...
    let loaded: xai_webuild_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a WeBuild subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: xai_webuild_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a WeBuild subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<xai_webuild_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/webuild-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
fn build_truncated_preserves_small_query_truncates_context() {
    ...
    assert!(
        ...
        "webuild ordering: query block first"
    );
...
//
// `webuild_home()` is a process-wide `OnceLock`, so the real async method is
