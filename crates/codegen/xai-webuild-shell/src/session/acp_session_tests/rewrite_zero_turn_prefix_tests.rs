...
/// Local skill is always listable, so it renders into the webuild markdown skill
/// catalog when the pending baseline is drained for a different agent.
fn regression_skill() -> xai_webuild_tools::implementations::skills::types::SkillInfo {
...
/// Regression: a zero-turn agent rebuild INTO a webuild/Default agent
/// must re-inject the baseline skill `<system-reminder>`. `initialize()`
/// is otherwise the only place skills are surfaced for the webuild agent, so
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "reminder must carry the webuild skill catalog header:\n{text}",
            );
            assert!(
