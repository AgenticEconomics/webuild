...
//! handshake against rmcp 2.1 in `xai-webuild-mcp/tests/repro_sse_flood.rs`): a
...
use xai_webuild_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "webuild_com_testconnector";
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use xai_webuild_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `webuild_com_*` tools enter entry-B.
