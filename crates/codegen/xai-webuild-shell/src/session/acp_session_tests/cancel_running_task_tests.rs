...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(xai_webuild_sampler::SamplerConfig {
...
async fn cancel_running_task_teardown_clears_running_and_pending_work() {
    ...
    local
        .run_until(async {
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                xai_webuild_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = xai_hunk_tracker::HunkTrackerActor::spawn(
            ...
            agent
                .tool_bridge()
                .update_resource(
                    xai_webuild_tools::implementations::webuild_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![xai_webuild_sampling_types::ToolCall {
                    id: "call-1".into(),
...
async fn cancel_propagates_to_sampler_handle_so_no_further_emission() {
    ...
    local
        .run_until(async {
            ...
            let cfg = xai_webuild_sampler::SamplerConfig {
                api_key: Some("test-key".to_string()),
                ...
                temperature: None,
                top_p: None,
                api_backend: xai_webuild_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                client_version: None,
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                xai_webuild_sampler::SamplingEvent,
            >();
            let sampler_handle = xai_webuild_sampler::SamplerActor::spawn(
                cfg,
                xai_webuild_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                xai_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![
                    ConversationItem::User(xai_webuild_sampling_types::UserItem { content :
                    vec![xai_webuild_sampling_types::ContentPart::Text { text : "hi".into(),
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use xai_webuild_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, xai_webuild_sampling_types::ContentPart::Text { text } if
