...
use xai_webuild_tools::registry::types::{SessionContext, ToolConfig, ToolServerConfig};
...
async fn web_search_uses_model_override_from_config_end_to_end() {
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("webuild-web-search-e2e"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("webuild-web-search-e2e/state.json"),
        memory_backend: None,
        web_search_config: xai_webuild_tools::implementations::web_search::WebSearchConfig::Enabled {
...
async fn web_search_errors_when_configured_model_cannot_be_resolved() {
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: std::env::temp_dir(),
        session_folder: std::env::temp_dir().join("webuild-web-search-disabled"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("webuild-web-search-disabled/state.json"),
        memory_backend: None,
        web_search_config: xai_webuild_tools::implementations::web_search::WebSearchConfig::Disabled,
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: Default::default(),
        video_gen_config: Default::default(),
