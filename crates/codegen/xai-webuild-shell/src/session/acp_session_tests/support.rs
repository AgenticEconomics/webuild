...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> xai_webuild_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> xai_webuild_agent::Agent {
    use xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalTool;
    use xai_webuild_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// WeBuild-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_webuild_build_agent_with_todo() -> xai_webuild_agent::Agent {
    use xai_webuild_tools::implementations::webuild_build::todo::TodoWriteTool;
    use xai_webuild_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> xai_webuild_agent::Agent {
    use xai_webuild_tools::implementations::webuild_build::enter_plan_mode::EnterPlanModeTool;
    use xai_webuild_tools::implementations::webuild_build::exit_plan_mode::ExitPlanModeTool;
    use xai_webuild_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<xai_webuild_tools::registry::types::ToolConfig>,
) -> xai_webuild_agent::Agent {
    test_agent_from_config(
        xai_webuild_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        xai_webuild_agent::AgentDefinition::default_webuild_build(),
        std::sync::Arc::new(xai_webuild_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: xai_webuild_agent::AgentDefinition,
    backend: std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>,
) -> xai_webuild_agent::Agent {
    use xai_webuild_tools::computer::local::LocalFs;
    use xai_webuild_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("webuild-test"),
    ...
    xai_webuild_agent::Agent::new(
        definition,
        xai_webuild_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        xai_webuild_agent::ReminderPolicy::default(),
        xai_webuild_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = xai_webuild_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(xai_webuild_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let actor = SessionActor {
        ...
        permissions: xai_webuild_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
