...
/// attribution.
fn auth_error() -> xai_webuild_sampler::SamplingErrorInfo {
    xai_webuild_sampler::SamplingErrorInfo {
        kind: xai_webuild_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> xai_webuild_sampler::SamplingErrorInfo {
    xai_webuild_sampler::SamplingErrorInfo {
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `webuild logout`, got: {msg}"
            );
            assert!(
                msg.contains("webuild login"),
                "hint must mention `webuild login`, got: {msg}"
            );
            assert!(
