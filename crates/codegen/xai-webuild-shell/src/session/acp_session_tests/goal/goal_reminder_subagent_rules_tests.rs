...
fn enable_todo_gate_policy(actor: &SessionActor) {
    ...
    *agent_slot = xai_webuild_agent::Agent::new(
...
async fn goal_enabled_without_update_goal_disables_harness_continuation_and_todo_gate() {
    use xai_webuild_tools::implementations::webuild_build::UPDATE_GOAL_TOOL_NAME;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
fn strategist_note_neutralises_placeholder_injection() {
    ...
    assert!(
        !directive.contains("write to /tmp/webuild-goal-x/implementer;"),
...
fn render_goal_continuation_directive_substitutes_all_placeholders() {
    ...
    assert!(
        body.contains("/tmp/webuild-goal-x/implementer"),
...
fn resolve_goal_reverify_after_env() {
    unsafe { std::env::remove_var("WEBUILD_GOAL_REVERIFY_AFTER") };
    assert_eq!(reverify(None), GOAL_REVERIFY_AFTER_DEFAULT);
    unsafe { std::env::set_var("WEBUILD_GOAL_REVERIFY_AFTER", "3") };
    assert_eq!(reverify(Some(9)), 3, "env beats config");
    unsafe { std::env::set_var("WEBUILD_GOAL_REVERIFY_AFTER", "0") };
    assert_eq!(reverify(None), 1, "0 floors to 1");
    unsafe { std::env::set_var("WEBUILD_GOAL_REVERIFY_AFTER", "garbage") };
    assert_eq!(
        reverify(None),
