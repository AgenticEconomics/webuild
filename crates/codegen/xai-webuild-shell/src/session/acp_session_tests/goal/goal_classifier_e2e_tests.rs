...
//!
//! Tests that mutate `WEBUILD_GOAL_CLASSIFIER` carry the
...
use xai_webuild_tools::implementations::webuild_build::update_goal::{RejectReason, UpdateGoalInput};
const ENV_FLAG: &str = "WEBUILD_GOAL_CLASSIFIER";
...
struct MockCoordinator {
    ...
    describe_outcome: StdArc<
        parking_lot::Mutex<
            xai_webuild_tools::implementations::webuild_build::task::types::SubagentDescribeOutcome,
        >,
    >,
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(xai_webuild_tools::implementations::webuild_build::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
fn seed_channel_with_acks(
    ...
    tokio::sync::oneshot::Receiver<
        xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck,
    >,
> {
    let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
    *actor.goal_update_rx.borrow_mut() = Some(rx);
...
async fn goal_classifier_stall_early_exit_pauses_with_no_progress() {
    ...
    local
        .run_until(async {
            ...
            let (actor, tmp) = make_actor(Some(coord.tx.clone()), true).await;
            use xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck;
...
async fn goal_classifier_env_override_disables_when_remote_enabled() {
    ...
    assert!(
        !resolved.value,
        "WEBUILD_GOAL_CLASSIFIER=0 must disable even with remote=true",
    );
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck::Accepted
                { .. },)
            );
        })
        .await;
...
fn render_ack_classifier_achieved_is_success() {
    use xai_webuild_tools::implementations::webuild_build::update_goal::{
...
use xai_webuild_tools::implementations::webuild_build::task::types::SubagentDescribeOutcome;
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                xai_webuild_tools::implementations::webuild_build::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                xai_webuild_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                xai_webuild_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
