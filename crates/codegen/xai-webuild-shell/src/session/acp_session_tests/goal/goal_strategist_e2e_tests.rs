...
//!
//! Tests mutate `WEBUILD_GOAL_CLASSIFIER` so they carry `serial`.
...
use xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<xai_webuild_tools::implementations::webuild_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        xai_webuild_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
