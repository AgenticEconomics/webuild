    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                xai_webuild_tools::implementations::webuild_build::update_goal::envelope_for_test(
                    xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalInput {
                        completed: Some(true),
                        message: None,
                        blocked_reason: None,
                    },
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(xai_webuild_tools::implementations::webuild_build::update_goal::envelope_for_test(xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
...
-> xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalEnvelope {
    let input = xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalInput {
        completed: Some(true),
        message: None,
        blocked_reason: None,
    };
