use super::support::*;
use super::*;
use xai_webuild_tools::reminders::task_completion::consumed_completion_ids;
use xai_webuild_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use xai_tool_types::{
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use xai_webuild_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
...
struct OneTaskTerminal {
    tasks: Vec<xai_webuild_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl xai_webuild_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: xai_webuild_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        xai_webuild_tools::computer::types::TerminalRunResult,
        xai_webuild_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<xai_webuild_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> xai_webuild_tools::computer::types::KillOutcome {
        xai_webuild_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<xai_webuild_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<xai_webuild_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> xai_webuild_tools::computer::types::TaskSnapshot {
    xai_webuild_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
