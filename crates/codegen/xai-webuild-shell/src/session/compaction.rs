...
use xai_webuild_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `WEBUILD_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("WEBUILD_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    use xai_webuild_sampling_types::ConversationItem;
    #[test]
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("WEBUILD_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("WEBUILD_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG WEBUILD_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                xai_webuild_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::AutoCompactSuppressed {
    ...
    async fn run_compact_inner(
        ...
        trigger: xai_webuild_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            xai_webuild_telemetry::events::CompactionTrigger::Manual => "manual",
            xai_webuild_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = xai_webuild_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            xai_webuild_hooks::event::HookEventName::PreCompact,
            xai_webuild_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<xai_webuild_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<xai_webuild_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(xai_webuild_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<xai_webuild_sampling_types::HostedTool> =
            if use_backend_search {
        ...
        let auto_trigger = matches!(trigger, xai_webuild_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = xai_webuild_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match xai_webuild_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(xai_webuild_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(xai_webuild_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(xai_webuild_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::CompactionRetryDegraded {
                                    trigger,
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                xai_webuild_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                xai_webuild_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use xai_webuild_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn xai_webuild_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn xai_webuild_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : xai_webuild_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        xai_webuild_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            xai_webuild_hooks::event::HookEventName::PostCompact,
            xai_webuild_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &xai_webuild_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                xai_webuild_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<xai_webuild_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use tokio::sync::mpsc;
    use xai_webuild_paths::AbsPathBuf;
    use xai_webuild_workspace::file_system::MockFs;
    use xai_webuild_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        let chat_state_handle = xai_chat_state::ChatStateActor::spawn(
            vec![],
            xai_webuild_sampling_types::SamplingConfig {
        ...
        SessionActor {
            ...
            vcs_kind: xai_webuild_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: xai_webuild_sampler::SamplerHandle::noop(),
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use xai_webuild_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): WeBuild usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("WeBuild usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> xai_webuild_sampler::SamplingErrorInfo {
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = xai_webuild_sampler::SamplingErrorInfo {
                    kind: xai_webuild_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
