...
use xai_webuild_workspace::session::file_state::RewindPoint;

use crate::session::signals::SessionSignals;
use crate::session::storage::{JsonlStorageAdapter, StorageAdapter};
use crate::tools::todo::TodoState;
use crate::util::webuild_home::webuild_home;
...
/// A feedback entry persisted to `~/.webuild/sessions/.../feedback.jsonl`.
///
...
pub use xai_webuild_shared::session::session_dir;
...
pub fn session_exists_for_cwd(session_id: &str, cwd: &str) -> bool {
    let sessions_root = crate::util::webuild_home::webuild_home().join("sessions");
...
fn session_exists_for_cwd_in_root(session_id: &str, cwd: &str, sessions_root: &Path) -> bool {
    let encoded = crate::util::webuild_home::encode_cwd_dirname(cwd);
...
/// `webuild -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `webuild -r <remote_id>` before this fix was deployed.
...
/// Searches across ALL cwd directories under `~/.webuild/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.webuild/sessions/`.
///
...
fn resolve_local_session_any_cwd_in_root(session_id: &str, sessions_root: &Path) -> Option<String> {
    ...
    for entry in entries.flatten() {
        ...
        if is_persisted_session_dir(&session_path) {
            ...
            if let Some(decoded) = crate::util::webuild_home::decode_cwd_from_dirname(&path) {
                return Some(decoded);
            }
        }
    }
...
pub fn find_session_dir_by_id(session_id: &str) -> Option<PathBuf> {
    let sessions_root = webuild_home().join("sessions");
...
/// Separated so tests can use a tempdir without touching the real webuild home.
...
pub fn find_summary_by_session_id(session_id: &str) -> Option<Summary> {
    find_summary_by_session_id_in_root(session_id, &webuild_home().join("sessions"))
}
...
pub fn resumed_session_sandbox_profile(
    ...
    resumed_session_sandbox_profile_in_root(session_id, cwd, &webuild_home().join("sessions"))
}
...
pub struct Summary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_id: Option<String>,
    /// Absolute path to the `.webuild` directory, used by reconstruction.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub webuild_home: Option<String>,
...
/// Current `webuild_home` as a UTF-8 string, or `None` if the path isn't valid UTF-8.
pub fn webuild_home_string() -> Option<String> {
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            xai_webuild_workspace::session::git::resolve_persisted_session_git_metadata_sync(
        ...
        Ok(Self {
            ...
            request_id: None,
            webuild_home: webuild_home_string(),
            last_active_at: None,
...
/// Collect MCP server stderr logs from `~/.webuild/logs/mcp/` for inclusion in the session archive.
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = xai_webuild_config::webuild_home().join("logs").join("mcp");
        ...
        StorageMode::Writeback => {
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'webuild login' first.",
                )
            })?;
...
/// worktree containing this session's cwd so `webuild worktree gc` expires by
...
pub async fn list_summaries(cwd: Option<&str>) -> io::Result<Vec<Summary>> {
    let root_dir = crate::util::webuild_home::webuild_home();
    let storage: Box<dyn StorageAdapter> = Box::new(JsonlStorageAdapter::with_root(root_dir));
...
/// Walk `~/.webuild/sessions/` and delete files with mtime older than `ttl_days`.
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "xai_webuild_shell::session::persistence",
...
mod session_exists_tests {
    ...
    fn returns_false_when_root_does_not_exist() {
        let root = std::path::PathBuf::from("/nonexistent/webuild/sessions");
...
mod session_exists_for_cwd_tests {
    ...
    fn session_under_different_cwd_is_not_considered_present() {
        ...
        let encoded_a = crate::util::webuild_home::encode_cwd_dirname("/project/alpha");
    ...
    fn resolve_local_session_any_cwd_skips_stub_and_finds_real() {
        ...
        let encoded_a = crate::util::webuild_home::encode_cwd_dirname(cwd_a);
        let dir_a = root.join(&encoded_a).join(session_id);
        fs::create_dir_all(&dir_a).unwrap();
        fs::write(dir_a.join("summary.json"), b"{}").unwrap();
        ...
        let encoded_b = crate::util::webuild_home::encode_cwd_dirname(cwd_b);
...
mod find_local_child_tests {
    ...
    fn returns_none_when_no_child_exists() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("sessions");
        let encoded = crate::util::webuild_home::encode_cwd_dirname("/work");
    ...
    /// Regression: a second `webuild -r <remote_id>` must return the existing child
...
mod resolve_local_session_tests {
    ...
    // setting up the on-disk structures under a fake webuild home.
    ...
    fn exact_match_returns_original_id() {
        ...
        // (We can't call resolve_local_session directly because it uses webuild_home(),
    ...
    fn no_match_returns_none() {
        ...
        fs::create_dir_all(root.join(crate::util::webuild_home::encode_cwd_dirname(cwd))).unwrap();
...
mod repo_wide_resolution_tests {
    ...
    fn skips_images_only_stub_and_resolves_real_sibling() {
        ...
        let encoded = crate::util::webuild_home::encode_cwd_dirname(exact_cwd);
