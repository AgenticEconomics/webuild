...
use xai_webuild_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use xai_webuild_agent::prompt::skills::SkillsConfig;
use xai_webuild_sampler::SamplerConfig as SamplingConfig;
use xai_webuild_sampling_types::truncate_bytes;
...
use xai_webuild_tools::types::compat::CompatConfig;
use xai_webuild_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use xai_webuild_workspace::file_system::CodebaseIndexManager;
use xai_webuild_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use xai_webuild_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
impl xai_webuild_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        xai_webuild_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `xai-webuild-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<xai_webuild_sampler::SharedAttributionCallback>,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<xai_webuild_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<xai_webuild_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-webuild-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<xai_webuild_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<xai_webuild_agent::Agent>,
    ...
    /// Kill-switch (`WEBUILD_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, xai_webuild_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<xai_webuild_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<xai_webuild_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `xai-webuild-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: xai_webuild_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`xai_webuild_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`xai_webuild_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: xai_webuild_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == xai_webuild_tools::implementations::webuild_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<xai_webuild_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `webuild prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &xai_webuild_agent::PromptContext) {
...
) -> Option<xai_webuild_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use xai_webuild_tools::types::output::{MCPOutput, ToolOutput};
    use xai_webuild_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl xai_webuild_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use xai_webuild_tools::registry::types::ToolConfig;
    use xai_webuild_tools::tool_taxonomy::TOOL_META_KEY;
    use xai_webuild_workspace::permission::PermissionCommand;
    ...
    async fn prepare_tool_call_stamps_early_tool_call_and_refinement() {
        ...
        local
            .run_until(async {
                ...
                assert_eq!(t["namespace"], "webuild_build");
...
mod managed_gateway_tool_tests {
    ...
    use xai_webuild_tools::types::output::{MCPOutput, ToolOutput};
    use xai_webuild_tools::types::tool::{ToolKind, ToolNamespace};
    use xai_webuild_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<xai_webuild_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
