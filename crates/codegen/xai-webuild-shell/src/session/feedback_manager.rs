...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(xai_webuild_version::VERSION.to_string());
    s
}

/// Pipeline: persist → strip → submit → telemetry (telemetry fields captured
/// before strip). Callers merge `WEBUILD_USER_METADATA` and set
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `WEBUILD_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `WEBUILD_TELEMETRY_ENABLED`. These are analytics data that
...
impl FeedbackManager {
    ...
    pub(crate) async fn submit_text_feedback(
        ...
        if let Some(user_meta) =
            crate::agent::mvp_agent::parse_json_object_env("WEBUILD_USER_METADATA")
        {
