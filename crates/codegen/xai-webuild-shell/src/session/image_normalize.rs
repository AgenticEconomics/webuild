...
use bytes::Bytes;
use std::borrow::Cow;
use xai_webuild_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
...
/// `xai_webuild_tools::reminders` (free-fn shape because this module has no
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    ...
    use xai_webuild_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) =
        xai_webuild_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        xai_webuild_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !xai_webuild_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = xai_webuild_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn re_encode_fallback_notice_picks_tag_per_harness() {
        ...
        let webuild = render_re_encode_fallback_notice(&notes, false);
        assert!(webuild.contains("<system-reminder>"));
        assert!(webuild.contains("</system-reminder>"));
        assert!(!webuild.contains("<system_reminder>"));
        assert!(webuild.contains("<image_re_encode_fallback>"));
    }
    #[test]
    fn display_format() {
    ...
    fn image_dropped_notice_picks_tag_per_harness() {
        ...
        let webuild = render_image_dropped_notice(&notes, false);
        assert!(webuild.contains("<system-reminder>"));
        assert!(webuild.contains("</system-reminder>"));
        assert!(!webuild.contains("<system_reminder>"));
        assert!(webuild.contains("<image_dropped_notice>"));
        assert!(webuild.contains("Image 5"));
