...
//!
//! Receives [`xai_webuild_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`xai_webuild_mcp::liveness`]),
//! - the [`xai_webuild_mcp::servers::WeBuildClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`webuild_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `webuild_com_` prefix)
    /// or `local` (user `.webuild/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`xai_webuild_mcp::servers::ClientStateKind::Ready`]
...
/// [`xai_webuild_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && xai_webuild_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    fn managed_handshake_auth_rejection_maps_to_needs_auth() {
        let key = (
            "webuild_com_notion".to_string(),
            McpClientEventKind::HandshakeFailed,
        );
        let ev = McpClientEvent::HandshakeFailed {
            server: "webuild_com_notion".to_string(),
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "webuild_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `webuild_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("webuild_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use xai_webuild_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("webuild_com_slack"), // managed
