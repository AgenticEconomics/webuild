...
/// `WEBUILD_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `WEBUILD_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `WEBUILD_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `WEBUILD_GOAL_VERIFIER_N` / remote
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<xai_webuild_sampling_types::conversation::ConversationItem> {
    use xai_webuild_sampling_types::conversation::{ConversationItem, ToolCall};
...
mod tests {
    ...
    fn validate_details_path_in_root_keys_on_injected_temp_root() {
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/var/folders/zz/T/webuild-goal-abc/goal-classifier-abc-1.md"),
                mac_like,
            )
            .is_ok(),
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/tmp/webuild-goal-abc/goal-classifier-abc-1.md"),
                Path::new("/tmp"),
            )
            .is_ok(),
    ...
    fn gap_fingerprint_is_stable_across_scratch_path_churn() {
        ...
        let c = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/webuild-goal-1/out.log for criterion 2",
        ]);
        let d = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/webuild-goal-2/out.log for criterion 2",
        ]);
        assert_eq!(c, d);
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        use xai_webuild_tools::implementations::webuild_build::task::types::SubagentTypeSummary;
        let mut tool_names = std::collections::HashMap::new();
        tool_names.insert(
            xai_webuild_tools::types::tool::ToolKind::Read,
            "cursor_read".to_string(),
        );
        tool_names.insert(
            xai_webuild_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            xai_webuild_tools::types::tool::ToolKind::Search,
        ...
        let webuild = RoleToolNames::from_summary(&summary_with(&[
            (xai_webuild_tools::types::tool::ToolKind::Read, "read_file"),
            (xai_webuild_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (xai_webuild_tools::types::tool::ToolKind::Search, "grep"),
        ]))
        .apply(GOAL_VERIFIER_PROMPT_TEMPLATE);
        assert_no_tool_placeholders(&webuild);
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (xai_webuild_tools::types::tool::ToolKind::Read, "cursor_read"),
            (xai_webuild_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (xai_webuild_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
    ...
    fn render_skeptic_prompt_substitutes_kind_lens_and_leaves_no_placeholder() {
        let body = render_skeptic_prompt(
            ...
            "/tmp/webuild-goal-x/skeptic-0",
            "/tmp/webuild-goal-x/implementer",
            None,
            &RoleToolNames::inherit_defaults(),
            true,
        );
        ...
        assert!(body.contains("/tmp/webuild-goal-x/skeptic-0"));
        assert!(body.contains("/tmp/webuild-goal-x/implementer"));
        assert!(
    ...
    fn stage_inputs_resume<'a>(
        ...
        VerificationStageInputs {
            ...
            verifier_id,
            attempt,
            model_id: "webuild-test",
            goal_created_at: 0,
            plan_file: None,
            plan_baseline_file: None,
            implementer_scratch_dir: Path::new("/tmp/webuild-goal-test/implementer"),
    ...
    async fn verification_stage_n1_not_refuted_returns_achieved() {
        ...
        assert!(
            p.contains("/tmp/webuild-goal-test/implementer"),
    ...
    fn resolve_goal_verifier_count_env_clamps() {
        unsafe { std::env::set_var("WEBUILD_GOAL_VERIFIER_N", "0") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MIN
        );
        unsafe { std::env::set_var("WEBUILD_GOAL_VERIFIER_N", "99") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MAX
        );
        unsafe { std::env::set_var("WEBUILD_GOAL_VERIFIER_N", "garbage") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_COUNT,
            "invalid env falls through to the default"
        );
        unsafe { std::env::remove_var("WEBUILD_GOAL_VERIFIER_N") };
    }
    ...
    fn resolve_goal_verifier_count_precedence_and_clamp() {
        ...
        // env > config.
        unsafe { std::env::set_var("WEBUILD_GOAL_VERIFIER_N", "2") };
    ...
    fn resolve_goal_classifier_max_runs_env_clamps_and_no_ceiling() {
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER_MAX", "0") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER_MAX", "999") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER_MAX", "garbage") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::remove_var("WEBUILD_GOAL_CLASSIFIER_MAX") };
    }
    ...
    fn resolve_goal_classifier_max_runs_precedence_and_floor() {
        ...
        // env > config.
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER_MAX", "4") };
    ...
    fn resolve_strategist_every_default_tracks_cap_floored_at_one() {
        unsafe { std::env::remove_var("WEBUILD_GOAL_STRATEGIST_EVERY") };
    ...
    fn resolve_strategist_every_precedence_and_floor() {
        ...
        // env > config + remote.
        unsafe { std::env::set_var("WEBUILD_GOAL_STRATEGIST_EVERY", "7") };
        let r = cfg_strategist(Some(3), Some(4)).resolve_goal_strategist_every(10);
        assert_eq!(r.value, 7);
        assert_eq!(r.source, ConfigSource::Env);
        // invalid env falls through to the default (cap/2).
        unsafe { std::env::set_var("WEBUILD_GOAL_STRATEGIST_EVERY", "not-a-number") };
        assert_eq!(
            cfg_strategist(None, None)
                .resolve_goal_strategist_every(10)
                .value,
    ...
    async fn run_one_skeptic_fails_closed_when_scratch_root_squatted() {
        ...
        let inputs = SkepticInputs {
            ...
            attempt: 1,
            kind_lens: "",
            implementer_scratch: "/tmp/webuild-goal-test/implementer",
            scratch_dir_ready: true,
            prior_gaps: None,
        };
