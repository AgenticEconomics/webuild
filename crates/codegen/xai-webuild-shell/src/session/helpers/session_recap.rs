...
///    (webuild/SGLang via ChatCompletions/Responses) keeps reasoning VERBATIM so
...
///   `build_recap_items(...)` verbatim (keeps the webuild prefix KV cache warm;
...
pub(crate) fn budget_recap_items(
    ...
    // verbatim path keeps the webuild prefix cache warm.
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use xai_webuild_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> xai_webuild_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    #[test]
    fn budget_over_budget_strips_reasoning_even_on_webuild() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("z".repeat(40_000)),
        ];
        // webuild backend => strip_reasoning=false, but the over-budget branch must
    ...
    #[test]
    fn budget_fast_path_keeps_reasoning_on_webuild() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("small"),
        ];
        // Fits under a large window on webuild (strip_reasoning=false) => verbatim,
        // reasoning kept so the prefix KV cache stays warm.
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on webuild must keep reasoning verbatim"
        );
    }
