...
//! (`xai_webuild_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](xai_webuild_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](xai_webuild_compaction::FullReplaceError).
...
use xai_webuild_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use xai_webuild_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    xai_webuild_telemetry::session_ctx::log_event(CompactionRetryDegraded {
