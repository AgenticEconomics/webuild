...
pub(crate) use xai_webuild_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_webuild_conv_id = Some(sid.clone());
            message.x_webuild_req_id = Some(format!("xai-compact-{}", uuid::Uuid::new_v4()));
            message.x_webuild_session_id = Some(sid);
            message.x_webuild_agent_id = Some(xai_webuild_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = xai_webuild_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, xai_webuild_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_webuild_conv_id: Some(session_id.to_string()),
                x_webuild_req_id: Some(format!("xai-compact-{}", uuid::Uuid::new_v4())),
                x_webuild_session_id: Some(session_id.to_string()),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            xai_webuild_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            xai_webuild_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: xai_webuild_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            xai_webuild_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, xai_webuild_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        xai_webuild_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            xai_webuild_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            xai_webuild_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// (`summary_before_recent = false`).
    #[tokio::test]
    async fn webuild_build_compaction_drops_working_tail_regression_206460() {
        let conversation = vec![
            ConversationItem::system("You are a helpful assistant."),
            ConversationItem::user(
                "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
