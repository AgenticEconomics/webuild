...
/// Env lane (desktop `WEBUILD_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`WEBUILD_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn xai_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::WeBuildAuth {
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = xai_webuild_test_support::EnvGuard::set(
            "WEBUILD_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = xai_webuild_test_support::EnvGuard::unset("WEBUILD_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = xai_webuild_test_support::EnvGuard::set("WEBUILD_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = xai_webuild_test_support::EnvGuard::set("WEBUILD_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::WEBUILD_CHAT_MODE_ENV;
        let _chat_off = xai_webuild_test_support::EnvGuard::unset(WEBUILD_CHAT_MODE_ENV);
        let _desktop_off =
            xai_webuild_test_support::EnvGuard::unset("WEBUILD_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                xai_webuild_test_support::EnvGuard::set("WEBUILD_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = xai_webuild_test_support::EnvGuard::set(WEBUILD_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = xai_webuild_test_support::EnvGuard::unset(WEBUILD_CHAT_MODE_ENV);
        ...
        {
            let _on = xai_webuild_test_support::EnvGuard::set(WEBUILD_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
