...
//! `xai_webuild_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use xai_webuild_shell_session_support::managed_mcp::*;
...
pub fn merge_managed_mcp_servers(
    ...
    plugin_registry: Option<&xai_webuild_agent::plugins::PluginRegistry>,
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &xai_webuild_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &xai_webuild_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &xai_webuild_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: xai_webuild_tools::util::webuild_home::webuild_home().join("config.toml"),
    };
...
/// Auto-create `webuild_com_*` entries for managed configs not already in `merged`.
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = xai_webuild_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = xai_webuild_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn auto_inject_creates_server_for_unmatched_managed_config() {
        ...
        let slack = merged
            .iter()
            .find(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "webuild_com_slack"));
        let slack = slack.expect("should have auto-injected webuild_com_slack");
        match slack {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use xai_webuild_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
    ...
    fn merge_drops_server_denied_by_name_including_managed_prefix() {
        ...
        let tagged = apply_mcp_server_policy(
            vec![
                acp::McpServer::Http(
                    acp::McpServerHttp::new("webuild_com_slack", "https://mcp.slack.com/sse")
                        .headers(vec![]),
                ),
        ...
        let slack = tagged
            .iter()
            .find(|s| mcp_server_name(&s.server) == "webuild_com_slack")
    ...
    fn auto_inject_dedup_by_display_name_first_scope_wins() {
        ...
        let linear_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "webuild_com_linear"))
            .count();
    ...
    fn auto_inject_skips_existing_server() {
        ...
        let slack_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "webuild_com_slack"))
            .count();
    ...
    fn auto_inject_skips_disabled() {
        ...
        let disabled: std::collections::HashSet<String> =
            ["webuild_com_slack".to_string()].into_iter().collect();
        let mut merged = vec![];
        auto_inject_managed_servers_with_disabled(&mut merged, &managed, &disabled);

        let has_slack = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "webuild_com_slack"));
        let has_linear = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "webuild_com_linear"));
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".webuild")).unwrap();
        std::fs::write(
            cwd.path().join(".webuild").join("config.toml"),
            r#"
[mcp_servers.github]
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${WEBUILD_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.webuild/plugins/team-tool",
            "/home/user/.webuild/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.webuild/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use xai_webuild_agent::plugins::PluginRegistry;
        use xai_webuild_agent::plugins::PluginScope;
        use xai_webuild_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use xai_webuild_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: xai_webuild_agent::plugins::PluginOrigin::UserWeBuild,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use xai_webuild_agent::plugins::PluginScope;
        use xai_webuild_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use xai_webuild_agent::plugins::manifest::PluginManifest;
