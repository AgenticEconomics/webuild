...
/// sanitizer shared with `xai-webuild-mcp` so the advertised folder matches disk.
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("webuild_com_linear"),
            "webuild_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.webuild/projects/enc/mcps");
    ...
    async fn gateway_descriptor_collision_prunes_only_gateway_owned_files() {
        ...
        std::fs::write(
            ...
            serde_json::to_vec_pretty(&serde_json::json!({
                ...
                "inputSchema": {"type": "object"},
                "x-webuild-managed-gateway": true,
            }))
            .unwrap(),
        )
        .unwrap();
