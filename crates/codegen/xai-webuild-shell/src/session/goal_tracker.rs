...
/// scratch root (`<temp_dir>/webuild-goal-<id>`) and the verifier
...
/// Private per-goal scratch root: `<temp_dir>/webuild-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("webuild-goal-{verifier_id}"))
}
...
mod tests {
    ...
    /// `webuild-goal-<12 hex chars>` scratch-root format (and the
    ...
    /// `temp_dir()` + `verifier_id`: a `webuild-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("webuild-goal-vid123"));
        assert_eq!(
            implementer_scratch_dir("vid123"),
            std::env::temp_dir()
                .join("webuild-goal-vid123")
                .join("implementer"),
        );
        assert_eq!(
