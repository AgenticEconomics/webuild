...
pub use xai_webuild_telemetry::enums::PrCreationSource;
...
pub struct SessionSignals {
    ...
    // === Bash tool patterns (webuild_build) ===
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    xai_webuild_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        assert_eq!(snapshot.models_used.len(), 3); // grok-3 not duplicated, webuild-5 added
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        // ITL stats must survive the restore (regression test for webuild-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the webuild-critique bug describes: the
