...
//!
//! Used by both the ACP `x.ai/session/list` handler and the `webuild sessions`
...
pub async fn fetch_merged(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            xai_webuild_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
