...
//! `crate::session::prompt_queue::*` and cross-crate `xai_webuild_shell::session::prompt_queue::*`
...
mod tests {
    ...
    fn queue_changed_serializes_camel_case_with_session_id() {
        let payload = QueueChanged {
            ...
            entries: vec![QueueEntryWire {
                id: "p1".to_string(),
                version: 0,
                owner: Some("webuild-tui".to_string()),
                last_editor: None,
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("webuild-tui".to_string()),
            last_editor: Some("webuild-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
        };
        let json = serde_json::to_value(&entry).unwrap();
        assert_eq!(json["lastEditor"], "webuild-vscode");
