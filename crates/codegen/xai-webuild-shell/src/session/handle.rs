...
pub struct SessionHandle {
    ...
    pub permission_handle: xai_webuild_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<xai_webuild_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<xai_webuild_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
