...
use xai_webuild_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's WeBuild flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use xai_webuild_tools::implementations::skills::skill::{
...
/// `xai-webuild-tools` so it stays identical to the pager's `LoopCommand` and the
