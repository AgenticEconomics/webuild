...
use xai_webuild_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> xai_webuild_workspace::file_system::FileIndexDelta {
    use xai_webuild_workspace::file_system::FileIndexDelta;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = xai_webuild_workspace::session::git::get_branch(&self.cwd).await;
        let commit = xai_webuild_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = xai_webuild_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = xai_webuild_workspace::session::git::GitHeadChanged {
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                xai_webuild_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
