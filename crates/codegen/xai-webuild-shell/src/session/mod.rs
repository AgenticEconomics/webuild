...
pub(crate) fn is_cursor_user_template(
    _template: &xai_webuild_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &xai_webuild_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
pub use xai_webuild_shared::session::info;
pub mod managed_mcp;
...
pub mod persistence;
pub use xai_webuild_shared::placeholder_images;
pub mod plan_mode;
