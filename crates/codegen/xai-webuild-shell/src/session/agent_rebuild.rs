...
//! [`xai_webuild_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xai_webuild_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xai_webuild_agent::Agent`] owns an [`xai_webuild_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`xai_webuild_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use xai_webuild_agent::config::AgentDefinition;
use xai_webuild_agent::error::AgentBuildError;
use xai_webuild_agent::prompt::context::PromptAudience;
use xai_webuild_agent::prompt::skills::SkillsConfig;
use xai_webuild_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use xai_webuild_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_webuild_tools::implementations::webuild_build::ask_user_question::types::UserQuestionRequest;
...
use xai_webuild_tools::implementations::lsp::LspBackend;
...
use xai_webuild_tools::types::SharedApiKeyProvider;
use xai_webuild_tools::types::compat::CompatConfig;
use xai_webuild_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<xai_webuild_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<xai_webuild_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use xai_webuild_tools::implementations::webuild_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(xai_webuild_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_webuild_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use xai_webuild_tools::implementations::webuild_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            xai_webuild_tools::computer::local::LocalTerminalBackend::new_local(
                xai_webuild_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: xai_webuild_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(xai_webuild_tools::types::tool::ToolKind::Task)
            .expect("WeBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("WeBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_webuild_build())
                    .await
