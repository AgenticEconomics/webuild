...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> xai_webuild_hooks::runner::RunContext<'_> {
        xai_webuild_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[xai_webuild_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, xai_webuild_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: xai_webuild_hooks::event::HookEventName,
        payload: xai_webuild_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                xai_webuild_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    xai_webuild_telemetry::events::HookOutcome::Success,
                ),
                xai_webuild_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    xai_webuild_telemetry::events::HookOutcome::Error,
                ),
                xai_webuild_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: xai_webuild_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
