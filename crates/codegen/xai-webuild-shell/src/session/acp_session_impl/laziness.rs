...
impl SessionActor {
    ...
    /// sees "WeBuild is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let items = vec![
            ConversationItem::System(xai_webuild_sampling_types::SystemItem {
                content: std::sync::Arc::<str>::from(LAZINESS_CLASSIFIER_PROMPT),
            }),
            ConversationItem::User(xai_webuild_sampling_types::UserItem {
                content: vec![xai_webuild_sampling_types::ContentPart::Text {
        ...
        let request = ConversationRequest {
            ...
            // other tool-flavoured WeBuild variants) reject the field at
            ...
            x_webuild_conv_id: Some(session_id_str.clone()),
            x_webuild_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_webuild_session_id: Some(session_id_str),
