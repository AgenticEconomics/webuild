...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.webuild/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.webuild/`, `~/.claude/`,
/// `~/.webuild/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<xai_webuild_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<xai_webuild_agent::prompt::user_message::RuleEntry>,
    Vec<xai_webuild_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.webuild/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = xai_webuild_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for webuild ordering), so a failed offload never
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(xai_webuild_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: xai_webuild_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use xai_webuild_agent::prompt::agents_md::read_agents_config_with_paths;
        use xai_webuild_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xai_webuild_workspace::file_system::{git_status_short, jj_status};
        use xai_webuild_workspace::session::git::VcsKind;
    ...
    ) -> Vec<xai_webuild_agent::prompt::user_message::McpServerEntry> {
        use xai_webuild_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = xai_webuild_sampler::SamplingClient::new(sampler_config).map_err(|e| {
