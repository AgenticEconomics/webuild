...
fn drop_cli_catchall_allows(
    rules: Vec<xai_webuild_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<xai_webuild_workspace::permission::types::PermissionRule>,
    Vec<xai_webuild_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if xai_webuild_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use xai_webuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use xai_webuild_workspace::permission::rules::parse_permission_rule;
    use xai_webuild_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<xai_webuild_sampler::SharedAttributionCallback>,
    ...
    web_search_sampling_config: Option<xai_webuild_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: xai_webuild_tools::implementations::webuild_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: xai_webuild_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<xai_webuild_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<xai_webuild_workspace::permission::PermissionHandle>,
    api_key_provider: Option<xai_webuild_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<xai_webuild_hooks::discovery::HookRegistry>>,
    workspace_ops: xai_webuild_workspace::WorkspaceOps,
    cli_permission_rules: Vec<xai_webuild_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        xai_webuild_tools::implementations::webuild_build::scheduler::types::SchedulerHandle,
    >,
    ...
    xai_webuild_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(xai_webuild_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            xai_webuild_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = xai_webuild_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("webuild: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        xai_webuild_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(xai_webuild_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if xai_webuild_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    xai_webuild_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn xai_webuild_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            xai_webuild_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        xai_webuild_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            xai_webuild_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("WEBUILD_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "WEBUILD_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = xai_webuild_sampling_types::SamplingConfig {
    ...
    let auto_wake_delivered =
        xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        xai_webuild_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        xai_webuild_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let terminal_backend: std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend> =
        if let Some(parent_tb) = parent_terminal_backend.filter(|_| startup_hints.is_subagent) {
            ...
            )) as std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>
        } else {
            let backend: std::sync::Arc<dyn xai_webuild_tools::computer::types::TerminalBackend> =
                std::sync::Arc::new(LocalTerminalBackend::new_local(resolve_search_shadows()));
            backend
        };
    let fs_backend: std::sync::Arc<dyn xai_webuild_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(xai_webuild_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(xai_webuild_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || xai_webuild_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                xai_webuild_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                xai_webuild_telemetry::enums::PermissionMode::Auto
            } else {
                xai_webuild_telemetry::enums::PermissionMode::Ask
            },
    ...
    let compaction_policy = xai_webuild_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        xai_webuild_tools::implementations::webuild_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn xai_webuild_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : xai_webuild_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : xai_webuild_telemetry::memory_log::TARGET, removed,
        ...
        xai_webuild_telemetry::session_ctx::log_event(
            xai_webuild_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        xai_webuild_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        xai_webuild_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        xai_webuild_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        xai_webuild_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<xai_webuild_tools::implementations::webuild_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::WeBuildTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::WeBuildWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::WeBuildPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = xai_webuild_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = xai_webuild_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_webuild_sampler::SamplingEvent>();
    let sampler_handle = xai_webuild_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<xai_webuild_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<xai_webuild_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = xai_webuild_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalEnvelope,
    >();
    let obs_bridge = {
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: xai_webuild_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match xai_webuild_workspace::session::git::discover_git_root(root) {
                xai_webuild_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    xai_webuild_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => xai_webuild_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(xai_webuild_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(xai_webuild_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            xai_webuild_tools::implementations::webuild_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : xai_webuild_telemetry::memory_log::TARGET, files = files.len(),
                ...
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use xai_webuild_tools::implementations::webuild_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use xai_webuild_tools::implementations::webuild_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = xai_webuild_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            xai_webuild_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        xai_webuild_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, xai_webuild_agent::AgentBuildError>,
    >();
