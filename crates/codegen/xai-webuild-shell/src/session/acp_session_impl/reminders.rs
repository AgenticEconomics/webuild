...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> xai_webuild_agent::ReminderPolicy {
    let mut policy = xai_webuild_agent::ReminderPolicy::default();
...
/// Wrapped in webuild's `<system-reminder>` shape by [`SessionActor::push_system_reminder`].
...
pub(super) fn todo_gate_active(
    policy: &xai_webuild_agent::system_reminder::ReminderPolicy,
    audience: xai_webuild_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    /// in `xai_webuild_tools::reminders`.
    pub(super) fn reminder_wrapper_tag(&self) -> &'static str {
        xai_webuild_tools::reminders::DEFAULT_REMINDER_TAG
    }
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    ...
                    .await;
                let read_tool_name =
                    xai_webuild_tools::reminders::task_completion::resolve_read_tool_name(&bridge)
                        .await;
                let reminder = xai_webuild_tools::reminders::task_completion::format_between_turn_bash_completions(
                    &bash_completions,
        ...
        let reminder =
            xai_webuild_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) fn todo_gate_policy(
        &self,
    ) -> Option<xai_webuild_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(xai_webuild_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
