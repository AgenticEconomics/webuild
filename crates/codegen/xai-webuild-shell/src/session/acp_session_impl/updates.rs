...
impl SessionActor {
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    xai_webuild_hooks::event::HookEventName::SubagentStart,
                    None,
                    xai_webuild_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = xai_webuild_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        xai_webuild_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            XaiSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    xai_webuild_hooks::event::HookEventName::SubagentEnd,
                    None,
                    xai_webuild_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            xai_webuild_hooks::event::HookEventName::Notification,
            None,
            xai_webuild_hooks::event::HookPayload::Notification {
