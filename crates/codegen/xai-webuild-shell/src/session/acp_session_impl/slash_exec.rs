...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::SlashCommandUsed {
        ...
        match action {
            ...
            BuiltinAction::SetYolo { enabled } => {
                ...
                if let Some(actual) = yolo_toggle_report(was, actual) {
                    ...
                    xai_webuild_telemetry::session_ctx::log_event(
                        xai_webuild_telemetry::events::YoloToggled {
                            enabled: actual,
                            previous_state: was,
                            trigger: xai_webuild_telemetry::events::YoloTrigger::SlashCommand,
                        },
                    );
                    tracing::info_span!(
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.webuild/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.webuild/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::HookAdded { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_slash_command_output(&msg).await;
                    }
                    None => {
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_slash_command_output(
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::PluginAdded {
                                    source: xai_webuild_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsInstall { source, trust } => {
                if source.is_empty() {
                    ...
                    if !trust {
                        let install_source =
                            xai_webuild_agent::plugins::git_install::parse_install_source(
                                &source, cwd,
                            );
                        let source_desc = match &install_source {
                            xai_webuild_agent::plugins::git_install::InstallSource::Git {
                                url,
                                ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            xai_webuild_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                xai_webuild_telemetry::session_ctx::log_event(
                                    xai_webuild_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn xai_webuild_tools::types::memory_backend::MemoryBackend,
