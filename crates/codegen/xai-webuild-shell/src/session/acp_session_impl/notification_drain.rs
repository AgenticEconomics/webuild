...
impl SessionActor {
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in xai_webuild_tools::implementations::webuild_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
    ...
    pub(super) fn drain_notifications_into_turn(
        ...
        use xai_webuild_tools::implementations::webuild_build::task::types::MonitorEventNotification;
        ...
        if let (Some(idx), Some(batch)) = (
            monitor_section_idx,
            xai_webuild_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
