...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: xai_webuild_tools::implementations::webuild_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`WEBUILD_GOAL_PLANNER`) and a plan
...
/// Override with `WEBUILD_GOAL_REVERIFY_AFTER` (floored at 1).
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                xai_webuild_tools::implementations::webuild_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
