...
impl SessionActor {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_plan() {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::PlanModeToggled {
                    enabled: true,
                    trigger: xai_webuild_telemetry::events::PlanModeTrigger::User,
                    turn_in_flight,
    ...
    /// first-entry/reentry distinction), or webuild's reentry/full variant.
