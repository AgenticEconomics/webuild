...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// WeBuild-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as webuild edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &xai_webuild_tools::implementations::webuild_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        xai_webuild_tools::normalization::merge_tool_meta(
            &toolset,
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => xai_webuild_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && xai_webuild_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(xai_webuild_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(xai_webuild_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            xai_webuild_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            xai_webuild_hooks::event::HookEventName::PostToolUseFailure,
                            xai_webuild_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    xai_webuild_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    xai_webuild_hooks::event::HookEventName::PostToolUse,
                    xai_webuild_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                ...
                .await;
            let (ext_file_path, ext_parameters) = if xai_webuild_telemetry::external::is_active() {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(xai_webuild_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                xai_webuild_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                xai_webuild_hooks::event::HookEventName::PreToolUse,
                None,
                xai_webuild_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    xai_webuild_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let xai_webuild_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                xai_webuild_workspace::permission::AccessKind::Read(p) => (
                    xai_webuild_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                xai_webuild_workspace::permission::AccessKind::Edit(p) => {
                    (xai_webuild_telemetry::events::AccessKind::Edit, p.clone())
                }
                xai_webuild_workspace::permission::AccessKind::Bash(cmd) => {
                    (xai_webuild_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                xai_webuild_workspace::permission::AccessKind::Grep { path, glob } => (
                    xai_webuild_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                xai_webuild_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (xai_webuild_telemetry::events::AccessKind::Mcp, name.clone())
                }
                xai_webuild_workspace::permission::AccessKind::WebFetch(u) => {
                    (xai_webuild_telemetry::events::AccessKind::Web, u.clone())
                }
                xai_webuild_workspace::permission::AccessKind::WebSearch(q) => {
                    (xai_webuild_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (xai_webuild_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    xai_webuild_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    xai_webuild_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    xai_webuild_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        xai_webuild_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        xai_webuild_hooks::event::HookEventName::PermissionDenied,
                        xai_webuild_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        xai_webuild_tools::implementations::webuild_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use xai_webuild_tools::implementations::webuild_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                xai_webuild_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
    ...
    async fn send_tool_call_start(
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        xai_webuild_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::Skill(skill) => {
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::SkillDispatched {
    ...
    /// `xai_webuild_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use xai_webuild_telemetry::enums::PrCreationSource;
        use xai_webuild_tools::util::git_detect;
        match &result.output {
            xai_webuild_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    xai_webuild_telemetry::session_ctx::log_event(
                        xai_webuild_telemetry::events::PrMerged {},
                    );
                }
            }
            xai_webuild_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: xai_webuild_tools::util::git_detect::PrRef,
        source: xai_webuild_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            xai_webuild_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                xai_webuild_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let xai_webuild_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                xai_webuild_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | xai_webuild_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            xai_webuild_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            xai_webuild_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`xai_webuild_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: xai_webuild_sampler::SamplingEvent,
    ) {
        use xai_webuild_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == xai_webuild_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                xai_webuild_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = xai_webuild_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use xai_webuild_tools::types::ToolInput;
    use xai_webuild_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use xai_webuild_tools::implementations::webuild_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use xai_webuild_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// WeBuild edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn webuild_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "webuild tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use xai_webuild_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use xai_webuild_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use xai_webuild_tools::implementations::webuild_build::exit_plan_mode::ExitPlanModeExtResponse;
...
mod wait_interrupt_tests {
    ...
    use xai_webuild_tools::types::output::ToolOutput;
