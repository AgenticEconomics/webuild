...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<xai_webuild_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            xai_webuild_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : xai_webuild_telemetry::memory_log::TARGET,
            ...
            xai_webuild_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            body_bytes_after, }) => {
            xai_webuild_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            .fire_hook(xai_webuild_hooks::event::HookEventName::SessionEnd, None,
            xai_webuild_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(& registry,
            xai_webuild_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(xai_webuild_hooks::event::HookEventName::Stop,
            None, xai_webuild_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(& registry,
            xai_webuild_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            xai_webuild_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::SessionEnded
            ...
            .push_capped(xai_webuild_tools::implementations::webuild_build::task::types::MonitorEventNotification
            ...
            xai_webuild_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(xai_webuild_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            session.fire_hook(xai_webuild_hooks::event::HookEventName::SessionStart, None,
            xai_webuild_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(& registry,
            xai_webuild_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(xai_webuild_hooks::event::HookEventName::SessionEnd, None,
            xai_webuild_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(xai_webuild_hooks::event::HookEventName::Stop,
            None, xai_webuild_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            xai_webuild_hooks::dispatcher::dispatch_non_blocking(& registry,
            xai_webuild_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[xai_webuild_sampling_types::ConversationItem],
    turn_number: usize,
