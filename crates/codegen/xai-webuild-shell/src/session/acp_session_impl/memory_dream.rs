...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &xai_webuild_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_webuild_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                xai_webuild_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        xai_webuild_telemetry::session_ctx::log_event(
            xai_webuild_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_webuild_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: xai_webuild_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_webuild_conv_id: Some(session_id.clone()),
                x_webuild_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_webuild_session_id: Some(session_id.clone()),
                x_webuild_agent_id: Some(xai_webuild_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: xai_webuild_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        xai_webuild_telemetry::session_ctx::log_event(
            xai_webuild_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => xai_webuild_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => xai_webuild_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => xai_webuild_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => xai_webuild_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::MemoryFlushed {
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    xai_webuild_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                xai_webuild_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_webuild_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = xai_webuild_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                xai_webuild_sampler::collect_response(events).await
            }
        };
