...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &xai_webuild_tools::implementations::webuild_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = xai_webuild_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        xai_webuild_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    xai_webuild_telemetry::session_ctx::log_event(
                        xai_webuild_telemetry::events::SlashCommandUsed {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::PluginUsed {
        ...
        xai_webuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        xai_webuild_telemetry::session_ctx::begin_prompt_id();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = xai_webuild_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || xai_webuild_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = xai_webuild_telemetry::events::PromptSubmitted {
            ...
            xai_webuild_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(xai_webuild_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            xai_webuild_hooks::event::HookEventName::UserPromptSubmit,
            xai_webuild_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                .await;
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::TurnCompleted {
                        outcome: xai_webuild_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::TurnCompleted {
                        outcome: xai_webuild_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                xai_webuild_telemetry::session_ctx::log_session_event(
                    xai_webuild_telemetry::events::ApiError {
                ...
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::TurnCompleted {
                        outcome: xai_webuild_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
                ...
                self.dispatch_hook(
                    xai_webuild_hooks::event::HookEventName::StopFailure,
                    xai_webuild_hooks::event::HookPayload::StopFailure {
                        error: format!("{err}"),
                    },
                    Some(prompt_id),
                    None,
        ...
        self.dispatch_hook(
            xai_webuild_hooks::event::HookEventName::Stop,
            xai_webuild_hooks::event::HookPayload::Stop {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = xai_webuild_tools::implementations::webuild_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = xai_webuild_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = xai_webuild_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use xai_webuild_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : xai_webuild_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        xai_webuild_telemetry::session_ctx::log_event(
            xai_webuild_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<xai_webuild_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_webuild_session_id = Some(self.session_info.id.to_string());
            request.x_webuild_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_webuild_agent_id = Some(xai_webuild_telemetry::id::agent_id());
            if request.x_webuild_deployment_id.is_none() {
                request.x_webuild_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(xai_webuild_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    xai_webuild_sampling_types::ConversationItem::Assistant(_) => {
