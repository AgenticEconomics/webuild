...
impl SessionActor {
    ...
    ) -> Option<xai_webuild_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(super) async fn reload_skills_from_disk(&self) -> usize {
        ...
        let new_skills = xai_webuild_agent::prompt::skills::list_skills_with_plugins(
            Some(cwd),
            &skills_config,
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &xai_webuild_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use xai_webuild_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn xai_webuild_auth::AuthCredentialProvider> = Arc::new(
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = xai_webuild_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(xai_webuild_tools::types::resources::DenyReadGlobs(
