...
impl SessionActor {
            ...
            Err(e) => {
                ...
                if terminal {
                    ...
                    crate::session::telemetry::emit_mcp_connection_span(
                        ...
                        Some(elapsed_ms),
                        None,
                        Some(xai_webuild_telemetry::events::McpErrorType::Auth.as_str()),
                    );
    ...
    /// or removed it from `~/.webuild/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.webuild/config.toml` + the project
    /// `.webuild/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    /// `WeBuildClientHandler`, and the liveness watcher is armed —
    ...
    ///
    /// The `WeBuildClientHandler` constructed inside `try_handshake`
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(xai_webuild_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(xai_webuild_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        tokio::task::spawn_local(async move {
            ...
            > = std::collections::HashMap::new();
            {
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => xai_webuild_telemetry::events::McpTransport::Stdio,
                                "sse" => xai_webuild_telemetry::events::McpTransport::Sse,
                                _ => xai_webuild_telemetry::events::McpTransport::Http,
                            };
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                xai_file_utils::events::McpErrorCategory::AuthRequired => {
                                    xai_webuild_telemetry::events::McpErrorType::Auth
                                }
                                xai_file_utils::events::McpErrorCategory::Timeout => {
                                    xai_webuild_telemetry::events::McpErrorType::Timeout
                                }
                                _ => xai_webuild_telemetry::events::McpErrorType::HandshakeFailed,
                            };
                            xai_webuild_telemetry::session_ctx::log_event(
                            ...
                            let detail = (!needs_auth).then(|| {
                                xai_webuild_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<xai_webuild_tools::types::tool_index::ServerSummary> {
        use xai_webuild_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            xai_webuild_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
