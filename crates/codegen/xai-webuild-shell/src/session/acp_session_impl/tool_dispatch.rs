...
/// - `file_path` — webuild_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), webuild_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — webuild_build (`read_file`, via `#[serde(rename)]`)
///
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = xai_webuild_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(xai_webuild_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from xai-webuild-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, webuild-shell would sanitize
