...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            xai_webuild_workspace::session::git::get_worktree_info(cwd),
            xai_webuild_workspace::session::git::get_branch(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<xai_webuild_tools::implementations::webuild_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_install_error(
        err: &xai_webuild_agent::plugins::install_registry::InstallError,
    ) -> String {
