...
pub enum SessionCommand {
    ...
    /// the `model` field sent in the `x-webuild-model-override` header and merges
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.webuild/hooks`,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<xai_webuild_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<xai_webuild_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<xai_webuild_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<xai_webuild_sampling_types::conversation::ConversationItem>>>,
    },
