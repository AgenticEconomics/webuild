...
impl JsonlStorageAdapter {
    pub fn new() -> Self {
        Self {
            dir_mode: SessionDirMode::FromRoot(crate::util::webuild_home::webuild_home()),
        }
    }
    ...
    fn session_dir(&self, info: &Info) -> PathBuf {
        match &self.dir_mode {
            SessionDirMode::FromRoot(root) => root
                .join("sessions")
                .join(crate::util::webuild_home::encode_cwd_dirname(&info.cwd))
    ...
    fn scan_session_dirs(&self, cwd: Option<&str>) -> Vec<PathBuf> {
        ...
        if let Some(cwd_str) = cwd {
            let enc = crate::util::webuild_home::encode_cwd_dirname(cwd_str);
    ...
    /// [`xai_webuild_sampling_types::upgrade_legacy_reasoning`] and emit
    ...
    fn read_chat_history_sync(
        ...
        for (line_idx, line) in contents.split(|b| *b == b'\n').enumerate() {
            ...
            let siblings =
                xai_webuild_sampling_types::upgrade_legacy_reasoning(&raw, &mut sibling_btc_ids_seen);
            for sib in siblings {
                match &sib {
...
/// `xai-webuild-subagent-resolution/src/context.rs` (it counts turns in the same
...
impl JsonlStorageAdapter {
    ...
    pub fn copy_session_data_sync(
        ...
        let target_summary = crate::session::persistence::Summary {
            ...
            request_id: None,
            webuild_home: crate::session::persistence::webuild_home_string(),
...
impl StorageAdapter for JsonlStorageAdapter {
    async fn init_session(&self, info: &Info, model_id: acp::ModelId) -> io::Result<Summary> {
        ...
        if Path::new(&summary_path).exists() {
            ...
            summary.sandbox_profile = xai_webuild_sandbox::configured_profile_name().map(String::from);
    ...
    async fn update_current_model_and_agent(
        ...
        reasoning_effort: Option<Option<xai_webuild_sampling_types::ReasoningEffort>>,
    ) -> io::Result<()> {
        self.apply_summary_patch(
            info,
            super::summary_write::SummaryPatch {
    ...
    async fn merge_rewind_points_from(&self, info: &Info, target_index: usize) -> io::Result<()> {
        let points = self.load_rewind_points(info).await?;
        let merged =
            xai_webuild_workspace::session::file_state::merge_rewind_points_from(points, target_index);
