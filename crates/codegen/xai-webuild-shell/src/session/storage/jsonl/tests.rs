...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use xai_webuild_paths::RelPathBuf;
    use xai_webuild_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use xai_webuild_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn test_copy_session_data_copies_tool_state() {
    ...
    let tool_state_json = serde_json::json!(
        { "state" : { "webuild_build.TodoState" : { "todos" : [] } } }
    );
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    xai_webuild_sandbox::set_configured_profile("workspace");
    let expected = xai_webuild_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item("mid")),
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::webuild_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::webuild_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::webuild_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::webuild_home::encode_cwd_dirname("/project");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::webuild_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::webuild_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::webuild_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::webuild_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::webuild_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// path end-to-end (loader + serde + xai_webuild_sampling_types::
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let xai_webuild_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    ...
    let xai_webuild_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
