...
//! both matched by the agent ([`xai_webuild_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use xai_webuild_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use xai_webuild_telemetry::events::ClientHookGateOutcome;
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = xai_webuild_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    xai_webuild_telemetry::session_ctx::log_event(
                        xai_webuild_telemetry::events::ClientHookGate {
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use xai_webuild_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    xai_webuild_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use xai_webuild_hooks::matcher::HookMatcher;
