...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    xai_webuild_config::MANAGED_CONFIG_FILENAME,
    xai_webuild_config::REQUIREMENTS_FILENAME,
    xai_webuild_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    xai_webuild_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            xai_webuild_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", xai_webuild_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            xai_webuild_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: WeBuildAuth) -> Option<WeBuildAuth> {
...
/// files (TUI tick vs `webuild login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `webuild setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `WEBUILD_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("WEBUILD_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xai-webuild-http` classifier.
...
/// Override with `WEBUILD_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("WEBUILD_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** WeBuildDeployment UUID (the id
...
/// Resolve deployment key from `WEBUILD_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (webuild setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("WEBUILD_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.webuild/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<WeBuildAuth>,
...
/// read-only `webuild setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if xai_webuild_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(xai_webuild_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        xai_webuild_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(xai_webuild_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            xai_webuild_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(xai_webuild_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &xai_webuild_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<xai_webuild_config::signed_policy::SignatureEnvelope> {
    use xai_webuild_config::signed_policy::now_unix;
    ...
    let claim = match xai_webuild_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &xai_webuild_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `webuild login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<WeBuildAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `webuild setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(WeBuildAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `webuild setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !xai_webuild_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::webuild_home::webuild_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            xai_webuild_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `webuild setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.webuild`.
    Installed,
...
/// Result of `webuild setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `webuild setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `webuild setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `webuild setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `webuild setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && xai_webuild_config::signed_policy::verification_active()
...
/// Run the `webuild setup` sync for the current principal. The caller must check
