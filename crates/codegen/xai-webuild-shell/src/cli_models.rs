//! Data APIs for `webuild models`. Clients own display.
...
/// Status for the `webuild models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `webuild_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.webuild_com_config.webuild_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_xai_api_key(
            agent_config.webuild_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("WEBUILD_AUTH"),
            EnvGuard::set("WEBUILD_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("WEBUILD_DEPLOYMENT_KEY"),
            EnvGuard::unset("WEBUILD_WS_ORIGIN"),
            EnvGuard::unset("WEBUILD_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        let (_dir, _g) = isolate_auth_sources();
        let token = WeBuildAuth {
            key: "session-token".into(),
            auth_mode: AuthMode::WebLogin,
            ..WeBuildAuth::test_default()
        };
        let json = serde_json::to_string(&token).unwrap();
        let _auth = EnvGuard::set("WEBUILD_AUTH", &json);
