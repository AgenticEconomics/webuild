...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "WEBUILD_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `WEBUILD_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `WEBUILD_CODE_XAI_API_KEY` is set.
...
/// `WEBUILD_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.webuild_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.webuild_com_config.oidc.as_ref().unwrap()`
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            WEBUILD_COM_METHOD_ID => Self::WeBuildCom,
    ...
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::WeBuildCom | Self::Oidc)
    }
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.webuild/auth.json".to_string())),
    )
}
...
pub fn webuild_com_auth_method(
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(WEBUILD_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// `~/.webuild/config.toml` skeleton from the bug report, walks it through
    ...
    /// Legacy `WEBUILD_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `WEBUILD_CODE_XAI_API_KEY` are set,
    ...
    // -- webuild login --legacy regression coverage ------------------------
    //
    // `webuild login --legacy` produces a WeBuildAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `WEBUILD_AUTH` env var (or via legacy
    ...
    // to-end. A regression in WEBUILD_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `WEBUILD_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn webuild_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, WeBuildAuth, WeBuildComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("WEBUILD_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `webuild login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = WeBuildAuth {
        ...
        // Provide it via WEBUILD_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("WEBUILD_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in WEBUILD_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("WEBUILD_AUTH");
        let _g2 = EnvGuard::unset("WEBUILD_AUTH_PATH");
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, WEBUILD_COM_METHOD_ID]
        );
