...
use crate::auth::{AuthManager, AuthMode, WeBuildAuth, run_auth_flow};
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = xai_webuild_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    xai_webuild_telemetry::unified_log::set_version(xai_webuild_version::VERSION);
    ...
    xai_file_utils::queue::cleanup_orphaned_uploads(
        &webuild_home::webuild_home(),
        xai_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by webuild-desktop when spawning `webuild agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("WEBUILD_CLIENT_VERSION") {
        crate::unified_log::info(
            "WEBUILD_CLIENT_VERSION",
            None,
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `webuild agent stdio` is a local/interactive entrypoint (spawned by
            // webuild-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_webuild_telemetry::unified_log::set_version(xai_webuild_version::VERSION);
    // `webuild agent [headless]` serves non-TUI automation; stamp proxy requests
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'webuild login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `webuild login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&webuild_home::webuild_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the webuild build URL for the first-connection callback
...
fn should_seed_shared_session(existing: Option<&WeBuildAuth>, session: &WeBuildAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
///
/// * `relay_on_demand == false` (default — explicit `webuild agent leader`
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: xai_webuild_version::VERSION.to_string(),
    })
    ...
    let auth: Option<WeBuildAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<WeBuildAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<WeBuildAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            let auth_scope = agent_config.webuild_com_config.auth_scope();
            // Gated on user_webuild_home() so a cwd-relative .webuild/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = xai_webuild_config::user_webuild_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.webuild/models_cache.json
                            // (another webuild process fetched a fresher /v1/models
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> WeBuildAuth {
        WeBuildAuth {
            key: key.into(),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::WeBuildComConfig {
            webuild_ws_url: format!("ws://{addr}"),
            webuild_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `webuild agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
