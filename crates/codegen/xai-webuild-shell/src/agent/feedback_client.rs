...
const CLIENT_VERSION_HEADER: &str = "x-webuild-client-version";

// ============================================================================
// Turn delta wire types (local to xai-webuild-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::webuild_auth_credentials::WeBuildAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::webuild_auth_credentials::WeBuildAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(xai_webuild_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::webuild_auth_credentials::WeBuildAuthCredentials,
    ) -> Arc<dyn xai_webuild_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(xai_webuild_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, xai_webuild_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "xai-webuild-cli")
        } else {
            builder
        }
    }
...
mod auth_refresh_tests {
    ...
    async fn send_empty_signs_request_with_active_auth() {
        ...
        let am = Arc::new(AuthManager::new(dir.path(), WeBuildComConfig::default()));
        am.hot_swap(WeBuildAuth {
