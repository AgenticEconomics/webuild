...
//! inside a cloned repository (`.mcp.json`, project `.webuild/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.webuild/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`xai_webuild_workspace::trust::TrustStore`]
//! reads/writes — lives in `xai-webuild-workspace` (client-side); this module keeps
...
//! on [`xai_webuild_workspace::folder_trust::decide`]; the consume-side nuance is
...
use xai_webuild_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `xai-webuild-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `xai_webuild_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use xai_webuild_workspace::folder_trust::grant_folder_trust;
use xai_webuild_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.webuild/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`xai_webuild_agent::plugins::TrustStore` at `~/.webuild/trusted-plugins`, plus the
...
/// [`xai_webuild_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = xai_webuild_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`xai_webuild_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.webuild/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    xai_webuild_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: xai_webuild_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != xai_webuild_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `webuild mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `webuild --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `webuild mcp doctor`). It MUST
...
///
/// Sources: project `.webuild/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.webuild/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.webuild/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            xai_webuild_tools::implementations::lsp::config::LspServerConfig,
            xai_webuild_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, xai_webuild_tools::implementations::lsp::config::LspServerConfig>
{
    xai_webuild_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use xai_webuild_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(xai_webuild_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; WEBUILD_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because WEBUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. WEBUILD_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because WEBUILD_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. WEBUILD_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because WEBUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; WEBUILD_HOME-isolated store; WEBUILD_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let webuild_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", webuild_home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        ...
        std::fs::create_dir_all(home.path().join(".webuild").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. WEBUILD_HOME-isolated so the trust store is empty;
        // WEBUILD_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = xai_webuild_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. WEBUILD_HOME-isolated so the trust store is empty;
        // WEBUILD_FOLDER_TRUST unset so the default-on feature flag applies.
        use xai_webuild_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). WEBUILD_HOME-isolated so the trust store is empty.
        use xai_webuild_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. WEBUILD_HOME-isolated (empty store).
        use xai_webuild_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".webuild").join("agents");
        ...
        .unwrap();
        let def = xai_webuild_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.webuild/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). WEBUILD_HOME-isolated (empty
        // store); WEBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".webuild").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + WEBUILD_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. WEBUILD_HOME-isolated so the seeded store is the temp one;
        // WEBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // WEBUILD_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(xai_webuild_version::TEST_VERSION_ENV);
        if option_env!("WEBUILD_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.webuild/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. WEBUILD_HOME-isolated (empty store);
        // WEBUILD_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".webuild").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // WEBUILD_HOME-isolated (empty store); WEBUILD_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".webuild").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // WEBUILD_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); WEBUILD_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use xai_webuild_agent::plugins::discovery::DiscoveryConfig;
        use xai_webuild_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".webuild").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // WEBUILD_HOME-isolated so the grant writes to a temp store; WEBUILD_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".webuild").join("hooks");
        ...
        let has_project_hook = |reg: &xai_webuild_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = xai_webuild_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &xai_webuild_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use xai_webuild_tools::implementations::lsp::config::LspServerConfig;
        use xai_webuild_tools::types::config_source::ConfigSource;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.webuild/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.webuild/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use xai_webuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use xai_webuild_tools::types::config_source::ConfigSource;

        // A `<cwd>/.webuild/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.webuild/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let webuild = tmp.path().join(".webuild");
        std::fs::create_dir_all(&webuild).unwrap();
        std::fs::write(webuild.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.webuild/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let webuild = tmp.path().join(".webuild");
        std::fs::create_dir_all(&webuild).unwrap();
        std::fs::write(webuild.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.webuild/config.toml [mcp_servers]`).
    ...
    /// `.webuild/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `webuild --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(xai_webuild_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("WEBUILD_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no WEBUILD_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(xai_webuild_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(xai_webuild_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("WEBUILD_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // WEBUILD_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("WEBUILD_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".webuild").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".webuild").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with WEBUILD_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. WEBUILD_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(xai_webuild_version::TEST_VERSION_ENV);
        if option_env!("WEBUILD_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. WEBUILD_HOME-isolated so the store starts
        // empty; `#[serial]` because WEBUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // WEBUILD_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(xai_webuild_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; WEBUILD_HOME-isolated and
        // WEBUILD_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEBUILD_HOME", home.path());
        let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".webuild").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".webuild").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.webuild/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".webuild")).unwrap();
        std::fs::write(tmp.path().join(".webuild").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
