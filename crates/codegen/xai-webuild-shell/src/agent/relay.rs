...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `webuild login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from WeBuild WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(xai_webuild_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                xai_webuild_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test WeBuildAuth with the given key.
    fn test_auth(key: &str) -> WeBuildAuth {
        WeBuildAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &WeBuildAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = WeBuildAuth {
    ...
    /// Helper: write a WeBuildAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &WeBuildAuth) {
