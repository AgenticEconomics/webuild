...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.webuild_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (webuild -p, subagents).
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&webuild_home);
        }
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    let webuild_auth = auth_manager.current().filter(|a| a.is_xai_auth());
    let user_id = webuild_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = webuild_auth.as_ref().and_then(|a| a.team_id.clone());
