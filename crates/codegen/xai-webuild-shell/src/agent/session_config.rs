...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct WeBuildSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("grok-build", "WeBuild"),
            model("grok-4.5", "WeBuild 4.5"),
        ];
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("grok-build", "WeBuild")];
        let current = acp::ModelId::from("grok-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn session_config_option_serializes_camel_case() {
        ...
        assert_eq!(v["label"], "WeBuild");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn webuild_session_detail_serializes_camel_case() {
        let detail = WeBuildSessionDetail::build(
            "sess-1".to_string(),
