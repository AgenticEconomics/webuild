...
use xai_webuild_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&webuild_home, WeBuildComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    /// Hot-reload the catalog from `~/.webuild/models_cache.json` after an
    ...
    /// When another webuild process sharing `~/.webuild` (a `--no-leader` run, a
    /// newer client, webuild-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `webuild_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::webuild_home::webuild_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.webuild_version.as_deref() != Some(xai_webuild_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            webuild_version: Some(xai_webuild_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "WeBuild" both route to `grok-build`)
...
fn resolve_prefetch_env_with_auth(auth: Option<WeBuildAuth>) -> Option<PrefetchEnv> {
...
fn resolve_prefetch_env(webuild_com_config: Option<WeBuildComConfig>) -> Option<PrefetchEnv> {
    let webuild_home = crate::util::webuild_home::webuild_home();
    let auth_manager = AuthManager::new(&webuild_home, webuild_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<WeBuildAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(webuild_com_config: Option<WeBuildComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(webuild_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("webuild-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeBuildComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("webuild-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeBuildComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use xai_webuild_sampling_types::ReasoningEffortOption;
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("webuild-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeBuildComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("webuild-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["webuild-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("webuild-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["webuild-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("webuild-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["webuild-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("webuild-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["webuild-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("webuild-legacy"));
    }
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("WEBUILD_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            ...
            make_entry_config_with_id(Some("grok-build"), "grok-build", Some("WeBuild")),
            make_entry_config_with_id(
                Some("webuild-composer-2.5-fast"),
                "webuild-composer-2.5-fast",
                Some("WeBuild Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("webuild-composer-2.5-fast"));
        assert_eq!(
