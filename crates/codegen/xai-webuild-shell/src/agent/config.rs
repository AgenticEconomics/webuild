...
use crate::auth::{AuthManager, WeBuildComConfig, OidcAuthConfig};
...
use std::sync::Arc;
use xai_webuild_agent::prompt::skills::SkillsConfig;
use xai_webuild_sampler::{AuthScheme, SamplerConfig};
use xai_webuild_sampling_types::{
...
use xai_webuild_tools::types::compat::{
...
pub struct EndpointsConfig {
    ...
    /// Env: `WEBUILD_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `WEBUILD_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `WEBUILD_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `WEBUILD_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `WEBUILD_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `WEBUILD_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `WEBUILD_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `WEBUILD_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `WEBUILD_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `WEBUILD_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `WEBUILD_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub webuild_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `WEBUILD_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub webuild_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `WEBUILD_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("WEBUILD_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `WEBUILD_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`webuild setup`): explicit `managed_config_url`,
    ...
    /// 1. `webuild_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.webuild_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `webuild_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.webuild_internal_otlp_headers) {
    ...
    /// `webuild_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.webuild_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.webuild_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `WEBUILD_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("WEBUILD_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("WEBUILD_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("WEBUILD_MODELS_BASE_URL"),
            models_list_url: env_string("WEBUILD_MODELS_LIST_URL"),
            feedback_base_url: env_string("WEBUILD_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("WEBUILD_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("WEBUILD_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("WEBUILD_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("WEBUILD_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("WEBUILD_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("WEBUILD_DEPLOYMENT_KEY"),
            managed_config_url: env_string("WEBUILD_MANAGED_CONFIG_URL"),
            ...
            webuild_internal_otlp_traces_endpoint: env_string("WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT"),
            webuild_internal_otlp_headers: env_string("WEBUILD_INTERNAL_OTLP_HEADERS"),
...
pub use xai_webuild_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use xai_webuild_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(xai_webuild_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: xai_webuild_tools::types::compat::CompatCell,
...
pub(crate) use xai_webuild_telemetry::config::env_telemetry_mode;
pub use xai_webuild_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.webuild/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                xai_webuild_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> xai_webuild_agent::plugins::discovery::DiscoveryConfig {
        xai_webuild_agent::plugins::discovery::DiscoveryConfig {
...
/// (`webuild workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `webuild workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "WEBUILD_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("WEBUILD_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("WEBUILD_SUGGESTIONS_AI")
...
///
/// These supplement the built-in scan locations (`.webuild/skills/`,
/// `.agents/skills/`, `~/.webuild/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `webuild inspect`.
    #[serde(skip)]
    pub model_override_warnings: Vec<super::config_model_override_parse::ModelOverrideWarning>,
    pub webuild_com_config: WeBuildComConfig,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Typed as `WeBuildComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<WeBuildComConfig>,
    /// `[desktop]` section — owned by webuild-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<xai_webuild_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `xai_webuild_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from WEBUILD_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<xai_webuild_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `WEBUILD_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.webuild/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xai_webuild_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, xai_webuild_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<xai_webuild_tools::implementations::webuild_build::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<xai_webuild_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<xai_webuild_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xai_webuild_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xai_webuild_agent::config::AgentDefinition) {
...
pub use xai_webuild_agent::config::AgentDefinition;
pub use xai_webuild_agent::config::Effort;
pub use xai_webuild_agent::config::PermissionMode;
pub use xai_webuild_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .webuild/agents/, ~/.webuild/agents/, built-ins)
...
pub struct AgentSelectionConfig {
    ...
    /// Supports environment variable expansion (e.g., `$HOME/.webuild/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            webuild_com_config: WeBuildComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(xai_webuild_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::webuild_home::webuild_home(),
            self.webuild_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.webuild_com_config.oidc.is_none() {
            config.webuild_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.webuild_com_config.oidc.is_none() && config.webuild_com_config.oauth2.is_none() {
            config.webuild_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("WEBUILD_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[webuild_com_config]`.
    /// `[webuild_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[webuild_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("webuild_com_config") {
                ...
                table.insert("webuild_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("WEBUILD_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `WEBUILD_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("WEBUILD_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("WEBUILD_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-webuild-doom-loop-check`
    ...
    /// `WEBUILD_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xai_webuild_sampling_types::DoomLoopRecoveryPolicy> {
        use xai_webuild_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("WEBUILD_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `WEBUILD_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    pub(crate) fn resolve_lsp_tools(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_LSP_TOOLS")
    ...
    pub(crate) fn resolve_web_fetch(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_WEB_FETCH")
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `WEBUILD_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_voice_mode(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_VOICE_MODE")
    ...
    /// `image_gen` tool gate. Default on; gated only by the `WEBUILD_IMAGE_GEN`
    /// env var and managed-config requirement pin.
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_IMAGE_GEN")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xai_webuild_tools::implementations::webuild_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("WEBUILD_IMAGE_EDIT").default(true).resolve()
    }
    ...
    /// Precedence: env `WEBUILD_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    /// image_gen_model_override` config > remote settings `image_gen_model_override`.
    /// `None` → default model (`webuild-imagine-image-quality`).
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "WEBUILD_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_write_file(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_WRITE_FILE")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `WEBUILD_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("WEBUILD_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `WEBUILD_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("WEBUILD_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `WEBUILD_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("WEBUILD_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEBUILD_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use webuild's default OAuth2 (xAI auth.x.ai).
    ///
    ...
    ///
    /// Priority: `--oauth` > WEBUILD_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_webuild_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("WEBUILD_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.webuild/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (WEBUILD_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("WEBUILD_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (WEBUILD_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `WEBUILD_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("WEBUILD_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (WEBUILD_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("WEBUILD_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (WEBUILD_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `WEBUILD_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("WEBUILD_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `WEBUILD_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(webuild_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("WEBUILD_ERROR_REPORTING"))
...
/// `WEBUILD_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn webuild_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("WEBUILD_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.webuild/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::webuild_home::webuild_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `WEBUILD_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        xai_webuild_config::load_merged_requirements().as_ref(),
        env_bool("WEBUILD_EXTERNAL_OTEL"),
...
/// by `webuild setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: xai_webuild_telemetry::external::config::ExternalClientInfo,
) -> Option<xai_webuild_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xai_webuild_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| xai_webuild_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xai_webuild_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = xai_webuild_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = xai_webuild_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        xai_webuild_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = xai_webuild_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    xai_webuild_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
///
/// Source of truth for built-in catalog rows is
/// `xai-webuild-models/default_models.json` (system model settings).
/// Optional `base_url` / `api_base_url` / `env_key` let a row pin a third-party
/// provider; when `base_url` is omitted the historical xAI dual-route defaults
/// still apply (`cli-chat-proxy` + `api.x.ai`).
...
struct DefaultModelJson {
    ...
    /// OpenAI-compatible (or other) inference base URL. When set, this model
    /// uses a single endpoint for all auth methods unless `api_base_url` is also set.
    base_url: Option<String>,
    /// Optional API-key-only base URL (dual routing with `base_url` for session auth).
    api_base_url: Option<String>,
    /// Provider API key env var name(s); string or array (first non-empty wins).
    env_key: Option<EnvKeys>,
}
/// Non-empty trimmed string from an optional JSON field.
fn non_empty_opt(s: Option<String>) -> Option<String> {
    s.map(|v| v.trim().to_owned()).filter(|v| !v.is_empty())
}
/// Resolve built-in row endpoints from `default_models.json` + global endpoints.
///
/// - Explicit `base_url` → third-party / pinned provider (optional `api_base_url`).
/// - No `base_url` → xAI dual route: session → proxy, API key → `api.x.ai`
///   (or an explicit `api_base_url` only).
fn default_model_endpoints(
    endpoints: &EndpointsConfig,
    base_url: Option<String>,
    api_base_url: Option<String>,
) -> (String, Option<String>) {
    let base = non_empty_opt(base_url);
    let api = non_empty_opt(api_base_url);
    match (base, api) {
        (Some(base), api) => (base, api),
        (None, Some(api)) => (endpoints.resolve_inference_base_url(), Some(api)),
        (None, None) => (
    ...
    entries
        ...
        .map(|m| {
            ...
            let (base_url, api_base_url) =
                default_model_endpoints(endpoints, m.base_url, m.api_base_url);
            let config = ModelEntryConfig {
                ...
                max_retries: None,
                api_key: None,
                env_key: m.env_key.filter(|k| !k.is_empty()),
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `WEBUILD_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<xai_webuild_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when WeBuild starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `webuild_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`webuild-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`WEBUILD_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`WEBUILD_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `WEBUILD_MCP_AUTO_RESTART` to opt out).
    ///
...
pub use xai_webuild_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.webuild_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xai-webuild-cli".to_string());
        headers
...
mod tests {
    ...
    use xai_webuild_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_webuild_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use xai_webuild_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    #[test]
    fn resolve_aux_model_honors_webuild_build_override() {
        let endpoints = EndpointsConfig::default();
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["WEBUILD_TEST_ENV_KEY_PRIMARY", "WEBUILD_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "WEBUILD_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEBUILD_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "WEBUILD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEBUILD_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "WEBUILD_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["WEBUILD_TEST_WS_PRIMARY", "WEBUILD_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEBUILD_TEST_WS_PRIMARY" => Some("   ".into()),
                "WEBUILD_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("WEBUILD_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("WEBUILD_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use xai_webuild_test_support::EnvGuard;
        let var = "WEBUILD_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        ...
        let primary = "WEBUILD_TEST_MULTI_ENV_PRIMARY";
        let alias = "WEBUILD_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use xai_chat_state::AuthType;
        use xai_webuild_test_support::EnvGuard;
        let primary = "WEBUILD_TEST_EMPTY_ENV_PRIMARY";
        let alias = "WEBUILD_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "WEBUILD_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "WEBUILD_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = xai_webuild_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_adds_api_key_to_default_model() {
        let dm = crate::models::default_model();
        let baked_base = default_model_entries(&EndpointsConfig::default())
            .get(dm)
            .expect("baked default model")
            .info
            .base_url
            .clone();
        ...
        assert_eq!(
            model.info.base_url, baked_base,
            "base_url should inherit from default_models.json, not be stale"
        );
    }
    #[test]
    fn baked_default_is_qwen_chat_completions_not_xai_proxy() {
        let dm = crate::models::default_model();
        assert_eq!(dm, "qwen3.7-max");
        let endpoints = EndpointsConfig::default();
        let defaults = default_model_entries(&endpoints);
        let qwen = defaults.get(dm).expect("default model present in catalog");
        assert_eq!(qwen.info.model, "qwen3.7-max");
        assert_eq!(qwen.info.api_backend, ApiBackend::ChatCompletions);
        assert_eq!(
            qwen.info.base_url,
            "https://dashscope.aliyuncs.com/compatible-mode/v1"
        );
        assert!(
            qwen.api_base_url.is_none(),
            "third-party default must not dual-route through api.x.ai"
        );
        assert_eq!(
            qwen.env_key.as_ref().map(|k| k.names()),
            Some(vec!["DASHSCOPE_API_KEY", "QWEN_API_KEY"])
        );
        // xAI model remains switchable via /model.
        let grok = defaults.get("grok-build").expect("xAI model still baked in");
        assert_eq!(grok.info.api_backend, ApiBackend::Responses);
        assert_eq!(grok.info.base_url, endpoints.proxy_url());
        assert_eq!(
            grok.api_base_url.as_deref(),
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xai_webuild_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xai_webuild_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xai-webuild-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_webuild_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["webuild[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["webuild["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_webuild_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.webuild_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn webuild_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.webuild_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.webuild_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.webuild_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.webuild_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, WeBuildComConfig};
        let base = WeBuildComConfig {
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "WEBUILD_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-webuild"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-webuild").unwrap();
        assert_eq!(model.info.model, "grok-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_pinned_base_url() {
        ...
        assert_eq!(
            sampling.base_url, model.info.base_url,
    ...
    #[test]
    #[serial]
    fn e2e_default_model_with_provider_env_key_routes_to_pinned_base_url() {
        let (_, models) = resolve_models_from_toml("", None);
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        ...
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("dashscope-key"));
        assert_eq!(
            sampling.base_url, model.info.base_url,
            "provider env_key should hit the pinned OpenAI-compatible endpoint"
        );
        unsafe { std::env::remove_var("DASHSCOPE_API_KEY") };
    }
    #[test]
    #[serial]
    fn e2e_xai_model_with_external_api_key_routes_to_api_xai() {
        let (_, models) = resolve_models_from_toml("", None);
        let model = models
            .get("grok-build")
            .expect("baked xAI model should exist for switching");
        ...
        assert_eq!(
            ...
            "xAI model external API key should route to api.x.ai via api_base_url"
        );
        unsafe { std::env::remove_var("XAI_API_KEY") };
    }
    #[test]
    fn e2e_xai_model_with_session_routes_to_proxy() {
        let (_, models) = resolve_models_from_toml("", None);
        let model = models
            .get("grok-build")
            .expect("baked xAI model should exist for switching");
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-webuild").unwrap();
        ...
        assert_eq!(
            ...
            "default catalog row keeps its pinned base_url under session auth"
        );
    }
    #[test]
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-webuild")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-webuild")),
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "WEBUILD_CLI_CHAT_PROXY_BASE_URL",
            "WEBUILD_XAI_API_BASE_URL",
            "WEBUILD_FEEDBACK_BASE_URL",
            "WEBUILD_TRACE_UPLOAD_URL",
            "WEBUILD_MANAGED_CONFIG_URL",
            "WEBUILD_MODELS_BASE_URL",
            "WEBUILD_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT",
            "WEBUILD_INTERNAL_OTLP_HEADERS",
            "WEBUILD_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEBUILD_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("WEBUILD_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEBUILD_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("WEBUILD_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("WEBUILD_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("WEBUILD_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("WEBUILD_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("WEBUILD_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("WEBUILD_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("WEBUILD_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("WEBUILD_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEBUILD_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("WEBUILD_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEBUILD_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("WEBUILD_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("WEBUILD_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("webuild-imagine-image")).resolve_image_gen_model_override(),
            Some("webuild-imagine-image".to_owned())
        );
        assert_eq!(
            with(Some("webuild-imagine-image-pro"), Some("webuild-imagine-image"))
                .resolve_image_gen_model_override(),
            Some("webuild-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("WEBUILD_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("WEBUILD_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("WEBUILD_GOAL");
            std::env::remove_var("WEBUILD_GOAL_CLASSIFIER");
            std::env::remove_var("WEBUILD_GOAL_PLANNER");
            std::env::remove_var("WEBUILD_GOAL_SUMMARY");
            std::env::remove_var("WEBUILD_GOAL_VERIFIER_N");
            std::env::remove_var("WEBUILD_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("WEBUILD_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("WEBUILD_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("WEBUILD_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEBUILD_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("WEBUILD_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEBUILD_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "WEBUILD_GOAL_USE_CURRENT_MODEL_ONLY";
...
model = "webuild-composer-2.5-fast"
agent_type = "cursor"
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "grok-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "webuild-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "grok-build", agent_type = "grok-build-plan" }
strategist_model = { model = "webuild-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let webuild_build = crate::util::config::GoalRoleModel {
            model: "grok-build".into(),
            agent_type: "grok-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "webuild-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(webuild_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(webuild_build),
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            webuild_internal_otlp_traces_endpoint: None,
            webuild_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `webuild_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_webuild_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                webuild_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: WEBUILD_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                webuild_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                webuild_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `webuild_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                webuild_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> xai_webuild_telemetry::external::config::ExternalClientInfo {
        xai_webuild_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("WEBUILD_EXTERNAL_OTEL", "1")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("WEBUILD_EXTERNAL_OTEL", "0")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("WEBUILD_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `WEBUILD_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("WEBUILD_SUBAGENTS");
            std::env::remove_var("WEBUILD_RESPECT_GITIGNORE");
            std::env::remove_var("WEBUILD_WEB_SEARCH_MODEL");
            std::env::remove_var("WEBUILD_SESSION_SUMMARY_MODEL");
            std::env::remove_var("WEBUILD_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("WEBUILD_CURSOR_RULES_ENABLED");
            std::env::remove_var("WEBUILD_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("WEBUILD_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("WEBUILD_CLAUDE_RULES_ENABLED");
            std::env::remove_var("WEBUILD_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("WEBUILD_MANAGED_MCPS_ENABLED");
            std::env::remove_var("WEBUILD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("WEBUILD_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "WEBUILD_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "WEBUILD_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "WEBUILD_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("WEBUILD_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("WEBUILD_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("WEBUILD_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("WEBUILD_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("WEBUILD_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("WEBUILD_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("WEBUILD_TELEMETRY_ENABLED") };
...
"#,
        ...
        xai_webuild_config::apply_version_overrides(&mut value, &v).unwrap();
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("webuild-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("webuild-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("WEBUILD_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEBUILD_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEBUILD_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("WEBUILD_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEBUILD_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEBUILD_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("WEBUILD_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEBUILD_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEBUILD_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("WEBUILD_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEBUILD_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEBUILD_MCP_RECURSIVE_CONFIG_WATCH", "false") };
