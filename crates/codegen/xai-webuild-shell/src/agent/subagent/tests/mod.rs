...
async fn subagent_inherits_session_cli_overrides() {
    use xai_webuild_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = xai_webuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/webuild/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/webuild/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn dummy_tracker(
    ...
    let cwd = xai_webuild_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn xai_webuild_workspace::file_system::AsyncFileSystem> = Arc::new(
        xai_webuild_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = xai_webuild_subagent_resolution::config::SubagentRole {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = xai_webuild_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = xai_webuild_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        xai_webuild_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use xai_webuild_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(xai_webuild_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use xai_webuild_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn handle_subagent_request_rejects_file_as_cwd() {
    ...
    let tmp_file = tmp_dir.path().join("webuild-test-cwd-file");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("WeBuild:read_file");
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use xai_webuild_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.parent_agent_name = Some("ai-oncall-bot".to_string());
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::WeBuildPlan.as_ref().to_string(),
    );
...
fn subagent_auth_type_rule() {
    ...
    let byok = byok_model_entry("webuild-byok");
    let plain = test_model_entry("webuild-plain");
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("webuild models"));
    }
    assert!(
...
fn test_sampling_config(model_slug: &str) -> xai_webuild_sampling_types::SamplingConfig {
