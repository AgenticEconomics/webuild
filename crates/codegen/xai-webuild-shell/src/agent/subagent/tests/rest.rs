...
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = xai_webuild_subagent_resolution::context::normalize_forked_context(
        items,
    );
    if let ConversationItem::User(u) = &conv[1] {
        let text = u
            ...
            .filter_map(|p| match p {
                xai_webuild_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = xai_webuild_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = xai_webuild_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = xai_webuild_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        xai_webuild_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
        ...
        .filter(|i| {
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                            p, xai_webuild_sampling_types::conversation::ContentPart::Text {
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/webuild-wt/sa-snap".into()),
        snapshot_ref: Some("refs/webuild/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/webuild/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
        parsed.snapshot_ref.as_deref(), Some("refs/webuild/subagent-snapshots/sa-snap")
    );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/webuild-wt/subagent-x".into()),
        snapshot_ref: None,
        effective_model_id: None,
    }
}
...
fn update_subagent_meta_snapshot_ref_persists_to_disk() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/webuild/subagents/sa-write",
    ...
    assert_eq!(reread.snapshot_ref.as_deref(), Some("refs/webuild/subagents/sa-write"));
    assert_eq!(reread.status, "completed");
    assert_eq!(reread.worktree_path.as_deref(), Some("/tmp/webuild-wt/subagent-x"));
}
...
fn update_subagent_meta_snapshot_ref_reports_failure_when_meta_missing() {
    let dir = tempfile::TempDir::new().unwrap();
    assert!(
        ! update_subagent_meta_snapshot_ref(dir.path(), "refs/webuild/subagents/sa-missing",
        "completed")
    );
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/webuild/subagents/x",
        "completed")
    );
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(Some("refs/webuild/subagents/x"), reread.snapshot_ref.as_deref());
...
async fn set_completed_snapshot_ref_updates_in_memory_entry() {
    ...
    coordinator
        .set_completed_snapshot_ref("sa-mem", "refs/webuild/subagents/sa-mem".into());
    let after = coordinator
        .resumable_source_for("sa-mem", "session-A", Path::new("/tmp"))
        .unwrap();
    assert_eq!(after.snapshot_ref.as_deref(), Some("refs/webuild/subagents/sa-mem"));
}
/// Unknown id is a no-op (entry already cap-evicted; meta.json still holds it).
#[test]
fn set_completed_snapshot_ref_unknown_id_is_noop() {
    let mut coordinator = SubagentCoordinator::new();
    coordinator.set_completed_snapshot_ref("ghost", "refs/webuild/subagents/ghost".into());
    assert!(
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    ...
    let ref_name = "refs/webuild/subagents/glue-1";
...
async fn gate_on_completion_clears_model_facing_worktree_path_but_resume_retains_it() {
    let wt = PathBuf::from("/tmp/webuild-wt/subagent-disp-1");
    let mut coordinator = SubagentCoordinator::new();
    ...
    coordinator
        .set_completed_snapshot_ref("disp-1", "refs/webuild/subagents/disp-1".into());
    ...
    assert_eq!(Some("refs/webuild/subagents/disp-1"), src.snapshot_ref.as_deref());
}
...
async fn gate_on_completion_retains_worktree_path_when_not_removed() {
    let wt = PathBuf::from("/tmp/webuild-wt/subagent-keep-1");
    let mut coordinator = SubagentCoordinator::new();
...
async fn disposal_completes_before_subagent_is_observable() {
    ...
    let ref_name = "refs/webuild/subagents/order-1";
    let snapshot_ref = crate::session::worktree::snapshot_subagent_worktree(
            &wt,
            &repo,
            ref_name,
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.webuild/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
        worktree.as_deref(),
        Some(Path::new("/home/user/.webuild/worktrees/myrepo/subagent-sub-wt",)),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/webuild-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.webuild/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_accounts_for_images() {
    use xai_webuild_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
    let text_only = vec![
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    let dir = std::env::temp_dir()
        .join("webuild-test-durable-resume")
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/webuild-wt/sa-dur".into()),
        snapshot_ref: None,
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/webuild-wt/sa-dur"));
...
fn durable_fallback_rejects_running_status() {
    let dir = std::env::temp_dir()
        .join("webuild-test-durable-status")
        .join(uuid::Uuid::now_v7().to_string());
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
        xai_webuild_subagent_resolution::validate_resume_identity("general-purpose", None, &
        source,).is_ok()
    );
    assert_eq!(
...
fn durable_meta_roundtrips_effective_model_id() {
    let dir = std::env::temp_dir()
        .join("webuild-test-model-roundtrip")
        .join(uuid::Uuid::now_v7().to_string());
    let _ = std::fs::create_dir_all(&dir);
    let meta = SubagentMeta {
...
fn resume_model_pinning_overrides_default_resolution() {
    ...
    let resolved_model = "webuild-light";
...
fn non_cursor_persona_injected_as_system_reminder() {
    use xai_webuild_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(& result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_webuild_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn make_test_skill(
    ...
    xai_webuild_tools::implementations::skills::types::SkillInfo {
        ...
        scope: xai_webuild_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
