...
pub(crate) async fn handle_subagent_request(
    ...
    }
    {
        ...
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(xai_webuild_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = xai_webuild_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    } else if effective_runtime.isolation != xai_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("webuild-subagent-worktrees").join(&request.id)
            }
        };
    ...
    {
        use xai_webuild_tools::implementations::webuild_build::task::MAX_SUBAGENT_DEPTH;
    ...
    {
        let reminder = xai_webuild_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = xai_webuild_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            xai_webuild_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<xai_webuild_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = xai_webuild_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<xai_webuild_tools::registry::types::ToolConfig> = vec![
            (& webuild_build::ReadFileTool).into(), (& webuild_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = xai_webuild_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == xai_webuild_hooks::event::HookEventName::Stop {
                            s.event = xai_webuild_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    xai_webuild_agent::config::McpServerRef::Named(name) => {
                    ...
                    xai_webuild_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            xai_webuild_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            xai_webuild_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    xai_webuild_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                xai_webuild_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    xai_webuild_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            xai_webuild_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        xai_webuild_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        xai_webuild_telemetry::events::Outcome::Cancelled
    } else {
        xai_webuild_telemetry::events::Outcome::Error
    };
    xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/webuild/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
