...
use xai_webuild_tools::implementations::webuild_build::task::types::*;
use xai_webuild_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<xai_webuild_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: xai_webuild_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::WeBuildAuth>,
    ...
    pub parent_terminal_backend: Option<Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<xai_webuild_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<xai_webuild_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig,
    ...
    pub plugin_registry: Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<xai_webuild_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<xai_webuild_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<xai_webuild_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<xai_webuild_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: xai_webuild_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<xai_webuild_sampler::SharedAttributionCallback>,
    ...
    pub parent_tool_snapshot: Option<Vec<xai_webuild_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<xai_webuild_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: xai_webuild_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: xai_webuild_tools::types::compat::CompatConfig,
    /// Shared set of IDs delivered via auto-wake synthetic prompts.
    pub auto_wake_delivered:
        Option<xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut xai_webuild_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("WEBUILD_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("WEBUILD_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use xai_webuild_subagent_resolution::ResumeSourceData;
...
use xai_webuild_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &xai_webuild_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (xai_webuild_sampler::SamplerConfig, acp::ModelId) {
    use xai_webuild_agent::config::ModelOverride;
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &xai_webuild_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (xai_webuild_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &xai_webuild_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &xai_webuild_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = xai_webuild_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(xai_webuild_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[xai_webuild_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<xai_webuild_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<xai_webuild_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        xai_webuild_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[xai_webuild_sampling_types::conversation::ConversationItem],
) -> bool {
    use xai_webuild_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<xai_webuild_sampling_types::conversation::ConversationItem>,
    ...
    let (conversation, prefix_len) =
        xai_webuild_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &xai_webuild_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use xai_webuild_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<xai_webuild_agent::config::AgentDefinition> {
    let mut def = xai_webuild_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<xai_webuild_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || xai_webuild_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = xai_webuild_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// agent: `WeBuildOrchestrator` parents give children
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut xai_webuild_agent::config::AgentDefinition,
) {
...
fn summarize_tool_config(
    config: &xai_webuild_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
    use std::collections::HashMap;
...
fn resolve_subagent_permission_mode(
    requested: xai_webuild_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> xai_webuild_agent::config::PermissionMode {
    use xai_webuild_agent::config::PermissionMode;
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    xai_webuild_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
/// auto-backgrounded (non-destructively). Env override: `WEBUILD_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("WEBUILD_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<xai_webuild_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = xai_webuild_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = xai_webuild_tools::reminders::wrap_reminder(&message);
