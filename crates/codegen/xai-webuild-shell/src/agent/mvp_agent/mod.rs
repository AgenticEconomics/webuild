...
use xai_webuild_telemetry::id::{agent_id, agent_instance_id};
use xai_webuild_telemetry::session_ctx::log_event;
use xai_webuild_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use xai_webuild_workspace::permission::{ClientType, PermissionEvent};
...
use xai_webuild_workspace::session::git::GitDiscoveryResult;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::WeBuildAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperWeBuildPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "WeBuildPro" => jwt_claim == "superwebuild",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperWeBuildPro" => jwt_claim == "superwebuild_heavy",
        "SuperWeBuildLite" => jwt_claim == "superwebuild_lite",
        _ => false,
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a xai_webuild_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[xai_webuild_announcements::RemoteAnnouncement]>,
    last_emitted: &[xai_webuild_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<xai_webuild_announcements::RemoteAnnouncement>> {
    let current = xai_webuild_announcements::filter_expired_at(
...
/// Override with `WEBUILD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("WEBUILD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// list. Set by `enforce_webuild_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.webuild/`).
    ///
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        xai_webuild_tools::implementations::webuild_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: xai_webuild_tools::implementations::webuild_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        xai_webuild_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        xai_webuild_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: xai_webuild_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled webuild-desktop's first
    ...
    workspace_ops: RefCell<Option<xai_webuild_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<xai_webuild_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &xai_webuild_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<xai_webuild_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !xai_webuild_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = xai_webuild_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use xai_webuild_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-webuild-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-webuild-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = xai_webuild_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_webuild_code_access(&self, auth: &crate::auth::WeBuildAuth) {
        if !auth.is_xai_auth() {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings webuild_build_access_gate)"
            );
    ...
    ///    against `~/.webuild/bundled/` and the manifest.
...
/// absent — matching the `webuild_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_webuild_code_access` (initial login gate) and
