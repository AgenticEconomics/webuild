...
fn jwt_tier_claim_maps_free_and_paid() {
    ...
    assert_eq!(
        ...
        Some("superwebuild_heavy")
    );
    assert_eq!(
        jwt_tier_claim(&jwt_with_tier(6)).as_deref(),
        Some("superwebuild_lite")
    );
    assert_eq!(jwt_tier_claim(&jwt_with_tier(99)).as_deref(), Some("99"));
}
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::WeBuildAuth {
    crate::auth::WeBuildAuth {
        key: key.into(),
        auth_mode: mode,
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ("superwebuild", "WeBuildPro"),
        ("x_basic", "XBasic"),
        ("x_premium", "XPremium"),
        ("x_premium_plus", "XPremiumPlus"),
        ("superwebuild_heavy", "SuperWeBuildPro"),
        ("superwebuild_lite", "SuperWeBuildLite"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "WeBuildPro"));
...
fn harness_pair(id: &str) -> Vec<xai_webuild_sampling_types::conversation::ConversationItem> {
    use xai_webuild_sampling_types::ToolCall;
    use xai_webuild_sampling_types::conversation::ConversationItem;
    vec![
...
async fn upload_harness_trace_turns_numbers_siblings_and_persists_counter() {
    ...
    let queue = crate::upload::trace::spawn_upload_queue(
        queue_home.path(),
        &queue_cfg,
        Some(xai_webuild_version::VERSION),
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_webuild_build() {
    let prev = std::env::var("WEBUILD_AGENT").ok();
    unsafe {
        std::env::remove_var("WEBUILD_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("WEBUILD_AGENT", v) }
    }
}
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xai_webuild_agent::AgentDefinition::from_json(&serde_json::json!(
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_webuild_build_family() {
    let prev = std::env::var("WEBUILD_AGENT").ok();
    unsafe {
        std::env::remove_var("WEBUILD_AGENT");
    }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use xai_webuild_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".webuild").join("agents");
    ...
    assert_eq!(
        def.model,
        xai_webuild_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("WEBUILD_AGENT", v) },
        None => unsafe { std::env::remove_var("WEBUILD_AGENT") },
    }
}
#[test]
...
#[test]
fn null_agent_type_falls_back_to_session_default_webuild_build_plan() {
    assert_eq!(
...
#[test]
fn null_agent_type_falls_back_to_session_default_webuild_build() {
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use xai_webuild_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = xai_webuild_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xai_webuild_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig::default(),
        video_gen_config: xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xai_webuild_tools::implementations::webuild_build::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xai_webuild_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(xai_webuild_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: xai_webuild_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        ...
        hook_registry: None,
        workspace_ops: xai_webuild_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xai_webuild_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use xai_webuild_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", false, Some("webuild-tui")),
        ),
        (
            sid_vscode.clone(),
    ...
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("webuild-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", true, Some("webuild-tui")),
        ),
        (
            sid_other.clone(),
    ...
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("webuild-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("webuild-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("webuild-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::WeBuildWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::WeBuildAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::WeBuildAuth::test_default()
            });
            use acp::Agent as _;
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, WeBuildComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), WeBuildComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::WeBuildAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, WeBuildComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), WeBuildComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("webuild-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("webuild-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("webuild-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("webuild-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("webuild-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::WeBuildWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::WeBuildTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::WeBuildWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::WEBUILD_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::WeBuildAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(WeBuildAuth::test_default());
...
/// Minimal agent whose `webuild_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.webuild_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use xai_webuild_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEBUILD_DISABLE_API_KEY_AUTH");
...
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{WEBUILD_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use xai_webuild_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEBUILD_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
...
async fn cached_token_fallthrough_falls_to_webuild_com_without_credentials() {
    use crate::agent::auth_method::{
        WEBUILD_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use xai_webuild_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEBUILD_DISABLE_API_KEY_AUTH");
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xai_webuild_tools::implementations::webuild_build::video_gen::{
    ...
    fn zdr_s3() -> ZdrVideoOutputS3Config {
        ZdrVideoOutputS3Config {
            ...
            key_prefix: "webuild-videos/".into(),
            expires_secs: 900,
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::WeBuildAuth::test_default());
    assert!(
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xai_webuild_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = xai_webuild_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(xai_webuild_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_webuild_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV);
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (webuild-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("webuild-tui"));
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use xai_webuild_test_support::EnvGuard;
    use xai_webuild_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("WEBUILD_HOME", home.path());
    let _sim = EnvGuard::set(xai_webuild_version::TEST_VERSION_ENV, "0.0-sim");
...
fn ann(id: &str) -> xai_webuild_announcements::RemoteAnnouncement {
    xai_webuild_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<xai_webuild_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xai_webuild_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xai_webuild_announcements::RemoteAnnouncement {
