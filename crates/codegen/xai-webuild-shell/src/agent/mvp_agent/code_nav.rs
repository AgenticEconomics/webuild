...
impl MvpAgent {
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        let git_root = xai_webuild_workspace::session::git::find_git_root_from_path(cwd).ok();
        match &setting {
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = xai_webuild_workspace::session::git::find_git_root_from_path(cwd)
