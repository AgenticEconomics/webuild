...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        xai_webuild_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::webuild_home::webuild_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    xai_webuild_workspace::permission::cleanup_stale_permission_state(
        ...
        xai_webuild_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_webuild_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("webuild-web") => client_type = ClientType::WeBuildWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("grok-code-extension") => client_type = ClientType::Extension,
                Some("webuild-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.webuild_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.webuild_com_config.auth_provider_label.clone(),
                cfg.webuild_com_config.auth_provider_command.is_some(),
                cfg.webuild_com_config.oidc.is_some(),
                issuer,
            )
        };
        ...
        xai_webuild_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "webuild_home" : crate ::util::webuild_home::webuild_home().display()
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("WEBUILD_AGENT_METADATA");
                    serde_json::json!(
                        { "webuildShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (xai_webuild_mcp::wire::MCP_SDK) :
                        ...
                        xai_webuild_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().webuild_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(xai_webuild_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().webuild_com_config.preferred_method, Some(crate
            ...
            auth_method::WEBUILD_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let webuild_ctx = self.auth_manager.webuild_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_webuild_oauth(cli_oauth);
                tracing::debug!(
                ...
                let auth_result = if !auth_meta.headless {
                    ...
                    tokio::select! {
                        ...
                        webuild_ctx, crate ::auth::AuthChannels { url_tx : Some(url_tx),
                    ...
                    tokio::select! {
                        biased; _ = cancel.cancelled() => { cancelled = true;
                        Err(anyhow::anyhow!("Authentication cancelled")) } r = crate
                        ::auth::run_auth_flow(& self.auth_manager, webuild_ctx, auth_meta
    ...
    async fn new_session(
        ...
        xai_webuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || xai_webuild_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = xai_webuild_telemetry::context::collect_git_context(&cwd_str);
                let ev = xai_webuild_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                xai_webuild_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match xai_webuild_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        xai_webuild_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        xai_webuild_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        xai_webuild_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => xai_webuild_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            xai_webuild_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = xai_webuild_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : xai_webuild_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a webuild worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            xai_webuild_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use xai_webuild_workspace::session::git::RestoreKind;
            let outcome = xai_webuild_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = xai_webuild_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let is_webuild_build = persisted_model.0.starts_with("grok-build");
        let same_family_fallback = if is_webuild_build {
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    xai_webuild_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = xai_webuild_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = xai_webuild_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(xai_webuild_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = xai_webuild_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = xai_webuild_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `webuild login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_webuild_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_webuild_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == xai_webuild_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                xai_webuild_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            xai_webuild_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
