...
//! (`x.ai/folder_trust/request`) that asks a GUI client (webuild-desktop) to decide
...
//! verdict is [`xai_webuild_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.webuild/lsp.json` servers start on the NEXT session open (the durable grant
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = xai_webuild_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                xai_webuild_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a xai_webuild_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a xai_webuild_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.webuild/hooks`, `.cursor/hooks.json`),
