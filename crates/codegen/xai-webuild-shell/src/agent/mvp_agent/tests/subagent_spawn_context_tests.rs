...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = xai_webuild_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                xai_webuild_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    xai_webuild_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                xai_webuild_workspace::permission::AccessKind::Read(Some(".env".into())),
                xai_webuild_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        xai_webuild_workspace::permission::Decision::PolicyDeny(_)
                    ),
