...
impl MvpAgent {
    ...
    pub(crate) fn sync_process_static_api_key(&self, preferred_model_id: Option<&str>) {
        if self.cfg.borrow().webuild_com_config.api_key_auth_disabled() {
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::WeBuildAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
                if self.is_session_based_auth() {
                    ...
                    if auth.is_some() {
                        xai_webuild_telemetry::unified_log::info(
    ...
    /// must not block the ACP response (webuild-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled webuild-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            xai_webuild_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                xai_webuild_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current WeBuildAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::WeBuildAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &xai_webuild_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on webuild-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<xai_webuild_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    xai_webuild_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    xai_webuild_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match xai_webuild_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => xai_webuild_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().webuild_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_xai_api_key(
                self.cfg.borrow().webuild_com_config.api_key_auth_disabled(),
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let Some(method_id) = self.cached_token_fallthrough_method_id() else {
            ...
            xai_webuild_telemetry::unified_log::warn(
        ...
        let meta = if method_id.0.as_ref() == auth_method::WEBUILD_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::WeBuildAuth) {
        ...
        let (
            ...
            let webuild_user_id = is_xai.then(|| user_id.clone());
            let webuild_team_id = is_xai.then(|| team_id.clone()).flatten();
        ...
        xai_webuild_telemetry::client::init(
            telemetry_config,
            telemetry_mode,
            webuild_user_id,
            webuild_team_id,
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<xai_webuild_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| xai_webuild_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("xai-webuild-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use xai_webuild_tools::implementations::webuild_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig {
        use xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig;
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `WEBUILD_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `WEBUILD_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if !cfg.webuild_com_config.api_key_auth_disabled() {
        ...
        if relay_sync_enabled {
            tracing::info!("[webuild] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_xai_auth {
            tracing::info!(
                "[webuild] Relay sync: DISABLED (no auth - run 'webuild login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: xai_webuild_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: xai_webuild_tools::implementations::webuild_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().webuild_com_config.auth_provider_command.clone(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<xai_webuild_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> xai_webuild_tools::implementations::webuild_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<xai_webuild_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or WEBUILD_RELAY_SYNC_ENABLED)
    ...
    pub async fn execute_hooks_action(
            ...
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&xai_webuild_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<xai_webuild_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::WeBuildSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<xai_webuild_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(xai_webuild_version::VERSION.to_string()),
                workspace_type: None,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                xai_webuild_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// 5. `WEBUILD_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `WEBUILD_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`xai_webuild_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<xai_webuild_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> xai_webuild_agent::AgentDefinition {
        use xai_webuild_agent::AgentDefinition;
        let webuild_agent_env_set = std::env::var("WEBUILD_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(xai_webuild_agent::config::is_strict_harness_agent_type);
        if !webuild_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = xai_webuild_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = xai_webuild_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("WEBUILD_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("grok-build-concise") | Some("webuild_build_concise") => {
                AgentDefinition::webuild_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::webuild_build_plan()
                    }
                }
            }
            Some(name) => {
                xai_webuild_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::webuild_build_plan)
            }
            None => AgentDefinition::webuild_build_plan(),
        };
        if !webuild_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = xai_webuild_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn xai_webuild_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                .unwrap_or(false)
                || std::env::var("WEBUILD_LOC_TRACKING")
        ...
        let mut session_env = xai_webuild_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                xai_webuild_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            xai_webuild_agent::config::ModelOverride::Override(id) => {
            ...
            xai_webuild_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = xai_webuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = xai_webuild_tools::util::webuild_home::webuild_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".webuild").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                xai_webuild_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = xai_webuild_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = xai_webuild_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    xai_webuild_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
