...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.webuild/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** webuild process
    /// sharing the same `~/.webuild` (the writer may also be this process;
...
/// Watches `~/.webuild/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.webuild/config.toml`, `.mcp.json`,
...
/// processes (e.g. `webuild login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.webuild/` (catches `<cwd>/.webuild/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.webuild/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let webuild_home_buf = webuild_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(webuild_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(webuild_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(webuild_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(webuild_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %webuild_home.display(),
                    error = %e,
                    "failed to watch webuild home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.webuild/config.toml` so the loop above
        // watches `<cwd>/.webuild/`), the call below installs a
        ...
        tracing::info!(
            webuild_home = %webuild_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.webuild/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.webuild/config.toml` trigger a
    ...
    /// `<cwd>/.webuild/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.webuild/` does not yet
/// exist at session-open time, the `.webuild/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.webuild/` followed by a
/// write to `<cwd>/.webuild/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let webuild_dir = cwd.join(".webuild");
    if let Err(e) = debouncer
        .watcher()
        .watch(&webuild_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .webuild directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let webuild_dir = cwd.join(".webuild");
    if let Err(e) = debouncer.watcher().unwatch(&webuild_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .webuild directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.webuild/` hasn't
...
/// Watches skill directories (`~/.webuild/skills/`, `<repo>/.webuild/skills/`, etc.)
...
/// recursively: `webuild_home` (`~/.webuild`, or `$WEBUILD_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.webuild` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, webuild_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, webuild_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".webuild", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, webuild_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(webuild_home) {
        return true;
    }
...
/// `~/.webuild` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](xai_webuild_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_webuild_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &webuild_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &webuild_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only webuild_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_webuild_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let webuild_home = home.join(".webuild");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &webuild_home, Some(home));

        // webuild_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&webuild_home));
        ...
        assert!(!g(&home.join("repo").join(".webuild")));
    }

    /// Regression for the ~/.webuild inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.webuild/worktrees/`) must not
    ...
    /// A write to `<webuild_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another webuild process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.webuild/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let webuild_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_webuild = cwd.path().join(".webuild");
        fs::create_dir_all(&project_webuild).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_webuild.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_webuild.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.webuild/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.webuild/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_webuild = new_cwd.path().join(".webuild");
        fs::create_dir_all(&project_webuild).unwrap();
        fs::write(project_webuild.join("config.toml"), "").unwrap();
