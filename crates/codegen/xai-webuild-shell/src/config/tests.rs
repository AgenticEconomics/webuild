...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "WEBUILD_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$WEBUILD_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${WEBUILD_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$WEBUILD_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$WEBUILD_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the WEBUILD_MEMORY env var.
...
/// Run `f` with WEBUILD_MEMORY explicitly unset.
fn without_webuild_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("WEBUILD_MEMORY", None, f)
}
/// Run `f` with WEBUILD_MEMORY set to a specific value.
fn with_webuild_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("WEBUILD_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_webuild_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_webuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "WEBUILD_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_webuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "WEBUILD_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_webuild_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "WEBUILD_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_webuild_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "WEBUILD_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_webuild_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override WEBUILD_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_webuild_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, WEBUILD_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_webuild_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override WEBUILD_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$WEBUILD_TEST_USER $WEBUILD_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the WEBUILD_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with WEBUILD_SUBAGENTS explicitly unset.
fn without_webuild_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("WEBUILD_SUBAGENTS", None, f)
}
/// Run `f` with WEBUILD_SUBAGENTS set to a specific value.
fn with_webuild_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("WEBUILD_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_webuild_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_webuild_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "WEBUILD_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_webuild_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "WEBUILD_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_webuild_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override WEBUILD_SUBAGENTS=0");
        },
    );
}
#[test]
    ...
    with_webuild_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "WEBUILD_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "WEBUILD_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("WEBUILD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "WEBUILD_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "WEBUILD_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "WEBUILD_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("WEBUILD_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_webuild_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_webuild_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "WEBUILD_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("WEBUILD_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_webuild_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_webuild_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_webuild_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn tools_config_env_var_disables() {
    with_webuild_respect_gitignore(
        "0",
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let tc = ToolsConfig::resolve(&config);
    ...
    with_webuild_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "WEBUILD_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".webuild/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".webuild/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".webuild/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".webuild").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".webuild/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".webuild/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".webuild").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".webuild").join("bundled");
    std::fs::create_dir_all(workspace.join(".webuild").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".webuild").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".webuild").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".webuild").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".webuild/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".webuild/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".webuild/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".webuild/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".webuild/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".webuild/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
    ...
    std::fs::remove_file(home.join(".webuild/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".webuild/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.webuild/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn roles_coexist_with_models_and_toggle() {
    ...
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.enabled);
    assert_eq!(cfg.models.get("explore").map(| s | s.as_str()), Some("webuild-fast"));
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "WEBUILD_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
#[test]
fn validate_hooks_path_rejects_outside_webuild_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("must be under ~/.webuild/"),
        "should mention ~/.webuild/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let webuild_home = crate::util::webuild_home::webuild_home();
    let traversal = format!("{}/../evil", webuild_home.display());
...
#[test]
fn validate_hooks_path_accepts_webuild_hooks_subdir() {
    let webuild_home = crate::util::webuild_home::webuild_home();
    let valid_path = webuild_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.webuild/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use xai_webuild_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own webuild yolo.
#[test]
...
fn simulate_release_build() -> xai_webuild_test_support::EnvGuard {
    xai_webuild_test_support::EnvGuard::set(xai_webuild_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). WEBUILD_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores WEBUILD_HOME even on panic). No user-global
/// `$WEBUILD_HOME/config.toml` is seeded: `webuild_home()` is `OnceLock`-cached,
...
fn resolve_effective_plugins_config_gates_project_paths_on_folder_trust() {
    ...
    let webuild = repo.path().join(".webuild");
...
/// in `xai-webuild-agent`. An ABSOLUTE plugin path is used so the merged
...
/// WEBUILD_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_webuild_agent::plugins::{TrustStore, discover_plugins};
    use xai_webuild_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("WEBUILD_HOME", home.path());
    let _flag = EnvGuard::unset("WEBUILD_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. WEBUILD_HOME-isolated (empty
/// store); WEBUILD_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let webuild = repo.path().join(".webuild");
    std::fs::create_dir_all(&webuild).unwrap();
    std::fs::write(webuild.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
