...
/// Default scopes for the xAI OAuth2 provider. Includes `webuild-cli:access`
...
pub struct WeBuildComConfig {
    ...
    /// Login button label (env: `WEBUILD_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `WEBUILD_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `WEBUILD_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[webuild_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`WEBUILD_OAUTH2_ISSUER` / `WEBUILD_OAUTH2_CLIENT_ID`).
///
...
/// auth.json scope key used by the pre-OIDC `webuild login --legacy` flow.
...
impl WeBuildComConfig {
    ...
    /// `WEBUILD_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `webuild login --devbox` / `--api-key` bypass
...
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("WEBUILD_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("WEBUILD_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("WEBUILD_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("WEBUILD_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("WEBUILD_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("WEBUILD_OAUTH2_REFERRER")
...
impl Default for WeBuildComConfig {
    fn default() -> Self {
        ...
        Self {
            webuild_ws_origin: std::env::var("WEBUILD_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            webuild_ws_url: std::env::var("WEBUILD_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "xai-webuild-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("WEBUILD_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("WEBUILD_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("WEBUILD_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("WEBUILD_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for webuild's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `WEBUILD_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `WEBUILD_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("WEBUILD_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("WEBUILD_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("WEBUILD_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("WEBUILD_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("WEBUILD_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: WeBuildComConfig = toml::from_str(
            r#"
        ...
        let cfg: WeBuildComConfig = toml::from_str("").expect("parse empty");
