...
fn write_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &WeBuildAuth) {
...
async fn oidc_refresher_e2e_full_refresh_cycle() {
    ...
    let mgr = Arc::new(
        AuthManager::new(dir.path(), WeBuildComConfig::default()).with_proxy_base_url(&base_url),
    );
...
fn expired_oidc_for(base_url: &str) -> WeBuildAuth {
    WeBuildAuth {
        key: "old-expired-token".into(),
        create_time: Utc::now() - Duration::hours(2),
        user_id: "user-42".into(),
...
impl AuthSnapshot for EmptySnapshot {
    fn current(&self) -> Option<WeBuildAuth> {
        None
    }
    fn expired_auth(&self) -> Option<WeBuildAuth> {
        None
    }
    fn read_disk_auth(&self) -> Option<WeBuildAuth> {
        None
    }
