...
use xai_webuild_telemetry::events::{AuthTokenKind, ManualAuthReason};
...
fn expired_oidc(base_url: &str) -> WeBuildAuth {
    WeBuildAuth {
        key: "expired-at".into(),
...
async fn auth_backend_contract_dead_token_emits_typed_manual_auth_event() {
    ...
    for (source, want_surface) in [
        ...
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(
            AuthManager::new(dir.path(), WeBuildComConfig::default()).with_proxy_base_url(&url),
        );
    ...
    let ok_manager = Arc::new(
        AuthManager::new(ok_dir.path(), WeBuildComConfig::default()).with_proxy_base_url(&ok_url),
    );
