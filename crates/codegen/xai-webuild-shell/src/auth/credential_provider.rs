...
use crate::util::webuild_auth_credentials::WeBuildAuthCredentials;
use reqwest::RequestBuilder;
use std::sync::Arc;
use xai_webuild_auth::{
...
fn api_key_id_for(auth: Option<&crate::auth::WeBuildAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_webuild_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: WeBuildAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = WeBuildAuthCredentials::new(None);
...
pub(crate) fn embedding_session_credentials(
...
) -> xai_webuild_memory::EndpointScopedCredentials {
    ...
    xai_webuild_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"webuild-shell"`   — classic WeBuild CLI / TUI (xai-webuild-shell)
/// - `"webuild-pager"`   — new WeBuild Pager / TUI (xai-webuild-pager)
/// - `"webuild-desktop"` — WeBuild Desktop app
/// - `"webuild-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-webuild-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-webuild-client-identifier`
///
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_webuild_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = WeBuildAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = WeBuildAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_webuild_telemetry::external::set_identity(
            xai_webuild_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-webuild-pager` and `xai-webuild-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> xai_webuild_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let webuild_com_config = crate::auth::WeBuildComConfig::default();
    let exporter = xai_webuild_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = webuild_com_config.token_header.clone();
    let webuild_home = crate::util::webuild_home::webuild_home();
    let bootstrap = Arc::new(AuthManager::new(&webuild_home, webuild_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    xai_webuild_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::WeBuildComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use xai_webuild_auth::AuthCredentialProvider;
    /// Serializes tests that pin `WEBUILD_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `WEBUILD_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("WEBUILD_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("WEBUILD_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("WEBUILD_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("WEBUILD_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> WeBuildAuth {
        WeBuildAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<WeBuildAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::WeBuildComConfig::default(),
        ));
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(WeBuildAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: xai_webuild_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_webuild_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    /// user token, matching the precedence in `WeBuildAuthCredentials::apply`.
