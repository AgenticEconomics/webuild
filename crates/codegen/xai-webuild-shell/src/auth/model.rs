...
/// auth.json scope key for plain API key auth (desktop login, `webuild login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from webuild-desktop login or `webuild login --api-key`)
    ApiKey,
}
...
pub struct WeBuildAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_webuild_code_access: Option<bool>,
...
impl std::fmt::Debug for WeBuildAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WeBuildAuth")
...
impl WeBuildAuth {
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &WeBuildAuth) {
...
#[cfg(test)]
impl WeBuildAuth {
    /// Returns a `WeBuildAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// WeBuildAuth { key: "my-key".into(), ..WeBuildAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, WeBuildAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `webuild login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<WeBuildAuth> {
...
/// Early-invalidation buffer. Override with `WEBUILD_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("WEBUILD_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &WeBuildAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &WeBuildAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> WeBuildAuth {
        WeBuildAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn is_xai_auth_matrix() {
        ...
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| WeBuildAuth {
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperWeBuildPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperWeBuildPro"));
    }
