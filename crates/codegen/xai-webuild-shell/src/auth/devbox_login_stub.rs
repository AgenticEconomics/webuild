...
//! points that can still be reached directly (`webuild login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::WeBuildAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<WeBuildAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<WeBuildAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `webuild login --devbox` entry point: always errors in this build.
pub async fn run_devbox_login(_config: &crate::agent::config::Config) -> anyhow::Result<WeBuildAuth> {
    anyhow::bail!(UNAVAILABLE)
}
