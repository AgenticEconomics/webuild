...
use crate::auth::config::WeBuildComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use xai_webuild_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, WeBuildAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<WeBuildAuth>>>,
    path: PathBuf,
    scope: String,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<WeBuildAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(webuild_home: &Path, webuild_com_config: WeBuildComConfig) -> Self {
        let scope = webuild_com_config.auth_scope();
        let proxy_base_url =
        ...
        xai_webuild_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "webuild_home": webuild_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "WEBUILD_HOME": std::env::var("WEBUILD_HOME").unwrap_or_else(|_| "(unset)".into()),
                "WEBUILD_AUTH_PATH": std::env::var("WEBUILD_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "WEBUILD_AUTH": std::env::var("WEBUILD_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // WEBUILD_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("WEBUILD_AUTH") {
            if let Ok(auth) = serde_json::from_str::<WeBuildAuth>(&inline_json) {
                return Self::assemble(
                    Some(auth),
                    webuild_home.join("auth.json"),
                    scope,
                    webuild_com_config,
                    proxy_base_url,
                    None,
            ...
            tracing::warn!("WEBUILD_AUTH set but failed to parse as JSON, falling back to file");
        }

        // WEBUILD_AUTH_PATH: custom file path (overrides default $WEBUILD_HOME/auth.json).
        let path = std::env::var("WEBUILD_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| webuild_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &WeBuildAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.webuild_com_config)?;
    ...
    fn vet_cached(&self, auth: WeBuildAuth) -> Option<WeBuildAuth> {
    ...
    pub(crate) fn current(&self) -> Option<WeBuildAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<WeBuildAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&WeBuildAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<WeBuildAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<WeBuildAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<WeBuildAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &WeBuildAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &WeBuildAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &WeBuildAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.webuild_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `WeBuildAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: WeBuildAuth) -> std::io::Result<WeBuildAuth> {
        ...
        match &write_result {
            Ok(()) => xai_webuild_telemetry::unified_log::info(
            ...
            Err(e) => xai_webuild_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: WeBuildAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut WeBuildAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn webuild_com_config(&self) -> &WeBuildComConfig {
        &self.webuild_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<WeBuildAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: WeBuildAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<WeBuildAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> WeBuildAuth {
        self.inner.read().clone().unwrap_or_else(|| WeBuildAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: WeBuildAuth) -> Option<WeBuildAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<WeBuildAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<WeBuildAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<WeBuildAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in WEBUILD_AUTH mode).
            DiskAuthState::Ok => {
                xai_webuild_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<WeBuildAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<WeBuildAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<WeBuildAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.webuild_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`webuild login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.webuild_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<WeBuildAuth, AuthError> {
        if self.webuild_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        xai_webuild_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) fn is_different_token(&self, candidate: &WeBuildAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<xai_webuild_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[webuild_com] auth_token_ttl` to drive a real schedule.
...
impl xai_webuild_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.webuild_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.webuild_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.webuild_com_config.preferred_method,
...
) -> xai_webuild_tools::types::SharedApiKeyProvider {
