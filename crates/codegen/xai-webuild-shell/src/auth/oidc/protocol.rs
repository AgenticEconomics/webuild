...
use super::super::config::{ForceLoginTeam, WeBuildComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, WeBuildAuth};
use base64::Engine;
...
pub fn is_configured(config: &WeBuildComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &WeBuildComConfig) -> Option<ForceLoginTeam> {
...
pub(super) fn build_webuild_auth(
...
mod tests {
    ...
    fn authorize_url_includes_required_oidc_params() {
        let config = OidcAuthConfig {
            ...
            audience: Some("api://webuild".into()),
        };
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "webuild-cli:access".into()],
            audience: None,
        };
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("webuild-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=webuild-desktop"));
        assert!(!url.contains("referrer=grok-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access webuild-cli:access api:access", "principal_type" : "Team",
