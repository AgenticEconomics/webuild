...
use super::super::config::{WeBuildComConfig, OidcAuthConfig};
use super::super::{AuthManager, WeBuildAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_webuild_auth, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to WeBuild.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.webuild_com_config().oauth2.as_ref();
    ...
    let principal_policy = login_principal_policy(auth_manager.webuild_com_config());
    ...
    let mut auth = build_webuild_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), WeBuildComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_webuild_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
