...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, WeBuildAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `webuild login` or set XAI_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-webuild-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-webuild-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
...
///
/// On success, persists credentials to `~/.webuild/auth.json` and returns
/// the authenticated `WeBuildAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `webuild login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, WeBuildComConfig};
    ...
    fn auth_manager_with_webuild_home(
        webuild_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(webuild_home, WeBuildComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let webuild_home = temp_dir.path().join(".webuild");
        std::fs::create_dir_all(&webuild_home).unwrap();
        let auth_manager = auth_manager_with_webuild_home(&webuild_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access webuild-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access webuild-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access webuild-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: WeBuildComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let webuild_home = temp_dir.path().join(".webuild");
        std::fs::create_dir_all(&webuild_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&webuild_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !webuild_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::WeBuildAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_webuild_home(temp_dir.path(), "http://127.0.0.1:9");
