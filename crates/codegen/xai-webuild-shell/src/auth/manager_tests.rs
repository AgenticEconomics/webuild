...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> WeBuildAuth {
    WeBuildAuth {
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older webuild version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = WeBuildAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = WeBuildAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(WeBuildAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = WeBuildAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = WeBuildAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `webuild login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `webuild logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `webuild login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_webuild_code_access`. The `&mut WeBuildAuth` signature already
...
/// contributor adding a token-like field to both `WeBuildAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = WeBuildAuth {
        key: "ROT_KEY".into(),
        ...
        has_webuild_code_access: Some(true),
    ...
    assert_eq!(disk.has_webuild_code_access, snapshot.has_webuild_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use xai_webuild_tools::types::ApiKeyProvider;

    let _xai = EnvGuard::unset("XAI_API_KEY");
    let _legacy = EnvGuard::unset("WEBUILD_CODE_XAI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), WeBuildComConfig::default()));
    mgr.hot_swap(WeBuildAuth {
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = WeBuildAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), WeBuildComConfig::default()).with_proxy_base_url(&base);

    let mut auth = WeBuildAuth {
        key: "tok".into(),
        ..WeBuildAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> WeBuildAuth {
    WeBuildAuth {
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || WeBuildAuth {
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), WeBuildComConfig::default()));
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use xai_webuild_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use xai_webuild_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, WeBuildComConfig::default()));
    ...
    use xai_webuild_telemetry::events::{
