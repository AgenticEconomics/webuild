...
pub(crate) struct ExternalAuthOutput {
    ...
    /// see [`WeBuildAuth::is_xai_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `WeBuildAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<WeBuildAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<WeBuildAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &WeBuildAuth) -> Option<WeBuildAuth> {
...
mod tests {
    ...
    #[test]
    fn sync_sets_webuild_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $WEBUILD_AUTH_EXPIRED", true).unwrap();
        assert_eq!(auth.key, "1");
    }

    #[test]
    fn refresh_carries_zdr_flags_forward() {
        let prev = WeBuildAuth {
