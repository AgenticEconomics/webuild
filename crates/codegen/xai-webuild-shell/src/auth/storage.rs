...
use super::model::{API_KEY_SCOPE, AuthMode, AuthStore, WeBuildAuth, lookup_auth};
...
pub fn read_token_by_scope(webuild_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = webuild_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `webuild login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `webuild login` to re-authenticate.")
    })
}

/// Read the API key from the `xai::api_key` scope in auth.json.
pub fn read_api_key(webuild_home: &Path) -> Option<String> {
    let path = webuild_home.join("auth.json");
...
pub fn store_api_key(webuild_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = webuild_home.join("auth.json");
...
pub fn clear_api_key(webuild_home: &Path) -> std::io::Result<()> {
    let path = webuild_home.join("auth.json");
