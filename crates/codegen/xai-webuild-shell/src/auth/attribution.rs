...
//!
//! 1. [`xai_webuild_telemetry::unified_log::warn`] for the local
//!    `~/.webuild/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`xai_webuild_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xai_webuild_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xai_webuild_sampler::SamplerConfig::attribution_callback`] at every
...
use xai_webuild_sampler::{Auth401AttributionCallback, SamplingConsumer};
use xai_webuild_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`xai_webuild_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xai_webuild_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xai_webuild_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `xai_webuild_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `xai-webuild-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xai-webuild-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.webuild/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `WeBuildAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `WeBuildAuth` (or
    ...
    // `webuild_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.webuild/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> WeBuildAuth {
        WeBuildAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `xai_webuild_tools::Auth401AttributionCallback`
