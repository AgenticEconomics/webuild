...
use crate::auth::model::WeBuildAuth;
use crate::auth::token_type::TokenType;
use xai_webuild_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&WeBuildAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        xai_webuild_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<WeBuildAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<WeBuildAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<WeBuildAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.webuild_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<WeBuildAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            xai_webuild_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<WeBuildAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<WeBuildAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &WeBuildAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, WeBuildAuth};
    ...
    fn rejected_cred() -> Option<WeBuildAuth> {
        Some(WeBuildAuth {
    ...
    impl TokenRefresher for OkRefresher {
        async fn refresh(&self, _reason: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            ...
            RefreshOutcome::Success(Box::new(WeBuildAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), WeBuildComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<WeBuildAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.webuild_com_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.webuild_com_config().auth_scope();
        let expired_different = WeBuildAuth {
