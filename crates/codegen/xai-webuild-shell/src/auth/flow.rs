...
use crate::auth::{AuthManager, WeBuildAuth, WeBuildComConfig, parse_output};
use crate::util::webuild_home;
...
/// session.
fn is_cached_credential_compatible(auth: &WeBuildAuth, webuild_com_config: &WeBuildComConfig) -> bool {
    let expected_issuer = webuild_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| webuild_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(webuild_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("WEBUILD_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `WEBUILD_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `webuild_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("WEBUILD_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
...
) -> anyhow::Result<(WeBuildAuth, bool)> {
    ...
    if is_refresh {
        cmd.env("WEBUILD_AUTH_EXPIRED", "1");
    }
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.webuild_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    webuild_com_config: &WeBuildComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = webuild_com_config.oidc.is_some(),
        has_oauth2 = webuild_com_config.oauth2.is_some(),
        has_external_auth = webuild_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, webuild_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, webuild_com_config)
        }) {
            xai_webuild_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = webuild_com_config.auth_provider_command {
    ...
    // `webuild login --devbox` uses run_devbox_login and is not gated here.
    if !webuild_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / WEBUILD_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(webuild_com_config) {
        return crate::auth::oidc::run_login_flow(webuild_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = webuild_com_config.oauth2 {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `webuild login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(webuild_com_config: &WeBuildComConfig) -> Option<WeBuildAuth> {
    let webuild_home = webuild_home::webuild_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&webuild_home, webuild_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(webuild_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(webuild_com_config).await {
        return Some(auth);
    }
    let webuild_home = webuild_home::webuild_home();
    let auth_manager = Arc::new(AuthManager::new(&webuild_home, webuild_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, webuild_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<WeBuildAuth> {
    auth_manager
        .current_or_expired()
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = webuild_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: WeBuildAuth) -> WeBuildAuth {
...
fn report_signed_in(auth: &WeBuildAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<WeBuildAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(webuild_com_config).await)
    } else {
        ensure_authenticated(webuild_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `webuild login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `WEBUILD_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `webuild login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.webuild_com_config, login_override).await {
        if config.webuild_com_config.oauth2.is_none() {
        ...
        let webuild_home = webuild_home::webuild_home();
        let auth_manager = Arc::new(AuthManager::new(&webuild_home, config.webuild_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.webuild_com_config) {
            eprintln!(
    ...
    // config was actually applied; `webuild setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `WEBUILD_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_webuild_telemetry::external::set_identity(
            xai_webuild_telemetry::external::IdentityAttrs::default(),
        );
        xai_webuild_telemetry::external::flush();
...
/// `webuild logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let webuild_home = webuild_home::webuild_home();
    let auth_manager = AuthManager::new(&webuild_home, config.webuild_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `WEBUILD_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("WEBUILD_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("WEBUILD_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("WEBUILD_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), WeBuildComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only webuild_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), WeBuildComConfig::default()));
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), WeBuildComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("WEBUILD_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("WEBUILD_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `webuild login` must use loopback, not device —
        ...
        let xai = WeBuildComConfig::default();
    ...
    fn oidc_auth(issuer: &str) -> WeBuildAuth {
        WeBuildAuth {
    ...
    fn weblogin_cred_is_never_compatible() {
        let cfg = WeBuildComConfig::default();
    ...
    fn pinned_cfg(team: &str) -> WeBuildComConfig {
        WeBuildComConfig {
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = WeBuildAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = WeBuildComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = WeBuildAuth {
        ...
        // WEBUILD_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into WeBuild: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `webuild login` passes `on_stderr=None`; stderr must be inherited so
