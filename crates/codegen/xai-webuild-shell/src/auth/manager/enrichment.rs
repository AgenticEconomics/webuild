...
use crate::auth::model::{WeBuildAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: WeBuildAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.webuild_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut WeBuildAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: WeBuildAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut WeBuildAuth, user_info: UserInfo) {
