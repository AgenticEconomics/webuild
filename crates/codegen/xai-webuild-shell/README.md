...
- **Using WeBuild**
  ...
  - [SSH Passthrough](#ssh-passthrough-webuild-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`webuild inspect`)](#introspection)
...
- [Building with WeBuild](#building-with-webuild) — headless API, ACP SDK integration
...
Credentials are stored in `~/.webuild/auth.json` and persist across sessions. Tokens expire after 7 days; WeBuild will prompt you to re-authenticate when needed.
...
**3. Run `webuild`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.webuild/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
WeBuild is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
4. **stdout** → captured by WeBuild and saved to `~/.webuild/auth.json` as the access token
5. exit 0 → success; exit non-zero → WeBuild falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. WeBuild reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want WeBuild to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells WeBuild when to proactively refresh. Without it, WeBuild assumes tokens last 30 days.
...
# Print ONLY the token to stdout (WeBuild captures this)
...
export WEBUILD_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export WEBUILD_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, WeBuild cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`webuild -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=webuild-cli")
...
When WeBuild needs to refresh an expired token, it re-runs your binary with `WEBUILD_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:
...
`WEBUILD_AUTH_EXPIRED` is optional — if your binary ignores it, WeBuild still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

WeBuild supports automatic credential refresh for external auth providers and OIDC. When WeBuild detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. WeBuild handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, WeBuild re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), WeBuild re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, WeBuild silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# WeBuild refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export WEBUILD_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `webuild login` before starting — WeBuild runs your binary automatically on first launch. You _can_ run `webuild login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, WeBuild tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
webuild --debug-file /tmp/webuild-auth.log -p "hello"
tail -f /tmp/webuild-auth.log
```
...
If you've authenticated with `webuild login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the webuild CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.webuild/auth.json)" \
  -H "X-XAI-Token-Auth: xai-webuild-cli" \
  -H "§HDR§x-webuild-model-override§: grok-build" \
  -d '{
    "model": "grok-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.webuild/auth.json` (set by `webuild login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-webuild-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `§HDR§x-webuild-model-override§: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `grok-build` which is on the default route, but always safe to include. |
...
> **Note:** `auth.json` tokens expire after 7 days. Run `webuild login` to refresh.
...
webuild --cwd ~/projects/my-app

# Start with an initial task
webuild --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
webuild --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
webuild --always-approve --prompt "Format all files"
```
...
Run WeBuild non-interactively from the command line. Use headless mode when you need to:
...
webuild -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
webuild -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
webuild -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
webuild -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
webuild -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
webuild -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
webuild -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
webuild -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
webuild -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
webuild --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
webuild -p "What does this project do?"

# Use a specific model
webuild -p "Optimize this function" -m grok-build

# Get JSON output for parsing
webuild -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
webuild -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
webuild -p "Remember: the secret number is 42" --output-format json
webuild -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
webuild -p "Continue where we left off" -c

# Run in a different directory
webuild -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
webuild -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
webuild -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
webuild -p "Generate a README" > README.md

# Parse JSON output with jq
webuild -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
webuild -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | webuild -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  webuild -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
webuild -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run WeBuild as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
```bash
webuild agent headless --webuild-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`webuild ssh`)

Use `webuild ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
webuild ssh user@host

# With SSH flags
webuild ssh -t user@host
webuild ssh -L 8080:localhost:8080 user@host

# With remote command
webuild ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `webuild ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with WeBuild

WeBuild can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `webuild -p` and parses JSON output.
...
class WeBuildChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["webuild", "-p", prompt, "-m", model, "--cwd", self.cwd,
...
class WeBuildChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("webuild", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await WeBuildACPChat(cwd=".").init()
...
class WeBuildACPChat {
  ...
  async init() {
    this.proc = spawn("webuild", ["agent", "stdio"]);
...
// Usage
const client = await new WeBuildACPChat(".").init();
...
WeBuild implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
WeBuild reads configuration from `~/.webuild/config.toml`. If the file doesn't exist, WeBuild uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: WEBUILD_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: WEBUILD_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: WEBUILD_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: WEBUILD_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: WEBUILD_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: WEBUILD_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: WEBUILD_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: WEBUILD_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `WEBUILD_TELEMETRY_BUILD_EVENTS_URL`, `WEBUILD_TELEMETRY_BUILD_EVENTS_API_KEY`, and `WEBUILD_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

WeBuild can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives WeBuild language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, WeBuild can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — WeBuild can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

WeBuild looks for server definitions in:

- project config: `<repo>/.webuild/lsp.json`
- user config: `~/.webuild/lsp.json`
...
- LSP tools are enabled (`WEBUILD_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
WEBUILD_LSP_TOOLS=1 webuild
```
...
If LSP tools are enabled but no usable server config is found, WeBuild emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
WeBuild does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `WEBUILD_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-webuild"

[model.company-webuild]
model = "grok-build"
base_url = "https://webuild-proxy.acme.com/"
name = "WeBuild Latest (Proxy)"
...
With this config, `webuild` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). WeBuild reads these files and appends their contents to the system prompt.

WeBuild scans for agent rules in this order:

1. `~/.webuild/` (global rules)
...
Within each directory, WeBuild checks for these filenames:
...
Skills are reusable prompt packages that extend WeBuild with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

WeBuild discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.webuild/skills/`           | Local | Highest  |
| `<repo_root>/.webuild/skills/` | Repo  | Medium   |
| `~/.webuild/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.webuild/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how WeBuild decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, WeBuild invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when WeBuild automatically invokes the skill. Be specific about trigger phrases and use cases.
...
WeBuild discovers agent definitions from `.webuild/agents/` (project), `~/.webuild/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `WEBUILD_AGENT` env var
...
```bash
webuild --agent-profile ./my-agent.md
# or
export WEBUILD_AGENT="my-custom-agent"
```
...
```bash
export WEBUILD_SUBAGENTS=0              # Environment variable
```
...
model = "grok-build"
prompt_file = ".webuild/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".webuild/personas/concise.md"  # or load from file
```

Both are also discovered from `.webuild/roles/*.toml` and `.webuild/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend WeBuild with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.webuild/plugins/`            | Project |
| `~/.webuild/plugins/`          | User    |
...
WeBuild discovers hooks from `.webuild/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `XAI_API_KEY`. If neither `api_key` nor `env_key` is set, WeBuild falls back to the global `XAI_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, WeBuild falls back to built-in defaults for known models.
...
**How it works:** When you override a built-in model, WeBuild starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `WEBUILD_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `webuild models` output). You also need a `[model.*]` entry so WeBuild knows
...
>
> [model.my-custom-model]              # 2. tell WeBuild how to reach it
...
Point WeBuild at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `WEBUILD_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `WEBUILD_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export WEBUILD_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
webuild
```

WeBuild fetches the model list from `{WEBUILD_MODELS_BASE_URL}/models` on startup and sends inference requests to `WEBUILD_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `WEBUILD_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.webuild/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, WeBuild uses API key auth (`Authorization: Bearer`) instead of session auth. `webuild login` is not required — only the API key.

---

## MCP Servers

Extend WeBuild's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.webuild/config.toml`:
...
MCP servers can also be configured per-project in `.webuild/config.toml`. WeBuild walks from the current directory up to the git repo root, loading `.webuild/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.webuild/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.webuild/config.toml` | This repository   | ↑        |
...
**Example:** commit a `.webuild/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.webuild/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.webuild/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.webuild/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `WEBUILD_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets WeBuild remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.webuild/memory/`:
- **Global** (`~/.webuild/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.webuild/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.webuild/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, WeBuild saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.webuild/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
WeBuild searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
webuild memory edit --global

# Show memory statistics: file count, chunk count, and index size
webuild memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.webuild/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.webuild/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, WeBuild emits the `webuild-shell-memory_injection`
...
WeBuild can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.webuild/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.webuild/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.webuild/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.webuild/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.webuild/sandbox.toml` (global) or `.webuild/sandbox.toml` (per-project):
...
```bash
webuild --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire webuild process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), WeBuild logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.webuild/sandbox-events.jsonl`
...
Use `webuild inspect` to see everything WeBuild discovers in the current directory:

```bash
webuild inspect          # human-readable output
webuild inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.webuild/skills/`, `~/.webuild/skills/`, plugins, and config paths
...
WeBuild automatically discovers configuration from Claude Code directories alongside native `.webuild/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How WeBuild uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.webuild/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by WeBuild at runtime.

---

## Built-in Tools

WeBuild includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `WEBUILD_WEB_FETCH=1`. 
...
WeBuild automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.webuild/sessions/`, organized by URL-encoded working directory:

```
~/.webuild/sessions/<encoded-cwd>/<session-id>/
...
webuild -p "Remember: X=42" -s my-session
webuild -p "What is X?" -s my-session

# Resume existing session (errors if not found)
webuild -p "Continue" -r my-session

# Continue most recent session in current directory
webuild -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
webuild -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.webuild/config.toml` | Configuration file                                  |
| `~/.webuild/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.webuild/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.webuild/memory/`     | Cross-session memory files and index                |
| `~/.webuild/skills/`     | User-scoped skill definitions                       |
| `~/.webuild/plugins/`    | User-scoped plugins                                 |
| `~/.webuild/agents/`     | User-scoped agent definitions                       |
| `.webuild/config.toml`   | Project-scoped config (MCP servers)                 |
| `.webuild/skills/`       | Project-scoped skill definitions                    |
| `.webuild/plugins/`      | Project-scoped plugins                              |
| `.webuild/agents/`       | Project-scoped agent definitions                    |
| `.webuild/hooks/`        | Project-scoped hooks                                |
| `.webuild/lsp.json`      | LSP server configuration                            |
...
| `WEBUILD_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.grok.com/v1`)                          |
| `WEBUILD_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `WEBUILD_MODELS_LIST_URL`          | Override the model list URL if it differs from `{WEBUILD_MODELS_BASE_URL}/models`                                              |
| `WEBUILD_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `WEBUILD_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `WEBUILD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `WEBUILD_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `WEBUILD_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `WEBUILD_HOME`                     | Override config directory (default: `~/.webuild`)                                                           |
| `WEBUILD_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `WEBUILD_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `WEBUILD_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `WEBUILD_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `WEBUILD_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `WEBUILD_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `WEBUILD_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `WEBUILD_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `WEBUILD_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `WEBUILD_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.webuild/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `WEBUILD_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `webuild` commands and flags.
...
webuild completions bash > ~/.local/share/bash-completion/completions/webuild
```

Reload your shell or run `source ~/.bashrc`.

Alternative (WeBuild-managed location):

```bash
mkdir -p ~/.webuild/completions/bash
webuild completions bash > ~/.webuild/completions/bash/webuild.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.webuild/completions/bash/webuild.bash" ]] && source "$HOME/.webuild/completions/bash/webuild.bash"
```
...
webuild completions zsh > ~/.zsh/completions/_webuild
```
...
```bash
mkdir -p ~/.webuild/completions/zsh
webuild completions zsh > ~/.webuild/completions/zsh/_webuild
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.webuild/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `webuild` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `webuild --debug` or `WEBUILD_LOG_FILE` instead:

```bash
# Per-session debug log (~/.webuild/debug/<sessionId>.txt)
webuild --debug

# Log to a custom path
WEBUILD_LOG_FILE=/tmp/webuild-debug.log webuild

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.webuild/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `WEBUILD_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
WEBUILD_LOG_FILE=/tmp/webuild-debug.log RUST_LOG="info,xai_webuild_shell::auth=debug" webuild
```
...
webuild --debug-file /tmp/webuild-auth.log -p "hello"
grep "auth:" /tmp/webuild-auth.log
```
...
cat ~/.webuild/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.webuild/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.webuild/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
