...
- **webuild login** now requests WeBuild Projects scopes so workspace listing works after consent.
...
- **`webuild plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using webuild -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **webuild inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct webuild --minimal --resume command.
...
- **WeBuild no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **webuild setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask WeBuild above the footer shortcuts.
...
- **`webuild worktree ls`** now works as a short alias for `webuild worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.webuild/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.webuild/worktrees/.
...
- **webuild inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain webuild launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive webuild agent stdio without silent parse drops on session/* calls.
...
- **webuild --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **webuild --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **webuild update** on Windows no longer fails when a previous .old executable is still running.
...
- **webuild models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `webuild plugin marketplace add`.
...
- **Plan files** now default to .webuild/plan.md to match WeBuild conventions.
...
- **--minimal** flag now shows in `webuild --help`.
...
- **Tool result previews** no longer paint opaque panels in `webuild --minimal`.
- **webuild wrap** now correctly handles quoted strings and shell aliases.
...
- **WeBuild.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **`webuild update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `webuild wrap`.
...
- **webuild agent stdio** now checks for updates in the background like other modes.
...
- **webuild wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `webuild agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `webuild wrap`** to run any command with local clipboard support.
...
- **Fixed `webuild agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or webuild-desktop.
...
- You can now pass --json-schema to webuild -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `WEBUILD_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **webuild -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in webuild -p and webuild agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `webuild` after running `webuild update`.
- Headless (`webuild -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`webuild plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **webuild sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **webuild quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, webuild sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, WEBUILD_HUNK_TRACKER, or config.
...
- **WeBuild** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`webuild dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`webuild mcp add`** now accepts positional arguments (e.g. `webuild mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`webuild update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.webuild.
...
- **MCP servers menu** and slash commands now work when starting webuild outside a project directory.
...
- **webuild --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `webuild --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `webuild update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`webuild --debug`** now produces per-session log files under ~/.webuild/debug/ even with a leader process.
...
- **`webuild update`** now restarts any older running leader so all clients get the new binary.
...
- **`webuild login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `webuild agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the WEBUILD_IMAGE_GEN_HARNESS env var.
...
- **webuild inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via WEBUILD_CURSOR_MCPS_ENABLED / WEBUILD_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside WeBuild and Claude directories.
...
- **image_gen** now uses the higher-quality webuild-imagine-image-quality model.
