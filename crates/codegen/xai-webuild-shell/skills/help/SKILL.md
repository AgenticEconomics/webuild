...
name: help
description: >
  WeBuild documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any WeBuild feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "WeBuild docs — config, MCP, auth, skills, commands"
---

# WeBuild Help

Answer the user's question about WeBuild setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.webuild/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.webuild/docs/user-guide/`. The available guides are:
   ...
   `~/.webuild/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.webuild/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.webuild/skills/<name>/SKILL.md`
   (read `~/.webuild/docs/user-guide/08-skills.md` for the SKILL.md format).