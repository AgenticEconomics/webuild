...
name: create-skill
description: >
  Interactively create a new WeBuild skill (SKILL.md + optional scripts/references).
  Use when the user wants to create a skill, scaffold a skill, or runs /create-skill.
metadata:
  short-description: "Create a new WeBuild skill"
---

# Create Skill

Interactively gather requirements from the user and create a fully working WeBuild skill on disk.
...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.webuild/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.webuild/skills/<name>/SKILL.md` - available in all projects
...
- Trigger phrases and keywords so WeBuild knows when to auto-invoke it
...
Where `<SKILL_DIR>` is:
- User scope: `~/.webuild/skills/<name>`
- Project scope: `<repo-root>/.webuild/skills/<name>`
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: WeBuild will invoke it when the description matches user intent
