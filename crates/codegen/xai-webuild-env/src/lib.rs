...
//! Backend environment presets for the WeBuild CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `WEBUILD_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct WeBuildEndpoints {
...
impl WeBuildEnvironment {
    ...
    pub fn is_production(&self) -> bool {
        matches!(self, WeBuildEnvironment::Production)
    }
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            WeBuildEnvironment::Production.env_prefix(),
            "WEBUILD_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_WEBUILD_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            WeBuildEnvironment::Production.relay_ws_url(),
            WeBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
