...
pub use xai_webuild_config::webuild_home;

/// Path to `$WEBUILD_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    webuild_home().join("pager.toml")
}

/// User-facing label for the user webuild directory (``~/.webuild`` or ``$WEBUILD_HOME``).
///
/// Derived from resolved [`webuild_home()`] vs `xai_webuild_config::default_webuild_home()`,
/// not from whether `WEBUILD_HOME` is set in the environment.
pub fn display_webuild_home_prefix() -> String {
    if webuild_home() == xai_webuild_config::default_webuild_home() {
        "~/.webuild".to_string()
    } else {
        "$WEBUILD_HOME".to_string()
    }
}

/// User-facing path under [`webuild_home()`], e.g. ``~/.webuild/config.toml``.
pub fn display_user_webuild_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_webuild_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`webuild_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let webuild = webuild_home();
    if let Ok(rest) = path_buf.strip_prefix(&webuild) {
        let prefix = display_webuild_home_prefix();
...
/// True when `path` is under user [`webuild_home()`] (not project `{cwd}/.webuild`).
pub fn is_under_user_webuild_home(path: &Path) -> bool {
    path.starts_with(webuild_home())
}
...
mod tests {
    ...
    #[test]
    fn display_webuild_home_prefix_default_install() {
        if std::env::var("WEBUILD_HOME").is_ok() {
            return;
        }
        assert_eq!(display_webuild_home_prefix(), "~/.webuild");
    }

    #[test]
    fn display_user_webuild_path_joins_relative() {
        let path = display_user_webuild_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".webuild") || path.contains("$WEBUILD_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_webuild() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.webuild/memory/MEMORY.md");
