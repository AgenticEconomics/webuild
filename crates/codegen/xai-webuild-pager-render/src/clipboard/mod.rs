...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(xai_webuild_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(xai_webuild_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `webuild wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `xai-webuild-pager`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `webuild` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `WEBUILD_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_WEBUILD_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `webuild`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("WEBUILD_OSC52_SINK").is_some()
            || std::env::var_os("LC_WEBUILD_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if xai_webuild_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
pub fn resolve_clipboard_route(ctx: &TerminalContext) -> ClipboardRoute {
    ...
    ClipboardRoute {
        ...
        // upstream `webuild wrap` sink is capturing our output and will forward
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = xai_webuild_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = xai_webuild_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`xai_webuild_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        xai_webuild_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = xai_webuild_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match xai_webuild_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use webuild wrap or /minimal."
            }
...
/// Write text and return a toast; emits `webuild-shell-clipboard_copy` when enabled.
...
fn log_clipboard_copy_event(
    ...
    if !xai_webuild_telemetry::client::is_enabled() {
        return;
    }
    xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    xai_webuild_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !xai_webuild_shared::clipboard::x11_display_env_present() {
        return None;
    }
    xai_webuild_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && xai_webuild_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    xai_webuild_telemetry::session_ctx::log_event(
        xai_webuild_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_webuild_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use xai_webuild_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    xai_webuild_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    xai_webuild_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    xai_webuild_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(xai_webuild_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match xai_webuild_shared::clipboard::get_image() {
        Ok(img) => {
