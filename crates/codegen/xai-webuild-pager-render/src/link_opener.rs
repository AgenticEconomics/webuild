...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("WEBUILD_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "WEBUILD_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    xai_webuild_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// Used by the SuperWeBuild upsell flow to attribute clicks to `referrer=grok-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/webuild session/image 1.jpg");
