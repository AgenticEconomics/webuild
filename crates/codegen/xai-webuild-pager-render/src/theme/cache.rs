...
//! lookup must be cheaper than re-loading from `~/.webuild/config.toml`.
...
//! `xai_webuild_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::WeBuildNight as u8);
...
/// back to `WeBuildNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::WeBuildNight as u8 => ThemeKind::WeBuildNight,
        x if x == ThemeKind::WeBuildDay as u8 => ThemeKind::WeBuildDay,
...
/// When `None`, `to_theme_kind()` defaults to `WeBuildNight` / `WeBuildDay`.
...
///
/// On the first call, reads from `~/.webuild/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    xai_webuild_markdown::set_color_level_cap(if locked {
        xai_webuild_markdown::ColorLevel::Basic
    } else {
        xai_webuild_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`WEBUILD_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `WeBuildNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::WeBuildNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `WeBuildNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xai_webuild_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = xai_webuild_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = xai_webuild_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::WeBuildNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::WeBuildDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::WeBuildNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::WeBuildDay);
            assert_eq!(applied, ThemeKind::WeBuildNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::WeBuildNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::webuildday().text_primary,
                "must not serve the cached (WeBuildDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // webuildday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::WeBuildDay), false),
                ThemeKind::WeBuildDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::WeBuildNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_webuildnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::WeBuildNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_webuildday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::WeBuildDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_webuildnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_webuildnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::WeBuildDay), true);
            assert_eq!(result, ThemeKind::WeBuildDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::WeBuildDay);
            assert_eq!(current_kind(), ThemeKind::WeBuildDay);
        });
    }
