...
/// in `config.toml`. When `None`, defaults to `WeBuildNight` / `WeBuildDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::WeBuildDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::WeBuildNight),
    }
}
