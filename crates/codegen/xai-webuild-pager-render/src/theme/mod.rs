...
//! The default theme is WeBuildNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! WeBuildNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub const ALL: &[ThemeKind] = &[
        ThemeKind::WeBuildNight,
        ThemeKind::WeBuildDay,
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::WeBuildNight, ThemeKind::WeBuildDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::WeBuildNight => "webuildnight",
            Self::TokyoNight => "tokyonight",
            Self::WeBuildDay => "webuildday",
    ...
    /// when quantized to 256 or 16 colors. WeBuildNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::WeBuildNight => false,
            Self::TokyoNight => true,
            Self::WeBuildDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "webuildnight" | "webuild-night" | "dark" => Some(Self::WeBuildNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "webuildday" | "webuild-day" | "light" | "day" => Some(Self::WeBuildDay),
...
/// `"webuildnight"` → `"WeBuild Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "webuildnight" => "WeBuild Night",
        "webuildday" => "WeBuild Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.webuild/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::WeBuildNight => Self::webuildnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::WeBuildDay => Self::webuildday(),
            ...
            // if reached, fall back to WeBuildNight.
            ThemeKind::Auto => Self::webuildnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `WEBUILD_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on webuildnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., WeBuildNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // WeBuildNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    xai_webuild_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::WeBuildNight.is_auto());
        assert!(!ThemeKind::WeBuildDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::webuildday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::webuildnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::webuildday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. WeBuildNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::WeBuildNight => Theme::webuildnight(),
                ThemeKind::WeBuildDay => Theme::webuildday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_webuildnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark WeBuildNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::webuildnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("webuildnight"),
            Some(ThemeKind::WeBuildNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::WeBuildNight));
        assert_eq!(ThemeKind::from_name("webuildday"), Some(ThemeKind::WeBuildDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::WeBuildDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("webuildnight", ThemeKind::WeBuildNight),
            ("webuild-night", ThemeKind::WeBuildNight),
            ("dark", ThemeKind::WeBuildNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("webuildday", ThemeKind::WeBuildDay),
            ("webuild-day", ThemeKind::WeBuildDay),
            ("light", ThemeKind::WeBuildDay),
            ("day", ThemeKind::WeBuildDay),
