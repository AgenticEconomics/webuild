...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    ...
    xai_webuild_tools::util::image_validate::validate_image_bytes_unrestricted(bytes, false)
        .ok()
        .map(|(w, h, _)| (w, h))
}
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = xai_webuild_shared::clipboard::mime_from_bytes(&bytes).to_owned();
...
use xai_webuild_shared::clipboard::mime_to_extension;
...
///
/// Delegates to [`xai_webuild_shared::clipboard::mime_to_extension`].
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = xai_webuild_shared::clipboard::mime_from_bytes(&data);
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = xai_webuild_shared::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(xai_webuild_shared::session::session_dir(&info).join("images"))
}
...
pub fn session_mermaid_dir(
    ...
    Some(xai_webuild_shared::session::session_dir(&info).join("mermaid"))
}
...
///   [`xai_webuild_shell::session::placeholder_images::load_placeholder_image`]
...
pub fn build_content_blocks_with_workspace(
    ...
    let allowed: Option<Vec<std::path::PathBuf>> =
        workspace_cwd.map(xai_webuild_shared::placeholder_images::default_allowed_prefixes);
...
pub fn build_content_blocks_with_prefixes(
    ...
    build_content_blocks_with_prefixes_and_caps(
        ...
        images,
        allowed_prefixes,
        xai_webuild_shared::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
/// [`xai_webuild_shell::session::placeholder_images::recover_orphan_placeholders_with_prefixes_and_caps`].
...
pub fn build_content_blocks_with_prefixes_and_caps(
    ...
    // attachment.
    let rewritten_text =
        xai_webuild_shared::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in &images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(
                    xai_webuild_shared::placeholder_images::display_number_meta(img.display_number),
                )),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = xai_webuild_shared::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match xai_webuild_shared::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
                ...
                recovered.push(
                    ImageContent::new(data, loaded.mime_type)
                        ...
                        .meta(Some(
                            xai_webuild_shared::placeholder_images::display_number_meta(
                                ph.display_number,
                            ),
                        )),
                );
...
mod tests {
    ...
    fn file_url_single_slash_rejected_at_anchor_gate() {
        ...
        // URL parsing runs.
        let pasted = "file:/tmp/should_not_exist_abc_webuild_pager.png";
    ...
    fn dropped_path_nonexistent_bare_path_not_intercepted() {
        ...
        let entries = dropped_paths("/tmp/definitely_does_not_exist_xyz_webuild_pager.txt");
        assert!(
            entries.is_empty(),
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            xai_webuild_shared::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
    ...
    fn build_blocks_uri_prefers_durable_session_path() {
        ...
        img.session_image_path = Some(PathBuf::from("/Users/test/.webuild/session/image.png"));
        let blocks = build_blocks_no_workspace("text".into(), vec![img]);
        assert_eq!(blocks.len(), 2);
        if let agent_client_protocol::ContentBlock::Image(ic) = &blocks[1] {
            assert_eq!(
                ic.uri.as_deref(),
                Some("file:///Users/test/.webuild/session/image.png"),
