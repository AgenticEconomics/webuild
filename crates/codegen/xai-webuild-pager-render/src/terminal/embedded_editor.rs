//! Detects whether webuild is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` webuild is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `xai-webuild-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
