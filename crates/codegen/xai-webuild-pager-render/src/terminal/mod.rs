...
pub enum TerminalName {
    ...
    JetBrains,
    /// WeBuild Desktop (Electron app).
    #[strum(to_string = "WeBuild Desktop")]
    WeBuildDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` webuild is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches webuild, which
    ...
    pub fn telemetry_snapshot(&self) -> xai_webuild_telemetry::events::TerminalTelemetry {
        ...
        xai_webuild_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: xai_webuild_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> xai_webuild_shared::session::FeedbackTerminalInfo {
        use xai_webuild_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(xai_webuild_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `webuild ssh` can
    // forward env vars.
...
/// `xai-webuild-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "webuilddesktop" => Some(TerminalName::WeBuildDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.webuild/pager.toml` and
