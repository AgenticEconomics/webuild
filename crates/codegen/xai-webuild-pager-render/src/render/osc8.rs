...
/// One path segment without spaces (`main.rs`, `.webuild`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — webuild
/// session media lives under `~/.webuild/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.webuild/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.webuild/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    #[test]
    fn scan_detects_webuild_session_media_path() {
        // Dot-directory (`.webuild`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line = make_line("Saved to /Users/alice/.webuild/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.webuild/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.webuild/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.webuild/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.webuild/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.webuild/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.webuild/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/webuild-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            &*resolve_link_target(&overlay.links()[0].target)
                .and_then(|resolved| resolved.osc8_url)
                .expect("url"),
            "file:///tmp/webuild-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/webuild-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/xai-webuild-pager/src/render` should
        // NOT produce a link for the `/xai-webuild-pager/src/render` substring.
        let line = make_line("find crates/codegen/xai-webuild-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
