...
//! Dark themes (WeBuildNight, TokyoNight) share `webuild-night.tmTheme`;
//! WeBuildDay uses `webuild-day.tmTheme` with deepened colors for light backgrounds.

use std::sync::OnceLock;

pub use xai_webuild_markdown::Syntect;
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_GROKNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/webuild-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::WeBuildDay => SYNTECT_GROKDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/webuild-day.tmTheme"))),
    }
}
