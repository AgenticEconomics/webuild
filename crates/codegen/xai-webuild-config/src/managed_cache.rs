...
//! OS-protected layers (root-owned `/etc/webuild`, MDM).

use std::path::Path;

use crate::paths::user_webuild_home;
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_webuild_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_webuild_home() {
        mark_managed_config_synced_at(&home, marker);
    }
}

/// Server-side WeBuildDeployment UUID from the last deploy-key managed-config
...
pub fn managed_deployment_id(key_fingerprint: &str) -> Option<String> {
    managed_deployment_id_at(user_webuild_home()?.as_path(), key_fingerprint)
}
...
pub fn confirmed_team_switch(new_team_id: &str) -> Option<String> {
    user_webuild_home().and_then(|home| confirmed_team_switch_at(&home, new_team_id))
}
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_webuild_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}
...
/// Override with `WEBUILD_DEPLOYMENT_CONFIG_CACHE_TTL_SECS` for testing.
fn managed_config_stale_threshold() -> std::time::Duration {
    if let Ok(s) = std::env::var("WEBUILD_DEPLOYMENT_CONFIG_CACHE_TTL_SECS")
