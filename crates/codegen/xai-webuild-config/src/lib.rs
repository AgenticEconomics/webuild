//! Config file loading for WeBuild.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/webuild/managed_config.toml`
//! 2. `$WEBUILD_HOME/managed_config.toml`
//! 3. `$WEBUILD_HOME/config.toml`
//! 4. `$WEBUILD_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/webuild/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.webuild`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_webuild_home, encode_cwd_dirname, ensure_sessions_cwd_dir, webuild_application,
    webuild_application_in, webuild_home, sessions_cwd_dir, system_config_dir, user_webuild_home,
};
pub use validation::{
