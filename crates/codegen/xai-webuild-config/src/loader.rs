...
use crate::paths::{system_config_dir, user_webuild_home};
...
/// webuild config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_webuild_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_webuild_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.webuild/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.webuild` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/webuild`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_webuild_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `WEBUILD_CAMPAIGNS_OVERRIDE` env layer.
...
/// `WEBUILD_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("WEBUILD_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`xai_webuild_shell::util::config::load_effective_config`) when remote campaigns
/// or `WEBUILD_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `WEBUILD_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_webuild_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("webuild-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("webuild-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// `WEBUILD_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `WEBUILD_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("WEBUILD_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads WEBUILD_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("WEBUILD_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("WEBUILD_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("WEBUILD_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("WEBUILD_CAMPAIGNS") },
        }
    }
}
