//! Filesystem locations for webuild config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static WEBUILD_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user webuild directory (`~/.webuild`, canonicalized) used when
/// `WEBUILD_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`webuild_home()`] is the default without duplicating the computation.
///
...
/// `~/.webuild/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_webuild_home` (deliberately standalone crate).
pub fn default_webuild_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".webuild")
}

/// Per-user config directory: `$WEBUILD_HOME` or `~/.webuild`. Created if needed.
pub fn webuild_home() -> PathBuf {
    WEBUILD_HOME
        .get_or_init(|| {
            let webuild_home = if let Ok(v) = std::env::var("WEBUILD_HOME") {
                PathBuf::from(v)
            } else {
                default_webuild_home()
            };
...
/// The user-global webuild home, but only when one genuinely resolves: `Some` when
/// `$WEBUILD_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`webuild_home()`], this never falls back to a cwd-relative `.webuild`, so callers
/// that *scan* user-global webuild resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.webuild` tree for the user-global one when no home resolves.
pub fn user_webuild_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("WEBUILD_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(webuild_home)
}

/// Canonical webuild application path: `$WEBUILD_HOME/bin/webuild` (Unix) or `webuild.exe` (Windows).
pub fn webuild_application() -> PathBuf {
    webuild_application_in(&webuild_home())
}

/// [`webuild_application`] under an explicit home instead of `$WEBUILD_HOME`.
pub fn webuild_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "webuild.exe" } else { "webuild" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/webuild/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/webuild"))
    } else {
        None
    }
}
...
/// `webuild_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    webuild_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = webuild_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_webuild_home_has_no_verbatim_prefix() {
        ...
        let home = default_webuild_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".webuild"));
    }
