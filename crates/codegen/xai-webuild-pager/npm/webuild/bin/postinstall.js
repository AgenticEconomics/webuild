#!/usr/bin/env node
// Runs once after npm install/update. Reads the webuild binary from the
// matching per-platform optional dependency (@webuild/webuild-<platform>)
// and installs it to ~/.webuild/bin/ using versioned filenames:
//
//   Unix:    webuild-<version>  +  webuild  (symlink)
//   Windows: webuild-<version>.exe  +  webuild.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.webuild', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@webuild/webuild: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@webuild/webuild-${key}`;
    try {
...
if (!version) {
    console.error('@webuild/webuild: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@webuild/webuild: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@webuild/webuild: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running webuild processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid webuild-* matching webuild-pager-*.
...
if (!platformDir) {
    console.error(`@webuild/webuild: platform package @webuild/webuild-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @webuild/webuild');
    process.exit(0);
}

installBinary('webuild', platformDir, `webuild${EXE}`);
cleanupOldVersions('webuild');
cleanupOldVersions('webuild-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.webuild');
...
// Persist the npm registry so `webuild update` and the trampoline use the same one.
const npmRegistry = process.env.WEBUILD_NPM_REGISTRY
    || (() => {
        try {
...
// Set WEBUILD_INSTALL_COMPLETIONS=1 to auto-generate to ~/.webuild/completions.
const WEBUILD_PATH = path.join(CANONICAL_DIR, `webuild${EXE}`);
if (process.env.WEBUILD_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.webuild', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'webuild.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_webuild');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(WEBUILD_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(WEBUILD_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.webuild/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: webuild completions bash > ~/.local/share/bash-completion/completions/webuild');
    console.log('     webuild completions zsh  > ~/.zsh/completions/_webuild');
}
