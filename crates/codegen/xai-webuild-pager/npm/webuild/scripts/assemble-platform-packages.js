...
//   1. Brotli-compresses the built binary into `../webuild-<platform>/bin/<bin>.br`
...
// (`@webuild/webuild`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw webuild binary is
...
const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'xai-webuild-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `webuild-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] webuild-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'webuild',
            envVar: 'WEBUILD_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'xai-webuild-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'webuild',
            envVar: 'WEBUILD_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'xai-webuild-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'webuild',
            envVar: 'WEBUILD_LINUX_X64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'xai-webuild-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'webuild',
            envVar: 'WEBUILD_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'xai-webuild-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'webuild.exe',
            envVar: 'WEBUILD_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'xai-webuild-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'webuild.exe',
            envVar: 'WEBUILD_WIN32_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'xai-webuild-pager.exe'),
        },
    ];
