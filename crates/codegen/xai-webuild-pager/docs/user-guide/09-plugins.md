...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `WEBUILD_PLUGIN_ROOT` and `WEBUILD_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
- **MCP servers** -- a `.mcp.json` file of server configurations
- **LSP servers** -- a `.lsp.json` file of language server configurations

If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, WeBuild discovers the components above from their standard directories.
...
|----------------------|-------------|
| `WEBUILD_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `WEBUILD_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

WeBuild sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (WeBuild also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

WeBuild discovers plugins from these locations, in priority order:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session -- loaded for that session only | Trusted automatically |
| `--plugin-dir` (CLI flag, `webuild agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.webuild/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.webuild/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

WeBuild also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `WeBuildOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `webuild agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
webuild plugin list [--json] [--available]   # List installed plugins (--available requires --json)
webuild plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
webuild plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
webuild plugin update [<name>]               # Omit the name to update all plugins
webuild plugin enable <name>
webuild plugin disable <name>
webuild plugin details <name>                # Show the plugin's component inventory
webuild plugin validate [<path>]             # Validate plugin.json (default: current directory)
webuild plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `webuild plugin install <source>` without `--trust` and WeBuild prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
or `WEBUILD_MARKETPLACE_REQUIRE_SHA=1`. Both are tighten-only: either one enables
...
```bash
webuild plugin marketplace list [--json]
webuild plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
webuild plugin marketplace remove <url>      # Git URL or local path of a configured source
webuild plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
webuild plugin marketplace add my-org/team-plugins
webuild plugin marketplace list
webuild plugin install my-org/team-plugins --trust
webuild plugin list
webuild plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `webuild plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.webuild/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `webuild plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.webuild/pager.toml`:
...
Place this file at `~/.webuild/settings.json` or `~/.claude/settings.json`.
...
WeBuild trusts plugins from `~/.webuild/plugins/` automatically. Project plugins in `.webuild/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
webuild plugin install <source> --trust
```
...
Run `webuild inspect` to see every discovered plugin and what it provides:

```bash
webuild inspect          # Show plugins with their skills, agents, hooks, and MCP servers
webuild inspect --json   # Emit machine-readable JSON
```
