...
Hooks let you run a script or send an HTTP request at key moments in a WeBuild session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that WeBuild calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.webuild/hooks
   ```

2. Create a hook file, e.g. `~/.webuild/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'WeBuild session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a WeBuild session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.webuild/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.webuild/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.webuild/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.webuild/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.webuild/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`WEBUILD_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
WeBuild accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). WeBuild skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, WeBuild maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
WeBuild sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `WEBUILD_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `WEBUILD_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `WEBUILD_SESSION_ID`     | The unique identifier of the current WeBuild session. |
| `WEBUILD_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `WEBUILD_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, WeBuild additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `WEBUILD_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `WEBUILD_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`WEBUILD_PLUGIN_ROOT`, `WEBUILD_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/webuild-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. WeBuild re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.webuild/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.webuild/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug WEBUILD_LOG_FILE=/tmp/webuild.log webuild`, then check `/tmp/webuild.log`.
