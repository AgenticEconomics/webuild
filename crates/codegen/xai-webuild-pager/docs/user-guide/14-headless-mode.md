...
Headless mode runs WeBuild non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
WeBuild processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, WeBuild emits an error object (process exit non-zero). Prompt-level
...
WeBuild may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `webuild -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
webuild -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
webuild -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
webuild -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
webuild -p "What's the secret number?" --resume abc123
```
...
webuild -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
webuild --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  webuild -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
WeBuild's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(webuild -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
webuild -p "Format all files" --yolo

# Run tests and fix failures
webuild -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
- Combine `--yolo` with `--rules` to set guardrails: `webuild -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug webuild -p "..." 2> debug.log`.

---

## Project Root Discovery

When WeBuild starts, it discovers the project root by walking upward from `--cwd`
...
WeBuild discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
WeBuild stores data in `~/.webuild` (override with `WEBUILD_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.webuild` read-only:
...
```bash
export XAI_API_KEY="xai-..."
export WEBUILD_DISABLE_AUTOUPDATER=1
webuild -p "..." --no-auto-update
```
...
| `WEBUILD_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`WEBUILD_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `WEBUILD_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$WEBUILD_HOME/bin/webuild`).
...
- Resume: `webuild -p "continue" --resume "<id>"` or `webuild -p "continue" --continue`
