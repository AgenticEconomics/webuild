...
- **`webuild dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.webuild/config.toml` under `[keybindings]` like every other shortcut.
...
```
 WeBuild · Dashboard — 4 agents · 2 awaiting
...
The state icon matches WeBuild's sibling views (
`tasks_pane`):
...
rebound via `~/.webuild/config.toml`.
...
Set `WEBUILD_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `webuild --bg`") would list
