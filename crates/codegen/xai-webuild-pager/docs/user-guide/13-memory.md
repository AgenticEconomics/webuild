...
Memory lets WeBuild recall facts, decisions, and patterns from earlier sessions. WeBuild indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each WeBuild session starts fresh: the model knows nothing about previous sessions. When you enable memory, WeBuild can:
...
3. `WEBUILD_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.webuild/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.webuild/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.webuild/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

WeBuild suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, WeBuild saves a structured metadata summary to that session's daily log. The summary contains:
...
WeBuilds the summary from conversation metadata without an LLM call, without added latency. WeBuild skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask WeBuild to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.webuild/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

WeBuild records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, WeBuild opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, WeBuild shows `Memory saved to ~/.webuild/memory/MEMORY.md`.

### Forget

Ask WeBuild to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.webuild/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what WeBuild remembers:

```
> what do you remember?
```

WeBuild searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.webuild/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, WeBuild confirms in the scrollback:

```
Memory saved to ~/.webuild/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what WeBuild has stored.
...
Dream also runs automatically. By default, WeBuild checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, WeBuild automatically searches memory for content relevant to the current project and injects it as context. This means WeBuild starts with knowledge from previous sessions without a reminder.
...
WeBuild searches memory automatically, but you can also trigger searches manually in the chat:
...
The `webuild memory` command manages memory from the shell. It has one subcommand, `clear`:

```bash
# Clear workspace memory (MEMORY.md, sessions/, and index.sqlite). This is the default scope.
webuild memory clear

# The same scope, stated explicitly
webuild memory clear --workspace

# Clear the global MEMORY.md
webuild memory clear --global

# Clear both workspace and global memory
webuild memory clear --all

# Skip the confirmation prompt (-y is the short form)
webuild memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.webuild/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.webuild/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, WeBuild applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, WeBuild uses the session's primary model. |
...
When a session memory is old, WeBuild attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, WeBuild watches `~/.webuild/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `webuild inspect` output.
2. Check the flag: `webuild --experimental-memory` or `WEBUILD_MEMORY=1`.
3. Check for `--no-memory` or `WEBUILD_MEMORY=0` overriding your config.
...
```bash
ls ~/.webuild/memory/
cat ~/.webuild/memory/MEMORY.md
$EDITOR ~/.webuild/memory/MEMORY.md
```
