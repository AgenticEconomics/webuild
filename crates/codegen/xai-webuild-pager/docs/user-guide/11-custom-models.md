...
WeBuild connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, new sessions start with **`qwen3.7-max`** (OpenAI-compatible chat
completions). The system catalog and default ID live in the embedded
`default_models.json` (crate `xai-webuild-models`). Built-in rows include:

| Model | Endpoint / protocol | Auth |
|-------|---------------------|------|
| `qwen3.7-max` (default) | DashScope compatible-mode `/v1` · `chat_completions` | `DASHSCOPE_API_KEY` or `QWEN_API_KEY` |
| `grok-build` | xAI dual route (session → cli-chat-proxy, API key → `api.x.ai`) · `responses` | `webuild login` or `XAI_API_KEY` |
...
```bash
webuild -p "Hello" -m qwen3.7-max
webuild -p "Hello" -m grok-build    # switch to xAI
```
...
Set a persistent default in `~/.webuild/config.toml`:

```toml
[models]
default = "qwen3.7-max"
# default = "grok-build"   # or any [model.*] / catalog id
```
...
WeBuild supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, WeBuild uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. WeBuild sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.webuild/config.toml` under `[model.<name>]` sections:
...
WeBuild resolves the API key in this order:
...
3. Your signed-in session token (from `webuild login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; WeBuild also accepts `WEBUILD_CODE_XAI_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells WeBuild when to trigger auto-compaction. When you override a known model, WeBuild inherits that model's context window. When you define a new model and omit `context_window`, WeBuild defaults to 200,000 tokens, so set it explicitly to match your provider.
...
When you override a built-in model, WeBuild starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which WeBuild sends verbatim.
...
Point WeBuild at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `WEBUILD_MODELS_BASE_URL` | Yes | Base URL for inference. WeBuild fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. WeBuild also accepts `WEBUILD_CODE_XAI_API_KEY`. |
| `WEBUILD_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, WeBuild inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, WeBuild uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `webuild login` -- the API key is enough.
...
```bash
export WEBUILD_WEB_SEARCH_MODEL="grok-4.20-multi-agent"
```

If you point web search at a custom model, you also need a `[model.*]` entry so WeBuild can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
