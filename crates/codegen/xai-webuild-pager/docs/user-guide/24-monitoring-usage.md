...
WeBuild CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through WeBuild's product-analytics backends.
...
- **Structurally separate** from product-internal telemetry: its exporters carry
  only the headers you configure, never product credentials.
- **Independent of product data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams — those
  settings govern product-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export WEBUILD_EXTERNAL_OTEL=1                  # master switch
...
`WEBUILD_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `WEBUILD_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `WEBUILD_TELEMETRY_ENABLED`, which controls product analytics — the two govern opposite-pointing data flows. |
...
> `WEBUILD_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`WEBUILD_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `webuild setup` managed config /
...
## Metrics (meter scope `ai.xai.webuild_code`)
...
sum by (model, type) (rate(webuild_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(webuild_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(webuild_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(webuild_code_tool_decision_total[1h]))
```
