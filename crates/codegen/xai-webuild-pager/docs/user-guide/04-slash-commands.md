...
- **Shell builtins** -- handled by the agent backend (xai-webuild-shell)
- **Pager builtins** -- handled by the TUI frontend (xai-webuild-pager)
...
When the context window fills up, WeBuild auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `webuild` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `WEBUILD_MEMORY=1`. `/remember` is always available.
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, WeBuild prompts you.
...
Set, manage, or check an autonomous goal. WeBuild works toward the objective across turns and reports progress.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.webuild/skills/commit/SKILL.md`, you can invoke it with:
