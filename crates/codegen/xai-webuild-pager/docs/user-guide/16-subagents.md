...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.webuild/agents/` or `~/.webuild/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.webuild/personas/`; applied during subagent resolution |
...
WeBuild discovers file-based personas from these locations, in priority order:

- `.webuild/personas/*.toml` (project)
- `~/.webuild/personas/*.toml` (user)
...
> **Note:** WeBuild applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, WeBuild resolves the effective model and reasoning effort in this order, highest priority first:
...
WeBuild manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
# instructions_file = ".webuild/personas/concise.md"  # or load from a file
```

WeBuild also discovers roles from `.webuild/roles/*.toml` and personas from `.webuild/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

WeBuild shows running and finished work in side panes on the agent screen:
