...
Agent mode runs WeBuild as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`webuild -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `webuild agent` command and apply to every mode. Pass them before the mode name, for example `webuild agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
...
```bash
webuild agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `WEBUILD_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, WeBuild defines extension methods under the `x.ai/` prefix (historical wire namespace; still used by this fork). These cover:
...
The tables here show representative methods in each category. The `x.ai/*` set is a WeBuild wire namespace and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
