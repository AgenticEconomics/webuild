...
WeBuild reads configuration from local config files, environment variables, and
...
Location: `~/.webuild/config.toml`

If the file does not exist, WeBuild uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); WeBuild writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `WEBUILD_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). WeBuild writes the change to
`[ui] vim_mode` in `~/.webuild/config.toml` immediately and applies it to every
...
`webuild` launches. Configure it from `/settings` → **Default screen mode**
...
`WEBUILD_SCROLL_SPEED`, `WEBUILD_SCROLL_MODE`, `WEBUILD_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `WEBUILD_SCROLL_LINES`. Precedence: env var →
...
(`WEBUILD_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`WEBUILD_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
MCP servers can also be configured per-project in `.webuild/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.webuild/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.webuild/config.toml` (current dir) > `<repo-root>/.webuild/config.toml` > `~/.webuild/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `WEBUILD_MEMORY=1`).
...
`webuild inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. WeBuild writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.webuild/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "webuild"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `webuild`. |
...
| WeBuild Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, WeBuild detects the terminal brand and selects the best
...
`$WEBUILD_EVENT`, `$WEBUILD_MESSAGE`, and `$WEBUILD_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'WeBuild' -message '$WEBUILD_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$WEBUILD_MESSAGE' ntfy.sh/my-webuild-alerts"
...
fires, try `condition = "always"` as a fallback. WeBuild supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by WeBuild; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= WEBUILD_EXTERNAL_OTEL)
...
| `WEBUILD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `WEBUILD_OIDC_ISSUER` | OIDC issuer URL |
| `WEBUILD_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `WEBUILD_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `WEBUILD_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `WEBUILD_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `WEBUILD_AGENT` | Custom agent definition path or name |
| `WEBUILD_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `WEBUILD_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `WEBUILD_LOG_FILE` log and headless stderr output |
...
| `WEBUILD_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `WEBUILD_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `WEBUILD_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `WEBUILD_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.webuild/config.toml` | Main configuration file |
| `~/.webuild/pager.toml` | TUI appearance configuration |
...
| `~/.webuild/lsp.json` | LSP server configuration (user-scoped) |
| `~/.webuild/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.webuild/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.webuild/` within your repository:

| File | What it configures |
|------|--------------------|
| `.webuild/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.webuild/config.toml`) |
| `.webuild/skills/` | Project-specific skills |
| `.webuild/hooks/` | Project-specific lifecycle hooks |
| `.webuild/agents/` | Project-specific agent definitions |
| `.webuild/lsp.json` | LSP server configuration |
| `.webuild/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.webuild/lsp.json` | All projects |
| Project | `.webuild/lsp.json` | Current repository |
...
1. **Project** -- `.webuild/lsp.json`
2. **User** -- `~/.webuild/lsp.json`
