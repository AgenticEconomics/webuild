...
Type a message and press `Enter` to send it. WeBuild reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, WeBuild asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `webuild --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.webuild/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `webuild -c`
...
- **Agent messages** -- WeBuild's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- WeBuild's reasoning process (collapsible)
...
WeBuild has built-in tools for:
...
# Experimental scrollback-native render mode. Sticky: plain `webuild` reopens in
...
```bash
webuild -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. WeBuild reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.webuild/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. WeBuild also reads `CLAUDE.md` files for compatibility.
