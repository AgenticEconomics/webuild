...
WeBuild supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.webuild/hooks/`
- **Project hooks** — from `.webuild/hooks/` in your repo
...
Plugin hooks automatically receive `WEBUILD_PLUGIN_ROOT` and `WEBUILD_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.webuild/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `webuild plugin marketplace add <source>`)
...
Sources land in `~/.webuild/config.toml`:
...
Or in `~/.webuild/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-webuild-hooks/examples/README.md) — ready-to-use sample hooks
