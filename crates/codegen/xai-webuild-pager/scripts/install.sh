#!/bin/bash
#
# WeBuild CLI installer — https://x.ai/cli/install.sh
#
# Auth: WEBUILD_DEPLOYMENT_KEY (takes precedence) or ~/.webuild/auth.json from `webuild login`.
# Env: WEBUILD_CHANNEL (stable|alpha|enterprise, default: stable), WEBUILD_BIN_DIR, WEBUILD_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   WEBUILD_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.webuild/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_webuild_token() {
    local auth_file="$HOME/.webuild/auth.json"
    local scope="$1"
...
# Resolve auth: WEBUILD_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$WEBUILD_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_webuild_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_webuild_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.webuild/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.webuild/auth.json." >&2
    fi
fi
...
DOWNLOAD_DIR="$HOME/.webuild/downloads"
BIN_DIR="${WEBUILD_BIN_DIR:-$HOME/.webuild/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${WEBUILD_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing WeBuild $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing WeBuild $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/webuild-$platform"
artifact_base="${BASE_URL}/webuild-${version}-${platform}"
...
echo "  Downloading webuild ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: WeBuild is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in webuild.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/webuild.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded webuild failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.webuild/bin and ~/.webuild/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/webuild"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/webuild and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.webuild/completions/bash" "$HOME/.webuild/completions/zsh"
"$BIN_DIR/webuild" completions bash > "$HOME/.webuild/completions/bash/webuild.bash" 2>/dev/null || true
"$BIN_DIR/webuild" completions zsh  > "$HOME/.webuild/completions/zsh/_webuild"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/webuild" completions fish > "$HOME/.config/fish/completions/webuild.fish" 2>/dev/null || true
fi
...
if [ -n "$WEBUILD_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${WEBUILD_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$WEBUILD_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.webuild/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.webuild/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.webuild/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.webuild/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "WeBuild $version installed to $BIN_DIR/webuild.exe" >&2
else
    echo "WeBuild $version installed to $BIN_DIR/webuild" >&2
fi

# --- Ensure webuild is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so webuild works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/webuild" "$candidate/webuild"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/webuild -> $BIN_DIR/webuild" >&2
...
# Also update shell config so ~/.webuild/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> webuild installer >>>
fish_add_path $HOME/.webuild/bin
# <<< webuild installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> webuild installer >>>
export PATH="$HOME/.webuild/bin:$PATH"
fpath=(~/.webuild/completions/zsh $fpath)
...
# <<< webuild installer <<<'
    ...
    if grep -qs "webuild installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> webuild installer >>>/ { skip=1; next }
            /# <<< webuild installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'webuild' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'webuild' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'webuild' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.webuild/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use webuild from cmd.exe or PowerShell, add %USERPROFILE%\\.webuild\\bin to your PATH." >&2
fi
