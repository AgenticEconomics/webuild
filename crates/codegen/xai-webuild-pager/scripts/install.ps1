#
# WeBuild CLI installer for PowerShell — https://x.ai/cli/install.ps1
#
# Auth: WEBUILD_DEPLOYMENT_KEY env var (takes precedence) or ~/.webuild/auth.json from `webuild login`.
...
#   $env:WEBUILD_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:WEBUILD_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:WEBUILD_VERSION) {
    $Version = $env:WEBUILD_VERSION
}
...
$WeBuildDir = Join-Path $env:USERPROFILE '.webuild'
...
function Read-WeBuildToken([string]$Scope) {
    $authFile = Join-Path $WeBuildDir 'auth.json'
...
if ($env:WEBUILD_DEPLOYMENT_KEY) {
    $AuthSource = 'deployment key'
    Write-Host 'Auth: using deployment key.' -ForegroundColor DarkGray
} else {
    $oidcToken = Read-WeBuildToken $OidcScope
    $legacyToken = Read-WeBuildToken $LegacyScope
    if ($oidcToken) {
        $AuthSource = 'auth.json (oidc)'
        Write-Host 'Auth: using OIDC token from ~/.webuild/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.webuild/auth.json.' -ForegroundColor DarkGray
    }
}
...
$DownloadDir = Join-Path $WeBuildDir 'downloads'
$BinDir = if ($env:WEBUILD_BIN_DIR) { $env:WEBUILD_BIN_DIR } else { Join-Path $WeBuildDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:WEBUILD_CHANNEL) { $env:WEBUILD_CHANNEL } else { 'stable' }
...
if ($AuthSource) {
    Write-Host "Installing WeBuild $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing WeBuild $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "webuild-$platform.exe"
$artifactBase = "$BaseUrl/webuild-$resolvedVersion-$platform"
...
foreach ($binName in @('webuild.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\webuild.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $WeBuildDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'webuild.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'webuild.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $WeBuildDir 'config.toml'
...
if ($env:WEBUILD_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:WEBUILD_PROXY_URL) { $env:WEBUILD_PROXY_URL } else { 'https://cli-chat-proxy.grok.com/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:WEBUILD_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $WeBuildDir 'managed_config.toml'
        $requirementsPath = Join-Path $WeBuildDir 'requirements.toml'
...
Write-Host "WeBuild $resolvedVersion installed to $BinDir\webuild.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so webuild works immediately.
...
Write-Host ''
Write-Host "Run 'webuild' or 'agent' to get started!" -ForegroundColor Cyan
