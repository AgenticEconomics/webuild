...
//!
//! These tests exercise the real `xai-webuild-pager` binary through a PTY using
...
/// enterprise deploy report: with `WEBUILD_GOAL=1`, `/goal` must
...
/// explicitly off (`WEBUILD_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `WEBUILD_FOLDER_TRUST=1` and a git repo that
...
/// also pins `WEBUILD_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
