...
//!
//! Uses `xai-webuild-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.webuild/auth.json` from `WEBUILD_AUTH_JSON` (path) or the
//! developer's `~/.webuild/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p xai-webuild-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use xai_webuild_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.webuild/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("WEBUILD_AUTH_JSON") {
    ...
    dirs_next_home()
        .map(|h| h.join(".webuild/auth.json"))
        .filter(|p| p.is_file())
}
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&webuild);
    if let Some(src) = auth_json_source() {
        let dest = webuild.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.webuild/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("WEBUILD_HOME".into(), webuild.display().to_string()),
    ...
    // of the developer's shell. `WEBUILD_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (xai_webuild_config::
    ...
    env.push((
        "WEBUILD_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("WEBUILD_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (xai-webuild-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "WEBUILD_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
