...
const PAID_TIER: &str = "SuperWeBuildPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "SuperWeBuild Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `superwebuild_heavy` = live
/// `/user` string `SuperWeBuildPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `WEBUILD_LOCAL_AUTH` dev issuer
...
/// `WEBUILD_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let webuild_home = content.home().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create temp .webuild");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("WEBUILD_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("WEBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("WEBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
