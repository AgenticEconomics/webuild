...
/// argv shape is the part `webuild wrap` owns. The quoting round-trip against a
...
fn wrap_not_found_alias_routes_via_shell_contract() {
    ...
    let (code, raw) = run_wrap(
        &["webuild-wrap-e2e-alias-xx", "with space"],
        &[("SHELL", &shell)],
    );
    ...
    assert!(
        raw.contains("ARG:webuild-wrap-e2e-alias-xx 'with space'"),
