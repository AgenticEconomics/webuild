...
//
// Bug: when the user has scrolled UP into webuild's scrollback (NOT pinned to the
// bottom / not following) and the terminal is resized HORIZONTALLY, webuild loses
...
// in follow/tail mode webuild re-pins to the bottom (no jump).
//
...
//
// Runs FULLSCREEN (alt-screen): webuild's mode-independent scroll-anchor logic is
// what's under test. `Viewport::Fullscreen` is autoresized on resize with no DSR
// cursor probe, so webuild itself re-wraps at the new width, and the alt-screen grid
// is not reflowed by the terminal — so the marker's position reflects webuild's own
...
///
/// webuild renders markdown with SOFT line breaks (a single `\n` becomes a space),
...
async fn resize_preserves_scroll_position() {
    ...
    // terminal resize with NO DSR cursor probe, so webuild re-wraps the scrollback
    // at the new width and the stale `scroll_offset` surfaces the jump in webuild's
    // OWN rendered output. The alt-screen grid is not reflowed by the terminal on
    // resize, so this isolates webuild's scroll-anchor logic with no harness confound.
