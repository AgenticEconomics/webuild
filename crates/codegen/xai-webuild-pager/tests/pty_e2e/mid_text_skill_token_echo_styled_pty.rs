...
use xai_webuild_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.webuild/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for
/// git workspaces (offline `webuild inspect` scans plain dirs too) — same shape
/// as the mid_text_skill_token_echo.yaml scenario's `git_init: true`.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".webuild").join("skills").join("test-skill");
