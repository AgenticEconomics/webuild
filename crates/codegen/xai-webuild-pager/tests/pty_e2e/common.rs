...
pub(crate) use xai_webuild_pager_pty_harness::{
...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("WEBUILD_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `xai_webuild_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `WEBUILD_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("WEBUILD_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`WEBUILD_TEST_VERSION`) and an explicit `WEBUILD_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/WEBUILD_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    // A self-built (unstamped) webuild auto-trusts and never prompts; simulate a
    ...
    // auto-trust.
    env.push(("WEBUILD_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set WEBUILD_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("WEBUILD_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".webuild")
        .join(xai_webuild_workspace::trust::TRUST_FILE_NAME);
    let store = xai_webuild_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&xai_webuild_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `xai_webuild_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let webuild_home = content.home().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create fake WEBUILD_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(webuild_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in xai-webuild-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.webuild/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{WEBUILD_HOME|HOME}/.webuild/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let webuild_home = content.home().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create .webuild");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let webuild_home = content.home().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create .webuild");
    // Minimal opt-in only — matches load_config's `{WEBUILD_HOME|HOME}/.webuild/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("WEBUILD_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "WEBUILD_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        xai_webuild_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `webuild wrap` run budget. Same contention math as the requirements-version
...
/// Run `webuild wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `WEBUILD_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("WEBUILD_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env).expect("spawn webuild wrap");
...
/// `webuild wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$WEBUILD_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("WEBUILD_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use xai_webuild_pager_pty_harness::host_clipboard::{
...
pub(crate) use xai_webuild_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
