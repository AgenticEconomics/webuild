...
use xai_webuild_pager::app::actions::Action;
use xai_webuild_pager::settings::{
    EnumChoice, PagerLocalSnapshot, SettingCategory, SettingKind, SettingMeta, SettingOwner,
    SettingValue, SettingsRegistry,
};
use xai_webuild_pager::views::settings_modal::{
...
fn make_state() -> SettingsModalState {
    ...
    xai_webuild_pager::app::set_voice_mode_enabled_for_test(true);
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    xai_webuild_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use xai_webuild_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    xai_webuild_pager::appearance::cache::set_keep_text_selection(
        xai_webuild_pager::appearance::TextSelection::Flash,
    );
    xai_webuild_pager::appearance::cache::set_show_thinking_blocks(true);
    xai_webuild_pager::appearance::cache::set_prompt_suggestions(true);
    xai_webuild_pager::appearance::cache::set_group_tool_verbs(true);
    xai_webuild_pager::appearance::cache::set_scroll_mode(
        xai_webuild_pager::appearance::ScrollMode::Auto,
    );
    xai_webuild_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    xai_webuild_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("webuildnight"),
            "auto_dark_theme" => SettingValue::Enum("webuildnight"),
            "auto_light_theme" => SettingValue::Enum("webuildday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = xai_webuild_pager::settings::default_value_for(meta);
...
// `crates/codegen/xai-webuild-pager/src/app/dispatch.rs::tests` (next to
...
fn pr4_mouse_click_in_theme_picker_is_no_op() {
    ...
    s.mode = SettingsModalMode::PickingEnum {
        key: "theme",
        choices_idx: 0,
        original_value: SettingValue::Enum("webuildnight"),
        supports_preview: true,
    };
    synth_rects(&mut s);
    let outcome = handle_settings_mouse(
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use xai_webuild_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use xai_webuild_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use xai_webuild_pager::views::settings_modal::ResetConfirmOverlay;
...
/// "Ask WeBuild" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask WeBuild"),
            "[{fixture_label}] docs footer (`Ask WeBuild`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to webuildday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    xai_webuild_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use xai_webuild_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use xai_webuild_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use xai_webuild_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use xai_webuild_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use xai_webuild_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("WeBuild 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        xai_webuild_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        xai_webuild_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_parses_aliases() {
    use xai_webuild_pager::slash::commands::privacy::parse_privacy_arg;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use xai_webuild_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use xai_webuild_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use xai_webuild_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use xai_webuild_pager::appearance::TextSelection;
    ...
    xai_webuild_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    xai_webuild_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn scroll_mode_picker_enter_dispatches_set_commit() {
    use xai_webuild_pager::appearance::ScrollMode;

    // Pin the cache-backed live value so the picker seeds at auto (idx 0)
    // regardless of sibling tests on this thread.
    xai_webuild_pager::appearance::cache::set_scroll_mode(ScrollMode::Auto);
    let mut s = make_state();
...
fn invert_scroll_enter_dispatches_typed_setter() {
    xai_webuild_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    xai_webuild_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    xai_webuild_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    xai_webuild_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    xai_webuild_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    xai_webuild_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
