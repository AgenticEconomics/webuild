...
#[ignore = "PTY e2e; run with cargo test -p xai-webuild-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared WEBUILD_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let webuild_home = content.home().join(".webuild");
    std::fs::create_dir_all(&webuild_home).expect("create WEBUILD_HOME");
    std::fs::write(
        webuild_home.join("config.toml"),
        format!("[models]\ndefault = \"{CONFIG_MODEL}\"\n"),
    )
    .expect("write config.toml");
    let socket = webuild_home.join("leader-e2e.sock");
    ...
    let state_path = webuild_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(webuild_home.join("config.toml")).expect("read config.toml");
    assert!(
