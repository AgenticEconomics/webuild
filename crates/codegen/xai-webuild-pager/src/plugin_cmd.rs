//! `webuild plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `xai_webuild_shell::plugin` (shared orchestration)
//! and lower crates (`xai-webuild-agent`, `xai-webuild-plugin-marketplace`). This
...
use xai_webuild_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use xai_webuild_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use xai_webuild_plugin_marketplace::SourceKind;
use xai_webuild_plugin_marketplace::git::SourceCacheLease;
use xai_webuild_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `webuild plugin list --json`. The `status` field acts as a
...
/// Typed entry for `webuild plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `webuild plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  webuild plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `webuild plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &xai_webuild_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = xai_webuild_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = xai_webuild_plugin_marketplace::load_sources(&config);
    sources.extend(xai_webuild_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in xai_webuild_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                xai_webuild_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &xai_webuild_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = xai_webuild_plugin_marketplace::git::default_cache_root();
            xai_webuild_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> xai_webuild_telemetry::events::InstallKind {
    if is_git {
        xai_webuild_telemetry::events::InstallKind::Git
    } else {
        xai_webuild_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: xai_webuild_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = xai_webuild_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use xai_webuild_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &xai_webuild_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `webuild plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               webuild plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `webuild plugin list` to see installed plugins."
        );
    }
    if let Err(e) = xai_webuild_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    xai_webuild_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = xai_webuild_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    xai_webuild_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. WeBuild discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `webuild plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `webuild plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use xai_webuild_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        xai_webuild_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &xai_webuild_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match xai_webuild_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use xai_webuild_plugin_marketplace::MarketplaceSource;
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  webuild plugin install sentry@xai-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  webuild plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = xai_webuild_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = xai_webuild_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
