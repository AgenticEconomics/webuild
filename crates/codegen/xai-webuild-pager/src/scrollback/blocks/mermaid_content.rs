...
//! the generic [`CodeBlockSpan`](xai_webuild_markdown::CodeBlockSpan) API) and
...
use xai_webuild_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_webuild_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_webuild_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_webuild_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `WeBuildDay` is the only light theme; every other concrete theme (and the
/// `WeBuildNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `xai_webuild_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::WeBuildDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_webuildday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::WeBuildDay),
            "WeBuildDay is the light theme"
        );
        for dark in [
            ThemeKind::WeBuildNight,
