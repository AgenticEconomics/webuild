...
fn shorten_path(path: &str) -> &str {
    let memory_root = xai_webuild_config::webuild_home().join("memory");
...
**File:** /root/.webuild/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.webuild/memory/MEMORY.md");
...
**File:** /root/.webuild/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.webuild/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured webuild memory root keep one trailing segment group.
        let memory_root = xai_webuild_config::webuild_home().join("memory");
