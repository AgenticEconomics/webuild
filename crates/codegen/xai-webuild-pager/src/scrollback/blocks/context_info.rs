...
use xai_webuild_shell::session::{ContextInfo, count_detail};
...
impl ContextInfoBlock {
    ...
    fn build_lines(&self, theme: &Theme, bar: BarLayout) -> Vec<Line<'static>> {
        ...
        // (routed from xai-webuild-shell's model config resolution). This makes
...
mod tests {
    use super::*;
    use xai_webuild_shell::session::TokenUsageCategory;
