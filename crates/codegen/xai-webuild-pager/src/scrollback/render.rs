...
pub(crate) fn map_hyperlinks_to_overlay(
    hyperlinks: &[xai_webuild_markdown::HyperlinkTarget],
...
mod tests {
    ...
    fn make_hyperlink(
        ...
        url: &str,
        id: u32,
    ) -> xai_webuild_markdown::HyperlinkTarget {
        xai_webuild_markdown::HyperlinkTarget {
            line_index: line,
            column_range: cols,
            url: url.to_string(),
    ...
    fn overlay_empty_hyperlinks_produces_nothing() {
        ...
        let links: &[xai_webuild_markdown::HyperlinkTarget] = &[];
        let mut overlay = LinkOverlay::new();
        map_hyperlinks_to_overlay(links, &output, 0, 0, 10, 0, 0, &[], &mut overlay);
    ...
    fn markdown_wrapped_session_media_path_fully_linkified() {
        ...
        let path = "/Users/alice/.webuild/sessions/%2FUsers%2Falice%2Fcode%2Fxai/\
    ...
    #[test]
    fn basename_headers_stay_webuild_owned_for_duplicate_and_outside_targets() {
        let cwd = std::path::PathBuf::from("/worktree");
        let terminal = official_vscode_remote_context();
        let cases = [
