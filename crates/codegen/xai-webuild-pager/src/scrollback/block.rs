...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: xai_webuild_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[xai_webuild_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use xai_webuild_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        if let RenderBlock::Subagent(b) = &mut block {
            ...
            b.model = Some("webuild-test".into());
        ...
        assert!(text.contains("webuild-test"), "got: {text:?}");
