...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `webuild wrap` on the local end, so
...
pub fn summarize_warnings(warnings: &[TerminalWarning]) -> Option<crate::startup::StartupWarning> {
    ...
    let is_ssh = xai_webuild_shell::util::clipboard::is_remote_session();
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(xai_webuild_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .webuild/sandbox.toml\nUser config: ~/.webuild/sandbox.toml"
        ),
...
/// Pure SSH `webuild wrap` recommendation — suggests launching the session
/// through `webuild wrap ssh <host>` on the user's local machine, which gives a
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `webuild
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && xai_webuild_shell::util::clipboard::wayland_data_control_supported(),
        is_wayland && xai_webuild_shell::util::clipboard::native_tool_name() == "wl-copy",
    )
}
...
pub fn format_clipboard_diagnostics(input: ClipboardDiagnosticsInput<'_>) -> ClipboardDiagnostics {
    ...
    let fix = match delivery {
        ...
        ClipboardDelivery::Unverified if input.is_ssh => {
            Some("webuild wrap <ssh command> or /minimal")
        }
        ClipboardDelivery::Unverified if input.container_no_display => {
            Some("webuild wrap <command> or /minimal")
        }
        ClipboardDelivery::Unverified => Some("webuild wrap or /minimal"),
        ClipboardDelivery::Failed if input.is_ssh => Some("webuild wrap <ssh command> or /minimal"),
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart WeBuild.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart WeBuild.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn clipboard_diagnostics_unknown_ssh_is_unverified() {
        ...
        for expected in [
            ...
            "wrap         off",
            "status       unverified",
            "fix          webuild wrap <ssh command> or /minimal",
        ] {
            assert!(
    ...
    fn clipboard_diagnostics_known_terminal_status() {
        ...
        assert!(
            unsupported
                .text
                .contains("fix          webuild wrap <ssh command> or /minimal")
        );
    ...
    fn clipboard_diagnostics_tmux_wrap_and_container() {
        ...
        assert!(
            container
                .text
                .contains("fix          webuild wrap <command> or /minimal")
        );
    }
    ...
    // -- ssh_wrap_hint: `webuild wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("webuild wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `webuild wrap` — adoption silences the hint by itself.
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::WeBuildDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::WeBuildDesktop));
    ...
    fn format_themes_env_line_lists_available_below_truecolor() {
        ...
        for level in [ColorLevel::Ansi256, ColorLevel::Basic, ColorLevel::None] {
            ...
            assert!(line.contains("webuildnight") && line.contains("webuildday"));
