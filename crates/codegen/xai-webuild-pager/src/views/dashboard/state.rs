...
/// `~/.webuild/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<xai_webuild_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_webuild_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.webuild/config.toml`.
///
...
/// `~/.webuild/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no webuild home"))?;
...
fn config_path() -> Option<PathBuf> {
    let home = xai_webuild_shell::util::webuild_home::webuild_home();
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/webuild");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/webuild");
    }
    ...
    /// per-key serial lock. The `WEBUILD_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(WEBUILD_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `WEBUILD_AGENT_DASHBOARD` key reads it concurrently.
