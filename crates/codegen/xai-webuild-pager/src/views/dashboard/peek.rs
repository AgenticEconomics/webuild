...
fn paint_peek_config_badge(
    ...
    // permission mode (the gate in xai-webuild-shell), so `plan` alone is the
...
mod tests {
    ...
    fn render_peek_shows_model_and_auto_approve_on_bottom_border() {
        ...
        panel.model_name = Some("WeBuild 4 Fast".to_string());
        panel.auto_approve = true;
        let bottom = badge_row(&panel, 6);
        assert!(
            bottom.contains("WeBuild 4 Fast"),
        ...
        qpanel.model_name = Some("WeBuild 4 Fast".to_string());
        let qbottom = badge_row(&qpanel, 8);
        assert!(
            qbottom.contains("WeBuild 4 Fast"),
        ...
        let mut plain =
            PeekPanelState::new(DashboardRowId::TopLevel(AgentId(0)), fields("Response"));
        plain.model_name = Some("WeBuild 4 Fast".to_string());
        ...
        let mut planp =
            PeekPanelState::new(DashboardRowId::TopLevel(AgentId(0)), fields("Response"));
        planp.model_name = Some("WeBuild 4 Fast".to_string());
