...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "WeBuild Test".to_string(),
            acp::ModelId::new(Arc::from("webuild-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("WeBuild Test", 4); // cursor between "WeBuild" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    ...
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"webuildnight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode, SettingsModalMode::Browse));
}
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
        ...
        SettingsModalMode::EditingValue {
            ...
            assert!(
                ...
                "validation_error must be Some for unknown model 'a' \
                 (catalog has 'WeBuild 4 Fast' only)",
            );
        }
    ...
    // (catalog has 'WeBuild 4 Fast'; "a" doesn't match) is
...
//    the "Tip · Ask WeBuild…" docs footer and the keybindings hints.
//
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask WeBuild to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("webuildday"),
        "width=40 must NOT render LONG path (no `webuildday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask WeBuild") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"webuildnight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"webuildnight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `webuildnight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `webuildnight`; entering the picker
            // captures `original_value = current value = webuildnight`,
...
/// misleading: on dark themes (WeBuildNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the WeBuildDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::WeBuildNight => crate::theme::Theme::webuildnight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::WeBuildDay => crate::theme::Theme::webuildday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::webuildnight(),
    };
    assert_ne!(
