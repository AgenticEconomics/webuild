...
use xai_webuild_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    /// permissions). Imported from `xai-webuild-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell WeBuild what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from webuild\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from webuild\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p xai-webuild-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/xai-webuild-pager
