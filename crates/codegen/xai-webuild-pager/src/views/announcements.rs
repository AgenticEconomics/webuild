...
use xai_webuild_announcements::visible_announcements;
...
fn is_critical(a: &xai_webuild_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &xai_webuild_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    a: &xai_webuild_announcements::RemoteAnnouncement,
    now: chrono::DateTime<chrono::Utc>,
) -> bool {
    is_critical(a) && !xai_webuild_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !xai_webuild_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &xai_webuild_announcements::RemoteAnnouncement) -> bool {
...
fn is_hidden(
    ...
    is_dismissible(a) && hidden_ids.contains(&xai_webuild_announcements::announcement_hide_key(a))
}
...
fn usable_cta(a: &xai_webuild_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &xai_webuild_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    announcements: &'a [xai_webuild_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a xai_webuild_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [xai_webuild_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a xai_webuild_announcements::RemoteAnnouncement, &'a str)> {
...
pub fn session_announcement_hide_keys_at(
    ...
    visible_announcements(announcements)
        ...
        .map(xai_webuild_announcements::announcement_hide_key)
        .collect()
}
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &xai_webuild_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| xai_webuild_announcements::AnnouncementCta {
    ...
    fn promo_cta_target_requires_usable_pair() {
        ...
        label_only.cta = Some(xai_webuild_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn render_promo_row_button_and_hide_affordances() {
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(row0.starts_with("[Get SuperWeBuild]"), "row0={row0:?}");
        assert!(
    ...
    fn render_promo_row_truncates_button_never_affordances() {
        let anns = [promo(
            ...
            "msg",
            Some((
                "Upgrade to SuperWeBuild Heavy for the exclusive preview",
                "https://x.ai",
            )),
        )];
        let area = Rect::new(0, 0, 50, 1);
