...
/// Env override for the whole feature: `WEBUILD_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "WEBUILD_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `WEBUILD_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "WEBUILD_PROMPT_SUGGESTIONS_MODEL";
...
pub struct PromptSuggestionController {
    ...
    /// `WEBUILD_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| xai_webuild_config::env_bool(PROMPT_SUGGESTIONS_ENV))
