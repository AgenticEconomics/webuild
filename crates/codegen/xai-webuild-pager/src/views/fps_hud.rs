//! Release-safe FPS readout — `/debug fps`, `WEBUILD_FPS` on release builds.
//!
//! The full frame profiler (`render::frame_metrics`, `WEBUILD_FPS`) is compiled
...
//!
//! `WEBUILD_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` as
...
//! HUD from startup, so `WEBUILD_FPS=1` is never a silent no-op
//! ([`HONORS_WEBUILD_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `WEBUILD_FPS` env gate: only where the dev
/// `FrameMetrics` overlay is compiled out. In debug/dev builds the env keeps
/// feeding that overlay alone.
const HONORS_WEBUILD_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD. `WEBUILD_FPS` enables it at startup on
/// release binaries ([`HONORS_WEBUILD_FPS_ENV`]); `/debug fps` toggles it
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("WEBUILD_FPS").ok())
    }
    /// `env` is the raw `WEBUILD_FPS` value; the truthiness rule (nonempty and
    /// not `"0"`) matches `FrameMetrics` and `WEBUILD_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_WEBUILD_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `WEBUILD_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {
...
mod tests {
    ...
    /// asserting against [`HONORS_WEBUILD_FPS_ENV`] keeps the test true under
    /// both cfgs (the dev half is pinned by the constant's shape, the same
    /// limitation as the `/debug` visibility test).
    #[test]
    fn webuild_fps_env_enables_hud_where_dev_overlay_absent() {
        for truthy in ["1", "full", " "] {
            assert_eq!(
                FpsHud::with_env(Some(truthy.into())).enabled(),
                HONORS_WEBUILD_FPS_ENV,
                "WEBUILD_FPS={truthy:?} must track the env-gate owner"
            );
        }
