...
pub enum ActiveModal {
    ...
    SessionPicker {
        ...
        content_results: Option<Vec<xai_webuild_shell::extensions::session_search::SearchSessionHit>>,
...
fn fit_docs_ask_webuild_tip(docs_path: &str, width: usize) -> String {
    use crate::render::line_utils::truncate_str;
    if width == 0 {
        return String::new();
    }
    let long =
        format!("Tip · Ask WeBuild about the docs ({docs_path}) — e.g. \"how do I set up MCP?\"");
    if long.width() <= width {
        return long;
    }
    let short = format!("Tip · Ask WeBuild about the docs · {docs_path}");
...
pub fn render_doc_picker_overlay(
    ...
    if let Some(tip_rect) = tip_area {
        let docs_path = crate::util::display_user_webuild_path(DOCS_USER_GUIDE_REL);
        let tip_line = fit_docs_ask_webuild_tip(&docs_path, tip_rect.width as usize);
...
mod doc_picker_tip_tests {
    use super::{
        ActiveModal, DOCS_USER_GUIDE_REL, fit_docs_ask_webuild_tip, howto_list_modal,
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        let path = crate::util::display_user_webuild_path(DOCS_USER_GUIDE_REL);
        let long =
            format!("Tip · Ask WeBuild about the docs ({path}) — e.g. \"how do I set up MCP?\"");
        let short = format!("Tip · Ask WeBuild about the docs · {path}");
        let path_only = format!("Tip · {path}");
        assert_eq!(fit_docs_ask_webuild_tip(&path, long.width()), long);
        assert_eq!(fit_docs_ask_webuild_tip(&path, short.width()), short);
        assert_eq!(fit_docs_ask_webuild_tip(&path, path_only.width()), path_only);
        for w in [5usize, 12, 20, 30] {
            let line = fit_docs_ask_webuild_tip(&path, w);
    ...
    fn doc_picker_renders_tip_with_path() {
        ...
        assert!(
            all.contains("Tip") && all.contains("Ask WeBuild"),
            "missing tip footer:\n{all}"
        );
        assert!(
