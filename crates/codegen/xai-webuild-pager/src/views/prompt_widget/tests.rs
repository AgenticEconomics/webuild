#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/webuild-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/webuild-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/webuild-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/webuild-test-image.png"),
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "WeBuild 4.5".to_string()),
        );
        ...
        // Accept arg completion → should replace "gr" with "WeBuild 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("WeBuild 4.5"),
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> xai_webuild_workspace::file_system::FuzzyMatchResult {
        xai_webuild_workspace::file_system::FuzzyMatchResult {
