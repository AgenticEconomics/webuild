...
use xai_webuild_tools::implementations::skills::types::SkillInfo;
...
pub fn plugin_group(plugin: &xai_hooks_plugins_types::PluginInfo) -> PluginGroup {
    ...
    match &plugin.origin {
        Some(PluginOrigin::ProjectWeBuild) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserWeBuild) => PluginGroup::new(2, "origin:user", "User"),
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> xai_webuild_telemetry::events::ExtensionsModalTab {
        use xai_webuild_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    AddMcpServer {
        name: String,
        config: Box<xai_webuild_shell::util::config::McpServerConfig>,
    },
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use xai_webuild_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let webuild = xai_webuild_config::webuild_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user webuild home (WEBUILD_HOME-aware)
    // or a project-scoped `{cwd}/.webuild/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User webuild home (WEBUILD_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(webuild.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.webuild/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".webuild" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $WEBUILD_HOME/hooks
    let global_hooks = webuild.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.webuild/hooks") || source_dir.contains("/.webuild/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&webuild) {
            let prefix = crate::util::display_webuild_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            xai_webuild_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_webuild_home(path) {
                    crate::util::display_user_webuild_path("skills")
            ...
            xai_webuild_tools::types::config_source::ConfigSource::Project { path } => {
                let s = path.display().to_string();
                if s.contains("/.webuild/") {
                    ".webuild/skills".into()
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.webuild/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // webuild-home branch is WEBUILD_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.webuild/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.webuild/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("webuild_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by organization")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "webuild_com_linear"),
    ...
    fn make_skill(
        ...
        xai_webuild_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: xai_webuild_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = xai_webuild_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    xai_webuild_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    xai_webuild_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.webuild/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    fn plugin_group_maps_each_origin_variant() {
        ...
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectWeBuild, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserWeBuild, 2, "origin:user", "User"),
            (
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        assert_eq!(group.key, "webuild-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        ...
        assert_eq!(plugin_group(&unknown).key, "webuild-mp:xAI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserWeBuild),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserWeBuild),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use xai_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserWeBuild);
