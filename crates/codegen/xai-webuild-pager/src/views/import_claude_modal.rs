...
//! are written to `.webuild/config.toml`.
...
use xai_webuild_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.webuild/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".webuild").join("config.toml");
...
mod tests {
    use super::*;
    use xai_webuild_shell::claude_import::PathKind;
    use xai_webuild_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use xai_webuild_shell::claude_import::ImportableItem;
