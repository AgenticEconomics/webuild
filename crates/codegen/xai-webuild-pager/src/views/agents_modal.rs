...
use xai_webuild_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use xai_webuild_shell::agent::config::AgentSelectionConfig;
use xai_webuild_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.webuild` vs `{cwd}/.webuild`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `WEBUILD_AGENT`,
...
/// Skips internal variants (WeBuildConcise, WeBuildPlan,
/// WeBuildPlanNoSubagents, WeBuildAskUser, Codex, Opencode,
/// CursorExtended, WeBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = xai_webuild_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.webuild/personas` and `{cwd}/.webuild/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = webuild_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".webuild").join("personas")),
        (ConfigFileScope::User, webuild_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match xai_webuild_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => xai_webuild_config::webuild_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".webuild").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.webuild/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.webuild/{subdir}` or `{cwd}/.webuild/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let webuild_home = xai_webuild_config::webuild_home();
    let in_user = dunce::canonicalize(webuild_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".webuild").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    xai_webuild_shell::config::load_effective_config()
        .ok()
        .and_then(|root| xai_webuild_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in xai-webuild-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    xai_webuild_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        xai_webuild_agent::config::PromptMode::Extend => "extend",
        xai_webuild_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use xai_webuild_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".webuild").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".webuild").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.webuild/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".webuild").join("personas").join("gone.toml");
