...
/// Display label for a section header, e.g. `"Managed by organization (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by organization ({count})"),
...
///
/// Priority: `webuild_com_` prefix or managed wire source → Managed; else plugin
...
pub fn section_for(server: &McpServerInfo) -> McpSectionId {
    if server.name.starts_with("webuild_com_") || server.wire_source == McpWireSource::Managed {
        McpSectionId::Managed
...
pub fn is_removable(server: &McpServerInfo) -> bool {
    server.wire_source == McpWireSource::Local && !server.name.starts_with("webuild_com_")
}
...
mod tests {
    ...
    #[test]
    fn section_for_webuild_com_with_plugin_label_is_managed() {
    ...
    #[test]
    fn is_removable_rejects_webuild_com_prefix() {
        let server = server_from_wire("webuild_com_slack", Some("local"), None);
        assert!(!is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("webuild_com_slack", Some("managed"), None);
