...
// (e.g. xai-webuild-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
pub struct SuggestionController {
    ...
    /// `WEBUILD_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `WEBUILD_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `x.ai/suggest` request.
    /// Resolved at construction from `WEBUILD_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: xai_webuild_config::env_bool("WEBUILD_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: xai_webuild_config::env_bool("WEBUILD_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("WEBUILD_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
