//! Local PTY wrapper: the engine behind `webuild wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `webuild wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `webuild` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner webuild
    // reads this (see `xai_webuild_pager_render::clipboard::osc52_sink_active`) to
    // *trust* OSC 52 even when it can't detect an OSC-52-capable terminal,
    // which is the usual SSH case (only `TERM` propagates, so the inner webuild
    ...
    // these two without clearing it. The canonical `WEBUILD_OSC52_SINK` is
    ...
    cmd.env("WEBUILD_OSC52_SINK", "1");
    cmd.env("LC_WEBUILD_OSC52_SINK", "1");
    ...
    // detached — `webuild wrap` is short-lived and exits with the child.
