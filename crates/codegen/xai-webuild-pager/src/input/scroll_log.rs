//! Scroll flight recorder — `WEBUILD_SCROLL_LOG` JSONL log of scroll-stream
...
//! (see [`super::mouse`]).
//!
//! Enablement: `WEBUILD_SCROLL_LOG=1` (or set-but-empty) logs to
//! `~/.webuild/logs/scroll-log-<timestamp>.jsonl`; any other non-`0` value is
//! used as the target path. Unset (or `0`, matching `WEBUILD_SCROLL_DEBUG`)
...
/// Appends [`ScrollLogRecord`]s to the `WEBUILD_SCROLL_LOG` file. Owned as
...
impl ScrollLogRecorder {
    /// Build from `WEBUILD_SCROLL_LOG` (see module docs for value semantics);
    /// `None` when unset or `0`.
    pub(crate) fn from_env_at(base: Instant) -> Option<Self> {
        let raw = std::env::var("WEBUILD_SCROLL_LOG").ok()?;
        let value = raw.trim();
        if value == "0" {
            return None;
        }
...
/// `~/.webuild/logs/scroll-log-<utc-ts>.jsonl` — the input-debug dump's dir
