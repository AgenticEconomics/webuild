//! Headless single-turn mode (`webuild -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_webuild_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use xai_webuild_shell::agent::auth_method::AuthMethodKind;
use xai_webuild_shell::agent::config::Config as AgentConfig;
use xai_webuild_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use xai_webuild_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_webuild_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<xai_webuild_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use xai_webuild_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<xai_webuild_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = xai_webuild_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut xai_webuild_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    xai_webuild_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `webuild login` to authenticate \
         (or `webuild login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         webuild login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `webuild login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !xai_webuild_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !xai_webuild_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &xai_webuild_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'webuild models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    xai_webuild_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = xai_webuild_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `webuild workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = xai_webuild_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        xai_webuild_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_webuild_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("WEBUILD_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = xai_webuild_shell::active_sessions::register(
            xai_webuild_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(xai_webuild_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(xai_webuild_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.webuild can't hang exit.
        let _ = xai_webuild_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = xai_webuild_shell::sampling::error::prompt_usage_from_error(&err)
...
mod tests {
    ...
    use super::*;
    use xai_webuild_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            xai_webuild_workspace::permission::types::PatternMode::Domain
        );
