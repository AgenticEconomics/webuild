//! `webuild completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "webuild" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("webuild");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "webuild", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "webuild", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `webuild worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:webuild-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:webuild-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("webuild");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "webuild", &mut buf);
    ...
    // so `webuild worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
