//! Host clipboard image paste mediated by `webuild wrap`.
//!
...
//! the inner `webuild`) can solicit the host pasteboard. That is intentional and
//! acceptable for `webuild wrap` because (1) the user opted into wrap on their
//! own host, (2) the answer stays inside their session, and (3) the remote
//! only requests when `osc52_sink_active()` (wrap already set
//! `WEBUILD_OSC52_SINK` / `LC_WEBUILD_OSC52_SINK`). Do not generalize this pattern
//! to untrusted multiplexers without an explicit allowlist.

use base64::Engine as _;
use xai_webuild_pager_render::clipboard::{ImageData, osc52_sink_active};

/// OSC body after `ESC ]` for a host image request.
pub const REQUEST_BODY: &[u8] = b"999;WeBuildWrapClipboardImage?";
...
/// Successful host image frame: `WEBUILD_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "WEBUILD_WRAP_IMG";

/// Host has no image (`WEBUILD_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "WEBUILD_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("WEBUILD_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("WEBUILD_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("WEBUILD_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("WEBUILD_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
