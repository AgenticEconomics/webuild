...
//!   `WEBUILD_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!   (`WEBUILD_MEMTRACE_THRESHOLD_MB`, default 1 GiB, then doubling), a full
...
//! ## Files
//!
//! `$WEBUILD_HOME/memtrace/<start-ts>-<pid>.jsonl` (+ `.1` after 4 MiB
...
//!
//! The lib cannot depend on jemalloc; `xai-webuild-pager-bin` installs:
...
/// Whether a trace sink is installed ([`start`] ran and `WEBUILD_MEMTRACE` is
...
/// Env: disable with `WEBUILD_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("WEBUILD_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("WEBUILD_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("WEBUILD_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$WEBUILD_HOME/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `WEBUILD_MEMTRACE=0`.
///
...
/// (`webuild --version`, `webuild trace …`) leave no files behind.
