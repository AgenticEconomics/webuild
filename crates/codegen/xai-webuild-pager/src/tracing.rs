...
/// firehose (`WEBUILD_DEBUG_LOG` / `WEBUILD_LOG_FILE`).
pub use xai_webuild_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `xai-webuild-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use xai_webuild_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "xai_webuild_shell=info,xai_webuild_pager=trace,xai_webuild_tools=info,xai_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = xai_webuild_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = xai_webuild_telemetry::instrumentation::layer();
    let sampling_log_layer = xai_webuild_telemetry::sampling_log::layer();
    let hooks_log_layer = xai_webuild_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    xai_webuild_telemetry::debug_log::install_firehose(registry, "tui");
