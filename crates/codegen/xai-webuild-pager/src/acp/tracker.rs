...
use xai_webuild_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use xai_webuild_tools::util::strip_redundant_session_cd;
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                xai_webuild_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && xai_webuild_shell::session::PromptOrigin::from_prompt_id(pid)
...
/// rather than a media file (the free / X Basic SuperWeBuild-upsell short-circuit).
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use xai_webuild_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(xai_webuild_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use xai_webuild_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    use super::*;
    use std::sync::Arc;
    /// Default meta with no timestamps (simulates old webuild-shell or tests that
    ...
    fn test_search_tool_call_flow() {
        use xai_webuild_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    /// No stream_start_ms (old webuild-shell) should not break anything.
    #[test]
    ...
    #[test]
    fn is_task_tool_recognizes_webuild_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(xai_webuild_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(xai_webuild_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(xai_webuild_tools::types::output::MediaGenOutput::uploaded(
    ...
    /// SuperWeBuild upsell as `ToolOutput::Text` on a `Completed` status. The media
