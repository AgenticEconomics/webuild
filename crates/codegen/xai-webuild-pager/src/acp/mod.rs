...
use xai_webuild_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a webuild-shell instance.
    pub is_webuild_shell: bool,
    ...
    pub auth_manager: std::sync::Arc<xai_webuild_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_webuild_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_webuild_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    xai_webuild_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_webuild_shell::util::webuild_home::webuild_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a webuild-shell agent
    let is_webuild_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("webuildShell"))
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "webuildShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "webuildShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_non_webuild_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.webuild/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_webuild_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    /// `startup_auth_webuild_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_webuild_shell::agent::auth_method::{WEBUILD_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(WEBUILD_COM_METHOD_ID, "WeBuild", None),
