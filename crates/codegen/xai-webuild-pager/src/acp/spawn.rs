...
//!
//! Simplified to only support WeBuildShell (in-process) mode.
...
/// Spawn a WeBuildShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_webuild_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<xai_webuild_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.webuild/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        xai_webuild_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
