...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("webuild");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("webuild");
        self.last_title.clear();
        self.last_title.push_str("webuild");
...
mod tests {
    ...
    #[test]
    fn webuild_only_produces_just_webuild() {
        let cfg = config_with_items(vec![TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "webuild");
    }

    #[test]
    fn session_name_and_webuild_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - webuild");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - webuild"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "webuild");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - webuild");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - webuild");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    #[test]
    fn empty_items_produces_webuild_fallback() {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("grok-3"),
            ..idle_state()
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "grok-3 - webuild");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - webuild");
    }
    ...
    #[test]
    fn reset_clears_state_and_emits_webuild() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::WeBuild]);
        let mut mgr = TitleManager::new(&cfg);
        let activity = TurnActivity::Thinking;
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "webuild");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("webuild"), "title missing 'webuild': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - grok-3 - workspace - webuild"
        );
    }
