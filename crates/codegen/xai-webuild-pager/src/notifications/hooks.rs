...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("WEBUILD_EVENT", event_str)
        .env("WEBUILD_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("WEBUILD_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'WEBUILD_EVENT=%s\\nWEBUILD_MESSAGE=%s\\nWEBUILD_SESSION_ID=%s\\n' \
             \"$WEBUILD_EVENT\" \"$WEBUILD_MESSAGE\" \"$WEBUILD_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("WEBUILD_EVENT=Turn complete"),
            "missing WEBUILD_EVENT: {content}"
        );
        assert!(
            content.contains("WEBUILD_MESSAGE=hello world"),
            "missing WEBUILD_MESSAGE: {content}"
        );
        assert!(
            content.contains("WEBUILD_SESSION_ID=sess-42"),
            "missing WEBUILD_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("WEBUILD_SESSION_ID"),
            "WEBUILD_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("WEBUILD_EVENT=Turn complete"));
        assert!(content.contains("WEBUILD_MESSAGE=test body payload"));
        assert!(content.contains("WEBUILD_SESSION_ID=test-session-123"));
    }
}
