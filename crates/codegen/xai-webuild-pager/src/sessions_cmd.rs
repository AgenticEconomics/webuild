...
use xai_webuild_shell::auth::{AuthManager, try_ensure_fresh_auth};
use xai_webuild_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // If the user has previously run the interactive `webuild` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.webuild_com_config).await;
    ...
    let client = xai_webuild_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = xai_webuild_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use xai_webuild_shell::session::merge::REMOTE_TIMEOUT;
            use xai_webuild_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = xai_webuild_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
