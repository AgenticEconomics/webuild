...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "WeBuild 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("grok-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "WeBuild 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "WeBuild 4.5");
        match result {
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("WeBuild 4.5") && i.insert_text == "WeBuild 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "WeBuild 4.3" && i.insert_text == "WeBuild 4.3")
        );
    }
    #[test]
    ...
    fn usage_manage_returns_open_url() {
        match run_usage("manage") {
            CommandResult::Action(Action::OpenUrl(url)) => {
                assert_eq!(url, "https://github.com/AgenticEconomics/webuild#readme");
            }
