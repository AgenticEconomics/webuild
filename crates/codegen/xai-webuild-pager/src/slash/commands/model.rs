...
use xai_webuild_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("WeBuild 4.5"); if we split on the last token
        // first, a shorter catalog entry ("WeBuild") would steal the prefix and
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("grok-4.5", "WeBuild 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "WeBuild 4.5").unwrap();
        assert_eq!(plain.insert_text, "WeBuild 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "WeBuild" (reasoning) and "WeBuild 4.5". `/model WeBuild 4.5`
        // must select the full name, not treat "4.5" as an effort on "WeBuild".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("webuild", "WeBuild");
        let (long_id, long_info) = model_with_reasoning("grok-4.5", "WeBuild 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "WeBuild 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(WeBuild 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("grok-4.5", "WeBuild 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "WeBuild 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model webuild 4.5`
    /// resolves to the same `ModelId` as `/model WeBuild 4.5`.
    #[test]
