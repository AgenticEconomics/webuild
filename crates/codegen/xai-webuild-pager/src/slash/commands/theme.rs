...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::WeBuildNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let webuildnight = items
                .iter()
                .find(|i| i.insert_text == "webuildnight")
                .expect("webuildnight should be in list");
            assert!(
                webuildnight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                webuildnight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "webuildnight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "webuildnight");
                }
                other => panic!("expected Action::SetTheme(\"webuildnight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = WeBuildNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for WeBuildNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "webuildnight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"webuildnight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> WeBuildDay.
            assert_eq!(Theme::current_kind(), ThemeKind::WeBuildDay);
        });
    }
    ...
    fn cancel_preview_restores_previous_kind() {
        with_test_env(|| {
            ...
            cmd.cancel_preview("webuildnight");
            assert_eq!(
                Theme::current_kind(),
                ThemeKind::WeBuildNight,
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::WeBuildDay);
            let cmd = ThemeCommand;
