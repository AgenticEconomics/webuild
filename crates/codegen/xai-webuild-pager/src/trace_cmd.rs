...
use xai_webuild_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $WEBUILD_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
    ...
    if !agent_config.is_trace_upload_enabled() {
        ...
        if !args.json {
            eprintln!(
                ...
                crate::util::display_user_webuild_path("config.toml")
            );
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            webuild_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    xai_webuild_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_webuild_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    webuild_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `webuild login` or set a deployment key. \
                 See {} for upload overrides.",
                crate::util::display_user_webuild_path("docs/user-guide")
            );
        }
    };
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            xai_webuild_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
            "No trace upload bucket configured. Set `WEBUILD_TELEMETRY_GCS_BUCKET`, \
             `WEBUILD_TRACE_UPLOAD_BUCKET`, or `endpoints.trace_upload_bucket` in \
    ...
    let upload_config = xai_webuild_shell::session::repo_changes::TraceExportConfig {
...
impl UploadAttempt<'_> {
    ...
    pub fn handle_failure(&self, error: &anyhow::Error) -> anyhow::Error {
        ...
        if self.json {
            ...
            eprintln!("  Retry:  webuild trace {}", self.session_id);
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "WeBuild version: {}", env!("VERSION_WITH_COMMIT"));
        let _ = writeln!(
            log,
            "OS:           {} {}",
            std::env::consts::OS,
...
async fn upload_with_retries(
    config: &xai_webuild_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = xai_webuild_shell::auth::ensure_authenticated_or_noninteractive(
