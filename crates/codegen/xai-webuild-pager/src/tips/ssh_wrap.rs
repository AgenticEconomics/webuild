//! SSH wrap tip: over SSH without `webuild wrap`, advertise that wrapping the
...
/// Ephemeral-tip dedup key for the SSH `webuild wrap` hint.
...
/// Build "Over SSH? Run `webuild wrap ssh <host>` locally for clipboard +
    ...
    EphemeralTip {
        ...
        ..EphemeralTip::new(
            ...
            Line::from(vec![
                ...
                Span::styled("webuild wrap ssh <host>", command),
...
mod tests {
    ...
    fn ssh_wrap_tip_advertises_local_wrap() {
        ...
        assert_eq!(
            text,
            "Over SSH? Run webuild wrap ssh <host> locally for clipboard + terminal restore"
        );
    }
