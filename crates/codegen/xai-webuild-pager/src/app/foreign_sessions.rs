...
use xai_webuild_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, webuild_home: &Path) -> [PathBuf; 2] {
        ...
        [
            webuild_home
                ...
                .join(skill)
                .join("SKILL.md"),
            webuild_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(webuild_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, webuild_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, webuild_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        webuild_home: webuild_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use xai_webuild_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/webuild"), |path| {
