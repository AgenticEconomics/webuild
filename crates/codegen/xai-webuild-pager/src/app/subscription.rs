...
/// `webuild_build_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `WEBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("WEBUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: xai_webuild_shell::auth::GateInfo) -> Vec<Effect> {
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: xai_webuild_shell::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> xai_webuild_shell::auth::GateInfo {
    ...
    fn may_be_free_tier_matrix() {
        ...
        app.subscription_tier = Some("SuperWeBuild Heavy".into());
        assert!(!app.may_be_free_tier());
    ...
    fn fire_subscription_check_guards_and_debounces() {
        ...
        paid.subscription_tier = Some("SuperWeBuild".into());
        assert!(
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = xai_webuild_shell::auth::GateInfo {
