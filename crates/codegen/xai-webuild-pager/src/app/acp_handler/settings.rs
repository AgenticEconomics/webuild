...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        xai_webuild_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = xai_webuild_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = xai_webuild_shell::config::load_effective_config().ok();
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/SuperWeBuild
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(xai_webuild_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("WEBUILD_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(xai_webuild_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        xai_webuild_shell::config::load_merged_requirements(),
        xai_webuild_shell::config::load_from_disk().ok(),
        xai_webuild_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = xai_webuild_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = xai_webuild_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = xai_webuild_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use xai_webuild_shell::util::config::resolve_tips;
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = xai_webuild_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        xai_webuild_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<xai_webuild_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `WEBUILD_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[xai_webuild_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = xai_webuild_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = xai_webuild_announcements::filter_expired(merged);
    ...
    if xai_webuild_announcements::prune_hidden_announcement_ids(
...
pub(super) fn pick_random_announcement(
    announcements: &[xai_webuild_announcements::RemoteAnnouncement],
) -> Option<xai_webuild_announcements::RemoteAnnouncement> {
...
/// `xai-webuild-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `xai-webuild-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
