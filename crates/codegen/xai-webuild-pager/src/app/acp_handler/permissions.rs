...
fn enqueue_permission(
    ...
    // 1. Parse bash highlights from request meta (imported from xai-webuild-shell).
    ...
    let bash_selection_count = bash_highlights
        .as_ref()
        .map(|h| xai_webuild_workspace::permission::default_always_allow_scope(&h.highlighted_words))
        .unwrap_or(0);
    ...
    let mcp_scope = perm
        ...
        .and_then(|m| {
            serde_json::from_value::<xai_webuild_workspace::permission::McpToolPermission>(
...
/// `xai-webuild-tools` for typed access to `command` and `description`.
...
fn build_permission_display(
    ...
    let bash_input = req.tool_call.fields.raw_input.as_ref().and_then(|v| {
        serde_json::from_value::<xai_webuild_tools::implementations::BashToolInput>(v.clone()).ok()
    });
    ...
    let title = if is_execute {
        ...
        if let Some(path) = file_path {
            ...
            format!(
                "Allow {}?",
                xai_webuild_workspace::permission::mcp_pretty_name_if_qualified(t)
            )
        } else {
