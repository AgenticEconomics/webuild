use super::*;
use xai_webuild_shell::sampling::error::format_rate_limited_user_message;
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        xai_webuild_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        xai_webuild_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        xai_webuild_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[xai_webuild_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &xai_webuild_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
    ...
    if is_credit_limit {
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use xai_webuild_tools::types::SessionMode;
