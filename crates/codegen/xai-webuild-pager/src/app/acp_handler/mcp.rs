...
/// `WEBUILD_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        xai_webuild_shell::util::config::resolve_mcp_push_server_status(
...
/// `xai-webuild-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`xai_webuild_shell::extensions::mcp::McpToolsChanged`] struct), and
...
/// ([`xai_webuild_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`xai_webuild_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use xai_webuild_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
...
/// on config reload (`crates/codegen/xai-webuild-shell/src/agent/mvp_agent.rs`
