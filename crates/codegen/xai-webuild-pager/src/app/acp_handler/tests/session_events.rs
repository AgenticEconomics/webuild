#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use xai_webuild_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use xai_webuild_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `webuild logout` /
    /// `webuild login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `webuild logout` then `webuild login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(xai_webuild_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
