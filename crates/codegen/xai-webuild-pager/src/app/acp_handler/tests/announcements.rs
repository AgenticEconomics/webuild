#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.webuild` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.webuild` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.webuild` reads).
    #[test]
