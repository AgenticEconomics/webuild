...
use xai_webuild_shell::extensions::notification::RetryState;
use xai_webuild_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> xai_webuild_shell::extensions::notification::ImageCompressedEntry {
    xai_webuild_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use xai_webuild_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Shared temp `WEBUILD_HOME` for disk-replay tests. `webuild_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `WEBUILD_HOME`
...
/// Runs `f` with a thread-local webuild home override so disk replay tests do not
/// depend on process-wide `webuild_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_webuild_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_webuild_home_for_tests(None);
    out
}
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = xai_webuild_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: xai_webuild_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use xai_webuild_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = xai_webuild_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "webuild_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "webuild_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("x.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
