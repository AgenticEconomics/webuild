...
/// (`[ui] mouse_reporting_toggle` / `WEBUILD_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    xai_webuild_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn voice_mode_config_value() -> Option<bool> {
    xai_webuild_shell::config::load_effective_config()
        .ok()
        .and_then(|cfg| {
            cfg.get("features")
                .and_then(|f| f.get("voice_mode"))
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use xai_webuild_shell::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("WEBUILD_VOICE_MODE")
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `xai-webuild-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`xai-webuild-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&xai_webuild_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<xai_webuild_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<xai_webuild_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `webuild`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<xai_webuild_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ...
    let webuild_com_config =
        match xai_webuild_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.webuild_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                xai_webuild_shell::auth::WeBuildComConfig::default()
            }
        };
    let refreshed_auth = xai_webuild_shell::auth::try_ensure_fresh_auth(&webuild_com_config).await;
    let early_prefetch =
        xai_webuild_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    xai_webuild_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(xai_webuild_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = xai_webuild_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("WEBUILD_HUNK_TRACKER").ok();
    ...
    let launch_auto = xai_webuild_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  webuild --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  webuild --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - webuild"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "webuild");
        assert_eq!(terminal_title_string(""), "webuild");
        assert_eq!(terminal_title_string("My chat"), "My chat - webuild");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["webuild-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["webuild-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["webuild-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["webuild-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["webuild-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["webuild-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = xai_webuild_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["webuild-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["webuild-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["webuild-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["webuild-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["webuild-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["webuild-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["webuild-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["webuild-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["webuild-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["webuild-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["webuild-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["webuild-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["webuild-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["webuild-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["webuild-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["webuild-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_webuild() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "webuild");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "WeBuild TUI",
                "",
                "Usage: webuild [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["webuild-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["webuild-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  webuild --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  webuild --resume sess-abc\n"));
    }
    #[test]
