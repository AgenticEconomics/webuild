...
/// [`decide`](xai_webuild_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("WEBUILD_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = xai_webuild_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(xai_webuild_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = xai_webuild_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match xai_webuild_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = xai_webuild_config::env_bool("WEBUILD_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("WEBUILD_SESSION_PICKER_GROUPED")
        .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    xai_webuild_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<xai_webuild_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == xai_webuild_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = xai_webuild_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = xai_webuild_shell::config::load_merged_requirements();
    let user_config = xai_webuild_shell::config::load_from_disk().ok();
    let managed_config = xai_webuild_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match xai_webuild_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = xai_webuild_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > WEBUILD_XAI_API_BASE_URL env > default).
            let endpoints_base =
                xai_webuild_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                xai_webuild_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to webuild-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = xai_webuild_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        xai_webuild_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        xai_webuild_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        xai_webuild_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use xai_webuild_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = xai_webuild_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let webuild_home = xai_webuild_tools::util::webuild_home::webuild_home();
            app.tip = xai_webuild_shell::util::tips::pick_and_advance(&app.tips, &webuild_home);
        }
    }

    let hints = xai_webuild_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.webuild/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        xai_webuild_shell::util::config::clamped_display_permission_mode(
            xai_webuild_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        xai_webuild_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = xai_webuild_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = xai_webuild_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // WEBUILD_SCROLL_SPEED/_MODE/_LINES + WEBUILD_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<xai_webuild_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`webuild "fix the bug"`). When
    ...
    // `webuild dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `WEBUILD_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("WEBUILD_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(xai_webuild_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart webuild");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.webuild/config.toml [ui]`
                // rather than `~/.webuild/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = xai_webuild_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> xai_webuild_shell::agent::config::UiConfig {
    use xai_webuild_shell::agent::config::UiConfig;
    let Ok(root) = xai_webuild_shell::config::load_effective_config() else {
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && xai_webuild_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
