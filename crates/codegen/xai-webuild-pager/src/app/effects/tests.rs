...
use super::*;
use xai_webuild_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use xai_webuild_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use xai_webuild_workspace::session::git::RestoreDegree;
...
/// Redirect `WEBUILD_HOME` to a tempdir for test isolation.
fn setup_webuild_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("WEBUILD_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xai_webuild_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        xai_webuild_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        xai_webuild_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_webuild_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xai_webuild_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            webuild_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// Neutralize `WEBUILD_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(WEBUILD_AGENT)]` (process-global env).
fn without_webuild_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("WEBUILD_AGENT", "")
}
...
#[serial_test::serial(WEBUILD_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_webuild_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> xai_webuild_shell::session::SessionInfoResponse {
    use xai_webuild_shell::session::acp_types::{ContextInfo, SessionInfoData};
    xai_webuild_shell::session::SessionInfoResponse {
