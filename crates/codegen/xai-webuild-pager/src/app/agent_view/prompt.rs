    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    xai_webuild_telemetry::session_ctx::log_event(
                        xai_webuild_telemetry::events::PromptSuggestion {
                            action: xai_webuild_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::PromptSuggestion {
                        action: xai_webuild_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
