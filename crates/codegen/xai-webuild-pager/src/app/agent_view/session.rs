...
impl AgentView {
    ...
    pub fn apply_full_context_info(&mut self, next: xai_webuild_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            ...
            None => {
                self.context_state = Some(xai_webuild_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
