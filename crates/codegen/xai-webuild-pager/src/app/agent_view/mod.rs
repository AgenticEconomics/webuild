...
pub struct AgentView {
    ...
    pub context_state: Option<xai_webuild_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<xai_webuild_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(xai_webuild_telemetry::events::CreditLimitChoice::PayAsYouGo);
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::SuperWeBuildUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use xai_webuild_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        xai_webuild_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = xai_webuild_tools::util::webuild_home::webuild_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &xai_webuild_tools::implementations::webuild_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
