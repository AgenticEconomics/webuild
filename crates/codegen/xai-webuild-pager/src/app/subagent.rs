...
thread_local! {
    static REPLAY_WEBUILD_HOME : std::cell::RefCell < Option < std::path::PathBuf >> = const
    { std::cell::RefCell::new(None) };
}
/// Override webuild home for disk-replay unit tests (thread-local; production never sets this).
#[cfg(test)]
pub(crate) fn set_replay_webuild_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_WEBUILD_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_webuild_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_WEBUILD_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_webuild_home(), parent_cwd, parent_session_id);
}
...
pub(crate) fn replay_inherited_updates(
    ...
    let home = effective_webuild_home();
    let updates =
        match xai_webuild_shell::session::storage::load_updates_for_replay_at(child_session_id, &home)
        {
            Ok(Some(u)) => u,
            Ok(None) => return,
            Err(e) => {
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_webuild_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_webuild_home_for_tests(None);
    }
    #[test]
