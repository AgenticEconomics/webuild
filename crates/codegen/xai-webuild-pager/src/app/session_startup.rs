...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain WeBuild session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: xai_webuild_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = xai_webuild_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = xai_webuild_shell::util::webuild_home::webuild_home().join("sessions");
    let encoded = xai_webuild_shell::util::webuild_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    xai_webuild_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = xai_webuild_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'webuild' to start a new session."
        )
    })?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &xai_webuild_shell::agent::config::Config,
) -> std::sync::Arc<xai_webuild_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(xai_webuild_shell::auth::AuthManager::new(
        &xai_webuild_shell::util::webuild_home::webuild_home(),
        agent_config.webuild_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.webuild_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if xai_webuild_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = xai_webuild_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = xai_webuild_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = xai_webuild_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use xai_webuild_shell::agent::session_registry_client::SessionRegistryClient;
    use xai_webuild_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = xai_webuild_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: xai_webuild_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["webuild"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["webuild", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_session_id_alone_is_new_with_id() {
        assert_eq!(
            parse(&["webuild", "--session-id", "my-id"])
                .session_startup_intent()
                .unwrap(),
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["webuild", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["webuild", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["webuild", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["webuild", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["webuild", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["webuild"])).chat_mode);
    }
