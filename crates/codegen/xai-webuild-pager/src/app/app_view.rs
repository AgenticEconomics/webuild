...
/// [`xai_webuild_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<xai_webuild_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: xai_webuild_shell::util::config::WorktreeHintMode) -> Self {
        use xai_webuild_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        xai_webuild_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            xai_webuild_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`xai_webuild_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `WEBUILD_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `WEBUILD_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("WEBUILD_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`xai_webuild_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => xai_webuild_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: xai_webuild_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`xai_webuild_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.webuild/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`WEBUILD_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `WEBUILD_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<xai_webuild_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<xai_webuild_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `WEBUILD_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `webuild dashboard`, the
    ...
    /// non-selectable headers. Gated by `WEBUILD_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `WEBUILD_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `xai-webuild-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<xai_webuild_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        xai_webuild_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `xai_webuild_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `WEBUILD_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: xai_webuild_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<xai_webuild_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `webuild wrap` tip: set after the first
    ...
    /// `webuild_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, xai_webuild_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `webuild_build_access_gate`. `Some` = blocked.
    pub gate: Option<xai_webuild_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperWeBuild", "Free").
    ...
    pub pending_gate_verification: Option<xai_webuild_shell::auth::GateInfo>,
    ...
    /// `~/.webuild/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `WEBUILD_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &xai_webuild_shell::util::config::RemoteSettings,
    ) -> Option<xai_webuild_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(xai_webuild_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &xai_webuild_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            xai_webuild_telemetry::session_ctx::log_event(
                xai_webuild_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: xai_webuild_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: xai_webuild_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: xai_webuild_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<xai_webuild_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && xai_webuild_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: xai_webuild_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(xai_webuild_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(xai_webuild_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `WEBUILD_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<xai_webuild_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_webuild_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        xai_webuild_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                xai_webuild_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            xai_webuild_telemetry::session_ctx::log_event(
                                xai_webuild_telemetry::events::SuperWeBuildUpsellShown {
                                    source:
                                        xai_webuild_telemetry::events::SuperWeBuildUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`xai_webuild_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use xai_webuild_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = xai_webuild_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                xai_webuild_telemetry::session_ctx::log_event(
                    xai_webuild_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: xai_webuild_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `webuild wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::ContextualTip {
                tip: xai_webuild_telemetry::events::ContextualTipKind::ImageInput,
                action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![xai_webuild_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![xai_webuild_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: xai_webuild_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = xai_webuild_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = xai_webuild_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&xai_webuild_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&xai_webuild_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = xai_webuild_shell::auth::AuthMeta {
            subscription_tier: Some("SuperWeBuild".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&xai_webuild_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperWeBuild".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperWeBuild")));
        assert!(!is_restricted_tier(Some("SuperWeBuild Heavy")));
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = xai_webuild_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(xai_webuild_workspace::foreign_sessions::RecentForeignSession {
                tool: xai_webuild_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![xai_webuild_workspace::file_system::FuzzyMatchResult {
