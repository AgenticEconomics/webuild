...
///
/// Special case for [`xai_webuild_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.webuild/config.toml`, and fire the
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == xai_webuild_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != xai_webuild_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
        ...
        .filter(|_| option_id.0.as_ref() == "allow-always-mcp")
    {
        let selection = match scope.selected {
            crate::views::permission_view::McpScope::Tool => {
                xai_webuild_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                }
            }
            crate::views::permission_view::McpScope::Server => match &scope.server_prefix {
                Some(prefix) => xai_webuild_workspace::permission::McpScopeSelection::Server {
                    server: prefix.clone(),
                },
                // Defensive: render path should disable Server when no prefix.
                None => xai_webuild_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                },
            },
        };
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(xai_webuild_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
