...
) -> xai_webuild_workspace::foreign_sessions::RecentForeignSession {
    xai_webuild_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_webuild_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(xai_webuild_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_webuild_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "WeBuild".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = xai_webuild_shell::agent::config::ModelSwitchIncompatibleAgentError {
...
fn same_agent_type_switch_no_modal() {
    ...
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "WeBuild A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "WeBuild B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = xai_webuild_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = xai_webuild_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = xai_webuild_shell::util::config::RemoteSettings {
...
fn test_gate() -> xai_webuild_shell::auth::GateInfo {
    xai_webuild_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(xai_webuild_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(xai_webuild_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
