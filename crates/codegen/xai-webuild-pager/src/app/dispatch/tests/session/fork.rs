...
fn worktree_forked_sets_session_id_eagerly_and_emits_load() {
    ...
    let worktree_path = PathBuf::from("/tmp/webuild-worktrees/pager-fork");
    let session_cwd = PathBuf::from("/tmp/webuild-worktrees/pager-fork/sub");
...
fn worktree_forked_with_restore_failure_shows_warning_banner() {
    ...
    let worktree_path = PathBuf::from("/tmp/webuild-worktrees/pager-fail");
    let session_cwd = PathBuf::from("/tmp/webuild-worktrees/pager-fail/sub");
    dispatch(
        Action::TaskComplete(TaskResult::WorktreeForked {
            agent_id: id,
