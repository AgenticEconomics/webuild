...
fn voice_on_welcome_creates_session_and_records() {
    if !xai_webuild_voice::AUDIO_SUPPORTED {
        return;
    }
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(xai_webuild_voice::VoiceCommand::PttPress)
    ));
}
#[test]
fn voice_final_routes_to_recording_session_not_active_view() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        xai_webuild_voice::VoiceEvent::UtteranceFinal {
            text: "hello".into(),
        },
    );
...
fn voice_auto_stops_when_leaving_recording_session() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(xai_webuild_voice::VoiceCommand::PttRelease)
    ));
}
#[test]
...
fn worktree_session_created_sets_session_and_cwd() {
    ...
    let id = AgentId(0);
    let worktree_path = PathBuf::from("/tmp/webuild-worktrees/pager-123");
...
fn worktree_session_preserves_subdirectory_offset() {
    let mut app = test_app();
    app.cwd = PathBuf::from("/repo/crates/codegen/xai-webuild-pager");
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.webuild/worktrees/repo/pager-123");
    let session_cwd = worktree_root.join("crates/codegen/xai-webuild-pager");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
fn worktree_session_created_drains_queued_prompts() {
    ...
    assert_eq!(app.agents[&id].session.queue_len(), 1);
    let worktree_path = PathBuf::from("/tmp/webuild-worktrees/pager-abc");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
            session_id: acp::SessionId::new("wt-drain-1"),
            worktree_path,
            session_cwd: PathBuf::from("/tmp/webuild-worktrees/pager-abc"),
            models: None,
        }),
        &mut app,
    );
...
/// trust. WEBUILD_HOME-isolated so the write hits a temp store, not the real one.
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(xai_webuild_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    app.yolo_policy_block = Some(POLICY_WARNING);
    {
        ...
        agent.deferred_session_mode = Some(xai_webuild_tools::types::SessionMode::Plan);
    }
...
fn dispatch_cycle_mode_pre_session_cycles_locally_and_creates_session() {
    ...
    assert_eq!(
        ...
        Some(xai_webuild_tools::types::SessionMode::Plan),
