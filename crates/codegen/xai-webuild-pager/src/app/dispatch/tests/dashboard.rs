...
/// `webuild dashboard` before login: the startup hook consumes the
/// `WEBUILD_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `webuild dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "grok-4.5", "WeBuild 4.5");
    open_dashboard(&mut app);
    app.cwd_has_git_ancestor = true;
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(xai_webuild_shell::sampling::types::ReasoningEffort::High),
            display: "WeBuild 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(xai_webuild_shell::sampling::types::ReasoningEffort::High)
        )),
...
/// `webuild dashboard`, and the startup hook.
#[serial_test::serial(WEBUILD_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    assert_eq!(pending.display, "WeBuild 4.5");
...
fn dashboard_slash_restricted_command_upsells_via_toast() {
    ...
    assert!(
        toast.contains("/imagine") && toast.contains("SuperWeBuild"),
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_webuild_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("WEBUILD_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
