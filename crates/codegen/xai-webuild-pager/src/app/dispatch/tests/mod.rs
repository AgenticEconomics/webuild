...
/// Plant a Build session under the process `webuild_home()` (OnceLock-cached;
/// do not rely on setting `WEBUILD_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = xai_webuild_shell::util::webuild_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xai_webuild_tools::implementations::webuild_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    xai_webuild_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    xai_webuild_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
