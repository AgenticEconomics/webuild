...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let response = xai_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![xai_hooks_plugins_types::MarketplaceScanResult {
            source_name: xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn slash_model_valid_dispatches_set_default_model_with_switch_and_persist() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "WeBuild 4.5".to_string()),
        );
    let effects = dispatch(Action::SendPrompt("/model WeBuild 4.5".into()), &mut app);
    assert_eq!(
        effects.len(),
        2,
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("webuild-test"));
    let info = acp::ModelInfo::new(id.clone(), "WeBuild Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "WeBuild 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("webuild-already"));
    let info = acp::ModelInfo::new(id.clone(), "WeBuild Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `clamp_to_terminal` might fold to WeBuildNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        crate::theme::cache::set(crate::theme::ThemeKind::WeBuildNight);
        let effects = dispatch(Action::SetTheme("webuildday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("webuildday"));
                assert_eq!(*rollback_value, SettingValue::Enum("webuildnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("webuildday"));
        assert!(
...
/// for the auto-dark sibling. Uses `webuildday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("webuildday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("webuildday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("webuildnight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("webuildnight"));
                assert_eq!(*rollback_value, SettingValue::Enum("webuildday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="webuildnight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="webuildday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on WeBuildNight even though we're committing a
/// different theme. Uses `webuildday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("webuildnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::WeBuildNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("webuildday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::WeBuildNight,
...
///
/// Uses `WeBuildDay` (non-truecolor-requiring) for the dark-mode
...
/// WeBuildNight. Using a non-truecolor theme avoids the clamp
...
fn set_auto_dark_theme_applies_when_theme_is_auto_and_system_is_dark() {
    with_theme_test_env(|| {
        ...
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::WeBuildDay,
...
/// Uses `webuildnight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from WeBuildDay, the
...
fn set_auto_dark_theme_does_not_apply_when_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoDarkTheme("webuildnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::WeBuildDay,
            "auto_dark_theme commit must NOT change live display when system=Light",
        );
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("webuildnight"),);
    });
}
/// Symmetric to the dark test: `set_auto_light_theme` applies only
/// when theme=auto + system=Light. Uses a non-truecolor theme
/// (`webuildnight`) for the same clamp reason as the dark variant.
#[test]
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("webuildnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::WeBuildNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("webuildday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("WeBuild Day"),
            "toast must use display name `WeBuild Day`, not canonical `webuildday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("WeBuild Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("WeBuild Night"));
    });
}
...
///
/// Uses non-truecolor themes (`webuildday` ↔ `webuildnight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("webuildnight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("webuildday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("webuildday"));
    });
}
