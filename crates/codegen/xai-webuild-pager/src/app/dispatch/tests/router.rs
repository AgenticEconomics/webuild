...
fn critical_announcement(id: &str) -> xai_webuild_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> xai_webuild_announcements::RemoteAnnouncement {
    xai_webuild_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(xai_webuild_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
/// the promo url (observed via the `WEBUILD_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("webuild-cta-open-{}.txt", std::process::id()));
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.webuild/skills/pick-best/SKILL.md", }
    );
