...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("superwebuild_heavy")));
    assert!(is_max_tier(Some("SuperWeBuild Heavy")));
    assert!(is_max_tier(Some("SUPERWEBUILD_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("superwebuild")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("superwebuild_heav")));
    assert!(!is_max_tier(Some("superwebuild_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_superwebuild() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("superwebuild"), "got: {url}");
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): WeBuild usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superwebuild_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superwebuild_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_webuild_telemetry::events::SuperWeBuildUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        (
            "Upgrade to SuperWeBuild",
            "For everyday coding and productivity tasks",
            Some(UPSELL_URL_UPGRADE),
        ),
        (
            "Upgrade to SuperWeBuild Heavy",
            "Get the most out of WeBuild. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use xai_webuild_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option SuperWeBuild
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_webuild_telemetry::events::SuperWeBuildUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with SuperWeBuild.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to SuperWeBuild");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to SuperWeBuild Heavy");
...
/// `WEBUILD_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
#[serial_test::serial(WEBUILD_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "webuild-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("WEBUILD_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("WEBUILD_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("webuild-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("WEBUILD_TEST_OPEN_URL_FILE", &url_file) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "webuild-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("WEBUILD_TEST_OPEN_URL_FILE", &bad) };
