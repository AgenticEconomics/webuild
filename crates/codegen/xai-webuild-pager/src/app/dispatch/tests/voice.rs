...
/// A free-tier user hitting the voice keybinding gets the SuperWeBuild upsell
...
fn voice_keybinding_on_restricted_tier_opens_upsell() {
    ...
    assert!(
        ...
        "restricted-tier voice keybinding must open the SuperWeBuild upsell"
    );
    assert!(
        !app.voice_listening(),
...
fn voice_interim_sets_then_error_clears_state() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        xai_webuild_voice::VoiceEvent::InterimTranscript {
            text: "partial".into(),
        },
    );
    ...
    crate::voice::handle_voice_event(
        &mut app,
        xai_webuild_voice::VoiceEvent::Error {
            message: "boom".into(),
        },
    );
    assert!(!app.voice_listening());
...
fn voice_stt_language_change_recycles_pipeline() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(xai_webuild_voice::VoiceCommand::Shutdown)
    ));
    assert!(matches!(
        effects.as_slice(),
