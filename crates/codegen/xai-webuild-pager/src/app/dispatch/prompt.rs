...
/// Start (if needed) and submit the initial prompt from `webuild "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::SmallScreen,
            action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
        });
    }
}

/// Show the one-shot "Over SSH? Run `webuild wrap ssh <host>` locally…" hint at
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::SshWrap,
            action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::PlanMode,
            action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::WordSelect,
            action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(xai_webuild_telemetry::events::ContextualTip {
        tip: xai_webuild_telemetry::events::ContextualTipKind::WordSelect,
        action: xai_webuild_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_webuild_telemetry::events::ContextualTip {
            tip: xai_webuild_telemetry::events::ContextualTipKind::SendNow,
            action: xai_webuild_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
            ...
            .is_restricted(invocation.token)
    {
        ...
        // another question modal already up, `open_superwebuild_upsell` would
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use xai_webuild_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use xai_webuild_telemetry::session_ctx::log_event;
