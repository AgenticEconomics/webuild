...
use xai_webuild_telemetry::events::{SuperWeBuildUpsell, SuperWeBuildUpsellClicked};
use xai_webuild_telemetry::session_ctx::log_event;

// Free-usage detection lives in shell next to the well-known code + 429 copy.
pub(crate) use xai_webuild_shell::sampling::error::is_free_usage_exhausted_error;
...
/// Whether the user is at the highest subscription tier (SuperWeBuild Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("superwebuild_heavy") and CCP display names ("SuperWeBuild Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "superwebuild_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://github.com/AgenticEconomics/webuild#readme";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://github.com/AgenticEconomics/webuild#readme";
...
///
/// **`max_tier = true`** (positively identified as SuperWeBuild Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_webuild_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using WeBuild",
            CreditLimitCardAction::PurchaseCredits,
            xai_webuild_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_webuild_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_webuild_telemetry::events::CreditLimitUpsellShown {
            surface: xai_webuild_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_webuild_telemetry::events::CreditLimitUpsellShown {
        surface: xai_webuild_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_webuild_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperWeBuildUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_superwebuild_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperWeBuild upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_superwebuild_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperWeBuild upsell modal. Controls the heading
...
fn open_superwebuild_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperWeBuildUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperWeBuild.",
            SuperWeBuildUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(xai_webuild_telemetry::events::SuperWeBuildUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperWeBuild".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperWeBuild Heavy".into(),
            description: "Get the most out of WeBuild. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_gate_refreshed(
    app: &mut AppView,
    settings: Option<xai_webuild_shell::util::config::RemoteSettings>,
) -> Vec<Effect> {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_webuild_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_webuild_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_superwebuild_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperWeBuildUpsellClicked {
        source: SuperWeBuildUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://github.com/AgenticEconomics/webuild#readme");
    // Funnel attribution: tag CLI-originated SuperWeBuild upsell clicks
