...
use super::billing::dispatch_open_superwebuild_url;
use super::ctx::{
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, xai_webuild_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(xai_webuild_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(xai_webuild_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::OpenSupergrokUrl => dispatch_open_superwebuild_url(app),
        Action::OpenUrl(url) => {
