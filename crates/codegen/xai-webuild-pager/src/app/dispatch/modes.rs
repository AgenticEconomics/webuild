...
/// from `~/.webuild/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        xai_webuild_tools::types::SessionMode::Plan.as_id()
    } else {
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: xai_webuild_telemetry::events::YoloTrigger::Pager,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(xai_webuild_tools::types::SessionMode::Plan.as_id()),
            }]
        }
