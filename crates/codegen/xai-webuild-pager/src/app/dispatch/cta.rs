...
pub(super) fn plugin_cta_candidates(
    ...
    for source in response.sources {
        let is_official = source.source_name == xai_webuild_plugin_marketplace::OFFICIAL_SOURCE_NAME
            || xai_webuild_plugin_marketplace::is_official_source_url(&source.source_url_or_path);
        if !is_official {
            continue;
        }
...
pub(super) fn plugin_cta_phase_for(
    ...
    use xai_webuild_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(xai_webuild_telemetry::events::PluginCtaInstalled {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(xai_webuild_telemetry::events::ExtensionsModalOpened {
                    trigger: xai_webuild_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = xai_webuild_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(xai_webuild_telemetry::events::PluginCtaImpression {
