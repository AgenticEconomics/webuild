...
/// Show the SuperWeBuild upsell when a tier-restricted (free / X Basic) user tries
        ...
        ActiveView::AgentDashboard => {
            if let Some(d) = app.dashboard.as_mut() {
                d.set_error_toast(&format!(
                    "/voice requires SuperWeBuild — upgrade at {}",
...
/// (free / X Basic), it shows the SuperWeBuild upsell instead of starting a session
...
pub(super) fn dispatch_enable_voice_mode(app: &mut AppView, from_hold: bool) -> Vec<Effect> {
    ...
    if !app.voice_mode_enabled || !xai_webuild_voice::AUDIO_SUPPORTED {
        return vec![];
    }
    ...
    // SuperWeBuild upsell instead of starting a doomed session (`/voice` itself is
