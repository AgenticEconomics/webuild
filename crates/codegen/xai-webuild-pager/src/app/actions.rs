...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: xai_webuild_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`webuild --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: xai_webuild_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    AnnouncementsOpenCta(xai_webuild_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"webuildnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `webuild dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.webuild/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: xai_webuild_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.webuild/config.toml`.
    ...
    /// Persist a typed setting to `~/.webuild/config.toml`. On failure,
    ...
    /// registry (`~/.webuild/active_sessions.json`).
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<xai_webuild_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<xai_webuild_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<xai_webuild_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<xai_webuild_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<xai_webuild_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<xai_webuild_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<xai_webuild_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
