...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + webuild_home OnceLock in the shared lib test binary); run: cargo test -p xai-webuild-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<webuild_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_webuild_home().join("sessions");
