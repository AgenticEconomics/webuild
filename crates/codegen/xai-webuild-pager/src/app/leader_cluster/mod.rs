...
//!
//! Env sandboxing follows this crate's `serial(WEBUILD_HOME)` idiom; note
//! `webuild_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_webuild_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `WEBUILD_LEADER_SOCKET`, `WEBUILD_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `webuild_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p xai-webuild-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The webuild home the agent actually persisted under: `webuild_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_webuild_home() -> PathBuf {
    xai_webuild_config::webuild_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same WEBUILD_HOME (two agents on one updates.jsonl is the
    ...
    _webuild_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(WEBUILD_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = webuild_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("WEBUILD_HOME", webuild_home.path()),
            crate::test_util::EnvVarGuard::set("WEBUILD_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("WEBUILD_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("WEBUILD_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("WEBUILD_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("WEBUILD_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (xai_webuild_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                xai_webuild_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(xai_webuild_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // xai-webuild-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: xai-webuild-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same WEBUILD_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `WEBUILD_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    webuild_ws_url: String::new(),
                    webuild_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
