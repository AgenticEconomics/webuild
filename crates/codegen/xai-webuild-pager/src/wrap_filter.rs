//! Streaming output filter for `webuild wrap`: OSC 52 clipboard interception,
...
///
/// Delegates to [`xai_webuild_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = xai_webuild_shell::util::clipboard::set_text(text) {
...
pub(crate) fn host_clipboard_image_frame() -> Vec<u8> {
    let image = xai_webuild_pager_render::clipboard::system_clipboard_get_image();
