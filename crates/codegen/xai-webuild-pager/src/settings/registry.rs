...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"webuildnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"WeBuild Night"`).
...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: xai_webuild_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`xai_webuild_voice::canonicalize_stt_language`] so the pager and
/// the STT client share one catalog (official WeBuild STT languages + client-only
/// `auto`). Unknown/blank/`None` → `en`.
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    xai_webuild_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `WEBUILD_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = xai_webuild_shell::models::default_model();
...
mod tests {
            ...
            match (meta.key, &meta.kind) {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in xai-webuild-tools"
                    );
                }
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        xai_webuild_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `xai_webuild_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = xai_webuild_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = xai_webuild_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match xai_webuild_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn current_value_for_auto_dark_theme_filters_auto_value() {
        ...
        assert_eq!(
            value,
            SettingValue::Enum("webuildnight"),
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: xai_webuild_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
