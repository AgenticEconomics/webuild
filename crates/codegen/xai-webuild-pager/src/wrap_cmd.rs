//! `webuild wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `webuild ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`webuild wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("webuild wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = xai_webuild_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("webuild wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `webuild wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`webuild wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`webuild wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `xai_webuild_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `webuild ssh`, `webuild wrap` does not gate on the terminal brand: the user
