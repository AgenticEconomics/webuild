...
//! render mode.
//!
//! `xai-webuild-pager` cannot depend on `xai-webuild-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-webuild-pager-bin`) wires it up once at
//! startup by calling `xai_webuild_pager_minimal::install()`. When nothing has
...
/// (`xai_webuild_pager_minimal::draw`).
