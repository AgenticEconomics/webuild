...
fn main() {
    ...
    let updates_path = match std::env::var("WEBUILD_SESSION_PATH") {
        Ok(p) => std::path::PathBuf::from(p),
        Err(_) => {
            eprintln!("Set WEBUILD_SESSION_PATH to a session's updates.jsonl path");
            eprintln!(
                "Example: WEBUILD_SESSION_PATH=$HOME/.webuild/sessions/<cwd-encoded>/019e0000-0000-7000-8000-000000000001/updates.jsonl"
            );
            return;
        }
    };
    ...
    if !updates_path.exists() {
        ...
        eprintln!("Set WEBUILD_SESSION_PATH env var to point to a session's updates.jsonl");
        return;
    }
