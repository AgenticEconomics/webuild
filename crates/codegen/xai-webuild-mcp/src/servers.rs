...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"webuild_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.webuild/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
impl McpState {
    ...
    /// `WeBuildClientHandler` cloned during `try_handshake` reads
...
/// MCP initialization strategy. Defined in `xai-webuild-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use xai_webuild_telemetry::enums::McpInitStrategy;
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        xai_webuild_telemetry::session_ctx::log_event(xai_webuild_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
/// Uses [`WeBuildClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, WeBuildClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`WeBuildClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`WeBuildClientHandler`]** constructed by
...
/// and the [`WeBuildClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to xai-webuild-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: xai_webuild_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, WeBuildClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("webuild-shell-{server_name}"),
    ...
    /// Build the [`WeBuildClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> WeBuildClientHandler {
        WeBuildClientHandler {
    ...
    /// references (the [`WeBuildClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            xai_webuild_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = xai_webuild_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.webuild/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = xai_webuild_config::webuild_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`webuild_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "webuild_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("webuild-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = xai_webuild_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = xai_webuild_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        xai_webuild_telemetry::session_ctx::log_event(
                            xai_webuild_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: xai_webuild_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    xai_webuild_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = xai_webuild_telemetry::instrumentation::timer("mcp_start_servers");
...
#[derive(Debug)]
pub struct WeBuildClientHandler {
...
impl ClientHandler for WeBuildClientHandler {
    ...
    // See the [`WeBuildClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("webuild_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("WEBUILD_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("webuild_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("webuild_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_webuild_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = WeBuildClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'webuild_com_notion' handshake failed: Auth required, when send initialize request"
        ));
