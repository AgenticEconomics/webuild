...
//!
//! Credentials are stored in `$WEBUILD_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `xai-webuild-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$WEBUILD_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$WEBUILD_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$WEBUILD_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user webuild home (set $WEBUILD_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$WEBUILD_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(xai_webuild_config::user_webuild_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$WEBUILD_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("webuild-mcp-credentials-test");
