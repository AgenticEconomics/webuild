...
const MCP_OAUTH_CLIENT_NAME: &str = "WeBuild";
...
// mode disabled, multiple `webuild` invocations).
//
// Layer 1 (cross-process): filesystem lock at $WEBUILD_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    xai_webuild_config::webuild_home().join(format!("mcp_auth_{safe}.lock"))
}
