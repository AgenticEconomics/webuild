...
///
/// Serializes to snake_case (`webuild_build`, `mcp`, …) for the
...
/// (e.g. `"WeBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "WeBuild")]
    WeBuild,
    #[serde(alias = "WeBuildConcise")]
    WeBuildConcise,
    #[serde(alias = "WeBuildHashline")]
    WeBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
