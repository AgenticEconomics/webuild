...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (webuild build harness) ─────────────
...
///
/// `"/path/.webuild/skills/my-skill/SKILL.md"` -> `"/path/.webuild/skills/"`
...
/// Used by the budgeted (webuild build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// WeBuild harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the webuild
...
mod tests {
    ...
    // ── budgeted mode: webuild build harness (budgeted XML) ───────────
