...
//!
//! Historically the agent hard-coded the dir lists `[".webuild", ".agents",
...
impl CompatCell {
    ...
    /// Whether WeBuild currently implements this compatibility surface.
    ///
...
pub const COMPAT_CELLS: [CompatCell; 18] = [
    CompatCell::new(
        ...
        "WEBUILD_CURSOR_SKILLS_ENABLED",
        Some(CompatRemoteKey::CursorSkills),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Rules,
        "WEBUILD_CURSOR_RULES_ENABLED",
        Some(CompatRemoteKey::CursorRules),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Agents,
        "WEBUILD_CURSOR_AGENTS_ENABLED",
        Some(CompatRemoteKey::CursorAgents),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Mcps,
        "WEBUILD_CURSOR_MCPS_ENABLED",
        Some(CompatRemoteKey::CursorMcps),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Hooks,
        "WEBUILD_CURSOR_HOOKS_ENABLED",
        Some(CompatRemoteKey::CursorHooks),
    ),
    CompatCell::new(
        CompatVendor::Cursor,
        CompatSurface::Sessions,
        "WEBUILD_CURSOR_SESSIONS_ENABLED",
        Some(CompatRemoteKey::CursorSessions),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Skills,
        "WEBUILD_CLAUDE_SKILLS_ENABLED",
        Some(CompatRemoteKey::ClaudeSkills),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Rules,
        "WEBUILD_CLAUDE_RULES_ENABLED",
        Some(CompatRemoteKey::ClaudeRules),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Agents,
        "WEBUILD_CLAUDE_AGENTS_ENABLED",
        Some(CompatRemoteKey::ClaudeAgents),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Mcps,
        "WEBUILD_CLAUDE_MCPS_ENABLED",
        Some(CompatRemoteKey::ClaudeMcps),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Hooks,
        "WEBUILD_CLAUDE_HOOKS_ENABLED",
        Some(CompatRemoteKey::ClaudeHooks),
    ),
    CompatCell::new(
        CompatVendor::Claude,
        CompatSurface::Sessions,
        "WEBUILD_CLAUDE_SESSIONS_ENABLED",
        Some(CompatRemoteKey::ClaudeSessions),
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Skills,
        "WEBUILD_CODEX_SKILLS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Rules,
        "WEBUILD_CODEX_RULES_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Agents,
        "WEBUILD_CODEX_AGENTS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Mcps,
        "WEBUILD_CODEX_MCPS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Hooks,
        "WEBUILD_CODEX_HOOKS_ENABLED",
        None,
    ),
    CompatCell::new(
        CompatVendor::Codex,
        CompatSurface::Sessions,
        "WEBUILD_CODEX_SESSIONS_ENABLED",
...
impl CompatConfig {
    ...
    /// priority order. `.webuild` and `.agents` are always included; `.claude`
    /// and `.cursor` are gated on their respective `skills` cell.
    ///
    /// Replaces the hard-coded `[".webuild", ".agents", ".claude", ".cursor"]`
    ...
    pub fn skill_config_dirs(&self) -> Vec<&'static str> {
        let mut dirs = vec![".webuild", ".agents"];
    ...
    /// Subdirectories scanned for `*.md` rules files. `.webuild/rules` is always
    ...
    pub fn rules_dirs(&self) -> Vec<&'static str> {
        let mut dirs = vec![".webuild/rules"];
...
mod tests {
    ...
    fn skill_config_dirs_all_on_matches_legacy_constant() {
        // Historical constant was `[".webuild", ".agents", ".claude", ".cursor"]`.
        assert_eq!(
            CompatConfig::default().skill_config_dirs(),
            vec![".webuild", ".agents", ".claude", ".cursor"]
        );
    }
    ...
    fn skill_config_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.skill_config_dirs(), vec![".webuild", ".agents", ".claude"]);

        c.claude.skills = false;
        assert_eq!(c.skill_config_dirs(), vec![".webuild", ".agents"]);

        // Only the `cursor` cell on (`claude` off): `cursor` still appended last.
        let mut c2 = CompatConfig::default();
        c2.claude.skills = false;
        assert_eq!(c2.skill_config_dirs(), vec![".webuild", ".agents", ".cursor"]);
    }

    #[test]
    fn rules_dirs_all_on_matches_legacy_constant() {
        // Historical `RULES_DIRS` was `[".webuild/rules", ".claude/rules", ".cursor/rules"]`.
        assert_eq!(
            CompatConfig::default().rules_dirs(),
            vec![".webuild/rules", ".claude/rules", ".cursor/rules"]
        );
    }
    ...
    fn rules_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.rules_dirs(), vec![".webuild/rules", ".claude/rules"]);
        c.claude.rules = false;
        assert_eq!(c.rules_dirs(), vec![".webuild/rules"]);
    }
