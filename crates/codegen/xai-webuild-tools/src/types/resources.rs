...
/// `"namespace.Name"` (e.g., `"webuild_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"webuild_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("webuild_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "webuild_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"webuild_build.ReadFile"`).
    id: String,
...
/// tool falls back to `Cwd/.webuild/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".webuild/plan.md";
...
/// [`Cwd`]`/.webuild/plan.md`, else the bare relative `.webuild/plan.md`.
...
register_resource!("webuild_build", "WebCitation", WebCitationCounter);
...
/// depending on `xai-webuild-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("webuild_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("webuild_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("webuild_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "webuild_build.ReadFile".to_string(),
            serde_json::json!({ "files_read" : ["loaded.rs"] }),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "webuild_build.Edit".to_string(),
            serde_json::json!(
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "webuild_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "webuild_build.Edit").is_none());
    }
    #[test]
