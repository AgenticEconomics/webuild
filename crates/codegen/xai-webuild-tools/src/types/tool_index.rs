...
//!
//! `ToolSearchIndex` is defined in `xai-webuild-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-webuild-shell`
...
/// in `xai-webuild-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-webuild-shell` after MCP initialization.
#[derive(Clone)]
