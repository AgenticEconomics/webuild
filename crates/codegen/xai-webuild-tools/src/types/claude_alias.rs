//! Canonical external-settings tool name ↔ WeBuild tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`xai-webuild-hooks`) needs the
//! WeBuild tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-webuild-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `webuild` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to WeBuild, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// WeBuild [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// WeBuild tool names this Claude tool maps to (empty when there is no direct
    /// WeBuild tool — the entry then only contributes a `kind`).
    webuild: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, webuild: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        webuild,
...
const fn match_only(claude: &'static str, webuild: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        webuild,
...
/// The WeBuild [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The WeBuild tool names a Claude matcher term fires on.
pub fn webuild_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.webuild)
...
/// The Claude names that map to `webuild_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(webuild_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.webuild.contains(&webuild_name))
        .map(|t| t.claude)
}

/// Every distinct WeBuild name the table references, for the `xai-webuild-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn webuild_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.webuild.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a WeBuild name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.webuild.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
