...
//!
//! `MemoryBackend` is defined in `xai-webuild-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-webuild-tools`.
/// Mirrors `xai_webuild_shell::session::memory_log::TARGET`.
