...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.webuild/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.webuild/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.webuild/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.webuild/ or ~/.claude/.
    User { path: PathBuf },
