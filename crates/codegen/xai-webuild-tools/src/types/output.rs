...
use crate::implementations::webuild_build::todo::{TodoItem, TodoState};
...
/// protocol boundary in `xai-webuild-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the webuild_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the webuild_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when WeBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::webuild_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::webuild_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::webuild_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::webuild_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::webuild_build::update_goal::UpdateGoalOutput),
...
mod tests {
    use super::*;
    use crate::implementations::webuild_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/webuild-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/webuild-worktree/wt-agent</worktree_path>"),
