...
/// (`xai-webuild-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-webuild-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-webuild-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-webuild-shell, calls ACP protocol)
#[async_trait::async_trait]
