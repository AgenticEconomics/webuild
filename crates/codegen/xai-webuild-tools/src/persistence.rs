...
mod tests {
    ...
    async fn writer_atomic_rename_never_exposes_partial_json() {
        ...
        assert!(parsed["state"]["webuild_build.WebCitation"].is_object());
    }
    ...
    async fn resources_flush_writes_pending() {
        ...
        let content = std::fs::read_to_string(&state_path).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&content).unwrap();
        // Should have "state" category with "webuild_build.WebCitation" key
        assert!(parsed["state"]["webuild_build.WebCitation"].is_object());
    }
    ...
    async fn save_and_flush_supersedes_older_pending_snapshot() {
        ...
        let content = std::fs::read_to_string(state_path).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&content).unwrap();
        assert_eq!(parsed["state"]["webuild_build.WebCitation"]["counter"], 2);
    }
    ...
    async fn save_and_flush_error_can_be_retried() {
        ...
        let content = std::fs::read_to_string(state_path).unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&content).unwrap();
        assert_eq!(parsed["state"]["webuild_build.WebCitation"]["counter"], 7);
    }
