...
//! Helper functions remain for use by `webuild_build/task_output/`.
...
mod tests {
    ...
    fn test_snapshot_to_result_prefers_display_command() {
        ...
        snapshot.display_command = Some("cargo test -p xai-webuild-shell".to_string());

        let result = snapshot_to_result(snapshot, "read_file", DEFAULT_TOOL_OUTPUT_BYTES);
        assert_eq!(
            result.command, "cargo test -p xai-webuild-shell",
