...
mod tests {
    ...
    use crate::implementations::webuild_build::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as WeBuildReadFileTool,
    };
    ...
    fn resources_with_webuild_rules_on_read(root: &Path) -> SharedResources {
        let mut resources = Resources::new();
        resources.insert(Cwd(root.to_path_buf()));
        resources.insert(FileSystem(Arc::new(LocalFs)));
        resources.insert(NotificationHandle(ToolNotificationHandle::noop()));
    ...
    #[tokio::test]
    async fn webuild_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let shared = resources_with_webuild_rules_on_read(root);
        ...
        let first = xai_tool_runtime::Tool::run(&WeBuildReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
