...
use regex::Regex;
use xai_webuild_config::shell::AmpersandSemantics;
...
pub struct BashParams {
    ...
    /// grok-build opts up to 10h via xai-webuild-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "webuild_build.Bash";
...
// particular production grok-build sets it to 10h in xai-webuild-shell's
...
/// [`xai_webuild_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `WEBUILD_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("WEBUILD_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in webuild_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the webuild_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`WEBUILD_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `WEBUILD_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = xai_webuild_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = xai_webuild_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing webuild_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (webuild_build backend) and can surface an educational
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // webuild-shell opts up to 10h via max_timeout_secs. Background unaffected.
