...
//! `LspTool` (Tool trait impl) under the `WeBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    xai_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.webuild/lsp.json or <cwd>/.webuild/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
