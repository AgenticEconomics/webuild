...
use crate::implementations::webuild_build::task::TaskTool;
use crate::implementations::webuild_build::task::backend::SubagentBackendResource;
use crate::implementations::webuild_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::webuild_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `WEBUILD_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("WEBUILD_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let webuild_build_bash = Expr::Value(ToolRequirement::Tool {
    ...
    let webuild_build_concise_bash = Expr::Value(ToolRequirement::Tool {
    ...
    vec![webuild_build_bash, webuild_build_concise_bash, opencode_bash]
}
...
mod tests {
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::webuild_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::webuild_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::webuild_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::webuild_build::task::types::SubagentEvent,
    ) -> crate::implementations::webuild_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::webuild_build::task::types::SubagentEvent::Query(q) => q,
