...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // The Task tool description for WeBuild is *never* taken from here.
        // It is always supplied via `ToolConfig::with_description(...)` using
        // the dynamically built string from `build_task_description()` in
        // xai-webuild-agent/src/builder.rs (HEADER + per-subagent blocks + FOOTER).
        //
        ...
        "<see build_task_description() in xai-webuild-agent>"
    }
