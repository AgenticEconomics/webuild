...
//! (in `xai-webuild-tools`) and the subagent coordinator (in `xai-webuild-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `xai-webuild-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "WeBuild:run_terminal_cmd" | "WeBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `xai-webuild-shell` relies on
...
register_resource!("webuild_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("webuild_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// xai-webuild-shell produces it, the channel protocol carries it, and the
...
register_resource!("webuild_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("webuild_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("webuild_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by xai-webuild-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-webuild-shell at turn start.
...
register_resource!("webuild_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("WeBuild:run_terminal_cmd", ToolKind::Execute),
                tc("WeBuild:read_file", ToolKind::Read),
                tc("WeBuild:list_dir", ToolKind::List),
                tc("WeBuild:grep", ToolKind::Search),
                tc("WeBuild:kill_task", ToolKind::KillTaskAction),
                tc("WeBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("WeBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
