...
const XAI_IMAGINE_MODEL: &str = "webuild-imagine-image-quality";
// Some Imagine models (e.g. `webuild-imagine-image`, selectable via `model_override`)
...
pub use xai_webuild_tools_api::slash_commands::{
...
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperWeBuild feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperWeBuild: https://grok.com/supergrok?referrer=grok-build. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// HTTP call and return the SuperWeBuild upsell prose instead. See
...
impl ImageGenClient {
    ...
    /// with the SuperWeBuild upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// SuperWeBuild upsell prose instead of a doomed request. Set by the
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("webuild-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("webuild-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("webuild-imagine-image")), None)
                .unwrap()
                .model,
            "webuild-imagine-image"
        );
    }
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperWeBuild upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperWeBuild"), "got: {}", t.text);
                assert!(t.text.contains("superwebuild?referrer=grok-build"));
            }
