...
const RG_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-rg/rg-",
    env!("WEBUILD_TOOLS_RG_VER"),
    "-",
    env!("WEBUILD_TOOLS_RG_TARGET"),
    ".bin"
));
...
fn resolve_bundled_rg() -> std::io::Result<PathBuf> {
    ...
    let p = crate::util::webuild_home().join("vendor").join(concat!(
        "rg-",
        env!("WEBUILD_TOOLS_RG_VER"),
        "-",
        env!("WEBUILD_TOOLS_RG_TARGET")
...
/// binary to ~/.webuild/vendor/ and returns that path.
