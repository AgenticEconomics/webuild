...
/// params are injected, `WEBUILD_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `WEBUILD_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "WEBUILD_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("webuild_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. WeBuild callers leave it `None`.
...
/// session-owned coordinator (in xai-webuild-shell), which performs an ACP
