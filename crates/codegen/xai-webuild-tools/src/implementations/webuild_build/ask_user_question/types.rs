...
//!
//! - **`xai-webuild-tools`** — tool blocks on a oneshot, formats the result.
//! - **`xai-webuild-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`xai-webuild-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `xai-webuild-tools`.
...
register_resource!("webuild_build", "UserQuestionSender", UserQuestionSender);
