...
mod versions;
use crate::types::schema::WeBuildIntegerSchema;
...
crate::register_resource!("webuild_build", "ReadFile", ReadFileParams);
...
/// an empty window (harness-aligned; not a WeBuild-line clamp).
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".webuild/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".webuild/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    /// phantom field only (start past any `split_inclusive` line), so WeBuild
