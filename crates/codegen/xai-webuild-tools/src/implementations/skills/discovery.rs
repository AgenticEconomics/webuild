...
///
/// `skills` is the standard layout (`.webuild/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into WeBuild sessions.
...
/// name (e.g. `~/.webuild/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into WeBuild.
...
/// `cwd` (exclusive). At each directory, checks for `.webuild/skills/`,
...
/// visits known config roots (`.webuild`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.webuild` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".webuild", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.webuild is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.webuild/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.webuild/ → kept (user content).
        let webuild_shell = tmp.path().join(".webuild").join("skills").join("shell");
        std::fs::create_dir_all(&webuild_shell).unwrap();
        std::fs::write(
            webuild_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (webuild_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.webuild/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .webuild skill in an intermediate dir.
        ...
        .unwrap();
        let webuild_skill = sub.join(".webuild").join("skills").join("webuild-dyn");
        std::fs::create_dir_all(&webuild_skill).unwrap();
        std::fs::write(webuild_skill.join("SKILL.md"), "---\nname: webuild-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"webuild-dyn"),
            "webuild-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only webuild-dyn discovered.
        ...
        assert!(
            names_off.contains(&"webuild-dyn"),
            "webuild-dyn missing: {names_off:?}"
        );
        assert!(
