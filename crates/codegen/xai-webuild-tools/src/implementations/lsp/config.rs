//! LSP server configuration from `.webuild/lsp.json`.
...
/// This is the canonical merge function — both session startup and `webuild inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::webuild_home::webuild_home().join("lsp.json");
    let project_path = cwd.join(".webuild").join("lsp.json");
...
/// Load LSP server configs from `~/.webuild/lsp.json` and `<cwd>/.webuild/lsp.json`.
