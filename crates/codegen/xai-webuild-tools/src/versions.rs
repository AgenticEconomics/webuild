...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"WeBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"WeBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `WeBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("WeBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("WeBuild:read_file", "legacy-0.4.10"),
            ("WeBuild:search_replace", "legacy-0.4.10"),
            ("WeBuild:get_task_output", "legacy-0.4.10"),
            ("WeBuild:kill_task", "legacy-0.4.10"),
            ("WeBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("WeBuild:run_terminal_cmd", "current"),
            ("WeBuild:read_file", "current"),
            ("WeBuild:search_replace", "current"),
            ("WeBuild:list_dir", "current"),
            ("WeBuild:grep", "current"),
            ("WeBuild:kill_task", "current"),
            ("WeBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "WeBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "WeBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "WeBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "WeBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn override_on_unmanaged_tool_errors() {
        let err =
            resolve_version("current", "WeBuild:web_search", Some("legacy-0.4.10")).unwrap_err();
        assert!(err.contains("unmanaged tool"));
    }

    #[test]
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "WeBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "WeBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // WeBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("WeBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "WeBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("WeBuild:run_terminal_cmd"));
        assert!(is_version_managed("WeBuild:read_file"));
        assert!(is_version_managed("WeBuild:search_replace"));
        assert!(is_version_managed("WeBuild:list_dir"));
        assert!(is_version_managed("WeBuild:grep"));
        assert!(is_version_managed("WeBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("WeBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "WeBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "WeBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("WeBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("WeBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "WeBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "WeBuild:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
