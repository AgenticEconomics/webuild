...
///   share it (webuild `read_file` → `"Read"`).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::WeBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::WeBuild => ("webuild_build", "WeBuild"),
                ToolNamespace::WeBuildConcise => ("webuild_build_concise", "WeBuildConcise"),
                ToolNamespace::WeBuildHashline => ("webuild_build_hashline", "WeBuildHashline"),
