...
//! after every tool call to check for SKILL.md files in `.webuild/skills/`,
...
/// Directories that contain skill definitions (`.webuild/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".webuild", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.webuild/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.webuild/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.webuild/) are outside the git root so the walk breaks early.
