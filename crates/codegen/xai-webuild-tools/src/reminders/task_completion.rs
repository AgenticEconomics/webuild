...
use crate::implementations::webuild_build::task::types::{
...
impl AutoWakeDeliveredIds {
    ...
    ///
    /// Used by the between-turn completion drain in `xai-webuild-shell` to
...
crate::register_resource!("webuild_build", "AutoWakeDeliveredIds", AutoWakeDeliveredIds);
...
pub fn format_monitor_events(
    events: &[crate::implementations::webuild_build::task::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        [event] => {
            ...
            let label =
                crate::implementations::webuild_build::monitor::event::sanitize_monitor_description(
                    label,
                );
            Some(format!(
        ...
        _ => {
            type Event = crate::implementations::webuild_build::task::types::MonitorEventNotification;
            ...
            for (task_id, group) in &groups {
                let description = group
                    ...
                    .filter(|d| !d.is_empty())
                    .unwrap_or("event");
                let description = crate::implementations::webuild_build::monitor::event::sanitize_monitor_description(
                    description,
                );
                let _ = write!(
                    buf,
...
/// - `xai-webuild-shell`'s `SessionActor` to sweep matching synthetic
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::webuild_build::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::webuild_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::webuild_build::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::webuild_build::task::types::GoalLoopActive(true));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        tokio::spawn(async move {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::webuild_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::webuild_build::task::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::webuild_build::task::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::webuild_build::monitor::event::wrap_monitor_event;
