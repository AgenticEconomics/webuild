...
//! heavyweight `xai-webuild-tools` dependency. This module re-exports the public
...
/// Note: the CLI also uses `WEBUILD_AGENT` as an
/// optional agent-definition selector for launching `webuild` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const WEBUILD_AGENT_ENV: &str = "WEBUILD_AGENT";

/// Sentinel value for [`WEBUILD_AGENT_ENV`] on agent tool terminals.
pub const WEBUILD_AGENT_ENV_VALUE: &str = "1";

/// Force `WEBUILD_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_webuild_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(WEBUILD_AGENT_ENV, WEBUILD_AGENT_ENV_VALUE);
}

/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${WEBUILD_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${WEBUILD_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
pub fn substitute_plugin_tokens(
    ...
    if let Some(root) = plugin_root {
        out = out
            ...
            .replace("${WEBUILD_PLUGIN_ROOT}", root);
    }
    if let Some(data) = plugin_data {
        out = out
            .replace("${CLAUDE_PLUGIN_DATA}", data)
            .replace("${WEBUILD_PLUGIN_DATA}", data);
    }
    out
}
...
#[cfg(test)]
mod tests {
    use super::{WEBUILD_AGENT_ENV, WEBUILD_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${WEBUILD_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${WEBUILD_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${WEBUILD_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(WEBUILD_AGENT_ENV, "WEBUILD_AGENT");
        assert_eq!(WEBUILD_AGENT_ENV_VALUE, "1");
    }
}
