...
//! (`xai-webuild-agent::prompt::ignore`) and the runtime tracker
//! (`AgentsMdTracker`).
...
pub fn is_ignored(gitignore: &Gitignore, path: &Path, git_root: Option<&Path>) -> bool {
    let check_path = match git_root {
        Some(root) => match path.strip_prefix(root) {
            ...
            // Outside the repo (e.g. ~/.webuild/Agents.md) → not ignored.
            Err(_) => return false,
        },
        None => {
...
mod tests {
    ...
    fn is_ignored_returns_false_for_path_outside_git_root() {
        ...
        // against the repo's .gitignore (e.g., ~/.webuild/Agents.md).
