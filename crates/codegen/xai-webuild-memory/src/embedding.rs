...
pub(super) fn build_middleware_client(
    ...
    xai_webuild_http::with_auth_retry(xai_webuild_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn xai_webuild_auth::AuthCredentialProvider> = std::sync::Arc::new(
        xai_webuild_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl xai_webuild_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = xai_webuild_http::shared_client()
