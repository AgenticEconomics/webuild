//! Agent definition types — parsed from `.webuild/agents/*.md` files.
...
use xai_webuild_tools::implementations::codex;
use xai_webuild_tools::implementations::webuild_build;
use xai_webuild_tools::implementations::webuild_build_concise;
use xai_webuild_tools::implementations::memory;
use xai_webuild_tools::implementations::opencode;
use xai_webuild_tools::implementations::search_tool;
use xai_webuild_tools::implementations::use_tool;
use xai_webuild_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard WeBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&webuild_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&webuild_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&webuild_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&webuild_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&webuild_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_webuild_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_webuild_build_toolset() -> ToolServerConfig {
    let mut tools = default_webuild_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&webuild_build::EnterPlanModeTool).into());
    tools.push((&webuild_build::ExitPlanModeTool).into());
    tools.push((&webuild_build::AskUserQuestionTool).into());
    tools.push((&webuild_build::WebSearchTool).into());
    tools.push((&webuild_build::ImageGenTool).into());
    tools.push((&webuild_build::ImageToVideoTool).into());
    tools.push((&webuild_build::ReferenceToVideoTool).into());
    tools.push((&webuild_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&webuild_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `webuild-computer` (workspace/sandbox) preset.
fn webuild_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&webuild_build::ReadFileTool).into(),
        (&webuild_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&webuild_build::ListDirTool).into(),
        (&webuild_build::GrepTool).into(),
        (&webuild_build::KillTerminalCommandTool).into(),
        (&webuild_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("grok-build", workspace_webuild_build_toolset()),
        ("grok-build-concise", webuild_build_concise_toolset()),
        ("grok-build-plan", webuild_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("webuild-computer", webuild_computer_toolset()),
    ]
}
...
fn default_webuild_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&webuild_build::ReadFileTool).into(),
            ...
            (&webuild_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&webuild_build::SchedulerCreateTool).into(),
            (&webuild_build::SchedulerDeleteTool).into(),
            (&webuild_build::SchedulerListTool).into(),
            (&webuild_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&webuild_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn webuild_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&webuild_build_concise::BashConciseTool).into(),
            (&webuild_build_concise::ReadFileConciseTool).into(),
            (&webuild_build_concise::SearchReplaceConciseTool).into(),
            (&webuild_build::ListDirTool).into(),
            (&webuild_build::GrepTool).into(),
            kill_task_tool_config(),
            (&webuild_build::TodoWriteTool).into(),
...
pub fn webuild_build_hashline_toolset(
    hashline_tools: Vec<xai_webuild_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_webuild_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&webuild_build::WebSearchTool).into(),
        (&webuild_build::SchedulerCreateTool).into(),
        (&webuild_build::SchedulerDeleteTool).into(),
        (&webuild_build::SchedulerListTool).into(),
        (&webuild_build::MonitorTool).into(),
...
fn webuild_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&webuild_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&webuild_build::UpdateGoalTool).into(),
            (&webuild_build::EnterPlanModeTool).into(),
            (&webuild_build::ExitPlanModeTool).into(),
            (&webuild_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&webuild_build::MonitorTool).into(),
            (&webuild_build::WebSearchTool).into(),
            (&webuild_build::WebFetchTool).into(),
            (&webuild_build::ImageGenTool).into(),
            (&webuild_build::ImageToVideoTool).into(),
            (&webuild_build::ReferenceToVideoTool).into(),
...
/// WeBuild + plan mode toolset WITHOUT subagent tools.
///
/// Same as `webuild_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn webuild_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&webuild_build::ReadFileTool).into(),
...
/// Default WeBuild toolset + `ask_user_question`.
///
/// Same as `default_webuild_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn webuild_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&webuild_build::ReadFileTool).into(),
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::WeBuild => AgentDefinition::default_webuild_build(),
            Self::WeBuildConcise => AgentDefinition::webuild_build_concise(),
            Self::WeBuildPlan => AgentDefinition::webuild_build_plan(),
            Self::WeBuildPlanNoSubagents => AgentDefinition::webuild_build_plan_no_subagents(),
            Self::WeBuildAskUser => AgentDefinition::webuild_build_ask_user(),
            ...
            Self::WeBuildOrchestrator => AgentDefinition::webuild_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .webuild/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_webuild_build_toolset")]
...
pub enum AgentScope {
    /// .webuild/agents/ (project-level, highest priority)
    Project,
    /// ~/.webuild/agents/ (user-level)
    User,
    /// ~/.webuild/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_webuild_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.webuild/agent-memory/<name>/`
    User,
    /// `<project>/.webuild/agent-memory/<name>/`
    Project,
    /// `<project>/.webuild/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_webuild_config::webuild_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".webuild/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".webuild/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `xai_webuild_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let webuild = xai_webuild_config::user_webuild_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), webuild.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".webuild/agents/") || path_str.contains(".webuild\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".webuild/bundled/agents/")
            || path_str.contains(".webuild\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_webuild_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["WeBuild:read_file", "WeBuildHashline:hashline_read"],
            [
                "WeBuild:search_replace",
                "WeBuildHashline:hashline_edit",
            ],
            ["WeBuild:grep", "WeBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_webuild_build_toolset(),
    ...
    pub fn default_webuild_build() -> Self {
        Self::base(
            BuiltinAgentName::WeBuild,
            "WeBuild agent for software engineering tasks.",
        )
    }
    /// WeBuild Concise agent definition — concise output format for SFT/RL.
    pub fn webuild_build_concise() -> Self {
        Self {
            tool_config: webuild_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::WeBuildConcise,
                "WeBuild agent with concise output format.",
            )
        }
    }
    /// WeBuild agent with plan mode tools.
    pub fn webuild_build_plan() -> Self {
        Self {
            tool_config: webuild_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::WeBuildPlan,
                "WeBuild agent with plan mode support.",
            )
        }
    }
    /// WeBuild + plan mode WITHOUT subagent tools.
    pub fn webuild_build_plan_no_subagents() -> Self {
        Self {
            tool_config: webuild_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::WeBuildPlanNoSubagents,
                "WeBuild agent with plan mode (no subagents).",
            )
        }
    }
    /// Default WeBuild agent with the `ask_user_question` tool.
    pub fn webuild_build_ask_user() -> Self {
        Self {
            tool_config: webuild_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::WeBuildAskUser,
                "WeBuild agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// WeBuild Orchestrator — GBL model with full WeBuild tools
    ...
    pub fn webuild_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::WeBuildOrchestrator,
                "WeBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_webuild_build_toolset();
        }
...
mod tests {
    ...
    fn webuild_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&webuild_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&webuild_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn webuild_computer_preset_is_curated_webuild_build_subset() {
        let gc = toolset_for_preset("webuild-computer").unwrap();
        let gb = toolset_for_preset("grok-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = webuild_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "webuild-computer tool `{}` must also ship in the grok-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "webuild-computer should be a curated subset of grok-build"
        );
    }
    #[test]
    fn webuild_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("webuild-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&webuild_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&webuild_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&webuild_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&webuild_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&webuild_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&webuild_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&webuild_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The webuild-computer preset must ship a full-file write tool (legacy
    ...
    #[test]
    fn webuild_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("webuild-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "webuild-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn webuild_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("webuild-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&webuild_build::LspTool).id,
            ToolConfig::from(&webuild_build::EnterPlanModeTool).id,
            ToolConfig::from(&webuild_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "webuild-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_webuild_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_webuild_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::WeBuildOrchestrator => true,
            BuiltinAgentName::WeBuild
            | BuiltinAgentName::WeBuildConcise
            | BuiltinAgentName::WeBuildPlan
            | BuiltinAgentName::WeBuildPlanNoSubagents
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.webuild/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.webuild/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_webuild_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have webuild_build tools"
        );
    }
    #[test]
    ...
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("grok-build", BuiltinAgentName::WeBuild),
            ("grok-build-concise", BuiltinAgentName::WeBuildConcise),
            ("grok-build-ask-user", BuiltinAgentName::WeBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_webuild_build();
