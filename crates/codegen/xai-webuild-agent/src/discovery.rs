...
//!
//! Searches `.webuild/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.webuild/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.webuild/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".webuild/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.webuild/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.webuild/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), webuild.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, webuild_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.webuild/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.webuild/agents/` walking from `cwd` up to repo root
/// 2. `~/.webuild/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.webuild/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user webuild agents, `.claude`
/// compat agents, then bundled. `.webuild` dirs resolve from `webuild_home`
/// (WEBUILD_HOME-aware) plus the legacy literal `~/.webuild` when WEBUILD_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    webuild_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.webuild, included only when it differs from webuild_home
    // (i.e. WEBUILD_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_webuild = home
        .map(|h| h.join(".webuild"))
        .filter(|legacy| webuild_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = webuild_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_webuild {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let webuild = xai_webuild_config::user_webuild_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), webuild.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, webuild_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let webuild = xai_webuild_config::user_webuild_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), webuild.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, webuild_home);
...
///
/// Project-level `.webuild/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let webuild = xai_webuild_config::user_webuild_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), webuild.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, webuild_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, webuild_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the WeBuild
...
/// Load project agent definitions from every `.webuild/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectWeBuild,
            PluginScope::User => PluginOrigin::UserWeBuild,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_webuild_when_webuild_home_differs() {
        let home = Path::new("/home/u");
        let webuild = Path::new("/custom/webuildhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(webuild))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&webuild.join("agents")));
        assert!(paths.contains(&home.join(".webuild").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&webuild.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".webuild").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_webuild_home_is_dot_webuild() {
        let home = Path::new("/home/u");
        let webuild = home.join(".webuild");
        let count = user_agent_dirs(Some(home), Some(&webuild))
            .into_iter()
            .filter(|(p, _)| *p == webuild.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.webuild/agents when webuild_home == ~/.webuild"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_webuild_build() {
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".webuild").join("agents");
        let agents_dir_2 = inner_dir.join(".webuild").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".webuild").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".webuild")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".webuild")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".webuild").join("agents");
        let bundled_dir = home.join(".webuild").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".webuild")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".webuild")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".webuild").join("agents");
        let bundled_dir = home.join(".webuild").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .webuild/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        ...
        assert_eq!(variant, BuiltinAgentName::WeBuildOrchestrator);
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.webuild/agents/) agent named "explore" should NOT shadow
