...
fn promote_tmp_to_dest(tmp: &Path, dest: &Path) -> std::io::Result<()> {
    #[cfg(test)]
    {
        if std::env::var_os("XAI_WEBUILD_TEST_FAIL_REFRESH_PROMOTE").is_some() {
...
mod tests {
    ...
    fn refresh_local_install_picks_up_new_agent() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".webuild").join("installed-plugins"));
        ...
        let trust = TrustStore::load_from(home.join(".webuild").join("trusted-plugins"));
    ...
    fn refresh_promote_failure_rolls_back_to_prior_snapshot() {
        ...
        let trust = TrustStore::load_from(home.join(".webuild").join("trusted-plugins"));
        let summary = {
            let _fail = EnvVarGuard::set("XAI_WEBUILD_TEST_FAIL_REFRESH_PROMOTE", "1");
    ...
    fn refresh_preserves_install_subdir_scope() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".webuild/installed-plugins"));
    ...
    fn refresh_keeps_stale_when_legacy_subdir_scope_lost() {
        ...
        let install_dir = home.join(".webuild").join("installed-plugins");
