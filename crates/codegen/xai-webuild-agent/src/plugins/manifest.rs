...
///
/// Replaces `${WEBUILD_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${WEBUILD_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`xai_webuild_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    xai_webuild_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".webuild-plugin")).unwrap();

        // Write manifest in .webuild-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".webuild-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${WEBUILD_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${WEBUILD_PLUGIN_DATA}/cache";
