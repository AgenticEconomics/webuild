...
fn run_git_in_capture(cwd: &Path, args: &[&str]) -> Result<std::process::Output, String> {
    ...
    cmd.args(args)
        ...
        .envs(xai_webuild_tools::util::pager_env());
    let output = cmd
        .output()
...
///
/// Each plugin has a data dir at `~/.webuild/plugin-data/<plugin_id>/`.
...
pub fn cleanup_plugin_data(repo: &InstalledRepo, scope: super::discovery::PluginScope) {
    let plugin_data_base = xai_webuild_config::webuild_home().join("plugin-data");
