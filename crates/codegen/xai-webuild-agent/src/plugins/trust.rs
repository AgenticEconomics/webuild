...
//!
//! Plugins from project directories (`.webuild/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.webuild/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.webuild/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.webuild/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_webuild_home() so a project's `.webuild/trusted-plugins` is never
        // read as the user trust store when neither WEBUILD_HOME nor a home dir resolves.
        let Some(webuild) = xai_webuild_config::user_webuild_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = webuild.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.webuild/trusted-plugins`.
    ...
    /// rewrites `~/.webuild/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.webuild/trusted-plugins`.
