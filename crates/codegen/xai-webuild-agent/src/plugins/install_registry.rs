...
/// Default install directory name under `~/.webuild/`.
...
/// Lives here (not in xai-webuild-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("XAI_WEBUILD_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.webuild/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        xai_webuild_config::webuild_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
...
pub enum InstallError {
    ...
    #[error(
        "refusing unpinned remote plugin code for '{plugin}' from {url}: \
         marketplace.require_sha / WEBUILD_MARKETPLACE_REQUIRE_SHA is enabled and \
