...
//! primitive consumed cross-crate by `xai-webuild-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.webuild/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.webuild`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `xai-webuild-workspace`, which depends on THIS crate) to keep the dep edge
