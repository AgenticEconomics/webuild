...
//! `TemplateRenderer` in `xai-webuild-tools`. This struct does NOT own a
...
/// Default identity on trim-tool-descriptions (`You are WeBuild released by Agentics Economics Labs`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "WeBuild";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "WeBuild Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "WeBuild Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.webuild/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a WeBuild agent"),
