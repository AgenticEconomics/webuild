...
//!
//! Searches from cwd to repo root, plus `~/.webuild/`. Also discovers
//! `*.md` files in `.webuild/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.webuild/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.webuild/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = xai_webuild_tools::util::webuild_home::webuild_home();
    ...
    // Always include webuild_home (~/.webuild/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `xai-webuild-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .webuild/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.webuild/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            xai_webuild_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
