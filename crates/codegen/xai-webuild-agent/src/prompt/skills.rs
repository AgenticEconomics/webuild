...
//! live in `xai_webuild_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use xai_webuild_tools::implementations::skills::types::skill_name_from_path;
pub use xai_webuild_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `xai_webuild_tools` directly.
pub use xai_webuild_tools::types::compat::CompatConfig;

use xai_webuild_tools::implementations::skills::discovery::{
...
///
/// Priority order: Local (cwd/.webuild/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.webuild/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.webuild/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.webuild/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.webuild/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &xai_webuild_tools::util::webuild_home::webuild_home(),
        compat,
    )
    .await;
...
pub fn collect_skill_config_dirs(
    ...
    let webuild_home = global_dir.to_path_buf();
    ...
    // config; `.webuild` and `.agents` are always present. When all cells are on
    // this list equals the historical `[".webuild", ".agents", ".claude", ".cursor"]`.
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.webuild/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
/// only visits known config roots (`.webuild`, `.agents`, `.claude`, `.cursor`),
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            xai_webuild_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(xai_webuild_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        xai_webuild_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || xai_webuild_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match xai_webuild_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".webuild").join("skills").join("dup"), "dup");
    ...
    fn find_skill_paths_flat_layout() {
        ...
        let webuild_dir = tmp.path().join(".webuild");

        write_skill_md(&webuild_dir.join("skills").join("alpha"), "alpha");
        write_skill_md(&webuild_dir.join("skills").join("beta"), "beta");

        let paths = find_skill_paths(&webuild_dir);
    ...
    fn find_skill_paths_nested_layout() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let webuild_dir = tmp.path().join(".webuild");
        let skills = webuild_dir.join("skills");
    ...
    fn find_skill_paths_no_skills_dir() {
        // .webuild exists but no skills/ subdirectory
        let tmp = tempfile::tempdir().unwrap();
        let webuild_dir = tmp.path().join(".webuild");
        fs::create_dir_all(&webuild_dir).unwrap();
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".webuild").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".webuild").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".webuild").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".webuild").join("skills");
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(xai_webuild_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    fn stamp_plugin_fields_sets_root_and_data() {
        ...
        let plugin = LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::UserWeBuild,
            trusted: true,
            enabled: true,
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".webuild").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        let auto_dir = repo_root.join(".webuild").join("skills").join("overlap-skill");
        ...
        assert!(
            matches!(
                ...
                Some(xai_webuild_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".webuild").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".webuild").join("skills").join("same");
    ...
    // these functions now live in xai-webuild-tools::implementations::skills::discovery
    // and xai-webuild-tools::types::skill_discovery_tracker, tested there.
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".webuild").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".webuild").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".webuild").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn min_plugin(name: &str) -> crate::plugins::LoadedPlugin {
        ...
        crate::plugins::LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::ProjectWeBuild,
            trusted: true,
            enabled: true,
            version: Some("1.0.0".to_string()),
            description: None,
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".webuild", ".agents", ".claude", ".cursor"] {
        ...
        assert!(ends_with(&dirs, ".webuild"), "webuild must remain: {dirs:?}");
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        // A `.claude` skill claiming a `.webuild`-owned name (both User scope)
        // was silently hidden before; it now re-keys to its dir basename.
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.webuild/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.webuild/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.webuild/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.webuild/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.webuild/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.webuild/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".webuild"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".webuild").join("skills");
