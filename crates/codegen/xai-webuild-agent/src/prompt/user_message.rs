...
//! - `Default` -- the legacy WeBuild prefix (built by the shell layer).
...
use xai_webuild_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy WeBuild prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .webuild/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_webuild_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.webuild/, ~/.claude/).
    ...
    /// (`~/.webuild/projects/<encoded-cwd>/mcps`). Surfaced in
