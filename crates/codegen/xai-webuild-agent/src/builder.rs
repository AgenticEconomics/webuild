...
use std::sync::Arc;
use xai_webuild_tools::bridge::ToolBridge;
use xai_webuild_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_webuild_tools::notification::ToolNotificationHandle;
use xai_webuild_tools::registry::types::SessionContext;
use xai_webuild_tools::types::tool::ToolKind;
/// The WeBuild [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-WeBuild
/// tool registry in `xai-webuild-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    xai_webuild_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.webuild/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<xai_webuild_tools::implementations::webuild_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn xai_webuild_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: xai_webuild_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend>>,
    image_gen_config: xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig,
    video_gen_config: xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        xai_webuild_tools::implementations::webuild_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: xai_webuild_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<xai_webuild_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<xai_webuild_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in xai-webuild-shell).
    ...
    preloaded_skills: Option<Vec<xai_webuild_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut xai_webuild_tools::registry::types::ToolServerConfig) {
    use xai_webuild_tools::implementations::webuild_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("WeBuild:enter_plan_mode");
    let missing_exit = !existing.contains("WeBuild:exit_plan_mode");
    let missing_ask = !existing.contains("WeBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&webuild_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&webuild_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&webuild_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut xai_webuild_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(xai_webuild_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: xai_webuild_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<xai_webuild_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn xai_webuild_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: xai_webuild_tools::implementations::webuild_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: xai_webuild_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `WEBUILD_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: xai_webuild_tools::implementations::webuild_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: xai_webuild_tools::implementations::webuild_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: xai_webuild_tools::implementations::webuild_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: xai_webuild_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the WeBuild Task description.
    ...
    ///
    /// When disabled, `WeBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.webuild/skills, ~/.webuild/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use xai_webuild_tools::implementations::webuild_build;
                tool_config.tools.push((&webuild_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use xai_webuild_tools::implementations::webuild_build;
                tool_config.tools.push((&webuild_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&xai_webuild_tools::implementations::webuild_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&xai_webuild_tools::implementations::webuild_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&xai_webuild_tools::implementations::webuild_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&xai_webuild_tools::implementations::webuild_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&xai_webuild_tools::implementations::webuild_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&xai_webuild_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let webuild_build_ns = xai_webuild_tools::types::tool::ToolNamespace::WeBuild.to_string();
            let mem_search_id = format!(
                "{webuild_build_ns}:{}",
                xai_webuild_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{webuild_build_ns}:{}",
                xai_webuild_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                xai_webuild_tools::types::tool::ToolNamespace::WeBuild,
            );
        ...
        if task_stripped {
            use xai_webuild_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::WeBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::WeBuildConcise, "run_terminal_cmd", true)
        ...
        if let xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["WeBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "WeBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full webuild toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                xai_webuild_tools::types::resources::TruncationCfg(
                    xai_webuild_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(xai_webuild_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(xai_webuild_sampling_types::HostedTool::XSearch);
            }
        }
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        xai_webuild_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: xai_webuild_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: xai_webuild_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("webuild", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use xai_webuild_tools::types::template_renderer::TemplateRenderer;
        use xai_webuild_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                ...
                profile: AgentDefinition::default_webuild_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::webuild_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::webuild_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::webuild_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_webuild_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::implementations::webuild_build::ask_user_question::AskUserQuestionParams;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        use xai_webuild_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_webuild_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "WeBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_webuild_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("WeBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::implementations::webuild_build::bash::BashParams;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        use xai_webuild_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_webuild_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their WeBuild
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_webuild_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their webuild
    ...
    /// treat `mcp__*` as always-on, so webuild keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use xai_webuild_tools::computer::local::LocalTerminalBackend;
        use xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig;
        use xai_webuild_tools::implementations::web_search::WebSearchConfig;
        use xai_webuild_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_webuild_build();
    ...
    /// Compat `ToolSearch` meta-tool maps to webuild's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, xai_webuild_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, xai_webuild_sampling_types::HostedTool::XSearch)),
