...
This crate extracts a first-class `Agent` type from `xai-webuild-shell`.
...
`xai-webuild-shell`, another in-process host, or a headless batch runner.
...
in `.webuild/agents/` (project-level) or `~/.webuild/agents/` (user-level).

```rust
use xai_webuild_agent::{AgentDefinition, AgentBuilder};
use xai_webuild_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".webuild/agents/code-reviewer.md")?;
...
```rust
use xai_webuild_agent::discovery;

// Find all .md files in .webuild/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xai-webuild-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xai-webuild-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xai-webuild-agent` depends on it for tool setup.
- **`xai-webuild-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xai-webuild-agent` (AGENTS.md
...
```bash
# Check
cargo check -p xai-webuild-agent

# Test
cargo test -p xai-webuild-agent

# Clippy
cargo clippy -p xai-webuild-agent --fix --allow-dirty
