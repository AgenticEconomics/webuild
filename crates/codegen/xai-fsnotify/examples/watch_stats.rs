...
//! startup latency, under either strategy (`WEBUILD_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! WEBUILD_FSNOTIFY_PER_DIR=0 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! WEBUILD_FSNOTIFY_PER_DIR=1 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "webuild holds 55k inotify watches" report).
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("WEBUILD_FSNOTIFY_PER_DIR").unwrap_or_default();
