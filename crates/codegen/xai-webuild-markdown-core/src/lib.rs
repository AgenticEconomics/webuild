//! Headless markdown analysis sharing WeBuild's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-webuild-markdown` so analysis matches what WeBuild
//! Build actually renders 1:1.
//!
//! After parsing, WeBuild applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set WeBuild uses to render markdown.
///
...
/// Offset event stream from WeBuild's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with WeBuild's options; count elements and flag structural issues.
