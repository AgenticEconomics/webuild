...
use xai_webuild_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    xai_webuild_telemetry::log_event(xai_webuild_telemetry::events::ToolCallCompleted {
    ...
    assert_eq!(harness.scope_name, "ai.xai.webuild_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("webuild-cli"),
        "service.name=webuild-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
