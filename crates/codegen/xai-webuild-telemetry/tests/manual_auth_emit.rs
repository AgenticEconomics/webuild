...
//! `webuild-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use xai_webuild_telemetry::client;
use xai_webuild_telemetry::config::{TelemetryConfig, TelemetryMode};
use xai_webuild_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_webuild_shell_manual_auth() {
    ...
    xai_webuild_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "webuild-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no webuild-shell-manual_auth POST received"
        );
