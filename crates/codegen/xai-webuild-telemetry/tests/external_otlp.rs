...
fn external_stream_end_to_end() {
    ...
    let mut cfg = xai_webuild_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "WEBUILD_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = xai_webuild_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    xai_webuild_telemetry::external::init(Some(cfg));
    assert!(xai_webuild_telemetry::external::is_active());
    ...
    assert!(!xai_webuild_telemetry::is_enabled());
    xai_webuild_telemetry::log_event(xai_webuild_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: xai_webuild_telemetry::enums::PermissionMode::Ask,
    });
    xai_webuild_telemetry::log_event(xai_webuild_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
        client_identifier: Some("webuild-pager".into()),
    ...
    xai_webuild_telemetry::log_event(xai_webuild_telemetry::events::PromptSubmitted {
    ...
    xai_webuild_telemetry::log_event(xai_webuild_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("webuild-cli")),
        "service.name=webuild-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
    ...
    xai_webuild_telemetry::external::shutdown();
    assert!(
