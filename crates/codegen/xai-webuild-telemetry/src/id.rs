...
//!
//! Extracted from `xai-webuild-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$WEBUILD_HOME` is resolved through `xai-webuild-config::webuild_home`.
...
/// This function caches the result in `$WEBUILD_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xai_webuild_config::webuild_home().join("agent_id");
...
/// Write `$WEBUILD_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    xai_webuild_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
///
/// Enabled only when `WEBUILD_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("WEBUILD_TELEMETRY_SPECIAL_USER").as_deref(),
