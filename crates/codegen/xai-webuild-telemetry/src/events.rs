...
//!
//! These structs were extracted from `xai-webuild-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `webuild wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
// ---------------------------------------------------------------------------
// Pager events (called from xai-webuild-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when WeBuild is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperWeBuildUpsellShown {
    pub source: SuperWeBuildUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperWeBuildUpsellClicked {
    pub source: SuperWeBuildUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperWeBuildUpsell`]; lets the funnel attribute the click to the
...
pub struct ClipboardCopy {
    ...
    /// An explicit `webuild wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to WeBuild's upsell surface.
...
/// `webuild-shell-manual_auth` (the `manual_auth` binding gets the `webuild-shell-`
...
telemetry_event!(SuperWeBuildUpsellShown, "supergrok_upsell_shown");
telemetry_event!(SuperWeBuildUpsellClicked, "supergrok_upsell_clicked");
