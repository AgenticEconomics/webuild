...
//!
//! Extracted from `xai-webuild-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-webuild-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-webuild-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "webuild-shell-",
            EmitterOrigin::Workspace => "webuild-workspace-",
        }
    }
}
...
/// event-name prefix (`webuild-shell-*` vs `webuild-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "webuild-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "webuild-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("webuild-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("webuild-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "webuild-workspace-turn");
    }
