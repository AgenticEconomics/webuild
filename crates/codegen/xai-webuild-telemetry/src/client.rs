...
//!
//! Extracted from `xai-webuild-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`webuild-shell-` /
/// `webuild-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperWeBuild Heavy")
/// and JWT-derived keys ("superwebuild_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperWeBuild Heavy" | "superwebuild_heavy" => "superwebuild_heavy",
        "SuperWeBuild" | "superwebuild" => "superwebuild",
        "SuperWeBuild Lite" | "superwebuild_lite" => "superwebuild_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("WeBuild Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("webuild-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("webuild-shell-turn"), "turn");
        assert_eq!(
            event_value("webuild-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("webuild-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("webuild-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperWeBuild"), "superwebuild");
        assert_eq!(normalize_tier("SuperWeBuild Heavy"), "superwebuild_heavy");
        assert_eq!(normalize_tier("superwebuild_heavy"), "superwebuild_heavy");
        ...
        assert_eq!(normalize_tier("SuperWeBuild Lite"), "superwebuild_lite");
    ...
    /// (e.g. a future `"webuild-shell-ext-"` origin), an earlier `ALL` entry could
