//! Tracing layer for `target: "sampling_log"` → `~/.webuild/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `WEBUILD_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "WEBUILD_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = webuild_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
