...
//!
//! `xai-webuild-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "WEBUILD_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `WeBuildComConfig`).
///
/// Built by the binaries (`xai-webuild-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `xai_webuild_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"xai-webuild-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`webuild-pager`) -> `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"webuild-tui"`, `"webuild-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-webuild-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("webuild-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid WEBUILD_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &xai_webuild_auth::CredentialSnapshot,
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-webuild-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use xai_webuild_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("xai-webuild-cli"),
