...
//! file logger that writes to `~/.webuild/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `xai_webuild_hooks` or `xai_webuild_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! WEBUILD_HOOKS_LOG=1 webuild              # enable, write to ~/.webuild/logs/hooks.log
//! WEBUILD_HOOKS_LOG=/tmp/h.log webuild     # write to custom path
//! WEBUILD_HOOKS_LOG=0 webuild              # explicitly disable
//! tail -f ~/.webuild/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "WEBUILD_HOOKS_LOG";
...
///
/// Writes to `~/.webuild/logs/hooks.log` (or custom path via `WEBUILD_HOOKS_LOG`).
/// Filters to hooks (`xai_webuild_hooks`) and plugins (`xai_webuild_agent::plugins`) targets.
/// Set `WEBUILD_HOOKS_LOG=0` to disable, `WEBUILD_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_webuild_hooks=debug,xai_webuild_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || webuild_home().join("logs").join("hooks.log");
