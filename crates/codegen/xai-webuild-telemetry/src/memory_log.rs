...
//! file logger that writes to `~/.webuild/logs/memory.log`.
//!
//! ## When to use
//!
...
//! WEBUILD_MEMORY_LOG=0 webuild                # disable even when enabled
//! tail -f ~/.webuild/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "WEBUILD_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.webuild/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `WEBUILD_MEMORY_LOG=0` to disable, `WEBUILD_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || webuild_home().join("logs").join("memory.log");
