...
//!
//! Enterprise customers point the WeBuild CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `WEBUILD_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `WEBUILD_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &xai_webuild_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             WEBUILD_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
