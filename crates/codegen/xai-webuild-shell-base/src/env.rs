//! WeBuildEnvironment configuration for the shell crate family.
//!
//! The environment presets (per-environment endpoint URLs, the staging
//! trust check, `EnvVarGuard`) live in the [`xai_webuild_env`] leaf crate so
...
//! - `WEBUILD_GATEWAY_URL` — when set to a valid URL, `MvpAgent` spawns a
//!   per-session gateway bridge actor and routes prompts through
//!   it. Unset → falls back to [`WeBuildEnvironment::gateway_ws_url`] for
//!   sessions created in gateway mode; otherwise local-mode (unchanged).
#[cfg(any(test, feature = "test-support"))]
pub use xai_webuild_env::EnvVarGuard;
pub use xai_webuild_env::{
    WeBuildEnvironment, PROD_ASSET_SERVER_URL, PROD_CLI_CHAT_PROXY_BASE_URL, PROD_GATEWAY_WS_URL,
...
pub const WEBUILD_GATEWAY_URL_ENV: &str = "WEBUILD_GATEWAY_URL";
...
pub const WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV: &str = "WEBUILD_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
/// Parse `WEBUILD_GATEWAY_URL` into a [`url::Url`].
///
/// This build ignores the variable and stays in local mode (warns if set).
pub fn parse_gateway_url() -> Option<url::Url> {
    let raw = std::env::var(WEBUILD_GATEWAY_URL_ENV).ok()?;
    if raw.is_empty() {
        return None;
    }
    tracing::warn!(
        env = WEBUILD_GATEWAY_URL_ENV,
        "WEBUILD_GATEWAY_URL is set but this build does not support gateway-bridge mode; staying in local mode"
    );
    None
}
#[cfg(test)]
mod tests {
    ...
    fn parse_gateway_url_returns_none_when_unset() {
        let _env = EnvVarGuard::remove(WEBUILD_GATEWAY_URL_ENV);
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_when_empty() {
        let _env = EnvVarGuard::set(WEBUILD_GATEWAY_URL_ENV, "");
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_hard_off_without_gateway_bridge() {
        let _env = EnvVarGuard::set(WEBUILD_GATEWAY_URL_ENV, "wss://gateway.example.com/ws");
        assert!(
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(WEBUILD_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(
