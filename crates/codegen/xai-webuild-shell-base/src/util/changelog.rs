...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$WEBUILD_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `webuild_home()` OnceLock). A seeded `$WEBUILD_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("WEBUILD_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::webuild_home::webuild_home);
        Self {
    ...
    ///
    /// When `WEBUILD_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$WEBUILD_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`WEBUILD_HOME` /
    /// `WEBUILD_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$WEBUILD_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("WEBUILD_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$WEBUILD_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("webuild-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
