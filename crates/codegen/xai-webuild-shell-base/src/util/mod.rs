pub mod changelog;
pub mod event_id;
pub mod webuild_home;
pub mod secure_file;
pub mod tips;
pub mod uname;
pub use xai_webuild_shared::clipboard;
pub use xai_webuild_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True if `pid` is a webuild process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_webuild_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("webuild"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    #[test]
    fn is_webuild_process_self_true_impossible_pid_false() {
        assert!(is_webuild_process(std::process::id()));
        assert!(!is_webuild_process(u32::MAX));
    }
}
