...
//! `xai_webuild_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `WEBUILD_POOL_*` /
//! `WEBUILD_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `WEBUILD_SAMPLER_SHARED_CLIENT=0` falls back to
...
use xai_webuild_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `xai_webuild_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `xai_webuild_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use xai_webuild_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `xai-webuild-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `xai-webuild-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `xai-webuild-telemetry` consumes
// the same type via `xai_webuild_sampler::OriginClientInfo`. The shell-specific
...
/// `WEBUILD_CLIENT_VERSION` env vars. Returns `None` when
/// `WEBUILD_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`WEBUILD_CLIENT_NAME` env var, default `"webuild-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("WEBUILD_CLIENT_NAME").unwrap_or_else(|_| "webuild-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`webuild -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-webuild-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `webuild -p`, `run_headless_inner` for
/// `webuild agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn xai_webuild_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(xai_webuild_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "webuild-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "webuild-desktop".to_string(),
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "webuild-pager".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("webuild-desktop/1.2.3 webuild-shell/"));
        ...
        assert!(without_version.starts_with("webuild-web webuild-shell/"));
        assert!(!without_version.starts_with("webuild-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "webuild-shell".to_string(),
        ...
        assert_eq!(ua.render(), "webuild-shell/0.1.171 (macos; aarch64)");
    }
}
