...
//! this crate and `xai-webuild-shell` can share them without duplication.
use std::collections::BTreeSet;
use xai_webuild_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
///
/// Stored here (rather than only in `xai-webuild-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in xai-webuild-tools. The mapping between them
/// happens in `run_compact_inner()` (xai-webuild-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (xai-webuild-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in xai-webuild-tools.
...
/// handled by the consumer (e.g. `xai-webuild-shell`), which has access to
...
/// `xai-chat-state` and `xai-webuild-shell` share the same format.
...
/// `run_compact_inner` in `xai-webuild-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = xai_webuild_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = xai_webuild_sampling_types::repair_dangling_tool_calls(
        items,
        xai_webuild_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use xai_webuild_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    fn call(id: &str) -> xai_webuild_sampling_types::ToolCall {
        xai_webuild_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.webuild/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.webuild/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by xai-webuild-shell:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use xai_webuild_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use xai_webuild_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use xai_webuild_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use xai_webuild_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
        ...
        assert!(
            ...
            "reasoning must be kept when strip_reasoning = false (WeBuild backends)"
        );
    ...
    fn fit_counts_user_images_against_budget() {
        use xai_webuild_sampling_types::ContentPart;
    ...
    fn fit_counts_encrypted_reasoning_against_budget() {
        use xai_webuild_sampling_types::rs;
