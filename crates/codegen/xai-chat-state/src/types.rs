...
/// emitter in `xai-webuild-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (webuild login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `xai_webuild_sampling_types::SamplingConfig` (which is secret-free).
