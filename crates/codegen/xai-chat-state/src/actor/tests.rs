...
use tokio::sync::mpsc;
use xai_webuild_sampling_types::{ConversationItem, SamplingConfig};
...
async fn record_last_turn_usage_round_trip() {
    use xai_webuild_sampling_types::TokenUsage;
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_webuild_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_webuild_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use xai_webuild_sampling_types::ToolSpec;
...
/// Simulates the exact sequence that `xai-webuild-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(xai_webuild_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use xai_webuild_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use xai_webuild_sampling_types::ApiBackend;
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            xai_webuild_sampling_types::AssistantItem {
...
fn serialize_via_public_api(
    req: &xai_webuild_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    xai_webuild_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &xai_webuild_sampling_types::ConversationRequest,
    extended: &xai_webuild_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(xai_webuild_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(xai_webuild_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use xai_webuild_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
