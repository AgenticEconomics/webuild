...
use xai_webuild_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use xai_webuild_pager::client_identity::PAGER_CLIENT_VERSION;
use xai_webuild_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use xai_webuild_shell::agent::config::Config as AgentConfig;
use xai_webuild_shell::leader::{
...
use xai_webuild_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.webuild_ws_origin {
        config.webuild_com_config.webuild_ws_origin = v.clone();
    }
    if let Some(v) = &args.webuild_ws_url {
        config.webuild_com_config.webuild_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &xai_webuild_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   WeBuild agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `webuild -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use xai_webuild_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(xai_webuild_telemetry::sampling_log::layer())
        .with(xai_webuild_telemetry::instrumentation::layer())
        .with(xai_webuild_telemetry::hooks_log::layer())
        .with(xai_webuild_telemetry::otel_layer::build_otel_layer(
            xai_webuild_telemetry::otel_layer::OtelClientInfo {
                client_name: "webuild-pager",
                client_version: xai_webuild_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            xai_webuild_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    xai_webuild_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    xai_webuild_telemetry::external::init(
        xai_webuild_shell::agent::config::resolve_external_otel_config(
            xai_webuild_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: xai_webuild_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `webuild setup`: rendering + exit codes only; fetch logic lives in `xai_webuild_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use xai_webuild_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `webuild login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export WEBUILD_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:WEBUILD_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  webuild setup");
        eprintln!();
        eprintln!("Or add the key to ~/.webuild/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's WeBuild administrator."
        );
        std::process::exit(1);
    }
    if json {
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. Ask your WeBuild administrator to publish one."
                    );
                }
            }
            Err(e) => {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `webuild setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = xai_webuild_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !xai_webuild_shell::util::is_webuild_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a webuild process, removing stale lock");
        ...
        if let Err(e) = xai_webuild_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(xai_webuild_shell::env::WeBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, xai_webuild_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = xai_webuild_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = xai_webuild_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "webuild-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&xai_webuild_shell::leader::ControlPayload>,
...
/// Env override for the `webuild workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "WEBUILD_WORKSPACE_COMMAND";
/// Resolution of the `webuild workspace` gate. `Unknown` is kept separate from
...
/// The `WEBUILD_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&xai_webuild_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for webuild on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<xai_webuild_shell::util::config::RemoteSettings> {
    join_early_prefetch(xai_webuild_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`webuild workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `webuild workspace`. Check your \
             network connection (run `webuild login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.webuild_com_config.webuild_ws_url;
    let socket = socket_path_for_ws_url(ws_url);
    LeaderClient::connect(
        socket,
        "webuild-workspace-cli",
        ClientMode::Stdio,
        ClientCapabilities::default(),
    )
    .await
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a webuild session, or run `webuild workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = xai_webuild_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use xai_webuild_shell::auth::ensure_authenticated;
    xai_webuild_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = xai_webuild_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`webuild workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.webuild/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.webuild_com_config,
        false,
        Some("No cached credentials found. Run `webuild login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.webuild_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    xai_webuild_telemetry::sentry::flush_on_shutdown();
    xai_webuild_telemetry::otel_layer::shutdown_otel();
    xai_webuild_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "webuild: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<xai_webuild_pager::app::AgentArgs>,
    ...
    let _otel_guard = xai_webuild_telemetry::otel_layer::otel_guard();
    xai_webuild_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => xai_webuild_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = xai_webuild_shell::agent::models::start_early_prefetch(None);
    xai_webuild_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "WeBuild (pager) - v{}",
            xai_webuild_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                xai_webuild_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(xai_webuild_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = xai_webuild_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("webuild: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = xai_webuild_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&xai_webuild_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &xai_webuild_shell::util::webuild_home::webuild_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = xai_webuild_shell::leader::LeaderEnvUrls::from(&agent_config.webuild_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = xai_webuild_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            xai_webuild_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(xai_webuild_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = xai_webuild_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = xai_webuild_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `WEBUILD_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `webuild dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `webuild dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `WEBUILD_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !xai_webuild_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.webuild/config.toml and \
             unsetting WEBUILD_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("WEBUILD_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<xai_webuild_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xai_webuild_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<xai_webuild_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(xai_webuild_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    xai_webuild_shell::heap_profile::install(xai_webuild_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    xai_webuild_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    xai_webuild_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xai_webuild_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xai_webuild_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = xai_webuild_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    xai_webuild_pager::memory_trace::start(
        xai_webuild_shell::util::webuild_home::webuild_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = xai_webuild_config::validate_requirements() {
        eprintln!("Couldn't start WeBuild: {e}");
        eprintln!();
        eprintln!(
            "Update WeBuild to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = xai_webuild_telemetry::sentry::init(xai_webuild_telemetry::sentry::Config {
        client: "webuild-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: xai_webuild_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    xai_webuild_pager::docs::extract_user_guide_docs(&xai_webuild_shell::util::webuild_home::webuild_home());
    xai_crash_handler::install_terminal_restore_only();
    if xai_webuild_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = xai_webuild_shell::util::webuild_home::webuild_home().join("crash");
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("WeBuild crashed during your last session.");
    ...
    let crashed = xai_webuild_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("WEBUILD_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("WEBUILD_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(xai_webuild_shell::agent::chat_modes::WEBUILD_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(xai_webuild_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("WEBUILD_DEBUG_LOG", path);
            std::env::remove_var("WEBUILD_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("WEBUILD_DEBUG_LOG", "1");
        set_if_unset("WEBUILD_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        xai_webuild_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return xai_webuild_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        xai_webuild_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        xai_webuild_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    xai_webuild_shell::config::apply_sandbox(
        None,
    ...
    xai_webuild_shell::http::set_client_name(if is_interactive {
        xai_webuild_workspace::permission::ClientType::WeBuildPager
    } else {
        xai_webuild_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        xai_webuild_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `webuild-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                xai_webuild_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xai_webuild_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = xai_webuild_telemetry::otel_layer::otel_guard();
                return xai_webuild_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = xai_webuild_telemetry::otel_layer::otel_guard();
                let config = xai_webuild_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_webuild_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_webuild_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_webuild_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_webuild_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xai_webuild_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_webuild_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return xai_webuild_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_webuild_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                xai_webuild_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = xai_webuild_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_webuild_shell::auth::run_cli_logout(&config)?;
                xai_webuild_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = xai_webuild_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(xai_webuild_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == xai_webuild_pager::headless::OutputFormat::Plain {
                args.output_format = xai_webuild_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return xai_webuild_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            xai_webuild_pager::headless::HeadlessOptions {
    ...
    let result = xai_webuild_pager::app::run(args, bg_update_rx).await;
    xai_webuild_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `webuild` to start.");
            } else {
                eprintln!("Update did not complete. Run `webuild update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `webuild update` child
...
/// fall back to a fresh blocking `webuild update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = xai_webuild_shell::env::WeBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                xai_webuild_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("WEBUILD_NPM_REGISTRY"))
        .ok()
        .or_else(xai_webuild_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = xai_webuild_shell::config::load_effective_config_disk_only()
        && let Some(ch) = xai_webuild_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("WEBUILD_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<webuild_home>/bin/webuild` resolves to, the
...
/// npm shim hardcodes `~/.webuild`, so a custom `WEBUILD_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, webuild_home: &std::path::Path) -> bool {
    if webuild_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = xai_webuild_config::webuild_application_in(webuild_home);
...
/// Handle `webuild-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `webuild update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in xai_webuild_shell::leader::discover_leaders().await {
        if d.classification != xai_webuild_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match xai_webuild_shell::leader::LeaderClient::connect(
            socket_path,
            "webuild-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        )
        .await
        ...
        match client
            ...
            .await
        {
            Ok(Ok(xai_webuild_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(xai_webuild_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            xai_webuild_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: xai_webuild_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/webuild-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            xai_webuild_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!xai_webuild_shell::heap_profile::prof_available());
            return;
        }
        assert!(xai_webuild_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(xai_webuild_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(xai_webuild_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        xai_webuild_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_webuild_target() {
        let home =
            std::env::temp_dir().join(format!("webuild-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("webuild")),
            &home
        ));
        ...
        let target = home.join("downloads").join("webuild-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("webuild")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("webuild")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("webuild-9.9.9");
    ...
    /// `webuild dashboard` flags the startup hook without forcing leader mode —
    /// the dashboard is independent of leader mode, so the launch keeps
    /// whatever leader setting the user (or config) chose.
    #[serial_test::serial(WEBUILD_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["webuild", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("WEBUILD_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("WEBUILD_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `webuild dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(WEBUILD_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["webuild", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(WEBUILD_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("WEBUILD_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("WEBUILD_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("WEBUILD_WORKSPACE_COMMAND", "off") };
