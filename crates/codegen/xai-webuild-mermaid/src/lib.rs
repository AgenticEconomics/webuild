...
//!
//! ```
//! use xai_webuild_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `WeBuildDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `WeBuildNight`, `TokyoNight`).
    Dark,
}
