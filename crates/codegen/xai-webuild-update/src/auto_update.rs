...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_webuild_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use xai_webuild_shell::util::config;
use xai_webuild_shell::util::webuild_home::{webuild_application, webuild_home};
...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `webuild update` to get the latest version.";
...
fn reinstall_hint(installer: &str) -> String {
    match installer {
        ...
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo xai-org-shared/grok-build --pattern 'webuild-*' --output webuild && chmod +x webuild".to_string(),
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of WeBuild is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of WeBuild is available.");
        }
        return Ok(());
    }
    ...
    println!("WeBuild - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_webuild_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_webuild_version();
...
/// background download, explicit `webuild update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_webuild_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_webuild_version();
...
/// managed `~/.webuild/bin/webuild` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("WEBUILD_INSTALLER") {
    ...
    if std::env::var_os("WEBUILD_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("WEBUILD_MANAGED_BY_INTERNAL").is_some() {
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `webuild update` child, `Some` only when a
...
/// (spawns `webuild update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `webuild update` child resolves
...
/// Launch "webuild update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("webuild update failed with {}", status);
            }
            Ok(None)
        }
        UpdateRunMode::NonBlocking => {
            ...
            xai_webuild_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Resolve the webuild binary path for re-execution after an update.
///
/// `current_exe()` resolves symlinks via `/proc/self/exe` (see proc(5)),
/// so it returns the old versioned target after a symlink swap.
/// Prefer `~/.webuild/bin/webuild` which always points to the latest version.
fn resolve_restart_exe() -> Result<std::path::PathBuf> {
    let canonical = webuild_application();
    if canonical.exists() {
        return Ok(canonical);
    }
...
/// Restart webuild with the original command-line arguments to pick up the update.
pub fn restart_webuild() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "WEBUILD_AUTO_UPDATE"));
    eprintln!("Restarting WeBuild...");
...
/// versioned name as the extension (`webuild-0.1.181-linux-x86_64` →
/// `webuild-0.1.tmp`) and therefore collides for every `0.1.x` version. The PID
...
/// versioned base like `webuild-0.1.181` doesn't collide via `with_extension`.
...
/// Delete `~/.webuild/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = webuild_home().join("models_cache.json");
...
/// Remove the stale `webuild-pager` symlink/binary from `~/.webuild/bin/` left by
...
/// config and writes to `~/.webuild/bin/`, so callers must isolate
/// `WEBUILD_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.webuild/downloads/`, not yet
/// activated as the managed `webuild`/`agent`.
...
async fn download_verified_from_base(
    ...
    let webuild_home = webuild_home();
    let download_dir = webuild_home.join("downloads");
    tokio::fs::create_dir_all(&download_dir).await?;

    let binary_name = format!("webuild-{}-{}", version, platform);
    let binary_path = download_dir.join(&binary_name);

    eprintln!("  Downloading webuild v{} ({})...", version, platform);
    ...
    // corrupt download is caught here and never becomes the active webuild.
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let webuild_home = webuild_home();
    let download_dir = webuild_home.join("downloads");
    let bin_dir = webuild_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.webuild/bin/{webuild,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "webuild", &download.version).await;
    cleanup_old_downloads(&download_dir, "webuild-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &webuild_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, webuild_home: &std::path::Path) {
    // Derive $HOME independently — webuild_home may be overridden via WEBUILD_HOME
    // env var, so webuild_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", webuild_home.join("completions/bash/webuild.bash")),
        ("zsh", webuild_home.join("completions/zsh/_webuild")),
        ("fish", user_home.join(".config/fish/completions/webuild.fish")),
    ];
...
///
/// When both paths share a grandparent (e.g. `~/.webuild/bin/webuild` and
/// `~/.webuild/downloads/webuild-0.1.203-linux-x86_64`), returns a relative path
/// like `../downloads/webuild-0.1.203-linux-x86_64`.  When they share the same
/// parent directory, returns just the filename.  Falls back to the absolute
/// `target` path for any other layout.
///
/// Relative symlinks survive Docker bind-mounts where `~/.webuild/` is mapped
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Same directory — just the filename (e.g. webuild-latest -> webuild-0.1.203-…)
    if target_parent == link_parent
        && let Some(name) = target.file_name()
    {
        return std::path::PathBuf::from(name);
    }
    // Sibling directories — ../target_dir/filename (e.g. bin/webuild -> ../downloads/webuild-…)
...
/// Swap `~/.webuild/bin/{webuild,agent}` to point at `binary_path`. Returns the
/// `webuild` link path (for [`regenerate_completions`]).
///
/// `webuild` and `agent` are first-class entry points that the bootstrap
/// installers (`install.sh`, `install.ps1`, `install-enterprise.sh`)
/// maintain in lockstep, and so must the updater — otherwise `webuild update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.webuild/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let webuild_name = if cfg!(windows) { "webuild.exe" } else { "webuild" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let webuild_link = bin_dir.join(webuild_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [webuild_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running webuild sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"webuild"` or `"webuild-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `webuild-*` matching `webuild-pager-*` or `webuild-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // Skip symlinks (e.g. webuild-latest).
        ...
        // binary (avoids `webuild-latest`, `webuild-pager-*` when prefix is `webuild`).
        let suffix = &name[prefix.len()..];
        if !suffix.starts_with(|c: char| c.is_ascii_digit()) {
            continue;
        }
        // Extract the version portion via the shared parser (handles the
        // internal `webuild-0.1.150-macos-aarch64`, pre-release, and npm
        // `webuild-0.1.150` layouts — see `version_from_versioned_binary_name`).
...
async fn heal_managed_install(installer: &str) {
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        #[cfg(unix)]
        reconcile_agent_to_webuild(&bin_dir).await;

        #[cfg(windows)]
        reconcile_agent_exe_to_webuild(&bin_dir).await;
    }
}

#[cfg(unix)]
async fn reconcile_agent_to_webuild(bin_dir: &std::path::Path) {
    let webuild_link = bin_dir.join("webuild");
    let agent_link = bin_dir.join("agent");

    let Ok(webuild_target) = tokio::fs::read_link(&webuild_link).await else {
        return;
    };
    if tokio::fs::metadata(&webuild_link).await.is_err() {
        return;
    }
    if let Ok(agent_target) = tokio::fs::read_link(&agent_link).await
        && agent_target == webuild_target
    {
        return;
    }
    match atomic_symlink_swap(&webuild_target, &agent_link).await {
        Ok(()) => tracing::info!(
            webuild_target = %webuild_target.display(),
            "reconciled agent bin symlink to webuild target"
        ),
...
#[cfg(windows)]
async fn reconcile_agent_exe_to_webuild(bin_dir: &std::path::Path) {
    let webuild_exe = bin_dir.join("webuild.exe");
    let agent_exe = bin_dir.join("agent.exe");

    if tokio::fs::metadata(&webuild_exe).await.is_err() {
        return;
    }
    match agent_exe_differs(&webuild_exe, &agent_exe).await {
        Ok(true) => {}
        Ok(false) => return,
        Err(e) => {
    ...
    match windows_replace_exe(&webuild_exe, &agent_exe).await {
        Ok(()) => tracing::info!("reconciled agent.exe to webuild.exe"),
        Err(e) => tracing::warn!("failed to reconcile agent.exe to webuild.exe: {e:#}"),
    }
}

#[cfg(windows)]
async fn agent_exe_differs(
    webuild: &std::path::Path,
    agent: &std::path::Path,
) -> std::io::Result<bool> {
    use tokio::io::{AsyncReadExt, BufReader};
    let webuild_len = tokio::fs::metadata(webuild).await?.len();
    match tokio::fs::metadata(agent).await {
        Ok(m) if m.len() != webuild_len => return Ok(true),
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(true),
        Err(e) => return Err(e),
    }
    let mut rg = BufReader::new(tokio::fs::File::open(webuild).await?);
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    xai_webuild_tools::util::detach_command(&mut cmd);
    cmd.envs(xai_webuild_tools::util::pager_env());
...
/// Download and install webuild from GitHub Releases (xai-org-shared/grok-build).
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    eprintln!(
        "  Downloading webuild v{} ({}) from GitHub Releases...",
        version, platform
    );
    ...
    // Update webuild-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/webuild -> ~/.webuild/downloads/webuild-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("webuild-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update webuild-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{webuild,agent} if either points directly into
    // ~/.webuild/downloads/ (legacy layout — skips the webuild-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["webuild", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".webuild/downloads/") && !target_str.ends_with("webuild-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "webuild", &version).await;
    cleanup_old_downloads(&download_dir, "webuild-pager", &version).await;
...
/// Check if other webuild processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any webuild process running from that vendored path will be SIGKILL'd by the
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.webuild/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_webuild_processes_running() {
    ...
    cmd.args(["-f", "webuild"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    xai_webuild_tools::util::detach_std_command(&mut cmd);
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other webuild process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other webuild sessions before updating.");
            eprintln!();
        }
    }
}
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_webuild_processes_running();

    let version_arg = match target {
        Some(ver) => format!("@webuild/webuild@{ver}"),
        None => {
...
/// Run the `webuild update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ webuild v{} installed successfully!", version);
        eprintln!("  Please restart WeBuild.");
    ...
    {
        eprintln!(
            "Forcing reinstall of WeBuild {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating WeBuild {} → {}", effective_current, install_target);
        &install_target
    };
    ...
    refresh_deployment_config().await;
    eprintln!("  ✓ webuild v{} installed successfully!", target_version);

    if !force && std::env::var_os("WEBUILD_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart WeBuild.");
    }
...
async fn refresh_deployment_config() {
    if !xai_webuild_shell::managed_config::has_principal() {
        return;
    }
    if !xai_webuild_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    xai_webuild_shell::managed_config::clear_orphan();
    if !xai_webuild_shell::config::is_managed_config_stale_for(
        &xai_webuild_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match xai_webuild_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `webuild setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `webuild setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        // name onto a single `webuild-0.1.tmp`; the helper must keep distinct
        // versions distinct AND make repeated attempts (same process, e.g.
        // concurrent tokio tasks) unique.
        let dest_181 = std::path::Path::new("/home/u/.webuild/downloads/webuild-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.webuild/downloads/webuild-0.1.182-linux-x86_64");
        ...
        assert!(
            name.starts_with("webuild-0.1.181-linux-x86_64."),
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.webuild/downloads").into(),
    ...
    async fn test_atomic_symlink_swap_creates_new_symlink() {
        ...
        let link = dir.path().join("webuild");
    ...
    async fn test_atomic_symlink_swap_replaces_regular_file() {
        ...
        let link = dir.path().join("webuild");
        // Simulate an old installation where webuild is a regular file.
    ...
    async fn test_reconcile_agent_repoints_diverged_agent() {
        ...
        std::fs::write(downloads.join("webuild-0.2.101-macos-aarch64"), "new").unwrap();
        std::fs::write(downloads.join("webuild-0.1.199-macos-aarch64"), "old").unwrap();

        std::os::unix::fs::symlink("../downloads/webuild-0.2.101-macos-aarch64", bin.join("webuild"))
            .unwrap();
        std::os::unix::fs::symlink("../downloads/webuild-0.1.199-macos-aarch64", bin.join("agent"))
            .unwrap();

        reconcile_agent_to_webuild(&bin).await;

        assert_eq!(
            std::fs::read_link(bin.join("agent")).unwrap(),
            std::path::PathBuf::from("../downloads/webuild-0.2.101-macos-aarch64"),
        );
        assert_eq!(std::fs::read_to_string(bin.join("agent")).unwrap(), "new");
        assert!(downloads.join("webuild-0.1.199-macos-aarch64").exists());
    }
    ...
    async fn test_reconcile_agent_heals_legacy_unversioned_agent() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(downloads.join("webuild-0.2.101-macos-aarch64"), "new").unwrap();
        std::fs::write(downloads.join("webuild-macos-aarch64"), "legacy").unwrap();

        std::os::unix::fs::symlink("../downloads/webuild-0.2.101-macos-aarch64", bin.join("webuild"))
            .unwrap();
        std::os::unix::fs::symlink("../downloads/webuild-macos-aarch64", bin.join("agent")).unwrap();
    ...
    async fn test_reconcile_agent_noop_when_consistent() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(downloads.join("webuild-0.2.101-macos-aarch64"), "new").unwrap();
        let target = "../downloads/webuild-0.2.101-macos-aarch64";
        std::os::unix::fs::symlink(target, bin.join("webuild")).unwrap();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_webuild_dangling() {
        let (_dir, bin, downloads) = managed_layout();
        std::os::unix::fs::symlink("../downloads/webuild-0.2.101-macos-aarch64", bin.join("webuild"))
            .unwrap();
        std::fs::write(downloads.join("webuild-0.1.199-macos-aarch64"), "old").unwrap();
        ...
        assert_eq!(
            std::fs::read_link(bin.join("agent")).unwrap(),
            std::path::PathBuf::from("../downloads/webuild-0.1.199-macos-aarch64"),
        );
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_webuild_not_symlink() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(bin.join("webuild"), "copy-binary").unwrap();
        std::fs::write(downloads.join("webuild-0.1.199-macos-aarch64"), "old").unwrap();
        std::os::unix::fs::symlink("../downloads/webuild-0.1.199-macos-aarch64", bin.join("agent"))
            .unwrap();
    ...
    async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
        ...
        let leftover_old = dir.path().join("webuild.tmp-link");
        let leftover_new = dir.path().join("webuild.123-0.tmp-link");
        ...
        // `webuild` link (no `.tmp-link` suffix) is untouched.
        ...
        let fresh = dir.path().join("webuild.999-9.tmp-link");
    ...
    async fn test_atomic_symlink_swap_with_absolute_target() {
        ...
        let binary = dir.path().join("webuild-0.1.141");
    ...
    async fn test_atomic_symlink_swap_with_relative_target() {
        ...
        std::fs::write(downloads.join("webuild-0.1.203"), "v203").unwrap();

        let rel_target = std::path::Path::new("../downloads/webuild-0.1.203");
        let link = bin.join("webuild");
        atomic_symlink_swap(rel_target, &link).await.unwrap();

        assert!(link.is_symlink());
        assert_eq!(
            std::fs::read_link(&link).unwrap(),
            std::path::PathBuf::from("../downloads/webuild-0.1.203")
        );
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        // bin/webuild -> ../downloads/webuild-0.1.203
        let target = std::path::Path::new("/home/alice/.webuild/downloads/webuild-0.1.203");
        let link = std::path::Path::new("/home/alice/.webuild/bin/webuild");
    ...
    fn test_relative_symlink_target_same_dir() {
        // downloads/webuild-latest -> webuild-0.1.203 (same directory)
        let target = std::path::Path::new("/home/alice/.webuild/downloads/webuild-0.1.203");
        let link = std::path::Path::new("/home/alice/.webuild/downloads/webuild-latest");
        let result = relative_symlink_target(target, link);
        assert_eq!(result, std::path::PathBuf::from("webuild-0.1.203"));
    }

    #[cfg(unix)]
    #[test]
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/webuild -> /home/alice/.webuild/downloads/webuild-0.1.203
        // Different grandparents — should stay absolute.
        let target = std::path::Path::new("/home/alice/.webuild/downloads/webuild-0.1.203");
        let link = std::path::Path::new("/usr/local/bin/webuild");
        let result = relative_symlink_target(target, link);
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.webuild/downloads/webuild-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.webuild/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".webuild");
        ...
        let bob_link = bob.join("bin").join("webuild");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        // Simulate 5 old webuild binaries in downloads dir.
        for v in ["0.1.140", "0.1.141", "0.1.142", "0.1.143", "0.1.144"] {
            std::fs::write(d.join(format!("webuild-{}-macos-aarch64", v)), v).unwrap();
        }
        // Current version.
        std::fs::write(d.join("webuild-0.1.145-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.145").await;

        // Current must survive.
        assert!(d.join("webuild-0.1.145-macos-aarch64").exists(), "current");
        // Newest old version (0.1.144) must survive.
        assert!(d.join("webuild-0.1.144-macos-aarch64").exists(), "N-1");
        // Everything else should be deleted.
        assert!(
            !d.join("webuild-0.1.143-macos-aarch64").exists(),
            "0.1.143 should be deleted"
        );
        assert!(
            !d.join("webuild-0.1.142-macos-aarch64").exists(),
            "0.1.142 should be deleted"
        );
        assert!(
            !d.join("webuild-0.1.141-macos-aarch64").exists(),
            "0.1.141 should be deleted"
        );
        assert!(
            !d.join("webuild-0.1.140-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // webuild and webuild-pager should not interfere with each other.
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64"), "old-webuild").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current-webuild").unwrap();
        std::fs::write(d.join("webuild-pager-0.1.140-macos-aarch64"), "old-pager").unwrap();
        std::fs::write(d.join("webuild-pager-0.1.141-macos-aarch64"), "current-pager").unwrap();

        // Cleanup only webuild — pager files must be untouched.
        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.141").await;

        assert!(d.join("webuild-0.1.141-macos-aarch64").exists());
        assert!(d.join("webuild-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
        assert!(
            d.join("webuild-pager-0.1.140-macos-aarch64").exists(),
            "pager untouched"
        );
        assert!(
            d.join("webuild-pager-0.1.141-macos-aarch64").exists(),
            "pager untouched"
        );
    }
    ...
    async fn test_cleanup_old_downloads_removes_stale_tmp_keeps_fresh_tmp() {
        ...
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64.tmp"), "partial").unwrap();
        make_stale(&d.join("webuild-0.1.140-macos-aarch64.tmp"));
        // Fresh tmp: a concurrent updater's in-flight download — kept, or
        // its atomic rename would fail with ENOENT.
        std::fs::write(d.join("webuild-0.1.142-macos-aarch64.77-0.tmp"), "inflight").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current").unwrap();

        cleanup_old_downloads(d, "webuild", "0.1.141").await;

        assert!(
            !d.join("webuild-0.1.140-macos-aarch64.tmp").exists(),
            "stale tmp cleaned up"
        );
        assert!(
            d.join("webuild-0.1.142-macos-aarch64.77-0.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_keeps_fresh_versioned_binary() {
        ...
        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("webuild-{v}-macos-aarch64")), v).unwrap();
        }
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current").unwrap();
        make_all_stale(d);
        ...
        std::fs::write(d.join("webuild-0.1.138-macos-aarch64"), "in-flight").unwrap();

        cleanup_old_downloads(d, "webuild", "0.1.141").await;

        assert!(d.join("webuild-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("webuild-0.1.140-macos-aarch64").exists(), "N-1 kept");
        assert!(
            d.join("webuild-0.1.138-macos-aarch64").exists(),
            "fresh just-renamed binary must NOT be deleted"
        );
        assert!(
            !d.join("webuild-0.1.139-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_skips_symlinks() {
        ...
        // webuild-latest is a symlink — must be skipped.
        let target = d.join("webuild-0.1.141-macos-aarch64");
        std::fs::write(&target, "current").unwrap();
        std::os::unix::fs::symlink(&target, d.join("webuild-latest")).unwrap();
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "webuild", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        std::fs::write(d.join("webuild-0.1.14-macos-aarch64"), "current").unwrap();
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64"), "old-140").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "old-141").unwrap();
        std::fs::write(d.join("webuild-0.1.13-macos-aarch64"), "old-13").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.14").await;

        // Current must survive.
        assert!(
            d.join("webuild-0.1.14-macos-aarch64").exists(),
            "current 0.1.14"
        );
        ...
        assert!(
            !d.join("webuild-0.1.13-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for webuild-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.148", "0.1.149", "0.1.150"] {
            std::fs::write(d.join(format!("webuild-pager-{}-linux-x64", v)), v).unwrap();
        }
        std::fs::write(d.join("webuild-pager-0.1.151-linux-x64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild-pager", "0.1.151").await;

        assert!(d.join("webuild-pager-0.1.151-linux-x64").exists(), "current");
        assert!(d.join("webuild-pager-0.1.150-linux-x64").exists(), "N-1 kept");
        assert!(
            !d.join("webuild-pager-0.1.149-linux-x64").exists(),
            "0.1.149 deleted"
        );
        assert!(
            !d.join("webuild-pager-0.1.148-linux-x64").exists(),
            "0.1.148 deleted"
        );
    }
    ...
    async fn test_cleanup_old_downloads_npm_layout() {
        // npm layout: files are just `webuild-{version}` (no platform suffix).
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("webuild-{}", v)), v).unwrap();
        }
        std::fs::write(d.join("webuild-0.1.141"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.141").await;

        assert!(d.join("webuild-0.1.141").exists(), "current");
        assert!(d.join("webuild-0.1.140").exists(), "N-1 kept");
        assert!(!d.join("webuild-0.1.139").exists(), "0.1.139 deleted");
        assert!(!d.join("webuild-0.1.138").exists(), "0.1.138 deleted");
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_alpha_versions() {
        // Alpha version filenames include pre-release tags:
        //   webuild-0.1.150-alpha.1-macos-aarch64
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        std::fs::write(d.join("webuild-0.1.148-alpha.1-macos-aarch64"), "alpha-148-1").unwrap();
        std::fs::write(d.join("webuild-0.1.148-alpha.2-macos-aarch64"), "alpha-148-2").unwrap();
        std::fs::write(d.join("webuild-0.1.149-alpha.1-macos-aarch64"), "alpha-149-1").unwrap();
        // Current version is the newest alpha.
        std::fs::write(d.join("webuild-0.1.150-alpha.1-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.150-alpha.1").await;

        // Current must survive.
        assert!(
            d.join("webuild-0.1.150-alpha.1-macos-aarch64").exists(),
            "current alpha"
        );
        // Newest old (0.1.149-alpha.1) kept as N-1.
        assert!(
            d.join("webuild-0.1.149-alpha.1-macos-aarch64").exists(),
            "N-1 alpha"
        );
        // Older alphas deleted.
        assert!(
            !d.join("webuild-0.1.148-alpha.2-macos-aarch64").exists(),
            "0.1.148-alpha.2 deleted"
        );
        assert!(
            !d.join("webuild-0.1.148-alpha.1-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        std::fs::write(d.join("webuild-0.1.148-macos-aarch64"), "stable-148").unwrap();
        std::fs::write(d.join("webuild-0.1.149-alpha.1-macos-aarch64"), "alpha-149").unwrap();
        std::fs::write(d.join("webuild-0.1.149-macos-aarch64"), "stable-149").unwrap();
        // Current is a stable release.
        std::fs::write(d.join("webuild-0.1.150-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.150").await;

        // Current must survive.
        assert!(d.join("webuild-0.1.150-macos-aarch64").exists(), "current");
        // Newest old is 0.1.149 stable (semver: 0.1.149 > 0.1.149-alpha.1).
        assert!(
            d.join("webuild-0.1.149-macos-aarch64").exists(),
        ...
        assert!(
            !d.join("webuild-0.1.148-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "v141").unwrap();

        // Invalid version string → cleanup must early-return without deleting.
        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "not-a-version").await;
        assert!(d.join("webuild-0.1.140-macos-aarch64").exists());
        assert!(d.join("webuild-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "webuild", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. webuild-latest, webuild-pager-* when prefix is webuild).
        std::fs::write(d.join("webuild-latest"), "alias").unwrap();
        std::fs::write(d.join("webuild-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        // webuild-latest and webuild-pager-* must be untouched.
        assert!(d.join("webuild-latest").exists());
        assert!(d.join("webuild-pager-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_unparseable_version_skipped() {
        ...
        std::fs::write(d.join("webuild-9garbage-macos-aarch64"), "junk").unwrap();
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(
            d.join("webuild-9garbage-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_multiplatform_in_same_dir() {
        ...
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "mac").unwrap();
        std::fs::write(d.join("webuild-0.1.141-linux-x86_64"), "linux").unwrap();
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64"), "old-mac").unwrap();
        std::fs::write(d.join("webuild-0.1.139-macos-aarch64"), "older-mac").unwrap();
        ...
        assert!(d.join("webuild-0.1.141-macos-aarch64").exists());
        assert!(d.join("webuild-0.1.141-linux-x86_64").exists());
        // N-1 (0.1.140) kept, older deleted.
        assert!(d.join("webuild-0.1.140-macos-aarch64").exists());
        assert!(!d.join("webuild-0.1.139-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_tmp_files_deleted_even_when_unparseable() {
        ...
        std::fs::write(d.join("webuild-junk.tmp"), "partial").unwrap();
        make_stale(&d.join("webuild-junk.tmp"));
        std::fs::write(d.join("webuild-0.1.140-macos-aarch64.tmp"), "partial2").unwrap();
        make_stale(&d.join("webuild-0.1.140-macos-aarch64.tmp"));
        std::fs::write(d.join("webuild-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(!d.join("webuild-junk.tmp").exists(), "junk tmp deleted");
        assert!(
            !d.join("webuild-0.1.140-macos-aarch64.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_three_olds_keeps_only_newest() {
        ...
        assert!(d.join("webuild-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("webuild-0.1.140-macos-aarch64").exists(), "N-1 only");
        assert!(!d.join("webuild-0.1.139-macos-aarch64").exists());
        assert!(!d.join("webuild-0.1.138-macos-aarch64").exists());
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_darwin_platform_recognized() {
        // The `darwin` alias for macOS is in PLATFORM_OS — versions on
        // webuild-X.Y.Z-darwin-* layouts must split correctly.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("webuild-0.1.140-darwin-arm64"), "v140").unwrap();
        std::fs::write(d.join("webuild-0.1.141-darwin-arm64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "webuild", "0.1.141").await;

        assert!(d.join("webuild-0.1.141-darwin-arm64").exists(), "current");
        assert!(d.join("webuild-0.1.140-darwin-arm64").exists(), "N-1");
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert_eq!(
            ...
            "Run `webuild update` to get the latest version."
        );
    }
    ...
    //   1. WEBUILD_INSTALLER (npm | internal | gh-release | gh)
    //   2. WEBUILD_MANAGED_BY_NPM       → npm
    //   3. WEBUILD_MANAGED_BY_INTERNAL  → internal
    ...
    impl InstallerEnvGuard {
        fn isolate() -> Self {
            const VARS: &[&str] = &[
                "WEBUILD_INSTALLER",
                "WEBUILD_MANAGED_BY_NPM",
                "WEBUILD_MANAGED_BY_INTERNAL",
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `WEBUILD_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("WEBUILD_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit WEBUILD_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("WEBUILD_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("WEBUILD_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // WEBUILD_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("WEBUILD_INSTALLER", "internal");
            std::env::set_var("WEBUILD_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    async fn test_windows_replace_exe_creates_dest_when_missing() {
        ...
        let dest = dir.path().join("webuild.exe");
    ...
    async fn test_windows_replace_exe_cleans_stale_old_backup() {
        ...
        let old = dir.path().join("webuild.exe.old");
    ...
    async fn test_windows_replace_exe_rollback_on_copy_failure() {
        // No stale .old: the aside IS webuild.exe.old, so this pins the
    ...
    async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
        ...
        let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with("webuild.exe.old.") && n.ends_with(".old"))
            })
            .collect();
        assert_eq!(
            asides.len(),
    ...
    async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
        ...
        let leftover_asides = std::fs::read_dir(dir.path())
            ...
            .filter(|e| {
                ...
                name.starts_with("webuild.exe.old.") && name.ends_with(".old")
            })
            .count();
    ...
    async fn test_windows_replace_exe_sweeps_accumulated_asides() {
        ...
        let aside_a = dir.path().join("webuild.exe.old.1234-0.old");
        let aside_b = dir.path().join("webuild.exe.old.1234-1.old");
