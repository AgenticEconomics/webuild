...
use xai_webuild_shell::env::WeBuildEnvironment;
use xai_webuild_shell::util::webuild_home::webuild_home;
...
///
/// Constructed once from `WeBuildEnvironment` at startup and threaded through the
...
pub struct UpdateConfig {
    ...
    /// Enterprise deployment key (WEBUILD_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &WeBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: xai_webuild_shell::auth::WeBuildComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = webuild_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = WeBuildVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<WeBuildVersion>(&version_str)
...
pub use xai_webuild_version::installed as get_installed_webuild_version;

/// Version of the managed webuild binary currently on disk, read from the
/// `~/.webuild/bin/webuild` symlink target (`../downloads/webuild-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `webuild update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.webuild/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = xai_webuild_shell::util::webuild_home::webuild_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "webuild")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`webuild-0.1.150-macos-aarch64`, including
/// pre-releases: `webuild-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`webuild-0.1.150`,
/// `webuild-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`webuild-latest`, `webuild-pager-*` when `bin_prefix` is
/// `webuild`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.webuild/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: WeBuildVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(xai_webuild_version::VERSION, &stable)
    })
}
...
/// `~/.webuild/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(xai_webuild_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = WeBuildVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`webuild-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("webuild-0.2.46-darwin-arm64", Some("0.2.46")),
            ("webuild-0.1.220-linux-x86_64", Some("0.1.220")),
            ("webuild-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("webuild-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("webuild-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("webuild-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("webuild-garbage-darwin-arm64", None),               // unparseable version
            ("webuild-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("webuild-latest", None),                             // symlink alias, not a version
            ("webuild", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "webuild").as_deref(),
                *expected,
        ...
        // prefix but not under "webuild".
        assert_eq!(
            version_from_versioned_binary_name("webuild-pager-0.1.5-darwin-arm64", "webuild-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // WeBuildVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: WeBuildVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = WeBuildVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: WeBuildVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<WeBuildVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<WeBuildVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = WeBuildVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use xai_webuild_shell::env::WeBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&WeBuildEnvironment::Production);
