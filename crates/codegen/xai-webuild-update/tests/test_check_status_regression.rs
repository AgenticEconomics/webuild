...
//! exact JSON shape produced by `webuild update --check --json` for the failure
...
use xai_webuild_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH, set `WEBUILD_INSTALLER=npm` so the auto-update
...
async fn check_status_surfaces_npm_403_in_error_field() {
    ...
    g.set_stderr(
        "npm error code E403\n\
         npm error 403 403 Forbidden - GET https://registry-mirror.example.invalid/api/npm/js-virtual/@xai-official%2fwebuild\n\
