...
//!
//! > `~/.webuild/bin/webuild` resolves to a binary that passes the smoke-test, OR it
...
use xai_webuild_update::auto_update::install_internal_from_base;
...
/// (`webuild` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `webuild` should resolve to after an install attempt.
...
/// to both `webuild` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p xai-webuild-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("WEBUILD_BLITZ_ITERS")
        .ok()
