...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `webuild update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `webuild update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `webuild-0.1.tmp`
...
use xai_webuild_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use xai_webuild_update::version::installed_on_disk_version;

/// Assert the active `~/.webuild/bin/webuild` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("webuild");
    assert!(link.is_symlink(), "webuild must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active webuild symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("webuild-{version}-{platform}"),
        "active webuild must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active webuild must pass the smoke-test");
}

/// Lay down a managed-install layout in the test WEBUILD_HOME:
/// `bin/webuild -> ../downloads/webuild-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("webuild-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `webuild update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.webuild/bin/webuild` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.webuild/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("webuild-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `webuild-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/webuild-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("webuild-{version}-{platform}"));
        assert_eq!(
