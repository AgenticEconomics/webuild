...
//! its own `OnceLock<WEBUILD_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the WEBUILD_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes WEBUILD_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// WEBUILD_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `WEBUILD_HOME`, initialized exactly once per test
/// binary. Once initialized, `xai_webuild_config::webuild_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("WEBUILD_INSTALLER");
            std::env::remove_var("WEBUILD_MANAGED_BY_NPM");
            std::env::remove_var("WEBUILD_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `WEBUILD_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_webuild_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("WEBUILD_TEST_VERSION", v) };
}
...
/// Minimal [`xai_webuild_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> xai_webuild_update::UpdateConfig {
    xai_webuild_update::UpdateConfig {
...
/// Backdate every file in `WEBUILD_HOME/downloads` by ~2 hours.
///
