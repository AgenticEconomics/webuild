...
//!
//! > After any install attempt, `$BIN_DIR/webuild` resolves to a binary that runs,
...
//!
//! The installer lives in the sibling `xai-webuild-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../xai-webuild-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> webuild installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".webuild").join("downloads");
    let bin = home.join(".webuild").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("webuild-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/webuild-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/webuild` from disk and re-run it: the active webuild must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_webuild_runs(home: &Path) {
    let link = home.join(".webuild").join("bin").join("webuild");
    assert!(link.is_symlink(), "webuild must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("webuild symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active webuild must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active webuild must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("WEBUILD_BIN_DIR", home.join(".webuild").join("bin"))
        .env("WEBUILD_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one webuild installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_webuild_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_webuild_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // webuild always runs (new good binary on success, previous-good on
        // rejection).
        assert_active_webuild_runs(home.path());
    }
}
