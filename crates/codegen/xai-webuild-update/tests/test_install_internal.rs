...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `WEBUILD_HOME`
//! tempdir so we can verify the full install pipeline:
//!   fetch version → download webuild binary → chmod → atomic symlink →
//!   cleanup_old_downloads → persist installer config.
//!
//! The function reads `webuild_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `WEBUILD_HOME` and run serially via
//! `#[serial]`.
...
use xai_webuild_update::UpdateConfig;
use xai_webuild_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main webuild binary download.
    Mock::given(method("GET"))
        .and(path(format!("/webuild-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let downloaded = home
        .join("downloads")
        .join(format!("webuild-0.1.181-{platform}"));
    assert!(downloaded.exists(), "binary downloaded: {downloaded:?}");
    assert_eq!(std::fs::read(&downloaded).unwrap(), b"#!/bin/sh\nexit 0\n");

    let symlink = home.join("bin").join("webuild");
    assert!(symlink.is_symlink(), "webuild symlink created");
    let target = std::fs::read_link(&symlink).unwrap();
    assert_eq!(
        target.file_name().unwrap(),
        format!("webuild-0.1.181-{platform}").as_str()
    );

    // `webuild` and `agent` move together — see `swap_managed_bin_links`.
    ...
    assert_eq!(agent_target, target, "agent and webuild point at same target");
}
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    let old_binary = download_dir.join(format!("webuild-0.1.180-{platform}"));
    std::fs::write(&old_binary, b"#!/bin/sh\nexit 0\n").unwrap();
    let rel_old = std::path::Path::new("..")
        .join("downloads")
        .join(format!("webuild-0.1.180-{platform}"));
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("webuild")).unwrap();
...
/// Rollback regression: if `agent` swap fails after `webuild` succeeded,
/// `webuild` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_webuild_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // webuild must be rolled back to the prior version.
    let webuild_target = std::fs::read_link(bin_dir.join("webuild")).unwrap();
    assert_eq!(
        webuild_target.file_name().unwrap(),
        format!("webuild-0.1.180-{platform}").as_str(),
        "webuild must be rolled back when agent swap fails"
    );
}

/// Absent-prior rollback regression: fresh install (no prior `webuild` /
/// `agent`), sabotaged `agent` swap must *remove* the just-created `webuild`
/// link so we don't leave it on the new binary while `agent` is absent.
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_webuild_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // No prior `webuild`. Sabotage `agent` swap: non-empty directory → EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("webuild").exists() && !bin_dir.join("webuild").is_symlink(),
        "precondition: webuild must not exist before install",
    );
    ...
    let webuild_path = bin_dir.join("webuild");
    assert!(
        !webuild_path.is_symlink() && !webuild_path.exists(),
        "webuild must be removed on rollback when there was no prior link",
    );
}
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate webuild-pager binary. Verify the
    // update removes the stale symlink from ~/.webuild/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("webuild-pager");
    ...
    assert!(
        ...
        "stale webuild-pager symlink should be removed"
    );
}
...
async fn install_internal_alpha_channel_resolves_max_of_alpha_and_stable() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/webuild-0.1.181-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_webuild_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    assert!(
        downloads.join(format!("webuild-0.1.181-{platform}")).exists(),
        "current"
    );
    assert!(
        downloads.join(format!("webuild-0.1.180-{platform}")).exists(),
        "N-1 retained"
    );
    assert!(
        !downloads.join(format!("webuild-0.1.179-{platform}")).exists(),
        "oldest deleted"
    );

    // Symlink updated to latest.
    let target = std::fs::read_link(home.join("bin").join("webuild")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("webuild")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_webuild_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
