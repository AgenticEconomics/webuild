...
use xai_webuild_update::auto_update::install_npm_for_test;
use xai_webuild_update::version::{
...
async fn fetch_npm_tag_passes_pkg_and_tag_to_npm() {
    ...
    assert!(args.contains("@webuild/webuild"), "args: {args}");
...
async fn fetch_npm_tag_alpha_appends_at_alpha_suffix() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@webuild/webuild@alpha"),
        "args: {}",
        log[0]
    );
}
...
async fn install_npm_calls_npm_with_version_arg() {
    ...
    assert!(args.contains("@webuild/webuild@0.1.181"), "args: {args}");
}
...
async fn install_npm_falls_back_to_dist_tag_on_no_target() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@webuild/webuild@latest"),
