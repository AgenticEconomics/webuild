...
//!
//! These tests touch global process state — `WEBUILD_HOME` (a `OnceLock` in
//! `xai-webuild-config`), `WEBUILD_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `WEBUILD_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the WEBUILD_HOME isolation used in other
//! integration tests.
...
use xai_webuild_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_webuild_home() {
    let _ = test_home();
    reset();
...
/// Write a `WeBuildVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `WeBuildVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under WEBUILD_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every webuild launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_webuild_version env override
//
// The function honors `WEBUILD_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_webuild_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("WEBUILD_TEST_VERSION", "9.9.9");
    }
    let v = xai_webuild_update::version::get_installed_webuild_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("WEBUILD_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        xai_webuild_update::version::get_installed_webuild_version()
    };

    unsafe {
        std::env::set_var("WEBUILD_TEST_VERSION", "0.0.0-test");
    }
    let overridden = xai_webuild_update::version::get_installed_webuild_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("WEBUILD_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = xai_webuild_update::version::get_installed_webuild_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("WEBUILD_TEST_VERSION", "not-a-version");
    }
    let v = xai_webuild_update::version::get_installed_webuild_version();
