...
use xai_webuild_update::auto_update::{download_silent, download_with_progress};
use xai_webuild_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_writes_body_to_dest() {
    ...
    Mock::given(method("GET"))
        .and(path("/webuild-0.1.181-macos-aarch64"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(body.clone()))
        .mount(&server)
        .await;

    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("webuild");
    let url = format!("{}/webuild-0.1.181-macos-aarch64", server.uri());
...
async fn download_silent_publishes_executable() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("webuild-0.1.181-linux-x86_64");
    download_silent(&format!("{}/bin", server.uri()), &dest)
        .await
        .unwrap();
...
async fn download_silent_to_nonexistent_parent_dir_fails() {
    ...
    let dest = tmp.path().join("missing-subdir").join("webuild");
    let err = download_silent(&format!("{}/x", server.uri()), &dest)
        .await
        .unwrap_err();
    let msg = format!("{err:#}").to_lowercase();
...
async fn download_with_progress_writes_body_with_content_length() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("webuild");
    download_with_progress(&format!("{}/webuild", server.uri()), &dest)
        .await
        .unwrap();
...
async fn download_silent_parallel_path_reassembles_bytes() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("webuild-binary");
    download_silent(&format!("{}/big", server.uri()), &dest)
        .await
        .unwrap();
