...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match xai_webuild_http::shared_client()
        .get(url)
...
pub use xai_webuild_workspace::permission::resolution::MANAGED_MCP_PREFIX;

/// `"Linear"` -> `"webuild_com_linear"`. Shares normalization and the
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    xai_webuild_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
...
/// 1. Server name starts with `MANAGED_MCP_PREFIX` ("webuild_com_")
/// 2. Server URL matches a managed config endpoint
///
/// This prevents false injection if a user accidentally names a server `webuild_com_*`
...
/// Inject managed OAuth headers into `webuild_com_`-prefixed MCP servers.
///
...
mod tests {
    ...
    fn reauth_first_attempt_allowed_then_backs_off() {
        let mut state = ManagedMcpState::default();
        let now = Utc::now();
        assert!(state.reauth_allowed("webuild_com_slack", now));
        assert!(!state.reauth_is_terminal("webuild_com_slack"));

        state.record_reauth_failure("webuild_com_slack", now);
        // 2^1 = 2s backoff: not eligible now, eligible after the window.
        assert!(!state.reauth_allowed("webuild_com_slack", now));
        assert!(state.reauth_allowed("webuild_com_slack", now + chrono::Duration::seconds(2)));
        assert!(!state.reauth_is_terminal("webuild_com_slack"));
    }
    ...
    fn reauth_goes_terminal_after_max_attempts() {
        ...
        assert!(!state.reauth_allowed("webuild_com_slack", now + chrono::Duration::seconds(3600)));
    }
    ...
    fn reauth_success_resets_cooldown() {
        ...
        state.record_reauth_success("webuild_com_slack");
        assert!(state.reauth_allowed("webuild_com_slack", now));
        assert!(!state.reauth_is_terminal("webuild_com_slack"));
    }
    ...
    fn complete_fetch_preserves_cooldown_clear_resets_it() {
        ...
        let now = Utc::now();
        {
            ...
            for _ in 0..MAX_REACTIVE_REAUTH_ATTEMPTS {
                state.record_reauth_failure("webuild_com_slack", now);
                state.record_reauth_failure("webuild_com_linear", now);
            }
            assert!(state.reauth_is_terminal("webuild_com_slack"));
            assert!(state.reauth_is_terminal("webuild_com_linear"));
            ...
            assert!(state.reauth_allowed("webuild_com_slack", now));
            assert!(state.reauth_allowed("webuild_com_linear", now));
            assert!(!state.reauth_is_terminal("webuild_com_slack"));
            assert!(!state.reauth_is_terminal("webuild_com_linear"));
        }
    }
