...
//! spawns the unchanged `/usr/local/bin/xai-webuild-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/xai-webuild-preview-proxy";
...
/// `webuild_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "webuild_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `xai-webuild-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`WEBUILD_SERVER_KEY` /
/// `WEBUILD_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
/// `xai-webuild-preview-proxy`'s `/__control/activity` route).
...
/// the default in `xai-webuild-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match xai-webuild-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "WEBUILD_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
