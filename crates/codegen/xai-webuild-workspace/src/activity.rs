...
/// `WEBUILD_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `WEBUILD_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `WEBUILD_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
...
/// The durability idle-hold cap from `WEBUILD_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("WEBUILD_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
