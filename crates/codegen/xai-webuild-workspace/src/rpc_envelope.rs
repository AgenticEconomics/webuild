...
//! (the envelope types are canonical in `xai_webuild_workspace_types::rpc`).
//!
//! `error_code` uses a non-wildcard match so the compiler enforces
//! coverage of new `WorkspaceError` variants.

pub use xai_webuild_workspace_types::rpc::{RpcEnvelope, RpcError};
...
pub fn error_code(err: &WorkspaceError) -> &'static str {
    match err {
        ...
        WorkspaceError::TurnActive(_) => xai_webuild_workspace_types::rpc::envelope::TURN_ACTIVE,
...
pub fn rpc_error_to_workspace(err: RpcError) -> WorkspaceError {
    if let Some(kind) =
        xai_webuild_workspace_types::rpc::deploy::DeployError::from_wire_code(&err.code)
    {
...
mod tests {
    ...
    fn error_code_round_trip_all_variants() {
        ...
        variants.extend(
            xai_webuild_workspace_types::rpc::deploy::DeployError::ALL
                .into_iter()
