...
//!
//! Reads OIDC credentials from `~/.webuild/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use xai_webuild_workspace::config::WorkspaceServerMetadata;
use xai_webuild_workspace::daemonize;
use xai_webuild_workspace::diag_server;
use xai_webuild_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_webuild_workspace";
...
struct Args {
    ...
    /// `WEBUILD_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "WEBUILD_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Override with `WEBUILD_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "WEBUILD_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/xai-webuild-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("WEBUILD_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use xai_webuild_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("WEBUILD_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized WEBUILD_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if xai_webuild_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via WEBUILD_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = xai_webuild_sandbox::is_active();
            ...
            tracing::info!(
                ...
                xai_webuild_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = xai_webuild_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("WEBUILD_SESSION_ID").ok();
    ...
    xai_webuild_workspace::init_metrics();
    let ws_handle = xai_webuild_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = xai_webuild_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            xai_webuild_workspace::handle::DrainReason::Sigterm,
        )
        .await;
