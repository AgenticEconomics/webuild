...
pub use xai_webuild_workspace_client::WorkspaceClient;
pub use xai_webuild_workspace_types::WorkspaceEvent;
...
/// environment (`WEBUILD_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `WEBUILD_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "webuild_workspace_upload_outcome_total",
        ...
        assert!(has(
            "webuild_workspace_toolset_swap_rejected_total",
            &[("reason", "turn_active"), ("trigger", "update_tool_config")]
        ));
        assert!(has(
            "webuild_workspace_orphan_lost_total",
            &[("reason", "sha_mismatch")]
        ));
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "webuild_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "webuild_workspace_permission_timeout_total")
        );
    }
}
