...
impl WorkspaceIdentity {
    ...
    /// `WeBuildAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `xai-webuild-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$WEBUILD_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("WEBUILD_SANDBOX_PROFILE").ok(),
            xai_webuild_sandbox::is_inside_bwrap(),
