...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "webuild-shell", alias = "webuild_shell")]
    Generic,
    /// WeBuild TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "webuild-tui", alias = "webuild_tui")]
    WeBuildTUI,
    /// WeBuild Web client - identified by clientIdentifier "webuild-web"
    #[serde(rename = "webuild_web")]
    WeBuildWeb,
    ...
    Extension,
    /// WeBuild Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to WeBuildTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"webuild-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"webuild_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "webuild-pager", alias = "webuild_pager")]
    WeBuildPager,
    /// WeBuild Desktop (Electron) client - identified by clientIdentifier "webuild-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "webuild_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `webuild-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "webuild-shell",
            Self::WeBuildTUI => "webuild-tui",
            Self::WeBuildWeb => "webuild-web",
            Self::Nebula => "nebula",
            Self::Extension => "grok-code-extension",
            Self::WeBuildPager => "webuild-pager",
            Self::Desktop => "webuild-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"webuild-web"`, `"webuild-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("webuild-web") => Self::WeBuildWeb,
            Some("nebula") => Self::Nebula,
            Some("grok-code-extension") => Self::Extension,
            Some("webuild-desktop") => Self::Desktop,
            Some("webuild-pager") => Self::WeBuildPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::WeBuildTUI | Self::WeBuildPager => "tui",
            Self::WeBuildWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&xai_webuild_tools::types::ToolInput> for AccessKind {
    fn from(input: &xai_webuild_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.webuild/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use xai_webuild_tools::implementations::webuild_build_hashline::edit::types::HashlineEditInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use xai_webuild_tools::implementations::webuild_build::bash::BashToolInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use xai_webuild_tools::implementations::use_tool::UseToolInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use xai_webuild_tools::implementations::webuild_build::monitor::types::MonitorInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use xai_webuild_tools::implementations::webuild_build::web_search::WebSearchInput;
        use xai_webuild_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_webuild_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("webuild-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("webuild_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
