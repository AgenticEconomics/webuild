...
static PERMISSION_TIMEOUT_TOTAL: LazyLock<IntCounter> = LazyLock::new(|| {
    register_int_counter!(
        "webuild_workspace_permission_timeout_total",
        "permission_request hooks whose reply timed out (backstop deadline fired)"
    )
    .expect("webuild_workspace_permission_timeout_total must register once")
});
...
pub const HITL_PERMISSION_LIVE_ENV: &str = "WEBUILD_HITL_PERMISSION_LIVE";
/// Whether the HITL-live permission path is enabled.
///
/// Intended long-term gate: the chat flag `webuild_chat_enable_hitl_live_path`,
...
pub fn access_kind_for_hub_tool(tool_name: &str, args: &Value) -> Option<AccessKind> {
    ...
    let name = name.strip_prefix("WeBuild:").unwrap_or(name);
    match name {
