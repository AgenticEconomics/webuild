...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"webuild_com_notion__notion-fetch"`)
    ...
    /// e.g. `"webuild_com_notion"`) for which the user has granted
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    xai_webuild_config::sessions_cwd_dir(cwd.as_str())
}
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = webuild_home().join("sessions");
...
mod tests {
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("webuild_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("webuild_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        state
            .allowed_mcp_servers
            .insert("webuild_com_slack".to_string());
        ...
        assert!(restored.allowed_mcp_servers.contains("webuild_com_slack"));
...
"#;
    ...
    async fn persist_and_load_roundtrip() {
        ...
        // We can't easily override webuild_home(), so instead test
