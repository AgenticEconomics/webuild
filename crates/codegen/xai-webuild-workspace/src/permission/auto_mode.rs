...
//!
//! Port of common agent auto-permission classifier semantics adapted to WeBuild's
...
/// mapped to WeBuild access kinds + known names).
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    xai_webuild_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    xai_webuild_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
