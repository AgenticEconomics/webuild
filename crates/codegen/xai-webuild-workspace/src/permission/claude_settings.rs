...
pub struct ClaudeSettings {
    ...
    /// webuild-only root legacy). Recognized values: `acceptEdits`,
...
///     under `permissions.*`. Root-level keys are **webuild legacy only** (not in
...
pub fn load_claude_settings(path: &Path) -> Option<ClaudeSettings> {
    ...
    // Canonical vendor settings store these under `permissions`; root is webuild legacy only.
...
///
/// Root `defaultMode` is webuild-only back-compat for older tests / hand-written
...
pub(crate) fn extract_default_mode(value: &serde_json::Value, path: &Path) -> Option<String> {
    ...
    // Nested key absent — optional webuild legacy root.
        ...
        Some(dm) => match dm.as_str() {
            ...
            None => {
                warn!(
                    path = %path.display(),
                    actual_type = %dm.type_of(),
                    "root defaultMode (webuild legacy): expected string, ignoring"
                );
                None
            }
        },
...
fn extract_additional_directories(value: &serde_json::Value, path: &Path) -> Option<Vec<String>> {
    ...
    // when it is present but the wrong type, do *not* resurrect the webuild-legacy
...
// permissions), consider extracting to a shared helper (e.g., in xai-webuild-hooks
...
// =============================================================================
//
// `xai-webuild-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in xai-webuild-shell (inaccessible from here at runtime). They also
    // set this env var so the workspace-resident gate honours the override
    // without a cross-crate dependency.
    if std::env::var("_WEBUILD_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = xai_webuild_config::user_webuild_home().map(|g| g.join("config.toml")) else {
        return false;
    };
