...
use xai_webuild_tools::implementations::webuild_build::web_fetch::{
...
fn permission_mode_artifact_str(mode: xai_webuild_telemetry::enums::PermissionMode) -> &'static str {
    use xai_webuild_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.webuild/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new webuild process, new agent
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            xai_webuild_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / WeBuildWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `WeBuildTUI | WeBuildPager | Desktop`).
    ...
    async fn auto_classifier_block_prompt_still_offers_always_approve() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // WeBuildPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::WeBuildPager);
                mgr.set_auto_mode(true);
