...
use xai_webuild_tools::implementations::webuild_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell WeBuild what to do differently";
...
///        `~/.webuild/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// (global yolo) option that is prepended for WeBuildTUI / WeBuildPager / Desktop.
///
...
/// (`WeBuildTUI`, `WeBuildPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::WeBuildTUI | ClientType::WeBuildPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `xai_webuild_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `xai-webuild-mcp` can
...
/// reached `xai_webuild_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"webuild_com_notion"` →
/// `"WeBuild Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"webuild_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server segment (everything before the single `__` separator,
    /// e.g. `"webuild_com_notion"`). `None` if the tool name has no `__`,
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for WeBuildTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For WeBuildTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::WeBuildPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::WeBuildPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::WeBuildWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::WeBuildPager, true);
        ...
        let off = prompter_with_gate(ClientType::WeBuildPager, false);
        assert!(off.bash_selection_meta(&access).is_none());
        // Generic client never gets the fancy-UI meta, even with the gate on.
        let generic = prompter_with_gate(ClientType::WeBuildWeb, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::WeBuildPager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::WeBuildTUI);
        let access = AccessKind::MCPTool {
            name: "linear__list".to_owned(),
            input: serde_json::Value::Null,
        };
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / WeBuildWeb / Extension / …) see `fallback_options`. The
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("webuild_com_notion"), "WeBuild Com Notion");
    ...
    /// Same pin as above for `WeBuildTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::WeBuildTUI, ClientType::Desktop] {
            let p = prompter(ct);
