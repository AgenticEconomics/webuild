...
use xai_webuild_mcp::servers::McpState;
use xai_webuild_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use xai_webuild_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<xai_webuild_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<xai_webuild_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<xai_webuild_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<xai_webuild_hooks::error::HookError>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<xai_webuild_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `WEBUILD_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$WEBUILD_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<webuild_home>/workspace` by default). The upload queue spills here.
    ...
    /// (`WEBUILD_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    ...
    /// enabled (`WEBUILD_WORKSPACE_TOOL_DEFS_ENABLED=true`).
...
impl WorkspaceShared {
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<xai_webuild_workspace_types::WorkspaceEvent> {
