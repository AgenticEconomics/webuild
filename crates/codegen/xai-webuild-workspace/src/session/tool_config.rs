...
use std::sync::Arc;
use xai_webuild_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<xai_webuild_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<xai_webuild_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            xai_webuild_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`WEBUILD_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("WEBUILD_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `WEBUILD_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: xai_webuild_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<xai_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$WEBUILD_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$WEBUILD_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(xai_webuild_tools::computer::local::LocalFs)
            as Arc<dyn xai_webuild_tools::computer::types::AsyncFileSystem>;
        let notification_handle = xai_webuild_tools::notification::ToolNotificationHandle::noop();
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = xai_webuild_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("xai-webuild-workspace/{version}"),
    );
    headers.insert("x-webuild-client-version".to_string(), version.to_string());
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "xai-webuild-cli".to_string());
        headers.insert(
...
/// `WEBUILD_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> xai_webuild_tools::implementations::webuild_build::web_fetch::WebFetchConfig
{
    use xai_webuild_tools::implementations::webuild_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("WEBUILD_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("WEBUILD_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("WEBUILD_WEB_SEARCH_MODEL").unwrap_or_else(|_| "grok-4.20-multi-agent".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("WeBuild:search_replace", Some(ToolKind::Edit)),
                tc("WeBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("WeBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("WeBuild:search_replace".to_owned(), ToolKind::Edit),
            ("WeBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("WeBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("WeBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("WeBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("WeBuild:read_file", None),
                test_support::tc("WeBuild:grep", None),
                test_support::tc("WeBuild:list_dir", None),
                test_support::tc("WeBuild:search_replace", None),
                test_support::tc("WeBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("WeBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"WeBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "WEBUILD_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use xai_webuild_tools::types::resources::{State, WebCitationCounter};
