...
/// Resolve `workspace_rewind_hunks` from `WEBUILD_WORKSPACE_REWIND_HUNKS` (default off).
pub(crate) fn rewind_hunks_enabled() -> bool {
    xai_webuild_config::env_bool("WEBUILD_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
/// Resolve `workspace_rewind_durable` from `WEBUILD_WORKSPACE_REWIND_DURABLE`
/// (default off). Off ⇒ the legacy in-memory-only path with no disk I/O.
pub(crate) fn rewind_durable_enabled() -> bool {
    xai_webuild_config::env_bool("WEBUILD_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
...
mod tests {
    ...
    async fn turn_end_with_durable_flag_off_writes_nothing_to_disk() {
        ...
        handle
            .on_turn_boundary("main", TurnBoundary::rewind_finalize(0))
            .await;
        let store_root = cwd.join(".webuild").join("rewind-checkpoints");
        assert!(
            !store_root.exists(),
