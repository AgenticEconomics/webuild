//! Hub [`AuthProvider`] from `~/.webuild/auth.json` for the standalone
...
//!
//! The in-leader `webuild workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let webuild = xai_webuild_config::user_webuild_home()
        .ok_or_else(|| anyhow::anyhow!("no user webuild home (set $WEBUILD_HOME or $HOME)"))?;
    Ok(webuild.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `webuild login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `webuild login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.webuild/auth.json`).
pub fn provider(
    hub_url: &Url,
