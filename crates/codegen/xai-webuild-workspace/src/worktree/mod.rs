...
//!
//! Moved from `xai-webuild-shell/src/session/worktree.rs` into the workspace
...
// Canonical in xai-webuild-workspace-types; re-exported for existing paths.
pub use xai_webuild_workspace_types::rpc::worktree::{
...
/// Resolve the webuild home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`xai_fast_worktree::resolve_webuild_home`), so checkout dirs and
/// the metadata DB always live under the same `.webuild` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_webuild_config::webuild_home()`,
/// so worktree paths also agree with trust/hooks and other webuild-home paths.
fn webuild_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_webuild_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.webuild/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    webuild_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.webuild/worktrees/<repo_name>`)
/// for a given source path, correctly handling webuild-managed worktrees.
///
/// When `source_path` is already under `~/.webuild/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the webuild worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = webuild_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.webuild/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the webuild-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.webuild/worktrees/`.
...
///
/// When the source path is already inside `~/.webuild/worktrees/<repo>/`, the
...
///
/// ACP equivalent of `webuild -w -r <session_id>` (optionally with `--ref`).
...
use xai_fast_worktree::{
    ...
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_webuild_home,
};
...
/// Scan `~/.webuild/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/webuild/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/webuild/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/webuild/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/webuild/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `WEBUILD_HOME` at an isolated tempdir (`resolve_webuild_home` re-reads
    ...
    /// and restores `WEBUILD_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (WeBuildHomeFixture is not visible across crates).
