...
/// [`xai_webuild_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> xai_webuild_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => xai_webuild_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => xai_webuild_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => xai_webuild_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => xai_webuild_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => xai_webuild_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](xai_webuild_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<xai_webuild_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    xai_webuild_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    xai_webuild_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(xai_webuild_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            xai_webuild_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: xai_webuild_workspace_types::FsEventKind,
) -> xai_codebase_graph::FileEvent {
    use xai_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        xai_webuild_workspace_types::FsEventKind::Created => FileEventKind::Created,
        xai_webuild_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        xai_webuild_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        xai_webuild_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.webuild/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use xai_webuild_workspace_types::FsEventKind as WsKind;
        assert_eq!(
