...
/// `WEBUILD_WORKSPACE_TERMINATION_GRACE_MS`. 45s fits under the K8s grace period.
const DEFAULT_TERMINATION_GRACE_MS: u64 = 45_000;
/// preStop-hook drain marker; override via `WEBUILD_WORKSPACE_DRAINING_FILE`.
...
static DRAIN_STARTED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "webuild_workspace_drain_started_total",
...
static DRAIN_COMPLETED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "webuild_workspace_drain_completed_total",
...
static DRAIN_DURATION: std::sync::LazyLock<Histogram> = std::sync::LazyLock::new(|| {
    register_histogram!(
        "webuild_workspace_drain_duration_seconds",
...
static DRAIN_LOST_ITEMS_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "webuild_workspace_drain_lost_items_total",
...
static PRODUCER_SPAWNED_AFTER_DRAIN_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "webuild_workspace_producer_spawned_after_drain_total",
...
static WORKSPACE_BIND_ZERO_TOOLS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_bind_zero_tools_total",
            "session.bind resolutions advertising zero model-facing tools, by reason",
            &["reason"]
        )
        .unwrap()
...
static WORKSPACE_BIND_FAILED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_bind_failed_total",
...
static WORKSPACE_BIND_UNSERVED_TOOLS_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "webuild_workspace_bind_unserved_tools_total",
...
static WORKSPACE_BIND_ADVERTISED_TOOLS: std::sync::LazyLock<Histogram> =
    std::sync::LazyLock::new(|| {
        register_histogram!(
            "webuild_workspace_bind_advertised_tools",
...
pub(crate) static WORKSPACE_TERMINAL_BACKEND_ORPHANED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_terminal_backend_orphaned_total",
...
static ENV_CAPTURE_PANIC_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "webuild_workspace_env_capture_panic_total",
...
pub(crate) static REWIND_CHECKPOINT_CAPTURE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_rewind_checkpoint_capture_total",
...
pub(crate) static REWIND_CHECKPOINT_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_rewind_checkpoint_finalize_total",
...
pub(crate) static REWIND_RESTORE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_rewind_restore_total",
...
pub(crate) static REWIND_CHECKPOINT_DURATION: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "webuild_workspace_rewind_checkpoint_duration_seconds",
...
pub(crate) static REWIND_NON_COMPLETED_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "webuild_workspace_rewind_non_completed_finalize_total",
...
impl WorkspaceHandle {
    ...
    /// Construct a handle with an explicit `$WEBUILD_WORKSPACE_HOME` and a
    ...
    fn build(
        ...
        let (hook_registry, hook_load_errors) = {
            use xai_webuild_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn xai_webuild_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                xai_webuild_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                xai_webuild_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use xai_webuild_tools::implementations::lsp::{
                ...
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    ...
                    true,
                    xai_webuild_tools::notification::ToolNotificationHandle::noop(),
                )));
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<xai_webuild_tools::registry::types::ToolServerConfig>,
    ...
    fn create_session_with_tracker_inner(
        ...
        let system_notify_channel = system_notifications
            .then(xai_webuild_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: xai_webuild_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(xai_webuild_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<xai_webuild_tools::registry::types::ToolServerConfig>,
    ...
    /// `WEBUILD_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `WEBUILD_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn hook_registry(&self) -> xai_webuild_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[xai_webuild_hooks::error::HookError] {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(xai_webuild_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(xai_webuild_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(xai_webuild_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    pub async fn start_session_mcp_servers(
        ...
        use xai_webuild_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        ...
        let mcp_results: Vec<
            Result<xai_webuild_mcp::servers::McpClient, xai_webuild_mcp::servers::McpError>,
        > = tokio::task::spawn_blocking(move || {
            use std::collections::HashMap;
            use xai_webuild_mcp::oauth_config::McpOAuthConfigMap;
            use xai_webuild_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
            ...
            rt_handle.block_on(xai_webuild_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                xai_webuild_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<xai_webuild_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<xai_webuild_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
            ...
            move |sid: xai_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                async move {
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || xai_webuild_tools::registry::types::ToolServerConfig {
                        tools: vec![],
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (activity_notify_handle, activity_notify_rx) =
            xai_webuild_tools::notification::types::ToolNotificationHandle::channel();
...
fn build_session_routed_handlers(
    toolset: &xai_webuild_tools::registry::types::FinalizedToolset,
    ws: &WorkspaceHandle,
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &xai_webuild_tools::notification::types::ToolNotification,
) {
    use xai_webuild_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        xai_webuild_tools::notification::types::ToolNotification,
    >,
) {
...
impl DrainReason {
    /// Stable `reason` label for `webuild_workspace_drain_started_total`.
...
impl DrainOutcome {
    /// Stable `outcome` label for `webuild_workspace_drain_completed_total`.
    pub fn as_str(self) -> &'static str {
        match self {
...
/// The SIGTERM drain budget from `WEBUILD_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("WEBUILD_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`WEBUILD_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("WEBUILD_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
/// sandbox; override via `WEBUILD_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    ...
    let api_base_url = std::env::var("WEBUILD_CLI_CHAT_PROXY_BASE_URL")
        .unwrap_or_else(|_| "https://cli-chat-proxy.grok.com/v1".to_string());
    let data_collection_disabled =
        std::env::var("WEBUILD_WORKSPACE_DATA_COLLECTION_DISABLED").as_deref() != Ok("false");
    ...
    let tool_config = xai_webuild_agent::workspace_webuild_build_toolset();
    ...
    if let Ok(dir) = std::env::var("WEBUILD_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("WEBUILD_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("WEBUILD_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
...
/// Resolve `$WEBUILD_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$WEBUILD_WORKSPACE_HOME` (operator override).
/// 2. `<webuild_home>/workspace`, where `<webuild_home>` honours `$WEBUILD_HOME` and
///    otherwise falls back to `~/.webuild` (see [`xai_webuild_config::webuild_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("WEBUILD_WORKSPACE_HOME")
        && !p.trim().is_empty()
    {
        return std::path::PathBuf::from(p);
    }
    xai_webuild_config::webuild_home().join("workspace")
}
...
/// (`WEBUILD_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
...
fn events_enabled() -> bool {
    std::env::var("WEBUILD_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `WEBUILD_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("WEBUILD_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`WEBUILD_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("WEBUILD_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
/// backing upload queue (tests, local mode). Never the real webuild home —
/// only [`connect_local_workspace`] resolves `$WEBUILD_WORKSPACE_HOME` — so the
/// queue-less default path can never collide with a real workspace's state dir.
fn ephemeral_workspace_home() -> std::path::PathBuf {
    std::env::temp_dir().join(format!("webuild-workspace-ephemeral-{}", std::process::id()))
}
/// Resolve `workspace_rewind_all_outcomes` from `WEBUILD_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    xai_webuild_config::env_bool("WEBUILD_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: xai_webuild_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use std::sync::Arc;
    use xai_webuild_tools::registry::types::ToolServerConfig;
    use xai_webuild_tools::types::tool::ToolKind;
    use xai_webuild_workspace_types::WorkspaceEvent;
    ...
    impl xai_webuild_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> xai_webuild_tools::types::tool::ToolNamespace {
            xai_webuild_tools::types::tool::ToolNamespace::MCP
        }
    ...
    impl xai_tool_runtime::Tool for BashCcoStub {
        ...
        type Output = xai_webuild_tools::types::output::ToolOutput;
        fn id(&self) -> xai_tool_protocol::ToolId {
        ...
        ) -> Result<xai_webuild_tools::types::output::ToolOutput, xai_tool_runtime::ToolError>
        {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(xai_webuild_tools::types::output::ToolOutput::Bash(
                xai_webuild_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        xai_webuild_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    async fn donation_entry_points_are_inert_without_a_hub() {
        ...
        assert!(
            handle
                .trace_donation_reporter("prod_webuild_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .log_donation_layer("prod_webuild_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .metric_donation_reporter("prod_webuild_workspace")
                .await
                .is_none(),
    ...
    async fn build_session_routed_handlers_skips_invalid_client_name_without_panic() {
        let handle = make_handle();
        let mut renamed = tc("WeBuild:read_file", Some(ToolKind::Read));
        ...
        let session = handle
            .create_session_with_config(
                ...
                Some(ToolServerConfig {
                    tools: vec![renamed, tc("WeBuild:grep", Some(ToolKind::Read))],
                    behavior_preset: None,
                }),
                CapabilityMode::All,
                None,
    ...
    async fn create_fingerprint_write_does_not_clobber_concurrent_rebind() {
        ...
        let fp_a = serde_json::to_value(&ToolServerConfig {
            tools: vec![tc("WeBuild:list_dir", Some(ToolKind::ListDir))],
            behavior_preset: None,
        })
        .ok();
    ...
    async fn toolset_terminal(
        toolset: &Arc<xai_webuild_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn xai_webuild_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<xai_webuild_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    pub(crate) fn background_capable_cfg() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                tc("WeBuild:read_file", Some(ToolKind::Read)),
                tc("WeBuild:run_terminal_cmd", Some(ToolKind::Execute)),
                tc(
                    "WeBuild:get_task_output",
                    Some(ToolKind::BackgroundTaskAction),
                ),
                tc("WeBuild:kill_task", Some(ToolKind::KillTaskAction)),
            ],
            behavior_preset: None,
        }
    }
    ...
    ///
    /// [`TerminalRunRequest`]: xai_webuild_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> xai_webuild_tools::computer::types::TerminalRunRequest {
        xai_webuild_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: xai_webuild_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: xai_webuild_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> xai_webuild_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn xai_webuild_tools::computer::types::TerminalBackend>,
        ) -> xai_webuild_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                xai_webuild_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> xai_webuild_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let xai_webuild_tools::types::output::ToolOutput::TaskOutput(
    ...
    /// `$WEBUILD_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $WEBUILD_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("WEBUILD_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("WEBUILD_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn fork_session_inherits_parent_tool_config_when_none() {
        ...
        let new_parent_baseline = ToolServerConfig {
            tools: vec![tc("WeBuild:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
    ...
    async fn fork_session_uses_explicit_tool_config_when_provided() {
        ...
        let custom = ToolServerConfig {
            tools: vec![
                tc("WeBuild:read_file", Some(ToolKind::Read)),
                tc("WeBuild:list_dir", Some(ToolKind::ListDir)),
            ],
            behavior_preset: None,
        };
        let child = handle
    ...
    async fn fork_session_uses_main_session_when_parent_session_id_is_none() {
        ...
        let baseline_ids: Vec<String> = child
            ...
            .map(|t| t.id.clone())
            .collect();
        assert_eq!(baseline_ids, vec!["WeBuild:read_file".to_string()]);
    }
    #[tokio::test]
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("xai-webuild-workspace-test-cwd-override");
    ...
    async fn on_mcp_snapshot_changed_emits_per_session_events_and_rebuilds() {
        ...
        handle
            ...
            .expect("subB ok");
        let mut rx = handle.shared.events.subscribe();
        let mcp_tool = tc("WeBuild:read_file", Some(ToolKind::Read));
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = xai_webuild_hooks::config::HookSpec {
                name: "injected".into(),
                event: xai_webuild_hooks::event::HookEventName::SessionStart,
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        let resolved = resolver(
            ...
            Some(serde_json::json!(
                { "metadata" : { "preset" : "webuild-computer", "capability_mode" :
                "all" }, }
            )),
        )
        .await
        ...
        assert!(
            reason.contains(xai_webuild_version::VERSION),
    ...
    async fn strict_bind_with_explicit_toolset_serves_it_end_to_end() {
        ...
        let resolved = resolver(
            ...
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "WeBuild:read_file" }] },
                }
            )),
        )
        .await
    ...
    async fn strict_rebind_with_corrected_toolset_heals_end_to_end() {
        ...
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({ "metadata" : { "preset" : "webuild-computer" } })),
        )
        .await
    ...
    fn owner_full_bind_metadata() -> serde_json::Value {
        serde_json::json!(
            ...
            "WeBuild:read_file" }, { "id" : "WeBuild:search_replace" }, { "id" :
            "WeBuild:grep" }, { "id" : "WeBuild:list_dir" },], }, }
        )
    }
    ...
    async fn bind_flow_rebinds_keep_backend_and_task_alive_end_to_end() {
        ...
        let bg_metadata = serde_json::json!(
            { "metadata" : { "tools" : [{ "id" : "WeBuild:read_file" }, { "id" :
            "WeBuild:run_terminal_cmd" }, { "id" : "WeBuild:get_task_output" }, {
            "id" : "WeBuild:kill_task" },] }, }
        );
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.webuild`.
    #[tokio::test]
