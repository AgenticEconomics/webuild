//! Integration tests for xai-webuild-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use xai_webuild_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use xai_webuild_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!xai_webuild_sandbox::should_restrict_child_network());
}
