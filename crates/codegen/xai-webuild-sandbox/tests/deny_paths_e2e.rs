...
fn profile_from_env() -> xai_webuild_sandbox::ProfileName {
    xai_webuild_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !xai_webuild_sandbox::is_inside_bwrap() {
            ...
            match xai_webuild_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = xai_webuild_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = xai_webuild_sandbox::SandboxManager::support_info();
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".webuild")).expect("mkdir .webuild");
    fs::write(
        tmp.join(".webuild").join("sandbox.toml"),
