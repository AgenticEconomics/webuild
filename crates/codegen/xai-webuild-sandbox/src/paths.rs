...
// ── WeBuild state directory ────────────────────────────────────────────────────

/// WeBuild state directory — always writable (`$WEBUILD_HOME` or `~/.webuild`).
pub(crate) fn webuild_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), webuild_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.webuild + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![webuild_home()];
    paths.extend(temp_writable_paths());
    paths
}
