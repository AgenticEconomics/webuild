...
//! cargo run -p xai-webuild-sandbox --example sandbox_smoke_test
//!
//! # Test strict profile
//! cargo run -p xai-webuild-sandbox --example sandbox_smoke_test -- strict
//!
//! # Test read-only profile
//! cargo run -p xai-webuild-sandbox --example sandbox_smoke_test -- read-only
//! ```
...
fn main() {
    ...
    let tmp_test = Path::new("/tmp/.webuild-sandbox-test");
