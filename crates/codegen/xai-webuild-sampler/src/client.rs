...
use xai_webuild_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use xai_webuild_sampling_types::ApiBackend;

/// Process-level fallback for the `x-webuild-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "webuild-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "webuild-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-webuild-*` headers. Optional fields are skipped when empty/`None`.
struct WeBuildRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl WeBuildRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-webuild-conv-id", self.conv_id)
            .header("x-webuild-req-id", self.req_id)
            .header("x-webuild-model-override", self.model_id)
            .header("x-webuild-session-id", self.session_id)
            .header("x-webuild-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-webuild-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-webuild-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-webuild-user-id", id);
        }
        b
    }
}
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xai_webuild_sampling_types::reported_cost_ticks(
        value
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-webuild-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-webuild-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xai_webuild_sampling_types::DoomLoopRecoveryPolicy>,
}
...
fn agent_version() -> String {
    xai_webuild_version::VERSION.to_string()
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-webuild-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-webuild-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-webuild-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-webuild-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-webuild-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    fn format_request_headers(
        &self,
        x_webuild_conv_id: &str,
        x_webuild_req_id: &str,
        model_id: &str,
        include_accept: bool,
    ) -> Vec<String> {
        ...
        req_headers.push(Self::format_header("x-webuild-conv-id", x_webuild_conv_id));
        req_headers.push(Self::format_header("x-webuild-req-id", x_webuild_req_id));
        req_headers.push(Self::format_header("x-webuild-model-override", model_id));
        if include_accept {
    ...
    pub async fn chat_completion(
        ...
        let x_webuild_conv_id = &payload.x_webuild_conv_id.clone().unwrap_or_default();
        let x_webuild_req_id = &payload.x_webuild_req_id.clone().unwrap_or_default();
        ...
        let webuild_headers = WeBuildRequestHeaders {
            conv_id: x_webuild_conv_id,
            req_id: x_webuild_req_id,
            model_id: &model_id,
            session_id: payload.x_webuild_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_webuild_turn_idx.as_deref(),
            agent_id: payload.x_webuild_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_webuild_deployment_id.as_deref(),
            user_id: payload.x_webuild_user_id.as_deref(),
        };
        let http_request = webuild_headers
    ...
    pub async fn chat_completion_stream(
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_webuild_conv_id, x_webuild_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_webuild_conv_id = request.x_webuild_conv_id.as_deref().unwrap_or_default();
        let x_webuild_req_id = request.x_webuild_req_id.as_deref().unwrap_or_default();
        ...
        let webuild_headers = WeBuildRequestHeaders {
            ...
            req_id: x_webuild_req_id,
            model_id: &model_id,
            session_id: request.x_webuild_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_webuild_turn_idx.as_deref(),
            agent_id: request.x_webuild_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_webuild_deployment_id.as_deref(),
            user_id: request.x_webuild_user_id.as_deref(),
        };
        ...
        xai_webuild_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = webuild_headers
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_webuild_conv_id, x_webuild_req_id, &model_id, false);
            let server_message = parse_error_bytes(bytes.as_ref());
    ...
    /// gate that adds the opt-in `x-webuild-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = webuild_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_webuild_conv_id = request.x_webuild_conv_id.clone();
        let x_webuild_req_id = request.x_webuild_req_id.clone();
        let x_webuild_session_id = request.x_webuild_session_id.clone();
        let x_webuild_turn_idx = request.x_webuild_turn_idx.clone();
        let x_webuild_agent_id = request.x_webuild_agent_id.clone();
        ...
        let extra_tools = xai_webuild_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_webuild_conv_id = x_webuild_conv_id;
        wrapper.x_webuild_req_id = x_webuild_req_id;
        wrapper.x_webuild_session_id = x_webuild_session_id;
        wrapper.x_webuild_turn_idx = x_webuild_turn_idx;
        wrapper.x_webuild_agent_id = x_webuild_agent_id;
...
mod tests {
    ...
    use xai_webuild_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_webuild_conv_id: None,
            x_webuild_req_id: None,
            x_webuild_session_id: None,
            x_webuild_turn_idx: None,
            x_webuild_agent_id: None,
            x_webuild_deployment_id: None,
            x_webuild_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "xai-webuild-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the webuild-shell agent product.
        assert!(ua.starts_with("my-client webuild-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = xai_webuild_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
