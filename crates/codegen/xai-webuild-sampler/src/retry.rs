...
use xai_webuild_sampling_types::SamplingError;
...
/// Resolve max API retries: `WEBUILD_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("WEBUILD_MAX_RETRIES").ok();
