...
//! it" buckets.
//!
//! `xai-webuild-sampler` is intentionally decoupled from `xai-webuild-shell`
...
/// tools live in `xai-webuild-tools`
/// (`crates/codegen/xai-webuild-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xai_webuild_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xai_webuild_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xai-webuild-shell/src/auth/manager.rs` to match, otherwise the
