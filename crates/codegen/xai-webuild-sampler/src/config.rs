...
//! `xai_webuild_sampling_types::SamplingConfig` so that the sampler crate
//! avoids transitive dependencies on shell-specific types
//! (`xai-webuild-tools`, etc.).
...
/// submit.
///
/// # Construction in `xai-webuild-shell`
///
...
/// composing chat-state's `xai_webuild_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-webuild-doom-loop-check` header on
