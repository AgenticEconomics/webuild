...
use xai_webuild_sampler::SamplingClient;
use xai_webuild_test_support::spawn_counting_server;

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`WEBUILD_SAMPLER_SHARED_CLIENT=0`,
/// `WEBUILD_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("WEBUILD_SAMPLER_SHARED_CLIENT");
            std::env::set_var("WEBUILD_POOL_MAX_IDLE", "2");
            std::env::set_var("WEBUILD_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
