...
//!
//! Catalog file lookup, in order: `.webuild-plugin/marketplace.json` (preferred),
//! `.webuild-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.webuild-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.webuild-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_webuild_dir() {
        let dir = tempfile::tempdir().unwrap();
        let webuild_dir = dir.path().join(".webuild-plugin");
        std::fs::create_dir_all(&webuild_dir).unwrap();
        std::fs::write(
            webuild_dir.join("marketplace.json"),
            r#"{"name": "webuild", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "webuild");
    }

    #[test]
    fn load_index_webuild_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".webuild-plugin", "webuild"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "webuild");
    }
