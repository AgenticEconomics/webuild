...
//! `.webuild-plugin/plugin-index.json` (preferred), then
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_webuild_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".webuild-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_webuild_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".webuild-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".webuild-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".webuild-plugin",
            r#"{
                "$schema": "https://x.ai/webuild/plugin-index.schema.json",
                "version": 1,
