//! Parse marketplace sources from `~/.webuild/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `WEBUILD_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    xai_webuild_config::env_bool("WEBUILD_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// and `known_marketplaces.json` files under `~/.webuild/` and `~/.claude/`.
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        xai_webuild_config::user_webuild_home(),
...
/// instead of `~/.webuild`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("WEBUILD_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("WEBUILD_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("WEBUILD_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
