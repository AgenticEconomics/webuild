...
//!    the lookup order — `.webuild-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = xai_webuild_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(xai_webuild_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let webuild_dir = dir.path().join(".webuild-plugin");
        std::fs::create_dir_all(&webuild_dir).unwrap();
        std::fs::write(
            webuild_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn webuild_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "webuild-plugin", "1.0.0");
        ...
        std::fs::write(
            webuild_dir.join("marketplace.json"),
            r#"{
                "name": "webuild-marketplace",
                "plugins": [{
                    "name": "webuild-plugin",
                    "description": "From the .webuild-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/webuild-plugin" },
                    "tags": ["webuild"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "webuild-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["webuild"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_webuild_file(dir: &Path, file: &str, content: &str) {
        let webuild_dir = dir.join(".webuild-plugin");
        std::fs::create_dir_all(&webuild_dir).unwrap();
        std::fs::write(webuild_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_webuild_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_webuild_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_webuild_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_webuild_file(dir.path(), "plugin-index.json", "not json");
