...
pub fn discover_worktrees(webuild_home: &Path) -> DiscoveryReport {
    let mut report = DiscoveryReport::default();
    scan_two_level_dir(
        &webuild_home.join("worktrees"),
        WorktreeKind::Session,
        &mut report,
    );
    scan_two_level_dir(
        &webuild_home.join("worktree_pool"),
        WorktreeKind::Pool,
        &mut report,
    );
    report
...
pub fn rebuild_worktree_db(
    ...
    let discovery = discover_worktrees(webuild_home);
...
mod tests {
    ...
    fn discover_session_worktrees() {
        ...
        let wt = webuild_home.join("worktrees/myrepo/worktree-abc123");
        make_fake_linked_worktree(&wt, "/repo/.git/worktrees/abc123");

        let report = discover_worktrees(webuild_home);
    ...
    fn discover_pool_worktrees() {
        ...
        let wt = webuild_home.join("worktree_pool/inst-1/pool-a");
    ...
    fn skips_dot_prefixed_and_markers() {
        ...
        let base = webuild_home.join("worktrees/myrepo");
    ...
    fn rebuild_registers_and_skips_duplicates() {
        ...
        let wt = webuild_home.join("worktrees/repo/worktree-sess1");
        make_fake_standalone_worktree(&wt);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();

        let r1 = rebuild_worktree_db(&db, webuild_home).unwrap();
        assert_eq!(r1.discovered, 1);
        assert_eq!(r1.registered, 1);
        assert_eq!(r1.already_tracked, 0);

        let r2 = rebuild_worktree_db(&db, webuild_home).unwrap();
    ...
    fn rebuild_keeps_same_basename_worktrees_in_different_repos() {
        ...
        let wt_a = webuild_home.join("worktrees/repo-a/wt-abc");
        let wt_b = webuild_home.join("worktrees/repo-b/wt-abc");
        make_fake_standalone_worktree(&wt_a);
        make_fake_standalone_worktree(&wt_b);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        let report = rebuild_worktree_db(&db, webuild_home).unwrap();
        ...
        let report2 = rebuild_worktree_db(&db, webuild_home).unwrap();
