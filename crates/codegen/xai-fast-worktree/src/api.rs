...
/// handle `~/.webuild/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if worktree_path.is_symlink() {
        ...
        if let Ok(Some(_)) = btrfs::is_btrfs_subvolume(&resolved) {
            ...
            // `btrfs` refuses non-subvolumes, the snapshot dir is webuild-owned, and
...
pub mod gc {
    ...
    /// symlinked `$WEBUILD_HOME` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.webuild/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.webuild/worktrees/<repo>/<session>):
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        xai_test_utils::require_git!();
        use xai_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate WEBUILD_HOME so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
        let _fx = crate::db::WeBuildHomeFixture::new();
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.webuild/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".webuild/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.webuild/worktrees/active-wt");
    ...
    mod metadata_integration {
        ...
        fn register_worktree_writes_correct_fields() {
            // Isolate WEBUILD_HOME so register_worktree's open_default write lands
            ...
            // (WEBUILD_HOME is process-global) can't INSERT-OR-REPLACE our row.
        ...
        fn gc_with_delegate_removes_expired_and_unregisters() {
            ...
            // WEBUILD_HOME == the gc DB dir so remove_worktree's open_default
