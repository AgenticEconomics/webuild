...
/// Record the source repo root in `<worktree>/.git/webuild-worktree-source`.
///
...
fn record_main_repo_marker(source: &Path, worktree: &Path) {
    ...
    let marker = git_dir.join("webuild-worktree-source");
    if marker.exists() {
        return;
    }
...
mod tests {
    ...
    fn test_marker_written_for_standalone_git_dir() {
        ...
        let marker = dest.join(".git/webuild-worktree-source");
    ...
    fn test_marker_skipped_for_linked_worktree() {
        ...
        assert!(!dest.join(".git/webuild-worktree-source").exists());
    }
