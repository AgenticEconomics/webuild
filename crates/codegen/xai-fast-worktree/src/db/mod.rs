...
impl WorktreeDb {
    /// Open (or create) the DB at `webuild_home/worktrees.db`.
    pub fn open(webuild_home: &Path) -> Result<Self> {
        Self::open_at(&webuild_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.webuild/worktrees.db`.
    ///
    /// Discovers webuild home via `$WEBUILD_HOME`, falling back to the canonicalized
    /// `$HOME/.webuild` (matching `xai_webuild_config::webuild_home`).
    ...
    pub fn open_default() -> Result<Self> {
        Self::open(&resolve_webuild_home()?)
    }
...
pub fn resolve_webuild_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("WEBUILD_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $WEBUILD_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .webuild
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with xai_webuild_config::default_webuild_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".webuild"))
}

/// Serializes tests that mutate the process-global `WEBUILD_HOME` env var so they
...
#[cfg(test)]
static WEBUILD_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`WEBUILD_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `WEBUILD_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `WEBUILD_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct WeBuildHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated webuild home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl WeBuildHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = WEBUILD_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("webuild-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via WEBUILD_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until WEBUILD_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("WEBUILD_HOME");
        unsafe { std::env::set_var("WEBUILD_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for WeBuildHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("WEBUILD_HOME", p),
                None => std::env::remove_var("WEBUILD_HOME"),
            }
        }
    }
}
