...
/// never wall-clock.
///
/// Knobs: WEBUILD_PERF_GIT_FILES (default 5000), WEBUILD_PERF_GIT_PICKS (default 10).
#[tokio::test]
#[ignore = "perf repro; run with --ignored --nocapture"]
async fn refresh_storm_scan_count_during_rebase() {
    let files = env_usize("WEBUILD_PERF_GIT_FILES", 5000);
    let picks = env_usize("WEBUILD_PERF_GIT_PICKS", 10);
