//! Memory-system configuration value types, extracted from xai-webuild-shell
...
//! `xai-webuild-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.webuild/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.webuild/memory/` are scanned:
