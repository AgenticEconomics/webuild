...
use xai_webuild_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-webuild-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `webuild_build_settings`
    ...
    /// default (on). Sits below env `WEBUILD_FOLDER_TRUST`, user
    ...
    /// remote settings `webuild_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `webuild_build_settings` remote settings key) to enable it. See
    ...
    /// `xai_webuild_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `WEBUILD_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `webuild_build_settings`).
    ...
    /// ghost text), from the `webuild_build_settings` remote settings flag. Sits below
    /// env (`WEBUILD_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    ...
    /// (e.g. `webuild-imagine-image`) instead of the default quality model
    /// (`webuild-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `WEBUILD_VOICE_MODE`. Free-tier SuperWeBuild upsell
    ...
    /// User's subscription tier from remote settings `webuild_build_access_gate`.
    /// E.g. "free", "premium", "superwebuild", "superwebuild_heavy".
    ...
    /// Whether the user is allowed to use WeBuild. Set by remote settings
    /// `webuild_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "SuperWeBuild", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `webuild_build_usage_redirect_url`
    ...
    /// `webuild_build_settings`. Per-model override on `ModelInfo`
    /// (`webuild_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`WEBUILD_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`WEBUILD_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`WEBUILD_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`WEBUILD_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `webuild workspace` CLI command (Computer Hub
    /// workspace exposure), from `webuild_build_settings.workspace_command_enabled`.
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `webuild wrap ssh` for remote sessions).
    #[serde(default)]
