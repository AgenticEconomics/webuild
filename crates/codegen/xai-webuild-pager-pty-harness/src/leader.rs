...
//! elected leader) plus a fixed leader socket beneath its `WEBUILD_HOME`; clients
...
impl LeaderCluster {
    ...
    /// The cluster's sessions root: `WEBUILD_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".webuild").join("sessions")
    }
