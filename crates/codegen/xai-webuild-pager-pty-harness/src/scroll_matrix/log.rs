//! `WEBUILD_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in
//! `xai-webuild-pager/src/input/scroll_log.rs`. [`ScrollLogLine`] mirrors it
...
/// Parse a `WEBUILD_SCROLL_LOG` JSONL file into records.
///
...
/// session (`WEBUILD_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
...
mod tests {
    ...
    // in `xai-webuild-pager/src/input/mouse/tests.rs`.
