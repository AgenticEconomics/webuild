//! Scripted TUI scenario runner for xai-webuild-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$WEBUILD_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&webuild_home)
                .with_context(|| format!("create scenario WEBUILD_HOME {}", webuild_home.display()))?;
            fs::write(webuild_home.join("config.toml"), config_toml)
