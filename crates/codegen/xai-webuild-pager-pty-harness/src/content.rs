...
//! [`MockInferenceServer`] from `xai-webuild-test-support` and provides the
...
pub use xai_webuild_test_support::mock_server::LogEntry;
pub use xai_webuild_test_support::mock_server::MockModelEntry as MockModel;
pub use xai_webuild_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use xai_webuild_test_support::sse;
pub use xai_webuild_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.webuild
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit WEBUILD_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // WEBUILD_HOME is set in the test runner's env).
            ("WEBUILD_HOME".into(), webuild_home),
            ("WEBUILD_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("WEBUILD_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("WEBUILD_PROMPT_SUGGESTIONS".into(), "false".into()),
            // Compatibility set_turns remains request-FIFO, so retries stay off.
            ("WEBUILD_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    /// on the SuperWeBuild upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("WEBUILD_HOME").as_deref(),
            content.home().join(".webuild").to_str()
        );
        assert_eq!(get("WEBUILD_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("WEBUILD_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("WEBUILD_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("WEBUILD_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("WEBUILD_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("WEBUILD_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("WEBUILD_MAX_RETRIES").as_deref(), Some("0"));
