//! `scroll-matrix` — scroll validation matrix sweep for xai-webuild-pager.
//!
...
use xai_webuild_pager_pty_harness::pager_binary;
use xai_webuild_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against xai-webuild-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_xai-webuild-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves WEBUILD_SCROLL_LOG against ITS cwd (the
