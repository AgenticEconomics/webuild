...
const CLIPBOARD_SINK_ENV_VARS: &[&str] = &["WEBUILD_OSC52_SINK", "LC_WEBUILD_OSC52_SINK"];

/// Host terminal identity markers stripped from the child environment.
///
/// The pager's terminal detection
/// (`xai-webuild-pager-render/src/terminal/mod.rs`:
...
fn apply_child_env(cmd: &mut CommandBuilder, env: &[(&str, &str)]) {
    ...
    // A harness launched under `webuild wrap` must not silently confirm clipboard
...
mod tests {
    ...
    fn apply_child_env_strips_all_host_terminal_markers() {
        ...
        cmd.env("WEBUILD_SCROLL_LOG", "/tmp/scroll.jsonl");
        ...
        assert_eq!(
            cmd.get_env("WEBUILD_SCROLL_LOG").and_then(|v| v.to_str()),
    ...
    fn apply_child_env_caller_env_overrides_survive_strips() {
        ...
        assert_eq!(
            cmd.get_env("WEBUILD_OSC52_SINK").and_then(|v| v.to_str()),
            Some("1"),
