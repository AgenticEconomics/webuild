...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build xai-webuild-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build xai-webuild-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_xai-webuild-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-webuild-pager-bin` (the composition-
///    root package that owns the `xai-webuild-pager` binary)
