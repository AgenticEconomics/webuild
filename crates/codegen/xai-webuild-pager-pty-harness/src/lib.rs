//! Unified PTY harness for xai-webuild-pager.
//!
...
//!    `xai-webuild-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `WEBUILD_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use xai_webuild_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through webuild's stdout. Used to reproduce the doubled-line class of bugs
    /// where webuild's diff renderer never re-asserts a region it didn't write
