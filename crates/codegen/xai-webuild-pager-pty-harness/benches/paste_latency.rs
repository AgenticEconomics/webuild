//! `paste-latency` — clipboard paste latency benchmark for `xai-webuild-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p xai-webuild-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p xai-webuild-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/webuild-old --mode text --json /tmp/paste-old.json
//! ```
...
use xai_webuild_pager_pty_harness::{
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for xai-webuild-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
