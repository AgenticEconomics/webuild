//! `pty-bench` — PTY benchmark CLI for `xai-webuild-pager`.
//!
...
//! ```bash
//! cargo bench -p xai-webuild-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/webuild-${VERSION}-linux-x86_64 \
//!   cargo bench -p xai-webuild-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for xai-webuild-pager",
    long_about = None,
)]
struct Cli {
