...
use xai_webuild_pager::app::app_view::{AuthState, TrustState};
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to WeBuild", bold)),
            );
...
mod tests {
    ...
    fn auth_hint_maps_auth_state() {
        use xai_webuild_pager::app::app_view::AuthMode;
    ...
    fn render_auth_shows_url_and_code() {
        ...
        assert!(text.contains("Sign in to WeBuild"), "header: {text:?}");
