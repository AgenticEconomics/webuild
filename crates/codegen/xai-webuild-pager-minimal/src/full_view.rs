...
use xai_webuild_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = xai_webuild_pager::appearance::cache::load_show_thinking_blocks();
        ...
        xai_webuild_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: xai_webuild_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(xai_webuild_pager::scrollback::block::RenderBlock::system(
                    "No conversation transcript to view yet",
                ));
        }
        return;
    }
    let path = std::env::temp_dir().join(format!("webuild-transcript-{}.ansi", uuid::Uuid::new_v4()));
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent.scrollback.push_block(
                    xai_webuild_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use xai_webuild_pager::scrollback::block::RenderBlock;
