...
use xai_webuild_pager::app::PagerTerminal;
use xai_webuild_pager::app::app_view::{ActiveView, AppView};
use xai_webuild_pager::minimal_api;
use xai_webuild_pager::render::Renderable;
use xai_webuild_pager::scrollback::state::ScrollbackState;
use xai_webuild_pager::scrollback::wrappers::EntryRenderer;
use xai_webuild_pager::theme::Theme;
use xai_webuild_pager::views::prompt_widget::PromptStyle;
use xai_webuild_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &xai_webuild_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(
    appearance: &xai_webuild_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    xai_webuild_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                xai_webuild_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(xai_webuild_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a xai_webuild_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &xai_webuild_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_watchers(agent: &xai_webuild_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == xai_webuild_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut xai_webuild_pager::app::agent_view::AgentView,
) -> Option<xai_webuild_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &xai_webuild_pager::app::agent_view::AgentView,
    activity: &Option<xai_webuild_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = xai_webuild_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = xai_webuild_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use xai_webuild_pager::views::context_bar::fmt_tokens;
...
fn minimal_pending_hint(
    pending: &Option<xai_webuild_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> xai_webuild_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use xai_webuild_pager::app::agent::AgentState;
        use xai_webuild_pager::scrollback::RenderBlock;
        use xai_webuild_pager::scrollback::entry::ScrollbackEntry;
        use xai_webuild_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &xai_webuild_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use xai_webuild_pager::acp::tracker::TurnActivity;
        use xai_webuild_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        xai_webuild_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        xai_webuild_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            xai_webuild_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(xai_webuild_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &xai_webuild_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use xai_webuild_pager::app::app_view::PendingAction;
        use xai_webuild_pager::input::key::KeyShortcut;
