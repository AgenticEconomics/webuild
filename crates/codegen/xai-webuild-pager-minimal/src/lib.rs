//! Minimal (scrollback-native) render mode — `webuild --minimal`.
//!
...
//! # Wiring
//!
//! `xai-webuild-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xai_webuild_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-webuild-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-webuild-pager`.
///
...
/// subsequent calls are ignored (see [`xai_webuild_pager::minimal_hook`]).
pub fn install() {
    xai_webuild_pager::minimal_hook::install(xai_webuild_pager::minimal_hook::MinimalHooks { draw });
}
