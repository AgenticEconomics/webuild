...
//! `xai-webuild-shell` and `xai-webuild-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `xai-webuild-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `xai-webuild-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// WeBuild's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `xai-webuild-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.webuild/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.webuild/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.webuild/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.webuild/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserWeBuild),
            conflict: None,
        };
