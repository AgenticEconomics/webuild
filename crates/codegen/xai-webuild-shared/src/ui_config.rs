...
use xai_webuild_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `WEBUILD_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A WeBuild STT
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `webuild wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: xai_webuild_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
