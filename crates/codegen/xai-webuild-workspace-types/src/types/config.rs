...
//! agent-session config types in `xai-webuild-config` and friends.
...
/// `xai-webuild-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_webuild_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
