//! Wire types for the `xai-webuild-workspace` API.
//!
...
//! `xai-webuild-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xai-webuild-workspace-types` (instead of `xai-webuild-workspace` or
/// `xai-webuild-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xai_webuild_workspace::permission`
