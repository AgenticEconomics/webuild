...
/// over `xai_webuild_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `xai_webuild_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `xai_webuild_tools`'s `TodoState`). `status`
...
/// in `xai-webuild-workspace/src/hub_server.rs`.
