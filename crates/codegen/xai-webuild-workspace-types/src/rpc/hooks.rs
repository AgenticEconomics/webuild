...
//!
//! `xai_webuild_hooks` pulls in `git2`/`reqwest`/`xai-webuild-tools`, too heavy for
...
/// Wire mirror of `xai_webuild_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `xai_webuild_hooks::config::HookSpec`.
///
...
/// `xai-webuild-hooks`, so a server-side test (`xai-webuild-workspace`'s
...
/// Wire mirror of `xai_webuild_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.webuild/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });
