...
//! `xai-webuild-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.webuild/skills
    Local,
    /// repo_root/.webuild/skills
    Repo,
    /// ~/.webuild/skills
    User,
    /// ~/.webuild/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.webuild/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the xai-webuild-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.webuild/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.webuild/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.webuild/plugins/infra-plugin",
            "plugin_data": "/root/.webuild/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
