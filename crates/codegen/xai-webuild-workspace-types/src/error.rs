...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xai_webuild_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xai-webuild-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xai_webuild_tools::ToolError`) happens
