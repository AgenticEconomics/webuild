...
impl InferenceRequestKind {
    fn classify(headers: &HeaderMap, body: &Value) -> Self {
        if nonempty_header(headers, "x-webuild-turn-idx").is_some() {
            return Self::Foreground;
        }
        if nonempty_header(headers, "x-webuild-req-id").is_some() {
...
impl InferenceOverrides {
    ...
    pub(crate) fn classify(
        ...
        let fingerprint =
            nonempty_header(headers, "x-webuild-req-id").map(|request_id| ModelCallFingerprint {
                endpoint,
                kind,
