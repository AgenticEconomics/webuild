//! Leader-mode (`webuild agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`webuild_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "WEBUILD_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`webuild_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "WEBUILD_BINARY_CLIENT";
...
/// (`WEBUILD_BINARY_LEADER`, else the shared [`webuild_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`WEBUILD_BINARY_CLIENT`,
/// else the shared [`webuild_binary`] resolution).
...
/// A `webuild agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&webuild_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export WEBUILD_* vars
            // (e.g. WEBUILD_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("WEBUILD_LEADER_SOCKET", home.join(".webuild").join("leader.sock"))
            .env("WEBUILD_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("WEBUILD_XAI_API_BASE_URL", server.url())
            ...
            // ~/.webuild/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "xai_webuild_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".webuild").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".webuild").join("leader.log")).unwrap_or_default()
}
