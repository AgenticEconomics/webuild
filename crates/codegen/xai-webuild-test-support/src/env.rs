...
fn local_webuild_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xai-webuild-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_webuild_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "xai-webuild-pager", "--bin", "xai-webuild-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xai-webuild-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xai-webuild-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xai-webuild-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve webuild binary: `WEBUILD_BINARY` env (CI) or a locally built `xai-webuild-pager` binary.
pub fn webuild_binary() -> PathBuf {
    if let Ok(path) = std::env::var("WEBUILD_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "WEBUILD_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_xai-webuild-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_webuild_binary_path();
    ensure_local_webuild_binary(&binary);
    binary
}
...
/// Point webuild at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox webuild on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit WEBUILD_HOME every spawned child shares the real
        // `%USERPROFILE%\.webuild` — test 1's models_cache.json (which embeds
        ...
        .env("WEBUILD_HOME", home.join(".webuild"))
        .env("WEBUILD_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("WEBUILD_XAI_API_BASE_URL", mock_url)
        .env("XAI_API_KEY", "test-key-for-ci")
        .env("WEBUILD_TELEMETRY_ENABLED", "false")
        .env("WEBUILD_FEEDBACK_ENABLED", "false")
        .env("WEBUILD_TRACE_UPLOAD", "false")
        .env("WEBUILD_INSTRUMENTATION", "disabled")
        ...
        .env("WEBUILD_DISABLE_AUTOUPDATER", "1");
}
