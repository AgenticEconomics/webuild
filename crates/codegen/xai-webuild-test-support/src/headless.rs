//! Headless mode (`webuild -p`) test runner.
//!
//! Runs the webuild binary as a subprocess with the mock server, captures output.
...
/// Run `webuild` with the given args against the mock server, with a 60s timeout.
...
pub async fn run_headless_with_cmd(mut cmd: tokio::process::Command) -> HeadlessResult {
    ...
    let mut child = cmd
        .spawn()
        .unwrap_or_else(|e| panic!("failed to spawn webuild binary at {}: {e}", binary.display()));
    ...
    .await
    {
        Ok(result) => (
            result.unwrap_or_else(|e| {
                panic!("failed to wait for webuild binary {}: {e}", binary.display())
            }),
            false,
        ),
        Err(_) => {
            ...
            let status = child.wait().await.unwrap_or_else(|e| {
                panic!(
                    "failed to kill timed out webuild binary {}: {e}",
                    binary.display()
                )
            });
            (status, true)
