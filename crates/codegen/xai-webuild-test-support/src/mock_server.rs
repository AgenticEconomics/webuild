...
mod tests {
    ...
    async fn post_foreground(
        ...
        reqwest::Client::new()
            ...
            .header("x-webuild-req-id", request_id)
            .header("x-webuild-turn-idx", "1")
    ...
    fn explicit_request_headers_override_tool_count_heuristic() {
        ...
        headers.insert("x-webuild-req-id", "title-request".parse().unwrap());
        assert!(
            !overrides
        ...
        headers.insert("x-webuild-turn-idx", "1".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
        ...
        headers.insert("x-webuild-req-id", "".parse().unwrap());
        headers.insert("x-webuild-turn-idx", "".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
    ...
    async fn overlapping_duplicate_replays_but_sequential_identical_request_claims_next() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                let response = reqwest::Client::new()
                    .post(url)
                    .header("x-webuild-req-id", "turn-id")
                    .header("x-webuild-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
    ...
    async fn cancelling_replay_waits_for_primary_before_satisfaction() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                reqwest::Client::new()
                    .post(url)
                    .header("x-webuild-req-id", "cancel-replay")
                    .header("x-webuild-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
