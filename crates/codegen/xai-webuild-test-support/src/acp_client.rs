//! ACP stdio clients for testing webuild sessions end-to-end: the typed
//! [`WeBuildStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
use crate::env::{webuild_binary, test_env_cmd_tokio};
use crate::headless::stderr_tail;
use crate::mock_server::MockInferenceServer;
use crate::process::spawn_piped_with_stderr_capture;

/// Spawn `webuild agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`WeBuildStdioClient`])
...
/// kill-list so a test can still set e.g. `WEBUILD_DEBUG_LOG=1` explicitly.
...
/// Drives `webuild agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`WeBuildStdioClient`] (`ClientSideConnection`,
