...
//! - [`WeBuildStdioClient`] — ACP client that drives `webuild agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `webuild agent --leader stdio` (unix)
//! - [`run_headless`] — Run `webuild -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`webuild_binary`] — Resolve the webuild binary path (WEBUILD_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{WeBuildStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, webuild_binary};
pub use headless::{
