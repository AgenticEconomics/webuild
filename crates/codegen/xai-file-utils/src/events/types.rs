...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `xai-webuild-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `xai-webuild-shell::session::events`.
    ...
    /// in `xai-webuild-shell::session::events`.
    ...
    /// discriminator constants in `xai-webuild-shell::session::events`.
    ...
    /// `xai-webuild-shell::session::events`.
    ...
    /// `xai-webuild-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `xai-webuild-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `xai-webuild-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `xai-webuild-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `xai-webuild-shell`'s `GoalPauseReason`. The two types
