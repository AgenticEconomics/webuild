...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".webuild", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
fn has_excluded_component(path: &Path) -> bool {
    for component in path.components() {
        if let std::path::Component::Normal(name) = component {
            ...
            if name_lower.starts_with(".webuild-") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn webuild_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".webuild")));
                assert!(!is_project_dir(&home.join(".webuild/bin")));
            }
        }

        #[test]
        fn webuild_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".webuild-proxy-work")));
            }
        }
