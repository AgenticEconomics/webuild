...
impl ResolvedStorageConfig {
    ...
    /// mirrors `WeBuildAuthCredentials::apply` precedence (deployment key wins).
...
pub struct UploadRetryPolicy {
    ...
    /// `WEBUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS`.
    pub auth_park_probe_interval: Duration,
}
pub const DEFAULT_AUTH_PARK_PROBE_INTERVAL: Duration = Duration::from_secs(300);
/// Smallest probe interval a `WEBUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS` override may
...
/// Resolve a `WEBUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `xai_webuild_workspace::recovery`.
...
struct UploadQueueItem {
    ...
    /// WeBuild client version string, stamped on the `gcs_queue_upload` tracing span.
...
pub struct UploadQueue {
    ...
    max_queue_bytes: u64,
    /// WeBuild client version string stamped on every `gcs_queue_upload` tracing span.
...
impl UploadQueue {
    ...
    pub fn spawn(
        ...
        Self::spawn_with_concurrency(webuild_home, resolver, retry_policy, DEFAULT_MAX_CONCURRENT)
    }
    ...
    pub fn spawn_with_concurrency(
        ...
        let queue_dir = webuild_home.join("upload_queue");
        if let Err(e) = std::fs::create_dir_all(&queue_dir) {
            tracing::warn!(error = % e, "Failed to create upload queue dir");
        }
        if let Some(raw_secs) = std::env::var("WEBUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring WEBUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("WEBUILD_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
    ...
    /// Set the webuild client version to stamp on every `gcs_queue_upload` span.
...
/// of entries removed.
pub fn cleanup_orphaned_uploads(webuild_home: &Path, max_age: Duration) -> u64 {
    let cleaned = cleanup_queue_dir(&webuild_home.join("upload_queue"), max_age, None);
