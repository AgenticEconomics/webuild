...
use xai_webuild_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticWeBuildAuth, StorageClient};
...
/// `StaticWeBuildAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticWeBuildAuth::new(Some(user_token.to_owned()));
