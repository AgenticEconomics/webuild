...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p xai-webuild-voice --bin voice-probe -- --seconds 5
//! ```

use std::path::PathBuf;

use xai_webuild_voice::{
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,xai_webuild_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                xai_webuild_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("WEBUILD_XAI_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("WEBUILD_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,xai_webuild_voice=debug)

Reads [voice] from ~/.webuild/config.toml unless --config PATH is set.
"#
    );
}
