...
//! field rename/retype in `webuild-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use xai_webuild_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "WeBuild:grep".to_owned(),
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "WeBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "WeBuild:read_file");
