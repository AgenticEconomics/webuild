...
mod tests {
    ...
    fn unset_params_is_ok_none() {
        assert_eq!(parse_params_json(0, "WeBuild:grep", None), Ok(None));
    }

    #[test]
    fn valid_object_is_returned() {
        let parsed = parse_params_json(0, "WeBuild:grep", Some(r#"{"max_results":50}"#)).unwrap();
        assert_eq!(
            parsed,
            Some(
    ...
    fn empty_string_is_a_parse_error() {
        let err = parse_params_json(3, "WeBuild:grep", Some("")).unwrap_err();
    ...
    fn name_override_unset_or_valid_is_ok() {
        assert_eq!(validate_name_override(0, "WeBuild:grep", None), Ok(()));
        for name in ["search", "WeBuild:grep", "a-b_C9"] {
            assert_eq!(
                validate_name_override(0, "WeBuild:grep", Some(name)),
                Ok(()),
                "name={name:?}"
            );
        }
    ...
    fn name_override_outside_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "dot.name"] {
            let err = validate_name_override(2, "WeBuild:grep", Some(name)).unwrap_err();
    ...
    fn all_ids_present_returns_none() {
        let entries = [entry("WeBuild:grep"), entry("WeBuild:read_file")];
        let allowed = allowed(&["WeBuild:grep", "WeBuild:read_file", "WeBuild:bash"]);
    ...
    fn empty_entries_returns_none() {
        assert_eq!(
            first_unknown_tool_id(&[], &allowed(&["WeBuild:grep"])),
            None
        );
    }
    ...
    fn first_unknown_id_is_returned_with_index() {
        let entries = [
            entry("WeBuild:grep"),
            entry("WeBuild:nonexistent"),
            entry("WeBuild:also_missing"),
        ];
        let allowed = allowed(&["WeBuild:grep"]);
        assert_eq!(
            first_unknown_tool_id(&entries, &allowed),
            Some((1, "WeBuild:nonexistent"))
        );
    }
}
