//! Shared API definitions for WeBuild tools: protobuf types, config validation,
...
pub mod pb {
    include!(concat!(env!("OUT_DIR"), "/xai.webuild.tools.v1.rs"));
}
...
///
/// Tool ids are colon-separated `Namespace:tool` (e.g. `WeBuild:grep`); the
...
mod default_client_name_tests {
    ...
    fn pins_first_colon_derivation() {
        assert_eq!(default_client_name("WeBuild:grep"), "grep");
