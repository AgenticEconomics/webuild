//! System model settings: default IDs + built-in catalog rows, loaded from
//! `default_models.json` (embedded at compile time).
//!
//! Edit that JSON to change the product default model, slug list, and (via
//! optional `base_url` / `api_backend` / `env_key` on each row) which
//! inference endpoint and protocol each built-in model uses.
//!
//! At runtime each model is resolved via:
//!   CLI flag > ENV var > config.toml `[models]` / `[model.*]` > remote
//!   settings > these defaults. Users can still switch to xAI or any custom
//!   provider with `/model`, `-m`, or `[model.<id>]` overrides.

use std::sync::LazyLock;

/// The raw JSON, embedded at compile time. Re-exported through the
/// `xai_webuild_shell::models` facade and consumed by `agent::config`, so it must
